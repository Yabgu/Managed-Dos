...
static void Check_Palette(void) {
	...
	case scalerMode16:
		for (i=render.pal.first;i<=render.pal.last;i++) {
			System::Byte r=render.pal.rgb[i].red;
			System::Byte g=render.pal.rgb[i].green;
			System::Byte b=render.pal.rgb[i].blue;
			System::UInt16 newPal = GFX_GetRGB(r,g,b);
	...
	default:
		for (i=render.pal.first;i<=render.pal.last;i++) {
			...
			System::Byte g=render.pal.rgb[i].green;
			System::Byte b=render.pal.rgb[i].blue;
			System::UInt32 newPal = GFX_GetRGB(r,g,b);
...
void RENDER_SetPal(System::Byte entry,System::Byte red,System::Byte green,System::Byte blue) {
...
static void RENDER_StartLineHandler(const void * s) {
	if (s) {
		const unsigned *src = (unsigned*)s;
		unsigned *cache = (unsigned*)(render.scale.cacheRead);
		for (int x=render.src.start;x>0;) {
...
static void RENDER_ClearCacheHandler(const void * src) {
	unsigned x, width;
	System::UInt32 *srcLine, *cacheLine;
	srcLine = (System::UInt32 *)src;
	cacheLine = (System::UInt32 *)render.scale.cacheRead;
...
bool RENDER_StartUpdate(void) {
	...
	render.scale.cacheRead = (System::Byte*)&scalerSourceCache;
...
extern unsigned PIC_Ticks;
void RENDER_EndUpdate( bool abort ) {
	if (GCC_UNLIKELY(!render.updating))
		return;
	RENDER_DrawLine = RENDER_EmptyLineHandler;
	if (GCC_UNLIKELY(CaptureState & (CAPTURE_IMAGE|CAPTURE_VIDEO))) {
		unsigned pitch, flags;
		flags = 0;
		...
		CAPTURE_AddImage( render.src.width, render.src.height, render.src.bpp, pitch,
			flags, fps, (System::Byte *)&scalerSourceCache, (System::Byte*)&render.pal.rgb );
	}
...
static unsigned MakeAspectTable(unsigned skip,unsigned height,double scaley,unsigned miny) {
	unsigned i;
	double lines=0;
	unsigned linesadded=0;
	for (i=0;i<skip;i++)
		Scaler_Aspect[i] = 0;
	...
	for (i=skip;i<height;i++) {
		lines += scaley;
		if (lines >= miny) {
			unsigned templines = (unsigned)lines;
			lines -= templines;
...
static void RENDER_Reset( void ) {
	unsigned width=render.src.width;
	unsigned height=render.src.height;
	...
	unsigned gfx_flags, xscale, yscale;
...
//		LOG_MSG("Scaler:%s",simpleBlock->name);
	...
	case 8:
		render.src.start = ( render.src.width * 1) / sizeof(unsigned);
		if (gfx_flags & GFX_CAN_8)
			...
			break;
	case 15:
			render.src.start = ( render.src.width * 2) / sizeof(unsigned);
	...
	case 16:
			...
			break;
	case 32:
			render.src.start = ( render.src.width * 4) / sizeof(unsigned);
			gfx_flags |= GFX_LOVE_32;
			gfx_flags = (gfx_flags & ~GFX_CAN_8) | GFX_RGBONLY;
			break;
	}
	...
	width *= xscale;
	unsigned skip = complexBlock ? 1 : 0;
...
void RENDER_SetSize(unsigned width,unsigned height,unsigned bpp,float fps,double ratio,bool dblw,bool dblh) {
	RENDER_Halt( );
...
void RENDER_Init(Section * sec) {
	...
	unsigned scalersize = render.scale.size;
