...
union GenReg32 {
	System::UInt32 dword[1];
	System::UInt16 word[2];
	System::Byte byte[4];
};
...
static INLINE System::UInt16 SegValue(SegNames index) {
	return (System::UInt16)Segs.val[index];
}
	
static INLINE RealPt RealMakeSeg(SegNames index,System::UInt16 off) {
	return RealMake(SegValue(index),off);	
}


static INLINE void SegSet16(unsigned index,System::UInt16 val) {
	Segs.val[index]=val;
