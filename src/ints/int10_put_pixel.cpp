...
static System::Byte cga_masks[4]={0x3f,0xcf,0xf3,0xfc};
static System::Byte cga_masks2[8]={0x7f,0xbf,0xdf,0xef,0xf7,0xfb,0xfd,0xfe};
...
void INT10_PutPixel(System::UInt16 x,System::UInt16 y,System::Byte page,System::Byte color) {
	...
	case M_CGA4:
		{
				if (real_readb(BIOSMEM_SEG,BIOSMEM_CURRENT_MODE)<=5) {
					System::UInt16 off=(y>>1)*80+(x>>2);
					if (y&1) off+=8*1024;

					System::Byte old=real_readb(0xb800,off);
					if (color & 0x80) {
						color&=3;
						old^=color << (2*(3-(x&3)));
					} else {
					...
					System::UInt16 off=(y>>2)*160+((x>>2)&(~1));
					off+=(8*1024) * (y & 3);

					System::UInt16 old=real_readw(0xb800,off);
					if (color & 0x80) {
						old^=(color&1) << (7-(x&7));
	...
	case M_TANDY16:
		{
			IO_Write(0x3d4,0x09);
			System::Byte scanlines_m1=IO_Read(0x3d5);
			System::UInt16 off=(y>>((scanlines_m1==1)?1:2))*(CurMode->swidth>>1)+(x>>1);
			off+=(8*1024) * (y & scanlines_m1);
			System::Byte old=real_readb(0xb800,off);
			System::Byte p[2];
			p[1] = (old >> 4) & 0xf;
			p[0] = old & 0xf;
			unsigned ind = 1-(x & 0x1);
	...
	case M_EGA:
		{
			...
			IO_Write(0x3ce,0x8);System::Byte mask=128>>(x&7);IO_Write(0x3cf,mask);
			...
			if (CurMode->plength!=(unsigned)real_readw(BIOSMEM_SEG,BIOSMEM_PAGE_SIZE))
				LOG(LOG_INT10,LOG_ERROR)("PutPixel_EGA_p: %x!=%x",CurMode->plength,real_readw(BIOSMEM_SEG,BIOSMEM_PAGE_SIZE));
			if (CurMode->swidth!=(unsigned)real_readw(BIOSMEM_SEG,BIOSMEM_NB_COLS)*8)
	...
	case M_CGA4:
		{
			System::UInt16 off=(y>>1)*80+(x>>2);
			if (y&1) off+=8*1024;
			System::Byte val=real_readb(0xb800,off);
			*color=(val>>(((3-(x&3)))*2)) & 3 ;
		}
		break;
	case M_CGA2:
