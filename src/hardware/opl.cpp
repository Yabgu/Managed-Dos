...
static System::Int16 wavtable[WAVEPREC*3];	// wave form table

// vibrato/tremolo tables
static System::Int32 vib_table[VIBTAB_SIZE];
static System::Int32 trem_table[TREMTAB_SIZE*2];

static System::Int32 vibval_const[BLOCKBUF_SIZE];
static System::Int32 tremval_const[BLOCKBUF_SIZE];

// vibrato value tables (used per-operator)
static System::Int32 vibval_var1[BLOCKBUF_SIZE];
static System::Int32 vibval_var2[BLOCKBUF_SIZE];
//static System::Int32 vibval_var3[BLOCKBUF_SIZE];
//static System::Int32 vibval_var4[BLOCKBUF_SIZE];

// vibrato/trmolo value table pointers
static System::Int32 *vibval1, *vibval2, *vibval3, *vibval4;
static System::Int32 *tremval1, *tremval2, *tremval3, *tremval4;
...
static const System::Byte regbase2modop[44] = {
...
static const System::Byte regbase2op[44] = {
...
#else
static const System::Byte regbase2modop[22] = {
	0,1,2,0,1,2,0,0,3,4,5,3,4,5,0,0,6,7,8,6,7,8
};
static const System::Byte regbase2op[22] = {
...
static System::UInt32 waveform[8] = {
	WAVEPREC,
	WAVEPREC>>1,
	WAVEPREC,
	(WAVEPREC*3)>>2,
...
static System::UInt32 wavemask[8] = {
	WAVEPREC-1,
	WAVEPREC-1,
	(WAVEPREC>>1)-1,
	(WAVEPREC>>1)-1,
...
static System::UInt32 wavestart[8] = {
	0,
	WAVEPREC>>1,
	0,
	WAVEPREC>>2,
...
void operator_advance(op_type* op_pt, System::Int32 vib) {
	op_pt->wfpos = op_pt->tcount;						// waveform position
	
	// advance waveform time
	op_pt->tcount += op_pt->tinc;
	op_pt->tcount += (System::Int32)(op_pt->tinc)*vib/FIXEDPT;

	op_pt->generator_pos += generator_add;
}

void operator_advance_drums(op_type* op_pt1, System::Int32 vib1, op_type* op_pt2, System::Int32 vib2, op_type* op_pt3, System::Int32 vib3) {
	System::UInt32 c1 = op_pt1->tcount/FIXEDPT;
	System::UInt32 c3 = op_pt3->tcount/FIXEDPT;
	System::UInt32 phasebit = (((c1 & 0x88) ^ ((c1<<5) & 0x80)) | ((c3 ^ (c3<<2)) & 0x20)) ? 0x02 : 0x00;

	System::UInt32 noisebit = rand()&1;

	System::UInt32 snare_phase_bit = (((unsigned)((op_pt1->tcount/FIXEDPT) / 0x100))&1);

	//Hihat
	System::UInt32 inttm = (phasebit<<8) | (0x34<<(phasebit ^ (noisebit<<1)));
	op_pt1->wfpos = inttm*FIXEDPT;				// waveform position
	// advance waveform time
	op_pt1->tcount += op_pt1->tinc;
	op_pt1->tcount += (System::Int32)(op_pt1->tinc)*vib1/FIXEDPT;
	...
	op_pt2->tcount += (System::Int32)(op_pt2->tinc)*vib2/FIXEDPT;
	...
	op_pt3->tcount += (System::Int32)(op_pt3->tinc)*vib3/FIXEDPT;
...
void operator_output(op_type* op_pt, System::Int32 modulator, System::Int32 trem) {
	if (op_pt->op_state != OF_TYPE_OFF) {
		op_pt->lastcval = op_pt->cval;
		System::UInt32 i = (System::UInt32)((op_pt->wfpos+modulator)/FIXEDPT);
		...
		op_pt->cval = (System::Int32)(op_pt->step_amp*op_pt->vol*op_pt->cur_wform[i&op_pt->cur_wmask]*trem/16.0);
	}
}
...
void operator_sustain(op_type* op_pt) {
	System::UInt32 num_steps_add = op_pt->generator_pos/FIXEDPT;	// number of (standardized) samples
	for (System::UInt32 ct=0; ct<num_steps_add; ct++) {
		op_pt->cur_env_step++;
	}
...
void change_attackrate(unsigned regbase, op_type* op_pt) {
	...
	if (attackrate) {
		...
		static System::Byte step_skip_mask[5] = {0xff, 0xfe, 0xee, 0xba, 0xaa}; 
...
void change_decayrate(unsigned regbase, op_type* op_pt) {
...
void change_releaserate(unsigned regbase, op_type* op_pt) {
...
void change_sustainlevel(unsigned regbase, op_type* op_pt) {
...
void change_waveform(unsigned regbase, op_type* op_pt) {
#if defined(OPLTYPE_IS_OPL3)
...
void change_keepsustain(unsigned regbase, op_type* op_pt) {
...
void change_vibrato(unsigned regbase, op_type* op_pt) {
...
void change_feedback(unsigned chanbase, op_type* op_pt) {
	int feedback = adlibreg[ARC_FEEDBACK+chanbase]&14;
	if (feedback) op_pt->mfbi = (System::Int32)(pow(FL2,(fltype)((feedback>>1)+8)));
	else op_pt->mfbi = 0;
}

void change_frequency(unsigned chanbase, unsigned regbase, op_type* op_pt) {
	// frequency
	System::UInt32 frn = ((((System::UInt32)adlibreg[ARC_KON_BNUM+chanbase])&3)<<8) + (System::UInt32)adlibreg[ARC_FREQ_NUM+chanbase];
	// block number/octave
	System::UInt32 oct = ((((System::UInt32)adlibreg[ARC_KON_BNUM+chanbase])>>2)&7);
	op_pt->freq_high = (System::Int32)((frn>>7)&7);

	// keysplit
	System::UInt32 note_sel = (adlibreg[8]>>6)&1;
	...
	// 20+a0+b0:
	op_pt->tinc = (System::UInt32)((((fltype)(frn<<oct))*frqmul[adlibreg[ARC_TVS_KSR_MUL+regbase]&15]));
	// 40+a0+b0:
...
void enable_operator(unsigned regbase, op_type* op_pt, System::UInt32 act_type) {
...
void disable_operator(op_type* op_pt, System::UInt32 act_type) {
...
void adlib_init(System::UInt32 samplerate) {
	int i, j, oct;

	int_samplerate = samplerate;

	generator_add = (System::UInt32)(INTFREQU*FIXEDPT/int_samplerate);
	...
	vibtab_add = static_cast<System::UInt32>(VIBTAB_SIZE*FIXEDPT_LFO/8192*INTFREQU/int_samplerate);
	vibtab_pos = 0;
	...
	System::Int32 trem_table_int[TREMTAB_SIZE];
	...
	for (i=0; i<TREMTAB_SIZE; i++) {
		...
		fltype trem_val2=(fltype)((fltype)((System::Int32)(trem_table_int[i]/4))*1.2/6.0/6.0);		// 1.2db (larger stepping)

		trem_table[i] = (System::Int32)(pow(FL2,trem_val1)*FIXEDPT);
		trem_table[TREMTAB_SIZE+i] = (System::Int32)(pow(FL2,trem_val2)*FIXEDPT);
	}

	// tremolo at 3.7hz
	tremtab_add = (System::UInt32)((fltype)TREMTAB_SIZE * TREM_FREQ * FIXEDPT_LFO / (fltype)int_samplerate);
	tremtab_pos = 0;

	for (i=0; i<BLOCKBUF_SIZE; i++) tremval_const[i] = FIXEDPT;


	static unsigned initfirstime = 0;
	if (!initfirstime) {
		initfirstime = 1;

		// create waveform tables
		for (i=0;i<(WAVEPREC>>1);i++) {
			wavtable[(i<<1)  +WAVEPREC]	= (System::Int16)(16384*sin((fltype)((i<<1)  )*PI*2/WAVEPREC));
			wavtable[(i<<1)+1+WAVEPREC]	= (System::Int16)(16384*sin((fltype)((i<<1)+1)*PI*2/WAVEPREC));
			wavtable[i]					= wavtable[(i<<1)  +WAVEPREC];
			// alternative: (zero-less)
/*			wavtable[(i<<1)  +WAVEPREC]	= (System::Int16)(16384*sin((fltype)((i<<2)+1)*PI/WAVEPREC));
			wavtable[(i<<1)+1+WAVEPREC]	= (System::Int16)(16384*sin((fltype)((i<<2)+3)*PI/WAVEPREC));
		...
		kslev[7][8] = 48;
		for (i=9;i<16;i++) kslev[7][i] = (System::Byte)(i+41);
		for (j=6;j>=0;j--) {
			for (i=0;i<16;i++) {
				oct = (int)kslev[j+1][i]-8;
				if (oct < 0) oct = 0;
				kslev[j][i] = (System::Byte)oct;
			}
		}
	}
...
void adlib_write(unsigned idx, System::Byte val) {
	System::UInt32 second_set = idx&0x100;
	adlibreg[idx] = val;
	...
	case ARC_TVS_KSR_MUL+0x10: {
		...
		int num = idx&7;
		unsigned base = (idx-ARC_TVS_KSR_MUL)&0xff;
		if ((num<6) && (base<22)) {
	...
	case ARC_KSL_OUTLEV+0x10: {
		...
		int num = idx&7;
		unsigned base = (idx-ARC_KSL_OUTLEV)&0xff;
		if ((num<6) && (base<22)) {
			unsigned modop = regbase2modop[second_set?(base+22):base];
			unsigned chanbase = second_set?(modop-18+ARC_SECONDSET):modop;
	...
	case ARC_ATTR_DECR+0x10: {
		// attack/decay rates
		int num = idx&7;
		unsigned base = (idx-ARC_ATTR_DECR)&0xff;
		if ((num<6) && (base<22)) {
			unsigned regbase = base+second_set;
	...
	case ARC_SUSL_RELR+0x10: {
		...
		int num = idx&7;
		unsigned base = (idx-ARC_SUSL_RELR)&0xff;
		if ((num<6) && (base<22)) {
			unsigned regbase = base+second_set;
	...
	case ARC_FREQ_NUM: {
		...
		unsigned base = (idx-ARC_FREQ_NUM)&0xff;
		if (base<9) {
	...
	case ARC_KON_BNUM: {
		...
		// regular 0xb0-0xb8
		unsigned base = (idx-ARC_KON_BNUM)&0xff;
		if (base<9) {
			int opbase = second_set?(base+18):base;
#if defined(OPLTYPE_IS_OPL3)
			if ((adlibreg[0x105]&1) && op[opbase].is_4op_attached) break;
	...
	case ARC_FEEDBACK: {
		...
		unsigned base = (idx-ARC_FEEDBACK)&0xff;
		if (base<9) {
			int opbase = second_set?(base+18):base;
			unsigned chanbase = base+second_set;
	...
	case ARC_WAVE_SEL+0x10: {
		int num = idx&7;
		unsigned base = (idx-ARC_WAVE_SEL)&0xff;
		if ((num<6) && (base<22)) {
#if defined(OPLTYPE_IS_OPL3)
...
unsigned adlib_reg_read(unsigned port) {
#if defined(OPLTYPE_IS_OPL3)
...
void adlib_write_index(unsigned port, System::Byte val) {
	opl_index = val;
#if defined(OPLTYPE_IS_OPL3)
	if ((port&3)!=0) {
		// possibly second set
...
static void OPL_INLINE clipit16(System::Int32 ival, System::Int16* outval) {
	if (ival<32768) {
		if (ival>-32769) {
			*outval=(System::Int16)ival;
		} else {
			*outval = -32768;
		}
	} else {
...
void adlib_getsample(System::Int16* sndptr, int numsamples) {
	int i, endsamples;
	op_type* cptr;

	System::Int32 outbufl[BLOCKBUF_SIZE];
#if defined(OPLTYPE_IS_OPL3)
	// second output buffer (right channel for opl3 stereo)
	System::Int32 outbufr[BLOCKBUF_SIZE];
#endif

	// vibrato/tremolo lookup tables (global, to possibly be used by all operators)
	System::Int32 vib_lut[BLOCKBUF_SIZE];
	System::Int32 trem_lut[BLOCKBUF_SIZE];
	...
	for (int cursmp=0; cursmp<samples_to_process; cursmp+=endsamples) {
		...
		memset((void*)&outbufl,0,endsamples*sizeof(System::Int32));
#if defined(OPLTYPE_IS_OPL3)
		// clear second output buffer (opl3 stereo)
		if (adlibreg[0x105]&1) memset((void*)&outbufr,0,endsamples*sizeof(System::Int32));
#endif

		// calculate vibrato/tremolo lookup tables
		System::Int32 vib_tshift = ((adlibreg[ARC_PERC_MODE]&0x40)==0) ? 1 : 0;	// 14cents/7cents switching
		...
		if (adlibreg[ARC_PERC_MODE]&0x20) {
			...
			if (adlibreg[ARC_FEEDBACK+6]&1) {
				...
				if (cptr[9].op_state != OF_TYPE_OFF) {
					if (cptr[9].vibrato) {
						vibval1 = vibval_var1;
						for (i=0;i<endsamples;i++)
							vibval1[i] = (System::Int32)((vib_lut[i]*cptr[9].freq_high/8)*FIXEDPT*VIBFAC);
					...
					for (i=0;i<endsamples;i++) {
						...
						System::Int32 chanval = cptr[9].cval*2;
						CHANVAL_OUT
					}
				}
			} else {
				...
				if ((cptr[9].op_state != OF_TYPE_OFF) || (cptr[0].op_state != OF_TYPE_OFF)) {
					if ((cptr[0].vibrato) && (cptr[0].op_state != OF_TYPE_OFF)) {
						vibval1 = vibval_var1;
						for (i=0;i<endsamples;i++)
							vibval1[i] = (System::Int32)((vib_lut[i]*cptr[0].freq_high/8)*FIXEDPT*VIBFAC);
					} else vibval1 = vibval_const;
					if ((cptr[9].vibrato) && (cptr[9].op_state != OF_TYPE_OFF)) {
						vibval2 = vibval_var2;
						for (i=0;i<endsamples;i++)
							vibval2[i] = (System::Int32)((vib_lut[i]*cptr[9].freq_high/8)*FIXEDPT*VIBFAC);
			...
			if (op[8].op_state != OF_TYPE_OFF) {
				...
				if (cptr[0].vibrato) {
					vibval3 = vibval_var1;
					for (i=0;i<endsamples;i++)
						vibval3[i] = (System::Int32)((vib_lut[i]*cptr[0].freq_high/8)*FIXEDPT*VIBFAC);
				...
				for (i=0;i<endsamples;i++) {
					...
					System::Int32 chanval = cptr[0].cval*2;
					CHANVAL_OUT
				}
			}
			...
			if ((op[7].op_state != OF_TYPE_OFF) || (op[16].op_state != OF_TYPE_OFF) ||
				...
				if ((cptr[9].vibrato) && (cptr[9].op_state == OF_TYPE_OFF)) {
					vibval4 = vibval_var2;
					for (i=0;i<endsamples;i++)
						vibval4[i] = (System::Int32)((vib_lut[i]*cptr[9].freq_high/8)*FIXEDPT*VIBFAC);
				...
				for (i=0;i<endsamples;i++) {
					...
					System::Int32 chanval = (op[7].cval + op[7+9].cval + op[8+9].cval)*2;
					CHANVAL_OUT
				}
			}
		}

		unsigned max_channel = NUM_CHANNELS;
#if defined(OPLTYPE_IS_OPL3)
		...
		for (int cur_ch=max_channel-1; cur_ch>=0; cur_ch--) {
			...
			if (adlibreg[ARC_FEEDBACK+k]&1) {
					...
					if (adlibreg[ARC_FEEDBACK+k+3]&1) {
						...
						if (cptr[0].op_state != OF_TYPE_OFF) {
							...
							for (i=0;i<endsamples;i++) {
								...
								System::Int32 chanval = cptr[0].cval;
								CHANVAL_OUT
							}
						}
						...
						if ((cptr[3].op_state != OF_TYPE_OFF) || (cptr[9].op_state != OF_TYPE_OFF)) {
							...
							for (i=0;i<endsamples;i++) {
								...
								System::Int32 chanval = cptr[3].cval;
								CHANVAL_OUT
							}
						}
						...
						if (cptr[3+9].op_state != OF_TYPE_OFF) {
							...
							for (i=0;i<endsamples;i++) {
								...
								System::Int32 chanval = cptr[3+9].cval;
								CHANVAL_OUT
							}
						}
					} else {
				...
				for (i=0;i<endsamples;i++) {
					...
					System::Int32 chanval = cptr[9].cval + cptr[0].cval;
					CHANVAL_OUT
				}
			} else {
#if defined(OPLTYPE_IS_OPL3)
					...
					if (adlibreg[ARC_FEEDBACK+k+3]&1) {
						...
						if ((cptr[0].op_state != OF_TYPE_OFF) || (cptr[9].op_state != OF_TYPE_OFF)) {
							...
							for (i=0;i<endsamples;i++) {
								...
								System::Int32 chanval = cptr[9].cval;
								CHANVAL_OUT
							}
						}
