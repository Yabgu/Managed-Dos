...
struct ipxnetaddr {
	System::Byte netnum[4];   // Both are big endian
	System::Byte netnode[6];
} localIpxAddr;

System::UInt32 udpPort;
bool isIpxServer;
bool isIpxConnected;
gcroot<IPaddress^> ipxServConnIp = gcnew IPaddress();			// IPAddress for client connection to server
gcroot<System::Net::Sockets::Socket^> ipxClientSocket =
gcnew System::Net::Sockets::Socket(
	System::Net::Sockets::AddressFamily::InterNetwork,
	System::Net::Sockets::SocketType::Dgram,
	System::Net::Sockets::ProtocolType::Udp);

int UDPChannel;						// Channel used by UDP connection
System::Byte recvBuffer[IPXBUFFERSIZE];	// Incoming packet buffer

static RealPt ipx_callback;

packetBuffer incomingPacket;

static System::UInt16 socketCount;
static System::UInt16 opensockets[SOCKTABLESIZE];

static System::UInt16 swapByte(System::UInt16 sockNum) {
...
#ifdef IPX_DEBUGMSG 
unsigned ECBSerialNumber = 0;
unsigned ECBAmount = 0;
#endif


ECBClass::ECBClass(System::UInt16 segment, System::UInt16 offset) {
...
void ECBClass::writeDataBuffer(System::Byte* buffer, System::UInt16 length) {
	if (databuffer != 0) delete[] databuffer;
	databuffer = new System::Byte[length];
	memcpy(databuffer, buffer, length);
	buflen = length;

}
bool ECBClass::writeData() {
	unsigned length = buflen;
	System::Byte* buffer = databuffer;
	fragmentDescriptor tmpFrag;
	setInUseFlag(USEFLAG_AVAILABLE);
	unsigned fragCount = getFragCount();
	unsigned bufoffset = 0;
	for (unsigned i = 0; i < fragCount; i++) {
		getFragDesc(i, &tmpFrag);
		for (unsigned t = 0; t < tmpFrag.size; t++) {
...
System::UInt16 ECBClass::getSocket(void) {
	return swapByte(real_readw(RealSeg(ECBAddr), RealOff(ECBAddr) + 0xa));
}

System::Byte ECBClass::getInUseFlag(void) {
	return real_readb(RealSeg(ECBAddr), RealOff(ECBAddr) + 0x8);
}

void ECBClass::setInUseFlag(System::Byte flagval) {
	iuflag = flagval;
	real_writeb(RealSeg(ECBAddr), RealOff(ECBAddr) + 0x8, flagval);
}

void ECBClass::setCompletionFlag(System::Byte flagval) {
	real_writeb(RealSeg(ECBAddr), RealOff(ECBAddr) + 0x9, flagval);
}

System::UInt16 ECBClass::getFragCount(void) {
	return real_readw(RealSeg(ECBAddr), RealOff(ECBAddr) + 34);
}

void ECBClass::getFragDesc(System::UInt16 descNum, fragmentDescriptor *fragDesc) {
	System::UInt16 memoff = RealOff(ECBAddr) + 30 + ((descNum + 1) * 6);
...
void ECBClass::NotifyESR(void) {
	System::UInt32 ESRval = real_readd(RealSeg(ECBAddr), RealOff(ECBAddr) + 4);
...
void ECBClass::setImmAddress(System::Byte *immAddr) {
	for (unsigned i = 0; i < 6; i++)
		real_writeb(RealSeg(ECBAddr), RealOff(ECBAddr) + 28 + i, immAddr[i]);
}

void ECBClass::getImmAddress(System::Byte* immAddr) {
	for (unsigned i = 0; i < 6; i++)
...
static bool sockInUse(System::UInt16 sockNum) {
...
static void OpenSocket(void) {
	System::UInt16 sockNum, sockAlloc;
...
static void IPX_AES_EventHandler(unsigned param)
{
	ECBClass* tmpECB = ECBList;
	ECBClass* tmp2ECB;
	while (tmpECB != 0) {
		tmp2ECB = tmpECB->nextECB;
		if (tmpECB->iuflag == USEFLAG_AESCOUNT && param == (unsigned)tmpECB->ECBAddr) {
...
static void handleIpxRequest(void) {
	...
	case 0x0007:	// SCHEDULE SPECIAL IPX EVENT
	{
		...
		PIC_AddEvent(IPX_AES_EventHandler,
			(1000.0f / (1193182.0f / 65536.0f))*(float)reg_ax, (unsigned)tmpECB->ECBAddr);
		...
		while (tmpECB) {
			...
			if (tmpECB->ECBAddr == ecbaddress) {
				if (tmpECB->getInUseFlag() == USEFLAG_AESCOUNT)
					PIC_RemoveSpecificEvents(IPX_AES_EventHandler, (unsigned)ecbaddress);
				tmpECB->setInUseFlag(USEFLAG_AVAILABLE);
				tmpECB->setCompletionFlag(COMP_CANCELLED);
				delete tmpECB;
				reg_al = 0;	// Success
	...
	case 0x0009:		// Get internetwork address
	{
		...
		System::Byte * addrptr = (System::Byte *)&localIpxAddr;
...
// Entrypoint handler
unsigned IPX_Handler(void) {
	handleIpxRequest();
	return CBRET_NONE;
}

// INT 7A handler
unsigned IPX_IntHandler(void) {
	handleIpxRequest();
	return CBRET_NONE;
}
...
static void pingAck(IPaddress^ retAddr) {
	...
	cli::pin_ptr<System::Byte> p = &packet[0];
	System::Byte * np = p;
	memcpy(np, &regHeader, sizeof(regHeader));
	ipxClientSocket->SendTo(packet, gcnew System::Net::IPEndPoint(retAddr->host, retAddr->port));
}
...
static void pingSend(void) {
	...
	System::Byte * np = p;
	memcpy(np, &regHeader, sizeof(regHeader));
	ipxClientSocket->SendTo(packet, gcnew System::Net::IPEndPoint(0xffffffff, 0xffff));
}

static void receivePacket(System::Byte *buffer, System::Int16 bufSize) {
	ECBClass *useECB;
	ECBClass *nextECB;
	System::UInt16 *bufword = (System::UInt16 *)buffer;
	System::UInt16 useSocket = swapByte(bufword[8]);
	IPXHeader * tmpHeader;
...
static void IPX_ClientLoop(void) {
	int numrecv;
	auto buff = gcnew cli::array<System::Byte>(IPXBUFFERSIZE); // (System::Byte *)recvBuffer;

	// Its amazing how much simpler UDP is than TCP

	int numberOfBytesRead = ipxClientSocket->Receive(buff);
	if (numberOfBytesRead > 0) {
		cli::pin_ptr<System::Byte> p = &buff[0];
		receivePacket(p, numberOfBytesRead);
	}
}
...
static void sendPacket(ECBClass* sendecb) {
	cli::array<System::Byte>^ outbuffer = gcnew cli::array<System::Byte>(IPXBUFFERSIZE);
	fragmentDescriptor tmpFrag;
	System::UInt16 i, fragCount, t;
	System::Int16 packetsize;
	int result;
	...
	for (i = 0; i < fragCount; i++) {
		...
		if (i == 0) {
			...
			// source netnum
			addrptr = (System::Byte *)&localIpxAddr.netnum;
			for (System::UInt16 m = 0; m < 4; m++) {
				real_writeb(tmpFrag.segment, tmpFrag.offset + m + 18, addrptr[m]);
			}
			// source node number
			addrptr = (System::Byte *)&localIpxAddr.netnode;
			for (System::UInt16 m = 0; m < 6; m++) {
	...
	{
		cli::pin_ptr<System::Byte> p = &outbuffer[0];

		System::UInt16 * wordptr = (System::UInt16 *)p;
		// Blank CRC
		//wordptr[0] = 0xffff;
		// Length
	...
	LOG_IPX("SEND crc:%2x", packetCRC(outbuffer, packetsize));

	if (!isloopback) {
		/*outPacket.channel = UDPChannel;
		outPacket.data = (System::Byte *)&outbuffer[0];
		outPacket.len = packetsize;
		outPacket.maxlen = packetsize;*/
		// Since we're using a channel, we won't send the IP address again

		ipxClientSocket->Send(outbuffer, 0, packetsize, System::Net::Sockets::SocketFlags::None);
	...
	if (isloopback || islocalbroadcast) {
		...
		pin_ptr<System::Byte> p = &outbuffer[0];
		receivePacket(p, packetsize);
...
static bool pingCheck(IPXHeader * outHeader) {
	cli::array<System::Byte>^ buffer = gcnew cli::array<System::Byte>(1024);
	pin_ptr<System::Byte> nbuffer = &buffer[0];

	IPXHeader *regHeader = (IPXHeader *)nbuffer;

	int nrec = ipxClientSocket->Receive(buffer);
	if (nrec > 0)
	{
...
bool ConnectToServer(char const *strAddr) {
	IPXHeader *regHeader;
	cli::array<System::Byte>^ regHeaderBuff = gcnew cli::array<System::Byte>(sizeof(IPXHeader));
	cli::pin_ptr<System::Byte> nregHeader = &regHeaderBuff[0];
	regHeader = (IPXHeader *)nregHeader;
	...
	gcroot<cli::array<System::String^>^ > ipport = (gcnew System::String(strAddr))->Split(':');
	System::Net::IPAddress ^addr = System::Net::IPAddress::Parse(ipport[0]);
	System::Net::IPEndPoint^ endpoint = gcnew System::Net::IPEndPoint(addr->Address, System::Int16::Parse(ipport[1]));
	ipxClientSocket->Bind(endpoint);

	//ipxClientSocket = SDLNet_TCP_Open(&ipxServConnIp);
	SDLNet_Write16(0xffff, regHeader->checkSum);
	SDLNet_Write16(sizeof(IPXHeader), regHeader->length);
	...
	int numsent = ipxClientSocket->Send(regHeaderBuff);

	if (!numsent) {
		//LOG_MSG("IPX: Unable to connect to server: %s", gcnew System::ComponentModel::Win32Exception()->ToString());
		ipxClientSocket->Close();
		return false;
	}
	else {
		...
		while (true) {
			...
			CALLBACK_Idle();
			int nRead = ipxClientSocket->Receive(regHeaderBuff);
		...
		if (cmd->FindCommand(1, temp_line)) {
			...
			if (strcasecmp("startserver", temp_line.c_str()) == 0) {
				if (!isIpxServer) {
					...
					startsuccess = IPX_StartServer((System::UInt16)udpPort);
					if (startsuccess) {
					...
					for (i = 0; i < SOCKETTABLESIZE; i++) {
						if (IPX_isConnectedToServer(i, ptrAddr)) {
							WriteOut("     %d.%d.%d.%d from port %d\n", CONVIP(ptrAddr->host),
								System::Net::IPAddress::NetworkToHostOrder(ptrAddr->port));
						}
					}
					WriteOut("\n");
				}
...
public:
	IPX(Section* configuration) :Module_base(configuration) {
		...
		System::UInt16 call_ipxesr1 = callback_esr.Get_callback();
		...
		//save registers
		phys_writeb(phyDospage + 0, (System::Byte)0xFA);    // CLI
		phys_writeb(phyDospage + 1, (System::Byte)0x60);    // PUSHA
		phys_writeb(phyDospage + 2, (System::Byte)0x1E);    // PUSH DS
		phys_writeb(phyDospage + 3, (System::Byte)0x06);    // PUSH ES
		phys_writew(phyDospage + 4, (System::UInt16)0xA00F); // PUSH FS
		phys_writew(phyDospage + 6, (System::UInt16)0xA80F); // PUSH GS

		// callback
		phys_writeb(phyDospage + 8, (System::Byte)0xFE);  // GRP 4
		phys_writeb(phyDospage + 9, (System::Byte)0x38);  // Extra Callback instruction
		phys_writew(phyDospage + 10, call_ipxesr1);        // Callback identifier

		// register recreation
		phys_writew(phyDospage + 12, (System::UInt16)0xA90F); // POP GS
		phys_writew(phyDospage + 14, (System::UInt16)0xA10F); // POP FS
		phys_writeb(phyDospage + 16, (System::Byte)0x07);    // POP ES
		phys_writeb(phyDospage + 17, (System::Byte)0x1F);    // POP DS
		phys_writeb(phyDospage + 18, (System::Byte)0x61);    // POPA
		phys_writeb(phyDospage + 19, (System::Byte)0xCF);    // IRET: restores flags, CS, IP

		// IPX version 2.12
		//phys_writeb(phyDospage+27,(System::Byte)0x2);
		//phys_writeb(phyDospage+28,(System::Byte)0x12);
	...
	~IPX() {
		...
		for (unsigned i = 0; i < 32; i++)
			phys_writeb(phyDospage + i, (System::Byte)0x00);
