...
public:
	...
	virtual System::UInt16 GetInformation(void) { return 0x8084; }
	virtual bool ReadFromControlChannel(PhysPt bufptr,System::UInt16 size,System::UInt16 * retcode){return false;}
	virtual bool WriteToControlChannel(PhysPt bufptr,System::UInt16 size,System::UInt16 * retcode){return false;}
};

class device_LPT1 : public device_NUL {
public:
   	device_LPT1() { SetName("LPT1");}
	System::UInt16 GetInformation(void) { return 0x80A0; }
};

bool DOS_Device::Read(System::Byte * data,System::UInt16 * size) {
	return Devices[devnum]->Read(data,size);
}

bool DOS_Device::Write(System::Byte * data,System::UInt16 * size) {
	return Devices[devnum]->Write(data,size);
}

bool DOS_Device::Seek(System::UInt32 * pos,System::UInt32 type) {
...
System::UInt16 DOS_Device::GetInformation(void) { 
	return Devices[devnum]->GetInformation();
}

bool DOS_Device::ReadFromControlChannel(PhysPt bufptr,System::UInt16 size,System::UInt16 * retcode) { 
	return Devices[devnum]->ReadFromControlChannel(bufptr,size,retcode);
}

bool DOS_Device::WriteToControlChannel(PhysPt bufptr,System::UInt16 size,System::UInt16 * retcode) { 
...
//	if(!name || !(*name)) return DOS_DEVICES; //important, but makename does it
	...
	for(System::Byte index = 0;index < DOS_DEVICES;index++) {
		if (Devices[index]) {
...
//TODO Give the Device a real handler in low memory that responds to calls
	for(unsigned i = 0; i < DOS_DEVICES;i++) {
		if(!Devices[i]){
			Devices[i] = adddev;
