...
static System::Byte static_functionality[0x10]=
{
...
void INT10_LoadFont(PhysPt font,bool reload,unsigned count,unsigned offset,unsigned map,unsigned height) {
	...
	IO_Write(0x3ce,0x6);
	if (IS_VGA_ARCH) IO_Write(0x3cf,(System::Byte)old_6);	//odd/even and b8000 adressing
	...
	if (reload) {
		...
		real_writeb(BIOSMEM_SEG,BIOSMEM_CHAR_HEIGHT,(System::Byte)height);
...
void INT10_SetupRomMemoryChecksum(void) {
	if (IS_EGAVGA_ARCH) { //EGA/VGA. Just to be safe
		...
		System::Byte sum = 0;
		PhysPt rom_base = PhysMake(0xc000,0);
		unsigned last_rombyte = 32*1024 - 1;		//32 KB romsize
		for (unsigned i = 0;i < last_rombyte;i++)
			sum += phys_readb(rom_base + i);	//OVERFLOW IS OKAY
		sum = (System::Byte)((256 - (unsigned)sum)&0xff);
		phys_writeb(rom_base + last_rombyte,sum);
	}
}


System::Byte int10_font_08[256 * 8] = {
...
System::Byte int10_font_14[256 * 14] = {
  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
System::Byte int10_font_16[256 * 16] = {
  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
