...
void vga_write_p3d5(unsigned port,unsigned val,unsigned iolen);
unsigned DEBUG_EnableDebugger(void);

void vga_write_p3d4(unsigned port,unsigned val,unsigned iolen) {
	crtc(index)=val;
}

unsigned vga_read_p3d4(unsigned port,unsigned iolen) {
	return crtc(index);
}
...
//	if (crtc(index)>0x18) LOG_MSG("VGA CRCT write %X to reg %X",val,crtc(index));
	...
	case 0x03:	/* End Horizontal Blanking Register */
		...
		/*
			0-4	Horizontal Blanking ends when the last 6 int of the character
	...
	case 0x05:	/* End Horizontal Retrace Register */
		...
		/*
			0-4	Horizontal Retrace ends when the last 5 int of the character counter
				equals this value.
			5-6	Number of character clocks to delay start of display after Horizontal
				Retrace.
	...
	case 0x06: /* Vertical Total Register */
		...
		/*	0-7	Lower 8 int of the Vertical Total. Bit 8 is found in 3d4h index 7
...
//TODO update cursor on screen
		...
		/*	0-7  Lower 8 int of the address of the cursor */
		break;
	case 0x10:	/* Vertical Retrace Start Register */
		crtc(vertical_retrace_start)=val;
		/*	
			0-7	Lower 8 int of Vertical Retrace Start. Vertical Retrace starts when
	...
	case 0x11:	/* Vertical Retrace End Register */
		...
		/*
			0-3	Vertical Retrace ends when the last 4 int of the line counter equals
				this value.
	...
	case 0x12:	/* Vertical Display End Register */
		if (val!=crtc(vertical_display_end)) {
			if (abs((int)val-(int)crtc(vertical_display_end))<3) {
		...
		/*
			0-7	Lower 8 int of Vertical Display End. The display ends when the line
	...
	case 0x15:	/* Start Vertical Blank Register */
		...
		/* 
			0-7	Lower 8 int of Vertical Blank Start. Vertical blanking starts when
				the line counter reaches this value. Bit 8 is found in 3d4h index 7
				bit 3.
		*/
		break;
	case 0x16:	/*  End Vertical Blank Register */
		crtc(end_vertical_blanking)=val;
		 /*
			0-6	Vertical blanking stops when the lower 7 int of the line counter
				equals this field. Some SVGA chips uses all 8 int!
		*/
		break;
...
//		VGA_DetermineMode();
	...
	case 0x18:	/* Line Compare Register */
		...
		/*
			0-7	Lower 8 int of the Line Compare. When the Line counter reaches this
...
unsigned vga_read_p3d5(unsigned port,unsigned iolen) {
