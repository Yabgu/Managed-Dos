...
static unsigned call_int16,call_irq1,call_irq6;

/* Nice table from BOCHS i should feel bad for ripping this */
#define none 0
static struct {
  System::UInt16 normal;
  System::UInt16 shift;
  System::UInt16 control;
  System::UInt16 alt;
...
bool BIOS_AddKeyToBuffer(System::UInt16 code) {
	if (mem_readb(BIOS_KEYBOARD_FLAGS2)&8) return true;
	System::UInt16 start,end,head,tail,ttail;
	if (machine==MCH_PCJR) {
...
static void add_key(System::UInt16 code) {
	if (code!=0) BIOS_AddKeyToBuffer(code);
}

static bool get_key(System::UInt16 &code) {
	System::UInt16 start,end,head,tail,thead;
	if (machine==MCH_PCJR) {
		/* should be done for cga and others as well, to be tested */
		start=0x1e;
		end=0x3e;
...
static bool check_key(System::UInt16 &code) {
	System::UInt16 head,tail;
	head =mem_readw(BIOS_KEYBOARD_BUFFER_HEAD);
	tail =mem_readw(BIOS_KEYBOARD_BUFFER_TAIL);
	if (head==tail) return false;
	code = real_readw(0x40,head);
...
static unsigned IRQ1_Handler(void) {
/* handling of the locks key is difficult as sdl only gives
 * states for numlock capslock. 
 */
	unsigned scancode=reg_al;	/* Read the code */

	System::Byte flags1,flags2,flags3,leds;
...
//LOG_MSG("key input %d %d %d %d",scancode,flags1,flags2,flags3);
	...
	case 0xb8:						/* Alt Released */
		...
		if( !( (flags3 &0x08) || (flags2 &0x02) ) ) { /* Both alt released */
			flags1 &= ~0x08;
			System::UInt16 token =mem_readb(BIOS_KEYBOARD_TOKEN);
			if(token != 0){
				add_key(token);
...
//	case 0x52:flags2|=128;break;//See numpad					/* Insert */
	...
	case 0x53: /* del . Not entirely correct, but works fine */
		...
		if(flags1 &0x08) {
			System::Byte token = mem_readb(BIOS_KEYBOARD_TOKEN);
			token = token*10 + (System::Byte)(scan_to_scanascii[scancode].alt&0xff);
...
/* In dosbox port 60 reads do this as well */
	System::Byte old61=IO_Read(0x61);
...
static bool IsEnhancedKey(System::UInt16 &key) {
...
static unsigned INT16_Handler(void) {
	System::UInt16 temp=0;
	switch (reg_ah) {
...
static void InitBiosSegment(void) {
	...
	System::Byte flag1 = 0;
	System::Byte leds = 16; /* Ack recieved */
