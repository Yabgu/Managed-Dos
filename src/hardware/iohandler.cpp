...
static unsigned IO_ReadBlocked(unsigned /*port*/, unsigned /*iolen*/) {
	return ~0;
}

static void IO_WriteBlocked(unsigned /*port*/, unsigned /*val*/, unsigned /*iolen*/) {
}

static unsigned IO_ReadDefault(unsigned port, unsigned iolen) {
	switch (iolen) {
	case 1:
...
void IO_WriteDefault(unsigned port, unsigned val, unsigned iolen) {
	switch (iolen) {
	case 1:
...
void IO_RegisterReadHandler(unsigned port, IO_ReadHandler * handler, unsigned mask, unsigned range) {
	while (range--) {
...
void IO_RegisterWriteHandler(unsigned port, IO_WriteHandler * handler, unsigned mask, unsigned range) {
	while (range--) {
...
void IO_FreeReadHandler(unsigned port, unsigned mask, unsigned range) {
	while (range--) {
...
void IO_FreeWriteHandler(unsigned port, unsigned mask, unsigned range) {
	while (range--) {
...
void IO_ReadHandleObject::Install(unsigned port, IO_ReadHandler * handler, unsigned mask, unsigned range) {
	if (!installed) {
		installed = true;
		m_port = port;
		m_mask = mask;
...
void IO_WriteHandleObject::Install(unsigned port, IO_WriteHandler * handler, unsigned mask, unsigned range) {
	if (!installed) {
		installed = true;
		m_port = port;
		m_mask = mask;
...
inline void IO_USEC_read_delay_old() {
	if (CPU_CycleMax > static_cast<System::Int32>((IODELAY_READ_MICROS*1000.0))) {
		// this could be calculated whenever CPU_CycleMax changes
		int delaycyc = static_cast<int>((CPU_CycleMax / 1000)*IODELAY_READ_MICROS);
...
inline void IO_USEC_write_delay_old() {
	if (CPU_CycleMax > static_cast<System::Int32>((IODELAY_WRITE_MICROS*1000.0))) {
		// this could be calculated whenever CPU_CycleMax changes
		int delaycyc = static_cast<int>((CPU_CycleMax / 1000)*IODELAY_WRITE_MICROS);
		if (CPU_Cycles > delaycyc) CPU_Cycles -= delaycyc;
		else CPU_Cycles = 0;
	}
}
...
#define IODELAY_READ_MICROSk (System::UInt32)(1024/1.0)
#define IODELAY_WRITE_MICROSk (System::UInt32)(1024/0.75)

inline void IO_USEC_read_delay() {
	int delaycyc = CPU_CycleMax / IODELAY_READ_MICROSk;
...
inline void IO_USEC_write_delay() {
	int delaycyc = CPU_CycleMax / IODELAY_WRITE_MICROSk;
...
static System::Byte crtc_index = 0;
const char* const len_type[] = { " 8","16","32" };
void log_io(unsigned width, bool write, unsigned port, unsigned val) {
	switch (width) {
	case 0:
		val &= 0xff;
		break;
	...
	if (write) {
		...
		if (port == 0x3d4) {
			if (width == 0) crtc_index = (System::Byte)val;
			else if (width == 1) crtc_index = (System::Byte)(val >> 8);
		}
...
void IO_WriteB(unsigned port, unsigned val) {
	...
	if (GCC_UNLIKELY(GETFLAG(VM) && (CPU_IO_Exception(port, 1)))) {
		...
		CPU_Push16(reg_ip);
		System::Byte old_al = reg_al;
		System::UInt16 old_dx = reg_dx;
		reg_al = val;
		reg_dx = port;
