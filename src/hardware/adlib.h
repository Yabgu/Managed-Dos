...
struct Timer {
	...
	void Start( const double& time, int scale ) {
		//Don't enable again
		if ( enabled ) {
			return;
		}
...
struct Chip {
	...
	bool Write( System::UInt32 addr, System::Byte val );
...
public:
	...
	virtual System::UInt32 WriteAddr( System::UInt32 port, System::Byte val ) = 0;
	//Write to a specific register in the chip
	virtual void WriteReg( System::UInt32 addr, System::Byte val ) = 0;
	//Generate a certain amount of samples
	virtual void Generate( MixerChannel* chan, unsigned samples ) = 0;
	//Initialize at a specific sample rate and mode
	virtual void Init( unsigned rate ) = 0;
	virtual ~Handler() {
	}
};

//The cache for 2 chips or an opl3
typedef System::Byte RegisterCache[512];
...
class Module: public Module_base {
	...
	union {
		System::UInt32 normal;
		System::Byte dual[2];
	} reg;
	void CacheWrite( System::UInt32 reg, System::Byte val );
	void DualWrite( System::Byte index, System::Byte reg, System::Byte val );
public:
	static OPL_Mode oplmode;
	MixerChannel* mixerChan;
	System::UInt32 lastUsed;				//Ticks when adlib was last used to turn of mixing after a few second
	...
	//Handle port writes
	void PortWrite( unsigned port, unsigned val, unsigned iolen );
	unsigned PortRead( unsigned port, unsigned iolen );
	void Init( Mode m );
