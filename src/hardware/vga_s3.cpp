...
void SVGA_S3_WriteCRTC(unsigned reg,unsigned val,unsigned iolen) {
	switch (reg) {
...
//TODO Base address
	...
	case 0x35:	/* CR35 CRT Register Lock */
		...
		/*
			0-3	CPU Base Address. 64k bank number. For the 801/5 and 928 see 3d4h
				index 51h int 2-3. For the 864/964 see index 6Ah.
			4	Lock Vertical Timing Registers (LOCK VTMG). Locks 3d4h index 6, 7
				(int 0,2,3,5,7), 9 bit 5, 10h, 11h int 0-3, 15h, 16h if set
	...
	case 0x43:	/* CR43 Extended Mode */
		...
		/*
			...
			(3d4h index 13h). (801/5,928) Only active if 3d4h index 51h int 4-5
			are 0
		*/
	...
	case 0x4c:  /* HGC start address high byte*/
		...
		if ((((unsigned)vga.s3.hgc.startaddr)<<10)+((64*64*2)/8) > vga.vmemsize) {
	...
	case 0x4e:  /* HGC pattern start X */
		vga.s3.hgc.posx = val & 0x3f;	// int 0-5
		break;
	case 0x4f:  /* HGC pattern start Y */
		vga.s3.hgc.posy = val & 0x3f;	// int 0-5
		break;
	...
	case 0x51:	/* Extended System Control 2 */
		vga.s3.reg_51=val & 0xc0;		//Only store int 6,7
		...
		/*
			...
			0-1	(928 +) Display Start Address bit 18-19
				int 16-17 are in index 31h int 4-5, int 0-15 are in 3d4h index
				0Ch,0Dh. For the 864/964 see 3d4h index 69h
			2	(80x) CPU BASE. CPU Base Address Bit 18.
			2-3	(928 +) Old CPU Base Address int 19-18.
				64K Bank register int 4-5. int 0-3 are in 3d4h index 35h.
				For the 864/964 see 3d4h index 6Ah
			4-5	Logical Screen Width Bit [8-9]. int 8-9 of the CRTC Offset register
	...
	case 0x55:	/* Extended Video DAC Control */
		...
		break;
		/*
			0-1	DAC Register Select int. Passed to the RS2 and RS3 pins on the
			...
			5	(80x,928) Hardware Cursor External Operation Mode. If set the two
				int of cursor data ,is output on the HC[0-1] pins for the video DAC
	...
	case 0x5e:	/* Extended Vertical Overflow */
		...
		/*
			0	Vertical Total bit 10. Bit 10 of the Vertical Total register (3d4h
				index 6). int 8 and 9 are in 3d4h index 7 bit 0 and 5.
			1	Vertical Display End bit 10. Bit 10 of the Vertical Display End
				register (3d4h index 12h). int 8 and 9 are in 3d4h index 7 bit 1
				and 6
			...
			4	Vertical Retrace Start bit 10. Bit 10 of the Vertical Start Retrace
				register (3d4h index 10h). int 8 and 9 are in 3d4h index 7 bit 2
				and 7.
...
unsigned SVGA_S3_ReadCRTC( unsigned reg, unsigned iolen) {
	switch (reg) {
...
//		return 0x44;	// Trio64 V+
	...
	case 0x31:	/* CR31 Memory Configuration */
//TODO mix in int from baseaddress;
	...
	case 0x69:	/* Extended System Control 3 */
		return (System::Byte)((vga.config.display_start & 0x1f0000)>>16); 
	case 0x6a:	/* Extended System Control 4 */
		return (System::Byte)(vga.svga.bank_read & 0x7f);
...
void SVGA_S3_WriteSEQ(unsigned reg,unsigned val,unsigned iolen) {
...
unsigned SVGA_S3_ReadSEQ(unsigned reg,unsigned iolen) {
	/* S3 specific group */
...
unsigned SVGA_S3_GetClock(void) {
	unsigned clock = (vga.misc_output >> 2) & 3;
	if (clock == 0)
		clock = 25175000;
	else if (clock == 1)
		clock = 28322000;
...
bool SVGA_S3_AcceptsMode(unsigned mode) {
