...
void MEM_BlockRead(PhysPt pt,void * data,unsigned size) {
	System::Byte * write=reinterpret_cast<System::Byte *>(data);
	while (size--) {
...
void MEM_BlockWrite(PhysPt pt,void const * const data,unsigned size) {
	System::Byte const * read = reinterpret_cast<System::Byte const * const>(data);
	while (size--) {
...
void MEM_StrCopy(PhysPt pt,char * data,unsigned size) {
	while (size--) {
		System::Byte r=mem_readb_inline(pt++);
		if (!r) break;
		*data++=r;
	}
	*data=0;
...
unsigned MEM_TotalPages(void) {
	return memory.pages;
}

unsigned MEM_FreeLargest(void) {
	unsigned size=0;unsigned largest=0;
	unsigned index=XMS_START;	
...
unsigned MEM_AllocatedPages(MemHandle handle) 
{
	unsigned pages = 0;
	while (handle>0) {
		pages++;
...
INLINE unsigned BestMatch(unsigned size) {
	unsigned index=XMS_START;	
	unsigned first=0;
	unsigned best=0xfffffff;
...
MemHandle MEM_AllocatePages(unsigned pages,bool sequence) {
	...
	if (!pages) return 0;
	if (sequence) {
		unsigned index=BestMatch(pages);
		if (!index) return 0;
		MemHandle * next=&ret;
		while (pages) {
			*next=index;
...
bool MEM_ReAllocatePages(MemHandle & handle,unsigned pages,bool sequence) {
	...
	MemHandle last;unsigned old_pages=0;
	while (index>0) {
		old_pages++;
		last=index;
...
void MEM_A20_Enable(bool enabled) {
	unsigned phys_base=enabled ? (1024/4) : 0;
	for (unsigned i=0;i<16;i++) PAGING_MapPage((1024/4)+i,phys_base+i);
...
void mem_unalignedwritew(PhysPt address,System::UInt16 val) {
	mem_writeb_inline(address,(System::Byte)val);val>>=8;
	mem_writeb_inline(address+1,(System::Byte)val);
}

void mem_unalignedwrited(PhysPt address,System::UInt32 val) {
	mem_writeb_inline(address,(System::Byte)val);val>>=8;
	mem_writeb_inline(address+1,(System::Byte)val);val>>=8;
	mem_writeb_inline(address+2,(System::Byte)val);val>>=8;
	mem_writeb_inline(address+3,(System::Byte)val);
}


bool mem_unalignedreadw_checked(PhysPt address, System::UInt16 * val) {
	System::Byte rval1,rval2;
	if (mem_readb_checked(address+0, &rval1)) return true;
	if (mem_readb_checked(address+1, &rval2)) return true;
	*val=(System::UInt16)(((System::Byte)rval1) | (((System::Byte)rval2) << 8));
	return false;
}

bool mem_unalignedreadd_checked(PhysPt address, System::UInt32 * val) {
	System::Byte rval1,rval2,rval3,rval4;
	if (mem_readb_checked(address+0, &rval1)) return true;
	if (mem_readb_checked(address+1, &rval2)) return true;
	if (mem_readb_checked(address+2, &rval3)) return true;
	if (mem_readb_checked(address+3, &rval4)) return true;
	*val=(System::UInt32)(((System::Byte)rval1) | (((System::Byte)rval2) << 8) | (((System::Byte)rval3) << 16) | (((System::Byte)rval4) << 24));
	return false;
}

bool mem_unalignedwritew_checked(PhysPt address,System::UInt16 val) {
	if (mem_writeb_checked(address,(System::Byte)(val & 0xff))) return true;val>>=8;
	if (mem_writeb_checked(address+1,(System::Byte)(val & 0xff))) return true;
	return false;
}

bool mem_unalignedwrited_checked(PhysPt address,System::UInt32 val) {
	if (mem_writeb_checked(address,(System::Byte)(val & 0xff))) return true;val>>=8;
	if (mem_writeb_checked(address+1,(System::Byte)(val & 0xff))) return true;val>>=8;
	if (mem_writeb_checked(address+2,(System::Byte)(val & 0xff))) return true;val>>=8;
	if (mem_writeb_checked(address+3,(System::Byte)(val & 0xff))) return true;
	return false;
}

System::Byte mem_readb(PhysPt address) {
	return mem_readb_inline(address);
}

System::UInt16 mem_readw(PhysPt address) {
	return mem_readw_inline(address);
}

System::UInt32 mem_readd(PhysPt address) {
	return mem_readd_inline(address);
}

void mem_writeb(PhysPt address,System::Byte val) {
	mem_writeb_inline(address,val);
}

void mem_writew(PhysPt address,System::UInt16 val) {
	mem_writew_inline(address,val);
}

void mem_writed(PhysPt address,System::UInt32 val) {
	mem_writed_inline(address,val);
}

static void write_p92(unsigned port,unsigned val,unsigned iolen) {	
...
void PreparePCJRCartRom(void) {
	...
	for (unsigned ct=0xd0;ct<0xe0;ct++) {
		memory.phandlers[ct] = &rom_page_handler;
	}
}
...
public:	
	MEMORY(Section* configuration):Module_base(configuration){
		...
		unsigned memsize=section->Get_int("memsize");
		...
		MemBase = new System::Byte[memsize*1024*1024];
