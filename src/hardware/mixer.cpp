...
static INLINE System::Int16 MIXER_CLIP(int SAMP) {
	if (SAMP < MAX_AUDIO) {
		if (SAMP > MIN_AUDIO)
			return SAMP;
...
static struct {
	System::Int32 work[MIXER_BUFSIZE][2];
	unsigned pos,done;
	unsigned needed, min_needed, max_needed;
	System::UInt32 tick_add,tick_remain;
	float mastervol[2];
...
System::Byte MixTemp[MIXER_BUFSIZE];
...
void MixerChannel::UpdateVolume(void) {
	volmul[0]=(int)((1 << MIXER_VOLSHIFT)*scale*volmain[0]*mixer.mastervol[0]);
	volmul[1]=(int)((1 << MIXER_VOLSHIFT)*scale*volmain[1]*mixer.mastervol[1]);
}
...
void MixerChannel::SetFreq(unsigned _freq) {
	freq_add=(_freq<<MIXER_SHIFT)/mixer.freq;
}

void MixerChannel::Mix(unsigned _needed) {
	needed=_needed;
	while (enabled && needed>done) {
		unsigned todo=needed-done;
		todo *= freq_add;
...
inline void MixerChannel::AddSamples(unsigned len, const Type* data) {
	int diff[2];
	unsigned mixpos=mixer.pos+done;
	freq_index&=MIXER_REMAIN;
	unsigned pos=0;unsigned new_pos;
...
thestart:
			...
			if ( sizeof( Type) == 1) {
				if (!signeddata) {
					if (stereo) {
						diff[0]=(((System::SByte)(data[pos*2+0] ^ 0x80)) << 8)-last[0];
						diff[1]=(((System::SByte)(data[pos*2+1] ^ 0x80)) << 8)-last[1];
					} else {
						diff[0]=(((System::SByte)(data[pos] ^ 0x80)) << 8)-last[0];
					}
				} else {
					if (stereo) {
				...
				if (signeddata) {
					if (stereo) {
						if (nativeorder) {
							...
							if ( sizeof( Type) == 2) {
								diff[0]=(System::Int16)host_readw((HostPt)&data[pos*2+0])-last[0];
								diff[1]=(System::Int16)host_readw((HostPt)&data[pos*2+1])-last[1];
							} else {
								diff[0]=(System::Int32)host_readd((HostPt)&data[pos*2+0])-last[0];
								diff[1]=(System::Int32)host_readd((HostPt)&data[pos*2+1])-last[1];
							}
						}
					} else {
						if (nativeorder) {
							...
							if ( sizeof( Type) == 2) {
								diff[0]=(System::Int16)host_readw((HostPt)&data[pos])-last[0];
							} else {
								diff[0]=(System::Int32)host_readd((HostPt)&data[pos])-last[0];
							}
						}
					}
				} else {
					if (stereo) {
						if (nativeorder) {
							diff[0]=(int)data[pos*2+0]-32768-last[0];
							diff[1]=(int)data[pos*2+1]-32768-last[1];
						} else {
							if ( sizeof( Type) == 2) {
								diff[0]=(int)host_readw((HostPt)&data[pos*2+0])-32768-last[0];
								diff[1]=(int)host_readw((HostPt)&data[pos*2+1])-32768-last[1];
							} else {
								diff[0]=(int)host_readd((HostPt)&data[pos*2+0])-32768-last[0];
								diff[1]=(int)host_readd((HostPt)&data[pos*2+1])-32768-last[1];
							}
						}
					} else {
						if (nativeorder) {
							diff[0]=(int)data[pos]-32768-last[0];
						} else {
							if ( sizeof( Type) == 2) {
								diff[0]=(int)host_readw((HostPt)&data[pos])-32768-last[0];
							} else {
								diff[0]=(int)host_readd((HostPt)&data[pos])-32768-last[0];
							}
						}
					}
				}
...
void MixerChannel::AddStretched(unsigned len,System::Int16 * data) {
	if (done>=needed) {
		LOG_MSG("Can't add, buffer full");	
		return;
	}
	unsigned outlen=needed-done;int diff;
	freq_index=0;
	unsigned temp_add=(len << MIXER_SHIFT)/outlen;
	unsigned mixpos=mixer.pos+done;done=needed;
	unsigned pos=0;
	diff=data[0]-last[0];
	while (outlen--) {
		unsigned new_pos=freq_index >> MIXER_SHIFT;
		if (pos<new_pos) {
			pos=new_pos;
			last[0]+=diff;
...
void MixerChannel::AddSamples_m8(unsigned len, const System::Byte * data) {
	AddSamples<System::Byte,false,false,true>(len,data);
}
void MixerChannel::AddSamples_s8(unsigned len,const System::Byte * data) {
	AddSamples<System::Byte,true,false,true>(len,data);
}
void MixerChannel::AddSamples_m8s(unsigned len,const System::SByte * data) {
	AddSamples<System::SByte,false,true,true>(len,data);
}
void MixerChannel::AddSamples_s8s(unsigned len,const System::SByte * data) {
	AddSamples<System::SByte,true,true,true>(len,data);
}
void MixerChannel::AddSamples_m16(unsigned len,const System::Int16 * data) {
	AddSamples<System::Int16,false,true,true>(len,data);
}
void MixerChannel::AddSamples_s16(unsigned len,const System::Int16 * data) {
	AddSamples<System::Int16,true,true,true>(len,data);
}
void MixerChannel::AddSamples_m16u(unsigned len,const System::UInt16 * data) {
	AddSamples<System::UInt16,false,false,true>(len,data);
}
void MixerChannel::AddSamples_s16u(unsigned len,const System::UInt16 * data) {
	AddSamples<System::UInt16,true,false,true>(len,data);
}
void MixerChannel::AddSamples_m32(unsigned len,const System::Int32 * data) {
	AddSamples<System::Int32,false,true,true>(len,data);
}
void MixerChannel::AddSamples_s32(unsigned len,const System::Int32 * data) {
	AddSamples<System::Int32,true,true,true>(len,data);
}
void MixerChannel::AddSamples_m16_nonnative(unsigned len,const System::Int16 * data) {
	AddSamples<System::Int16,false,true,false>(len,data);
}
void MixerChannel::AddSamples_s16_nonnative(unsigned len,const System::Int16 * data) {
	AddSamples<System::Int16,true,true,false>(len,data);
}
void MixerChannel::AddSamples_m16u_nonnative(unsigned len,const System::UInt16 * data) {
	AddSamples<System::UInt16,false,false,false>(len,data);
}
void MixerChannel::AddSamples_s16u_nonnative(unsigned len,const System::UInt16 * data) {
	AddSamples<System::UInt16,true,false,false>(len,data);
}
void MixerChannel::AddSamples_m32_nonnative(unsigned len,const System::Int32 * data) {
	AddSamples<System::Int32,false,true,false>(len,data);
}
void MixerChannel::AddSamples_s32_nonnative(unsigned len,const System::Int32 * data) {
	AddSamples<System::Int32,true,true,false>(len,data);
}
...
void MixerChannel::FillUp(void) {
	...
	Mix((unsigned)(index*mixer.needed));
	SDL_UnlockAudio();
}
...
static void MIXER_MixData(unsigned needed) {
	MixerChannel * chan=mixer.channels;
	while (chan) {
		chan->Mix(needed);
		chan=chan->next;
	...
	if (CaptureState & (CAPTURE_WAVE|CAPTURE_VIDEO)) {
		System::Int16 convert[1024][2];
		unsigned added=needed-mixer.done;
		if (added>1024) 
			added=1024;
		unsigned readpos=(mixer.pos+mixer.done)&MIXER_BUFMASK;
		...
		CAPTURE_AddWave( mixer.freq, added, (System::Int16*)convert );
	}
...
static void MIXER_Mix_NoSound(void) {
	...
	for (unsigned i=0;i<mixer.needed;i++) {
...
static void MIXER_CallBack(void * userdata, System::Byte *stream, int len) {
	unsigned need=(unsigned)len/MIXER_SSIZE;
	System::Int16 * output=(System::Int16 *)stream;
	unsigned reduce;
	unsigned pos, index, index_add;
	int sample;
...
//		LOG_MSG("Full underrun need %d, have %d, min %d", need, mixer.done, mixer.min_needed);
		...
		unsigned left = mixer.done - need;
		if (left < mixer.min_needed) {
			if( !Mixer_irq_important() ) {
				unsigned needed = mixer.needed - need;
				unsigned diff = (mixer.min_needed>needed?mixer.min_needed:needed) - left;
...
//			LOG_MSG("regular run need %d, have %d, min %d, left %d", need, mixer.done, mixer.min_needed, left);
			 ...
			 */
			unsigned diff = left - mixer.min_needed;
...
//		LOG_MSG("overflow run need %d, have %d, min %d", need, mixer.done, mixer.min_needed);
	...
	if(need != reduce) {
		while (need--) {
			unsigned i = (pos + (index >> MIXER_SHIFT )) & MIXER_BUFMASK;
			index += index_add;
...
MixerChannel* MixerObject::Install(MIXER_Handler handler,unsigned freq,const char * name){
	if(!installed) {
...
void MIXER_Init(Section* sec) {
	...
	spec.userdata=NULL;
	spec.samples=(System::UInt16)mixer.blocksize;
