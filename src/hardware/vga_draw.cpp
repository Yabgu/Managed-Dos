...
typedef System::Byte * (* VGA_Line_Handler)(unsigned vidstart, unsigned line);

static VGA_Line_Handler VGA_DrawLine;
static System::Byte TempLine[SCALER_MAXWIDTH * 4];

static System::Byte * VGA_Draw_1BPP_Line(unsigned vidstart, unsigned line) {
	const System::Byte *base = vga.tandy.draw_base + ((line & vga.tandy.line_mask) << vga.tandy.line_shift);
	System::UInt32 *draw = (System::UInt32 *)TempLine;
	for (unsigned x=vga.draw.blocks;x>0;x--, vidstart++) {
		unsigned val = base[(vidstart & (8 * 1024 -1))];
...
static System::Byte * VGA_Draw_2BPP_Line(unsigned vidstart, unsigned line) {
	const System::Byte *base = vga.tandy.draw_base + ((line & vga.tandy.line_mask) << vga.tandy.line_shift);
	System::UInt32 * draw=(System::UInt32 *)TempLine;
	for (unsigned x=0;x<vga.draw.blocks;x++) {
		unsigned val = base[vidstart & vga.tandy.addr_mask];
		vidstart++;
...
static System::Byte * VGA_Draw_2BPPHiRes_Line(unsigned vidstart, unsigned line) {
	const System::Byte *base = vga.tandy.draw_base + ((line & vga.tandy.line_mask) << vga.tandy.line_shift);
	System::UInt32 * draw=(System::UInt32 *)TempLine;
	for (unsigned x=0;x<vga.draw.blocks;x++) {
		unsigned val1 = base[vidstart & vga.tandy.addr_mask];
		++vidstart;
		unsigned val2 = base[vidstart & vga.tandy.addr_mask];
		++vidstart;
...
static System::Byte * VGA_Draw_CGA16_Line(unsigned vidstart, unsigned line) {
	const System::Byte *base = vga.tandy.draw_base + ((line & vga.tandy.line_mask) << vga.tandy.line_shift);
	const System::Byte *reader = base + vidstart;
	System::UInt32 * draw=(System::UInt32 *)TempLine;
	...
	unsigned i = 0,temp1,temp2,temp3,temp4;
	for (unsigned x=0;x<vga.draw.blocks;x++) {
		unsigned val1 = *reader++;
		unsigned val2 = val1&0xf;
		val1 >>= 4;
...
static System::Byte * VGA_Draw_4BPP_Line(unsigned vidstart, unsigned line) {
	const System::Byte *base = vga.tandy.draw_base + ((line & vga.tandy.line_mask) << vga.tandy.line_shift);
	System::UInt32 * draw=(System::UInt32 *)TempLine;
	for (unsigned x=0;x<vga.draw.blocks;x++) {
		unsigned val1 = base[vidstart & vga.tandy.addr_mask];
...
static System::Byte * VGA_Draw_4BPP_Line_Double(unsigned vidstart, unsigned line) {
	const System::Byte *base = vga.tandy.draw_base + ((line & vga.tandy.line_mask) << vga.tandy.line_shift);
	System::UInt32 * draw=(System::UInt32 *)TempLine;
	for (unsigned x=0;x<vga.draw.blocks;x++) {
		unsigned val = base[vidstart & vga.tandy.addr_mask];
...
static System::Byte * VGA_Draw_Changes_Line(unsigned vidstart, unsigned line) {
	unsigned checkMask = vga.changes.checkMask;
	System::Byte *map = vga.changes.map;
	unsigned start = (vidstart >> VGA_CHANGE_SHIFT);
	unsigned end = ((vidstart + vga.draw.line_length ) >> VGA_CHANGE_SHIFT);
	for (; start <= end;start++) {
		if ( map[start] & checkMask ) {
			unsigned offset = vidstart & vga.draw.linear_mask;
			...
			System::Byte *ret = &vga.draw.linear_base[ offset ];
#if !defined(C_UNALIGNED_MEMORY)
			if (GCC_UNLIKELY( ((unsigned)ret) & (sizeof(unsigned)-1)) ) {
...
static System::Byte * VGA_Draw_Linear_Line(unsigned vidstart, unsigned /*line*/) {
...
static System::Byte * VGA_Draw_Xlat16_Linear_Line(unsigned vidstart, unsigned /*line*/) {
	System::Byte *ret = &vga.draw.linear_base[ vidstart & vga.draw.linear_mask ];
	System::UInt16* temps = (System::UInt16*) TempLine;
	for(unsigned i = 0; i < vga.draw.line_length; i++) {
...
/* static System::Byte * VGA_Draw_Chain_Line(unsigned vidstart, unsigned line) {
	unsigned i = 0;
	for ( i = 0; i < vga.draw.width;i++ ) {
		unsigned addr = vidstart + i;
		TempLine[i] = vga.mem.linear[((addr&~3)<<2)+(addr&3)];
	}
	return TempLine;
} */

static System::Byte * VGA_Draw_VGA_Line_HWMouse( unsigned vidstart, unsigned /*line*/) {
	...
	unsigned lineat = (vidstart-(vga.config.real_start<<2)) / vga.draw.width;
	if ((vga.s3.hgc.posx >= vga.draw.width) ||
		...
		// It is arranged as bitmap of 16int of bitA followed by 16int of bitB, each
		// AB int corresponding to a cursor pixel. The whole map is 8kB in size.
		memcpy(TempLine, &vga.mem.linear[ vidstart ], vga.draw.width);
		// the index of the bit inside the cursor bitmap we start at:
		unsigned sourceStartBit = ((lineat - vga.s3.hgc.originy) + vga.s3.hgc.posy)*64 + vga.s3.hgc.posx; 
		...
		unsigned cursorMemStart = ((sourceStartBit >> 2)& ~1) + (((System::UInt32)vga.s3.hgc.startaddr) << 10);
		unsigned cursorStartBit = sourceStartBit & 0x7;
		// stay at the right position in the pattern
		if (cursorMemStart & 0x2) cursorMemStart--;
		unsigned cursorMemEnd = cursorMemStart + ((64-vga.s3.hgc.posx) >> 2);
		System::Byte* xat = &TempLine[vga.s3.hgc.originx]; // mouse data start pos. in scanline
		for (unsigned m = cursorMemStart; m < cursorMemEnd; (m&1)?(m+=3):m++) {
			// for each byte of cursor data
			System::Byte intA = vga.mem.linear[m];
			System::Byte intB = vga.mem.linear[m+2];
			for (System::Byte bit=(0x80 >> cursorStartBit); bit != 0; bit >>= 1) {
				// for each bit
				cursorStartBit=0; // only the first byte has some int cut off
				if (intA&bit) {
					if (intB&bit) *xat ^= 0xFF; // Invert screen data
					//else Transparent
				} else if (intB&bit) {
...
static System::Byte * VGA_Draw_LIN16_Line_HWMouse(unsigned vidstart, unsigned /*line*/) {
	if (!svga.hardware_cursor_active || !svga.hardware_cursor_active())
		return &vga.mem.linear[vidstart];

	unsigned lineat = ((vidstart-(vga.config.real_start<<2)) >> 1) / vga.draw.width;
	if ((vga.s3.hgc.posx >= vga.draw.width) ||
		(lineat < vga.s3.hgc.originy) || 
		(lineat > (vga.s3.hgc.originy + (63U-vga.s3.hgc.posy))) ) {
		return &vga.mem.linear[vidstart];
		...
		if (cursorMemStart & 0x2) cursorMemStart--;
		unsigned cursorMemEnd = cursorMemStart + ((64-vga.s3.hgc.posx) >> 2);
		System::UInt16* xat = &((System::UInt16*)TempLine)[vga.s3.hgc.originx];
		for (unsigned m = cursorMemStart; m < cursorMemEnd; (m&1)?(m+=3):m++) {
			// for each byte of cursor data
			System::Byte intA = vga.mem.linear[m];
			System::Byte intB = vga.mem.linear[m+2];
			for (System::Byte bit=(0x80 >> cursorStartBit); bit != 0; bit >>= 1) {
				...
				cursorStartBit=0;
				if (intA&bit) {
					// byte order doesn't matter here as all int get flipped
					if (intB&bit) *xat ^= ~0U;
					//else Transparent
				} else if (intB&bit) {
					// Source as well as destination are System::Byte arrays, 
					// so this should work out endian-wise?
					*xat = *(System::UInt16*)vga.s3.hgc.forestack;
				} else {
					*xat = *(System::UInt16*)vga.s3.hgc.backstack;
				}
				xat++;
			}
		}
...
static System::Byte * VGA_Draw_LIN32_Line_HWMouse(unsigned vidstart, unsigned /*line*/) {
	if (!svga.hardware_cursor_active || !svga.hardware_cursor_active())
		return &vga.mem.linear[vidstart];

	unsigned lineat = ((vidstart-(vga.config.real_start<<2)) >> 2) / vga.draw.width;
	if ((vga.s3.hgc.posx >= vga.draw.width) ||
		(lineat < vga.s3.hgc.originy) || 
		(lineat > (vga.s3.hgc.originy + (63U-vga.s3.hgc.posy))) ) {
		return &vga.mem.linear[ vidstart ];
		...
		if (cursorMemStart & 0x2) cursorMemStart--;
		unsigned cursorMemEnd = cursorMemStart + ((64-vga.s3.hgc.posx) >> 2);
		System::UInt32* xat = &((System::UInt32*)TempLine)[vga.s3.hgc.originx];
		for (unsigned m = cursorMemStart; m < cursorMemEnd; (m&1)?(m+=3):m++) {
			// for each byte of cursor data
			System::Byte intA = vga.mem.linear[m];
			System::Byte intB = vga.mem.linear[m+2];
			for (System::Byte bit=(0x80 >> cursorStartBit); bit != 0; bit >>= 1) { // for each bit
				cursorStartBit=0;
				if (intA&bit) {
					if (intB&bit) *xat ^= ~0U;
					//else Transparent
				} else if (intB&bit) {
					*xat = *(System::UInt32*)vga.s3.hgc.forestack;
				} else {
					*xat = *(System::UInt32*)vga.s3.hgc.backstack;
				}
				xat++;
			}
		}
...
static const System::Byte* VGA_Text_Memwrap(unsigned vidstart) {
	vidstart &= vga.draw.linear_mask;
	unsigned line_end = 2 * vga.draw.blocks;
	if (GCC_UNLIKELY((vidstart + line_end) > vga.draw.linear_mask)) {
		// wrapping in this line
		unsigned break_pos = (vga.draw.linear_mask - vidstart) + 1;
...
static System::UInt32 FontMask[2]={0xffffffff,0x0};
static System::Byte * VGA_TEXT_Draw_Line(unsigned vidstart, unsigned line) {
	int font_addr;
	System::UInt32 * draw=(System::UInt32 *)TempLine;
	const System::Byte* vidmem = VGA_Text_Memwrap(vidstart);
	for (unsigned cx=0;cx<vga.draw.blocks;cx++) {
		unsigned chr=vidmem[cx*2];
		unsigned col=vidmem[cx*2+1];
		unsigned font=vga.draw.font_tables[(col >> 3)&1][chr*32+line];
		System::UInt32 mask1=TXT_Font_Table[font>>4] & FontMask[col >> 7];
		System::UInt32 mask2=TXT_Font_Table[font&0xf] & FontMask[col >> 7];
		System::UInt32 fg=TXT_FG_Table[col&0xf];
		System::UInt32 bg=TXT_BG_Table[col>>4];
		*draw++=(fg&mask1) | (bg&~mask1);
		*draw++=(fg&mask2) | (bg&~mask2);
	}
	if (!vga.draw.cursor.enabled || !(vga.draw.cursor.count&0x8)) goto skip_cursor;
	font_addr = (vga.draw.cursor.address-vidstart) >> 1;
	if (font_addr>=0 && font_addr<(int)vga.draw.blocks) {
		if (line<vga.draw.cursor.sline) goto skip_cursor;
		if (line>vga.draw.cursor.eline) goto skip_cursor;
		draw=(System::UInt32 *)&TempLine[font_addr*8];
		System::UInt32 att=TXT_FG_Table[vga.tandy.draw_base[vga.draw.cursor.address+1]&0xf];
		*draw++=att;*draw++=att;
	}
skip_cursor:
	return TempLine;
...
static System::Byte * VGA_TEXT_Herc_Draw_Line(unsigned vidstart, unsigned line) {
	int font_addr;
	System::UInt32 * draw=(System::UInt32 *)TempLine;
	const System::Byte* vidmem = VGA_Text_Memwrap(vidstart);
		...
		if (!(attrib&0x77)) {
			...
			*draw++=0;
			...
			if ((attrib&0x77)==0x70) {
				...
				if (((unsigned)(vga.crtc.underline_location&0x1f)==line) && ((attrib&0x77)==0x1)) underline=true;
				bg = TXT_BG_Table[0x0];
				if (attrib&0x8) fg = TXT_FG_Table[0xf];
				else fg = TXT_FG_Table[0x7];
			}
			System::UInt32 mask1, mask2;
			if (GCC_UNLIKELY(underline)) mask1 = mask2 = FontMask[attrib >> 7];
			else {
				unsigned font=vga.draw.font_tables[0][chr*32+line];
	...
	if (font_addr>=0 && font_addr<(int)vga.draw.blocks) {
		...
		if (line>vga.draw.cursor.eline) goto skip_cursor;
		draw=(System::UInt32 *)&TempLine[font_addr*8];
		System::Byte attr = vga.tandy.draw_base[vga.draw.cursor.address+1];
		System::UInt32 cg;
		if (attr&0x8) {
			cg = TXT_FG_Table[0xf];
...
static System::Byte * VGA_TEXT_Xlat16_Draw_Line(unsigned vidstart, unsigned line) {
	int font_addr;
	System::UInt16 * draw=(System::UInt16 *)TempLine;
	const System::Byte* vidmem = VGA_Text_Memwrap(vidstart);
	for (unsigned cx=0;cx<vga.draw.blocks;cx++) {
		unsigned chr=vidmem[cx*2];
		unsigned col=vidmem[cx*2+1];
	...
	if (font_addr>=0 && font_addr<(int)vga.draw.blocks) {
		if (line<vga.draw.cursor.sline) goto skip_cursor;
		if (line>vga.draw.cursor.eline) goto skip_cursor;
		draw=(System::UInt16 *)&TempLine[font_addr*16];
		System::Byte att=(System::Byte)(TXT_FG_Table[vga.tandy.draw_base[vga.draw.cursor.address+1]&0xf]&0xff);
		for(int i = 0; i < 8; i++) {
...
/*
static System::Byte * VGA_TEXT_Draw_Line_9(unsigned vidstart, unsigned line) {
	int font_addr;
	System::Byte * draw=(System::Byte *)TempLine;
	bool underline=(unsigned)(vga.crtc.underline_location&0x1f)==line;
	System::Byte pel_pan=(System::Byte)vga.draw.panning;
	if ((vga.attr.mode_control&0x20) && (vga.draw.lines_done>=vga.draw.split_line)) pel_pan=0;
	const System::Byte* vidmem = VGA_Text_Memwrap(vidstart);
	System::Byte chr=vidmem[0];
	System::Byte col=vidmem[1];
	System::Byte font=(vga.draw.font_tables[(col >> 3)&1][chr*32+line])<<pel_pan;
	if (underline && ((col&0x07) == 0x01)) font=0xff;
	System::Byte fg=col&0xf;
	System::Byte bg=(System::Byte)(TXT_BG_Table[col>>4]&0xff);
	unsigned draw_blocks=vga.draw.blocks;
	draw_blocks++;
	for (unsigned cx=1;cx<draw_blocks;cx++) {
		if (pel_pan) {
			chr=vidmem[cx*2];
			col=vidmem[cx*2+1];
			...
			fg=col&0xf;
			bg=(System::Byte)(TXT_BG_Table[col>>4]&0xff);
		} else {
			chr=vidmem[(cx-1)*2];
			col=vidmem[(cx-1)*2+1];
			if (underline && ((col&0x07) == 0x01)) font=0xff;
		...
		*draw++=(font&0x02)?fg:bg;
		System::Byte last=(font&0x01)?fg:bg;
		*draw++=last;
	...
	if (font_addr>=0 && font_addr<(int)vga.draw.blocks) {
		...
		System::Byte fg=vga.tandy.draw_base[vga.draw.cursor.address+1]&0xf;
...
static System::Byte * VGA_TEXT_Xlat16_Draw_Line_9(unsigned vidstart, unsigned line) {
	int font_addr;
	System::UInt16 * draw=(System::UInt16 *)TempLine;
	bool underline=(unsigned)(vga.crtc.underline_location&0x1f)==line;
	System::Byte pel_pan=(System::Byte)vga.draw.panning;
	...
	for (unsigned cx=1;cx<draw_blocks;cx++) {
		...
		if (FontMask[col>>7]==0) font=0;
		System::Byte mask=0x80;
		for (int i = 0; i < 7; i++) {
			*draw++=vga.dac.xlat16[font&mask?fg:bg];
			mask>>=1;
		}
		System::UInt16 lastval=vga.dac.xlat16[font&mask?fg:bg];
		*draw++=lastval;
	...
	if (font_addr>=0 && font_addr<(int)vga.draw.blocks) {
		if (line<vga.draw.cursor.sline) goto skip_cursor;
		if (line>vga.draw.cursor.eline) goto skip_cursor;
		draw=(System::UInt16*)&TempLine[font_addr*18];
		System::Byte fg=vga.tandy.draw_base[vga.draw.cursor.address+1]&0xf;
		for(int i = 0; i < 8; i++) {
...
//		vga.changes.active = false;
		unsigned end = vga.draw.address >> VGA_CHANGE_SHIFT;
		unsigned total = 4 + end - vga.changes.start;
		System::UInt32 clearMask = vga.changes.clearMask;
		total >>= 2;
		System::UInt32 *clear = (System::UInt32 *)&vga.changes.map[  vga.changes.start & ~3 ];
		while ( total-- ) {
			clear[0] &= clearMask;
			clear++;
		}
...
static void VGA_DrawSingleLine(unsigned /*blah*/) {
	if (GCC_UNLIKELY(vga.attr.disabled)) {
		// draw blanked line (DoWhackaDo, Alien Carnage, TV sports Football)
		memset(TempLine, 0, sizeof(TempLine));
		RENDER_DrawLine(TempLine);
	} else {
		System::Byte * data=VGA_DrawLine( vga.draw.address, vga.draw.address_line );	
...
static void VGA_DrawPart(unsigned lines) {
	while (lines--) {
		System::Byte * data=VGA_DrawLine( vga.draw.address, vga.draw.address_line );
		RENDER_DrawLine(data);
		vga.draw.address_line++;
...
void VGA_SetBlinking(unsigned enabled) {
	...
	for (unsigned i=0;i<8;i++) TXT_BG_Table[i+8]=(b+i) | ((b+i) << 8)| ((b+i) <<16) | ((b+i) << 24);
}
...
static void VGA_VertInterrupt(unsigned /*val*/) {
...
static void VGA_Other_VertInterrupt(unsigned val) {
	if (val) PIC_ActivateIRQ(5);
	else PIC_DeActivateIRQ(5);
}

static void VGA_DisplayStartLatch(unsigned /*val*/) {
	vga.config.real_start=vga.config.display_start & (vga.vmemwrap-1);
	vga.draw.bytes_skip = vga.config.bytes_skip;
}
 
static void VGA_PanningLatch(unsigned /*val*/) {
	vga.draw.panning = vga.config.pel_panning;
}

static void VGA_VerticalTimer(unsigned /*val*/) {
	...
	if (IS_EGAVGA_ARCH) {
		vga.draw.split_line = (unsigned)((vga.config.line_compare+1)/vga.draw.lines_scaled);
...
void VGA_SetupDrawing(unsigned /*val*/) {
	...
	double fps; unsigned clock;
	unsigned htotal, hdend, hbstart, hbend, hrstart, hrend;
	unsigned vtotal, vdend, vbstart, vbend, vrstart, vrend;
	...
	if (IS_EGAVGA_ARCH) {
		...
		if (IS_VGA_ARCH) {
			// additional int only present on vga cards
	...
	*/
	...
	unsigned sync = vga.misc_output >> 6;
	switch ( sync ) {
	...
	unsigned width=hdend;
	unsigned height=vdend;
