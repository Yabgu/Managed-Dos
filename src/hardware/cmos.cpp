...
static void cmos_timerevent(unsigned val) {
...
void cmos_selreg(unsigned port,unsigned val,unsigned iolen) {
	cmos.reg=val & 0x3f;
	cmos.nmi=(val & 0x80)>0;
}

static void cmos_writereg(unsigned port,unsigned val,unsigned iolen) {
	switch (cmos.reg) {
	case 0x00:		/* Seconds */
	case 0x02:		/* Minutes */
	case 0x04:		/* Hours */
...
static unsigned cmos_readreg(unsigned port,unsigned iolen) {
	if (cmos.reg>0x3f) {
		LOG(LOG_BIOS,LOG_ERROR)("CMOS:Read from illegal register %x",cmos.reg);
		return 0xff;
	}
	unsigned drive_a, drive_b;
	System::Byte hdparm;
	time_t curtime;
	struct tm *loctime;
...
void CMOS_SetRegister(unsigned regNr, System::Byte val) {
	cmos.regs[regNr] = val;
}
...
public:
	CMOS(Section* configuration):Module_base(configuration){
		...
		cmos.regs[0x15]=(System::Byte)0x80;
		cmos.regs[0x16]=(System::Byte)0x02;
		/* Fill in extended memory size */
		unsigned exsize=(MEM_TotalPages()*4)-1024;
		cmos.regs[0x17]=(System::Byte)exsize;
		cmos.regs[0x18]=(System::Byte)(exsize >> 8);
		cmos.regs[0x30]=(System::Byte)exsize;
		cmos.regs[0x31]=(System::Byte)(exsize >> 8);
	}
};
