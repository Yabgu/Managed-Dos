...
static unsigned call_casemap;

static System::UInt16 dos_memseg=DOS_PRIVATE_SEGMENT;

System::UInt16 DOS_GetMemory(System::UInt16 pages) {
	if ((unsigned)pages+(unsigned)dos_memseg>=DOS_PRIVATE_SEGMENT_END) {
		E_Exit("DOS:Not enough memory for internal tables");
	}
	System::UInt16 page=dos_memseg;
	dos_memseg+=pages;
	return page;
}

static unsigned DOS_CaseMapFunc(void) {
	//LOG(LOG_DOSMISC,LOG_ERROR)("Case map routine called : %c",reg_al);
	return CBRET_NONE;
}

static System::Byte country_info[0x22] = {
...
void DOS_SetupTables(void) {
	...
	for(unsigned d=0;d<26;d++) real_writeb(dos.tables.dpb,d,d);

	/* Create a fake disk buffer head */
	seg=DOS_GetMemory(6);
	for (unsigned ct=0; ct<0x20; ct++) real_writeb(seg,ct,0);
