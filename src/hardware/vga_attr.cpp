...
unsigned read_p3c0(unsigned /*port*/,unsigned /*iolen*/) {
	// Wcharts, Win 3.11 & 95 SVGA
	unsigned retval = attr(index) & 0x1f;
	if (!(attr(disabled) & 0x1)) retval |= 0x20;
	return retval;
}
 
void write_p3c0(unsigned /*port*/,unsigned val,unsigned iolen) {
	if (!vga.internal.attrindex) {
		...
		*/
		...
		case 0x0c:		case 0x0d:		case 0x0e:		case 0x0f:
			if (attr(disabled) & 0x1) VGA_ATTR_SetPalette(attr(index),(System::Byte)val);
			/*
		...
		case 0x10: /* Mode Control Register */
			...
			if ((attr(mode_control) ^ val) & 0x04) {
				attr(mode_control)=(System::Byte)val;
				VGA_DetermineMode();
			...
			/*
				...
				6	If set pixels are 8 int wide. Used in 256 color modes.
				7	If set bit 4-5 of the index into the DAC table are taken from port
					3C0h index 14h bit 0-1, else the int in the palette register are
					used.
			*/
			break;
		case 0x11:	/* Overscan Color Register */
			attr(overscan_color)=(System::Byte)val;
			/* 0-5  Color of screen border. Color is defined as in the palette registers. */
			break;
		case 0x12:	/* Color Plane Enable Register */
			/* Why disable colour planes? */
			attr(color_plane_enable)=(System::Byte)val;
			/* 
				...
				4-5	Video Status MUX. Diagnostics use only.
					Two attribute int appear on int 4 and 5 of the Input Status
		...
		case 0x13:	/* Horizontal PEL Panning Register */
			...
			case M_TEXT:
				...
				else vga.config.pel_panning=(System::Byte)(val+1);
				break;
			case M_VGA:
			case M_LIN8:
		...
		case 0x14:	/* Color Select Register */
			...
			if (attr(color_select) ^ val) {
				attr(color_select)=(System::Byte)val;
				for (System::Byte i=0;i<0x10;i++) {
					VGA_ATTR_SetPalette(i,vga.attr.palette[i]);
				}
			}
			/*
				0-1	If 3C0h index 10h bit 7 is set these 2 int are used as int 4-5 of
					the index into the DAC table.
				2-3	These 2 int are used as bit 6-7 of the index into the DAC table
...
unsigned read_p3c1(unsigned /*port*/,unsigned iolen) {
//	vga.internal.attrindex=false;
	switch (attr(index)) {
			/* Palette */
	case 0x00:		case 0x01:		case 0x02:		case 0x03:
