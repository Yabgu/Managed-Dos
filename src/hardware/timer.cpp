...
static INLINE void BIN2BCD(System::UInt16& val) {
	System::UInt16 temp=val%10 + (((val/10)%10)<<4)+ (((val/100)%10)<<8) + (((val/1000)%10)<<12);
	val=temp;
}

static INLINE void BCD2BIN(System::UInt16& val) {
	System::UInt16 temp= (val&0x0f) +((val>>4)&0x0f) *10 +((val>>8)&0x0f) *100 +((val>>12)&0x0f) *1000;
	val=temp;
}
...
struct PIT_Block {
	...
	System::UInt16 read_latch;
	System::UInt16 write_latch;

	System::Byte mode;
	System::Byte latch_mode;
	System::Byte read_state;
	System::Byte write_state;
...
static bool counter_output(unsigned counter) {
...
static void status_latch(unsigned counter) {
	// the timer status can not be overwritten until it is read or the timer was 
	// reprogrammed.
...
static void counter_latch(unsigned counter) {
	...
	case 0:		/* Interrupt on Terminal Count */
		...
		if (index>p->delay) {
			...
			if(p->bcd) {
				...
				p->read_latch = (System::UInt16)(9999-index*(PIT_TICK_RATE/1000.0));
			} else {
				index = fmod(index,(1000.0/PIT_TICK_RATE)*(double)0x10000);
				p->read_latch = (System::UInt16)(0xffff-index*(PIT_TICK_RATE/1000.0));
			}
		} else {
			p->read_latch=(System::UInt16)(p->cntr-index*(PIT_TICK_RATE/1000.0));
		}
		break;
	case 1: // countdown
		if(p->counting) {
	...
	case 2:		/* Rate Generator */
		index=fmod(index,(double)p->delay);
		p->read_latch=(System::UInt16)(p->cntr - (index/p->delay)*p->cntr);
		break;
...
static void write_latch(unsigned port,unsigned val,unsigned /*iolen*/) {
//LOG(LOG_PIT,LOG_ERROR)("port %X write:%X state:%X",port,val,pit[port-0x40].write_state);
	unsigned counter=port-0x40;
	PIT_Block * p=&pit[counter];
...
static unsigned read_latch(unsigned port,unsigned /*iolen*/) {
//LOG(LOG_PIT,LOG_ERROR)("port read %X",port);
	System::UInt32 counter=port-0x40;
	System::Byte ret=0;
...
static void write_p43(unsigned /*port*/,unsigned val,unsigned /*iolen*/) {
//LOG(LOG_PIT,LOG_ERROR)("port 43 %X",val);
	unsigned latch=(val >> 6) & 0x03;
	switch (latch) {
	case 0:
	case 1:
	case 2:
