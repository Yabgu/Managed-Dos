...
public:
	...
	virtual unsigned readb(PhysPt addr);
	virtual unsigned readw(PhysPt addr);
	virtual unsigned readd(PhysPt addr);
	virtual void writeb(PhysPt addr,unsigned val);
	virtual void writew(PhysPt addr,unsigned val);
	virtual void writed(PhysPt addr,unsigned val);
	virtual HostPt GetHostReadPt(unsigned phys_page);
	virtual HostPt GetHostWritePt(unsigned phys_page);
	virtual bool readb_checked(PhysPt addr,System::Byte * val);
	virtual bool readw_checked(PhysPt addr,System::UInt16 * val);
	virtual bool readd_checked(PhysPt addr,System::UInt32 * val);
	virtual bool writeb_checked(PhysPt addr,unsigned val);
	virtual bool writew_checked(PhysPt addr,unsigned val);
	virtual bool writed_checked(PhysPt addr,unsigned val);
	unsigned flags;
};
...
void MEM_SetLFB(unsigned page, unsigned pages, PageHandler *handler, PageHandler *mmiohandler);
void MEM_SetPageHandler(unsigned phys_page, unsigned pages, PageHandler * handler);
void MEM_ResetPageHandler(unsigned phys_page, unsigned pages);
...
struct PagingBlock {
	...
	struct {
		...
		System::UInt32	phys_page[TLB_SIZE];
	} tlb;
#else
	...
	struct {
		unsigned used;
		System::UInt32 entries[PAGING_LINKS];
	} links;
	System::UInt32		firstmb[LINK_START];
	bool		enabled;
};
...
PageHandler * MEM_GetPageHandler(unsigned phys_page);


/* Unaligned address handlers */
System::UInt16 mem_unalignedreadw(PhysPt address);
System::UInt32 mem_unalignedreadd(PhysPt address);
void mem_unalignedwritew(PhysPt address,System::UInt16 val);
void mem_unalignedwrited(PhysPt address,System::UInt32 val);

bool mem_unalignedreadw_checked(PhysPt address,System::UInt16 * val);
bool mem_unalignedreadd_checked(PhysPt address,System::UInt32 * val);
bool mem_unalignedwritew_checked(PhysPt address,System::UInt16 val);
bool mem_unalignedwrited_checked(PhysPt address,System::UInt32 val);
...
static INLINE tlb_entry *get_tlb_entry(PhysPt address) {
	unsigned index=(address>>12);
	if (TLB_BANKS && (index > TLB_SIZE)) {
		unsigned bank=(address>>BANK_SHIFT) - 1;
...
static INLINE System::Byte mem_readb_inline(PhysPt address) {
	HostPt tlb_addr=get_tlb_read(address);
	if (tlb_addr) return host_readb(tlb_addr+address);
	else return (System::Byte)(get_tlb_readhandler(address))->readb(address);
}

static INLINE System::UInt16 mem_readw_inline(PhysPt address) {
	if ((address & 0xfff)<0xfff) {
		HostPt tlb_addr=get_tlb_read(address);
		if (tlb_addr) return host_readw(tlb_addr+address);
		else return (System::UInt16)(get_tlb_readhandler(address))->readw(address);
	} else return mem_unalignedreadw(address);
}

static INLINE System::UInt32 mem_readd_inline(PhysPt address) {
...
static INLINE void mem_writeb_inline(PhysPt address,System::Byte val) {
	HostPt tlb_addr=get_tlb_write(address);
	if (tlb_addr) host_writeb(tlb_addr+address,val);
	else (get_tlb_writehandler(address))->writeb(address,val);
}

static INLINE void mem_writew_inline(PhysPt address,System::UInt16 val) {
	if ((address & 0xfff)<0xfff) {
		HostPt tlb_addr=get_tlb_write(address);
...
static INLINE void mem_writed_inline(PhysPt address,System::UInt32 val) {
	if ((address & 0xfff)<0xffd) {
		HostPt tlb_addr=get_tlb_write(address);
...
static INLINE bool mem_readb_checked(PhysPt address, System::Byte * val) {
	HostPt tlb_addr=get_tlb_read(address);
	if (tlb_addr) {
...
static INLINE bool mem_readw_checked(PhysPt address, System::UInt16 * val) {
	if ((address & 0xfff)<0xfff) {
		HostPt tlb_addr=get_tlb_read(address);
		if (tlb_addr) {
...
static INLINE bool mem_readd_checked(PhysPt address, System::UInt32 * val) {
	if ((address & 0xfff)<0xffd) {
		HostPt tlb_addr=get_tlb_read(address);
		if (tlb_addr) {
...
static INLINE bool mem_writeb_checked(PhysPt address,System::Byte val) {
	HostPt tlb_addr=get_tlb_write(address);
	if (tlb_addr) {
...
static INLINE bool mem_writew_checked(PhysPt address,System::UInt16 val) {
	if ((address & 0xfff)<0xfff) {
		HostPt tlb_addr=get_tlb_write(address);
		if (tlb_addr) {
...
static INLINE bool mem_writed_checked(PhysPt address,System::UInt32 val) {
	if ((address & 0xfff)<0xffd) {
		HostPt tlb_addr=get_tlb_write(address);
		if (tlb_addr) {
