...
bool SERIAL_setCommParameters(COMPORT port,
			int baudrate, char parity, int stopint, int length) {
	...
	case SERIAL_2STOP: dcb.StopBits = ONE5STOPBITS; break;
...
int SERIAL_getmodemstatus(COMPORT port) {
	...
	// int are the same as return value
	return (int)dptr;
}
void SERIAL_setDTR(COMPORT port, bool value) {
	UCHAR masks[2];
