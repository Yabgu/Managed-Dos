	...
	void Run(void)
	{
		...
		if (type=="floppy" || type=="dir" || type=="cdrom") {
			...
			if(cmd->FindString("-freesize",mb_size,true)) {
				char teststr[1024];
				System::UInt16 sizemb = static_cast<System::UInt16>(atoi(mb_size.c_str()));
				if (type=="floppy") {
			...
			unsigned index=0;unsigned count=0;
...
// Nothing to do here.
			...
			System::Byte bit8size=(System::Byte) sizes[1];
			if (type=="cdrom") {
				int num = -1;
...
public:
	void Run(void) {
		...
		System::UInt16 umb_start=dos_infoblock.GetStartOfUMBChain();
		System::Byte umb_flag=dos_infoblock.GetUMBChainState();
		System::Byte old_memstrat=DOS_GetMemAllocStrategy()&0xff;
		if (umb_start!=0xffff) {
			if ((umb_flag&1)==1) DOS_LinkUMBsToMemChain(0);
			DOS_SetMemAllocStrategy(0);
		}

		System::UInt16 seg,blocks;blocks=0xffff;
		...
		if (umb_start!=0xffff) {
			...
			System::UInt16 largest_block=0,total_blocks=0,block_count=0;
			for (;; block_count++) {
				blocks=0xffff;
				DOS_AllocateMemory(&seg,&blocks);
				if (blocks==0) break;
			...
			System::Byte current_umb_flag=dos_infoblock.GetUMBChainState();
		...
		if (reg_al==0x80) {
			...
			System::UInt16 xms_seg=SegValue(es);System::UInt16 xms_off=reg_bx;
			reg_ah=8;
...
extern System::UInt32 floppytype;


class BOOT : public Program {
private:
   
	FILE *getFSFile_mounted(char const* filename, System::UInt32 *ksize, System::UInt32 *bsize, System::Byte *error) {
...
//				return NULL;
	...
	FILE *getFSFile(char const * filename, System::UInt32 *ksize, System::UInt32 *bsize,bool tryload=false) {
		System::Byte error = tryload?1:0;
...
public:
	...
	void Run(void) {
		...
		FILE *usefile_2=NULL;
		unsigned i=0; 
		System::UInt32 floppysize;
		System::UInt32 rombytesize_1=0;
		System::UInt32 rombytesize_2=0;
		System::Byte drive = 'A';
		std::string cart_cmd="";
		...
		while(i<cmd->GetCount()) {
			if(cmd->FindCommand(i+1, temp_line)) {
				...
				System::UInt32 rombytesize;
				FILE *usefile = getFSFile(temp_line.c_str(), &floppysize, &rombytesize);
				if(usefile != NULL) {
					if(diskSwap[i] != NULL) delete diskSwap[i];
					diskSwap[i] = new imageDisk(usefile, (System::Byte *)temp_line.c_str(), floppysize, false);
					if (usefile_1==NULL) {
						usefile_1=usefile;
		...
		bootSector bootarea;
		imageDiskList[drive-65]->Read_Sector(0,0,1,(System::Byte *)&bootarea);
			...
			if (machine!=MCH_PCJR) WriteOut(MSG_Get("PROGRAM_BOOT_CART_WO_PCJR"));
				...
				if (cart_cmd!="") {
					...
					if (cart_cmd=="?") {
						...
						for(unsigned dct=0;dct<MAX_SWAPPABLE_DISKS;dct++) {
				...
				System::UInt32 sz1,sz2;
				FILE *tfile = getFSFile("system.rom", &sz1, &sz2, true);
				if (tfile!=NULL) {
					fseek(tfile, 0x3000L, SEEK_SET);
					System::UInt32 drd=(System::UInt32)fread(rombuf, 1, 0xb000, tfile);
					if (drd==0xb000) {
				...
				System::UInt16 romseg=host_readw(&rombuf[0x1ce]);
				...
				if (cart_cmd=="") {
					System::UInt32 old_int18=mem_readd(0x60);
					/* run cartridge setup */
					SegSet16(ds,romseg);
					SegSet16(es,romseg);
					SegSet16(ss,0x8000);
					...
					System::UInt32 new_int18=mem_readd(0x60);
...
public:
	void Run(void) {
		...
		try {		
			...
			System::Byte vga_buffer[0x10000];
			unsigned data_written=0;
			unsigned data_read=fread(vga_buffer, 1, 0x10000, tmpfile);
			for (unsigned ct=0; ct<data_read; ct++) {
...
void LOADFIX::Run(void) 
{
	...
	System::UInt16 blocks = kb*1024/16;
	if (DOS_AllocateMemory(&segment,&blocks)) {
		DOS_MCB mcb((System::UInt16)(segment-1));
...
{
	// Get current drive
	System::Byte drive = DOS_GetDefaultDrive();
	if (Drives[drive]) {
...
public:
	...
	void Run(void) {
		...
		System::Byte c;System::UInt16 n=1;
		...
		if (type=="floppy" || type=="hdd" || type=="iso") {
			...
			if(fstype=="fat") {
				if (imgsizedetect) {
					...
					System::UInt32 fcsize = (System::UInt32)(ftell(diskfile) / 512L);
					System::Byte buf[512];
					fseek(diskfile, 0L, SEEK_SET);
					if (fread(buf,sizeof(System::Byte),512,diskfile)<512) {
						fclose(diskfile);
						WriteOut(MSG_Get("PROGRAM_IMGMOUNT_INVALID_IMAGE"));
						return;
					}
					...
					unsigned sectors=(unsigned)(fcsize/(16*63));
				...
				newImage = new imageDisk(newDisk, (System::Byte *)temp_line.c_str(), imagesize, (imagesize > 2880));
...
void KEYB::Run(void) {
	if (cmd->FindCommand(1,temp_line)) {
		if (cmd->FindString("?",temp_line,false)) {
			...
			unsigned keyb_error=0;
			std::string cp_string;
			System::Int32 tried_cp = -1;
