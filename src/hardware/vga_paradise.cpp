...
//       only 7 int of bank index (VGADOC and Ferraro agree on that) but also point that there are
...
void write_p3cf_pvga1a(unsigned reg,unsigned val,unsigned iolen) {
	...
	case 0x0c:
		...
		break;
	case 0x0d:
		// CRT control. int 3-4 contain int 16-17 of CRT start.
...
unsigned read_p3cf_pvga1a(unsigned reg,unsigned iolen) {
	if (pvga1a.locked() && reg >= 0x09 && reg <= 0x0e)
		return 0x0;
...
void SetClock_PVGA1A(unsigned which,unsigned target) {
	if (which < 4) {
...
unsigned GetClock_PVGA1A() {
	return pvga1a.clockFreq[(vga.misc_output >> 2) & 3];
}

bool AcceptsMode_PVGA1A(unsigned mode) {
