...
public:
	...
	bool ReadFromControlChannel(PhysPt bufptr,System::UInt16 size,System::UInt16 * retcode){return false;}
	bool WriteToControlChannel(PhysPt bufptr,System::UInt16 size,System::UInt16 * retcode){return false;}
private:
	System::Byte readcache;
	System::Byte lastwrite;
	struct ansi { /* should create a constructor which fills them with the appriorate values */
		bool esc;
		bool sci;
		bool enabled;
		System::Byte attr;
		System::Byte data[NUMBER_ANSI_DATA];
		System::Byte numberofarg;
		System::UInt16 nrows;
		System::UInt16 ncols;
		System::SByte savecol;
		System::SByte saverow;
		bool warned;
	} ansi;
};

bool device_CON::Read(System::Byte * data,System::UInt16 * size) {
	System::UInt16 oldax=reg_ax;
...
bool device_CON::Write(System::Byte * data,System::UInt16 * size) {
	System::UInt16 count=0;
	unsigned i;
	System::Byte col,row;
	...
	switch(data[count]){
		...
		case 'H':/* Cursor Pos*/
			...
			if(ansi.data[0] > ansi.nrows) ansi.data[0] = (System::Byte)ansi.nrows;
			if(ansi.data[1] > ansi.ncols) ansi.data[1] = (System::Byte)ansi.ncols;
		...
		case 'B': /*cursor Down */
			...
			row=CURSOR_POS_ROW(page) ;
			tempdata = (ansi.data[0]? ansi.data[0] : 1);
			if(tempdata + static_cast<unsigned>(row) >= ansi.nrows)
				{ row = ansi.nrows - 1;}
			else	{ row += tempdata; }
			INT10_SetCursorPos(row,col,page);
			ClearAnsi();
		...
		case 'C': /*cursor forward */
			...
			row=CURSOR_POS_ROW(page);
			tempdata=(ansi.data[0]? ansi.data[0] : 1);
			if(tempdata + static_cast<unsigned>(col) >= ansi.ncols) 
				{ col = ansi.ncols - 1;} 
			else	{ col += tempdata;}
			INT10_SetCursorPos(row,col,page);
			ClearAnsi();
		...
		case 'K': /* erase till end of line (don't touch cursor) */
			...
			//for(i = col;i<(unsigned) ansi.ncols; i++) INT10_TeletypeOutputAttr(' ',ansi.attr,true);
			INT10_SetCursorPos(row,col,page);
			ClearAnsi();
			break;
...
bool device_CON::Seek(System::UInt32 * pos,System::UInt32 type) {
	// seek is valid
	*pos = 0;
	return true;
}
...
System::UInt16 device_CON::GetInformation(void) {
	System::UInt16 head=mem_readw(BIOS_KEYBOARD_BUFFER_HEAD);
	System::UInt16 tail=mem_readw(BIOS_KEYBOARD_BUFFER_TAIL);
	...
	System::UInt16 start=mem_readw(BIOS_KEYBOARD_BUFFER_START);
	System::UInt16 end	=mem_readw(BIOS_KEYBOARD_BUFFER_END);
	head+=2;
...
void device_CON::ClearAnsi(void){
	for(System::Byte i=0; i<NUMBER_ANSI_DATA;i++) ansi.data[i]=0;
	ansi.esc=false;
	ansi.sci=false;
	ansi.numberofarg=0;
}
