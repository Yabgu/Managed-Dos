...
#if (SBPP == 9)
		for (unsigned x=0;x<SCALER_BLOCKSIZE;x++) {
			PTYPE pixel = PMAKE(src[x]);
			if (pixel != fc[x]) {
#else 
		for (unsigned x=0;x<SCALER_BLOCKSIZE;x+=sizeof(unsigned)/sizeof(SRCTYPE)) {
			if (*(unsigned const*)&src[x] != *(unsigned*)&sc[x]) {
#endif
				do {
					fc[x] = PMAKE(src[x]);
					sc[x] = src[x];
...
#define SCALERFUNC									\
{													\
	unsigned halfpixel=(((P & redblueMask) * 5) >> 3) & redblueMask;	\
