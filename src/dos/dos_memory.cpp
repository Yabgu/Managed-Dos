...
static System::UInt16 memAllocStrategy = 0x00;

static void DOS_CompressMemory(void) {
	System::UInt16 mcb_segment=dos.firstMCB;
	DOS_MCB mcb(mcb_segment);
	DOS_MCB mcb_next(0);

	while (mcb.GetType()!=0x5a) {
		mcb_next.SetPt((System::UInt16)(mcb_segment+mcb.GetSize()+1));
...
System::UInt16 DOS_GetMemAllocStrategy() {
...
bool DOS_AllocateMemory(System::UInt16 * segment,System::UInt16 * blocks) {
	DOS_CompressMemory();
	System::UInt16 bigsize=0;
	System::UInt16 mem_strat=memAllocStrategy;
	System::UInt16 mcb_segment=dos.firstMCB;

	System::UInt16 umb_start=dos_infoblock.GetStartOfUMBChain();
	if (umb_start==UMB_START_SEG) {
		/* start with UMBs if requested (int 7 or 6 set) */
	...
	System::UInt16 found_seg=0,found_seg_size=0;
	for (;;) {
		mcb.SetPt(mcb_segment);
		if (mcb.GetPSPSeg()==0) {
			/* Check for enough free memory in current block */
			System::UInt16 block_size=mcb.GetSize();			
			if (block_size<(*blocks)) {
				...
				*segment=mcb_segment+1;
				...
				switch (mem_strat & 0x3f) {
					case 0: /* firstfit */
						mcb_next.SetPt((System::UInt16)(mcb_segment+*blocks+1));
			...
			if ((mem_strat&0x80) && (umb_start==UMB_START_SEG)) {
				...
				if (found_seg) {
					...
					if ((mem_strat & 0x3f)==0x01) {
						...
						mcb_next.SetPt((System::UInt16)(found_seg+*blocks+1));
						mcb_next.SetPSPSeg(MCB_FREE);
						mcb_next.SetType(mcb.GetType());
						...
						*segment = found_seg+1+found_seg_size - *blocks;
						mcb_next.SetPt((System::UInt16)(*segment-1));
...
bool DOS_ResizeMemory(System::UInt16 segment,System::UInt16 * blocks) {
	...
	DOS_CompressMemory();
	System::UInt16 total=mcb.GetSize();
	...
	if (*blocks<total) {
		...
		mcb.SetSize(*blocks);
		mcb_next.SetPt((System::UInt16)(segment+*blocks));
...
void DOS_BuildUMBChain(bool umb_active,bool ems_active) {
	if (umb_active  && (machine!=MCH_TANDY)) {
		System::UInt16 first_umb_seg = 0xd000;
		System::UInt16 first_umb_size = 0x2000;
		...
		System::UInt16 cover_mcb=(System::UInt16)(mcb_segment+mcb.GetSize()+1);
		mcb.SetPt(cover_mcb);
		mcb.SetType(0x4d);
...
bool DOS_LinkUMBsToMemChain(System::UInt16 linkstate) {
	...
	System::UInt16 mcb_segment=dos.firstMCB;
	System::UInt16 prev_mcb_segment=dos.firstMCB;
	DOS_MCB mcb(mcb_segment);
...
static unsigned DOS_default_handler(void) {
...
void DOS_SetupMemory(void) {
	...
	System::UInt16 ihseg = 0x70;
	System::UInt16 ihofs = 0x08;
	real_writeb(ihseg,ihofs+0x00,(System::Byte)0xFE);	//GRP 4
	real_writeb(ihseg,ihofs+0x01,(System::Byte)0x38);	//Extra Callback instruction
	real_writew(ihseg,ihofs+0x02,callbackhandler.Get_callback());  //The immediate word
	real_writeb(ihseg,ihofs+0x04,(System::Byte)0xCF);	//An IRET Instruction
...
//	RealSetVec(0x0f,RealMake(ihseg,ihofs));		//Always a tricky one (soundblaster irq)
	...
	DOS_MCB mcb_devicedummy((System::UInt16)DOS_MEM_START);
...
//	mcb_devicedummy.SetFileName("SD      ");
	...
	System::UInt16 mcb_sizes=2;
	// Create a small empty MCB (result from a growing environment block)
	DOS_MCB tempmcb((System::UInt16)DOS_MEM_START+mcb_sizes);
	...
	DOS_MCB tempmcb2((System::UInt16)DOS_MEM_START+mcb_sizes);
	tempmcb2.SetPSPSeg(0x40);	// can be removed by loadfix
	tempmcb2.SetSize(16);
	mcb_sizes+=17;
	tempmcb2.SetType(0x4d);

	DOS_MCB mcb((System::UInt16)DOS_MEM_START+mcb_sizes);
	...
	if (machine==MCH_TANDY) {
		...
		mcb_devicedummy.SetPt((System::UInt16)0x2000);
		...
		mcb_devicedummy.SetPt((System::UInt16)0x17ff);
