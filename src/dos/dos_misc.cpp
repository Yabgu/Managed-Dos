...
static unsigned call_int2f,call_int2a;
...
static unsigned INT2F_Handler(void) {
	for(Multiplex_it it = Multiplex.begin();it != Multiplex.end();it++)
...
static unsigned INT2A_Handler(void) {
	return CBRET_NONE;
}
...
static bool DOS_MultiplexFunctions(void) {
	...
	case 0x1216:	/* GET ADDRESS OF SYSTEM FILE TABLE ENTRY */
		...
		if (reg_bx<16) {
			...
			unsigned sftofs=0x06+reg_bx*0x3b;
			...
			System::UInt32 handle=RealHandle(reg_bx);
			if (handle>=DOS_FILES) {
				...
				System::Byte drive=Files[reg_bx]->GetDrive();

				mem_writew(sftptr+sftofs+0x02,(System::UInt16)(Files[reg_bx]->flags&3));	// file open mode
				mem_writeb(sftptr+sftofs+0x04,(System::Byte)(Files[reg_bx]->attr));		// file attribute
				...
				System::UInt32 curpos=0;
				Files[reg_bx]->Seek(&curpos,DOS_SEEK_CUR);
				System::UInt32 endpos=0;
