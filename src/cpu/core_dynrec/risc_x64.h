...
#define DRC_PTR_SIZE_IM System::UInt64
...
static INLINE void gen_memaddr(HostReg reg,void* data) {
	System::Int64 diff = (System::Int64)data-((System::Int64)cache.pos+5);
	if ((diff<0x80000000LL) && (diff>-0x80000000LL)) {
		cache_addb(0x05+(reg<<3));
		// RIP-relative addressing is offset after the instruction 
		cache_addd((System::UInt32)(((System::UInt64)diff)&0xffffffffLL)); 
	} else if ((System::UInt64)data<0x100000000LL) {
		cache_addw(0x2504+(reg<<3));
		cache_addd((System::UInt32)(((System::UInt64)data)&0xffffffffLL));
	} else {
...
// this function can use FC_OP1/FC_OP2 as dest_reg which are
// not directly byte-accessible on some architectures
static void gen_mov_byte_to_reg_low_imm_canuseword(HostReg dest_reg,System::Byte imm) {
	cache_addb(0x66);
	cache_addb(0xb8+dest_reg);			// mov reg,imm
	cache_addw(imm);
}
...
static void gen_mov_direct_dword(void* dest,System::UInt32 imm) {
	...
	cache_addb(0x25);
	cache_addd((System::UInt32)(((System::UInt64)dest)&0xffffffffLL));
	cache_addd(imm);
}

// move a 64bit constant value into a full register
static void gen_mov_reg_qword(HostReg dest_reg,System::UInt64 imm) {
	cache_addb(0x48);
...
static void gen_add_direct_word(void* dest,System::UInt32 imm,bool dword) {
	...
	cache_addb(0x25);
	cache_addd((System::UInt32)(((System::UInt64)dest)&0xffffffffLL));
	if (dword) cache_addd((System::UInt32)imm);
	else cache_addw((System::UInt16)imm);
}
...
static System::UInt64 INLINE gen_call_function_setup(void * func,unsigned paramcount,bool fastcall=false) {
	// align the stack
	cache_addb(0x48);
	...
	System::UInt64 proc_addr=(System::UInt64)cache.pos;
...
static void INLINE gen_load_param_imm(unsigned imm,unsigned param) {
	...
	switch (param) {
		...
		case 1:			// mov param2,imm32
			gen_mov_dword_to_reg_imm(FC_OP2,(System::UInt32)imm);
			break;
#if defined (_MSC_VER)
		case 2:			// mov r8,imm32
			cache_addw(0xb849);
			cache_addq((System::UInt32)imm);
			break;
		case 3:			// mov r9,imm32
			cache_addw(0xb949);
			cache_addq((System::UInt32)imm);
		...
		case 2:			// mov rdx,imm32
			gen_mov_dword_to_reg_imm(HOST_EDX,(System::UInt32)imm);
			break;
		case 3:			// mov rcx,imm32
			gen_mov_dword_to_reg_imm(HOST_ECX,(System::UInt32)imm);
			break;
#endif
		default:
...
static void INLINE gen_load_param_addr(DRC_PTR_SIZE_IM addr,unsigned param) {
...
static System::UInt64 gen_create_branch_on_zero(HostReg reg,bool dword) {
	if (!dword) cache_addb(0x66);
	cache_addb(0x0b);					// or reg,reg
	cache_addb(0xc0+reg+(reg<<3));

	cache_addw(0x0074);					// jz addr
	return ((System::UInt64)cache.pos-1);
}

// short conditional jump (+-127 bytes) if register is nonzero
// the destination is set by gen_fill_branch() later
static System::UInt64 gen_create_branch_on_nonzero(HostReg reg,bool dword) {
	if (!dword) cache_addb(0x66);
	cache_addb(0x0b);					// or reg,reg
	cache_addb(0xc0+reg+(reg<<3));
...
#if C_DEBUG
	System::Int64 len=(System::UInt64)cache.pos-data;
	if (len<0) len=-len;
	if (len>126) LOG_MSG("Big jump %d",len);
#endif
	*(System::Byte*)data=(System::Byte)((System::UInt64)cache.pos-data-1);
}

// conditional jump if register is nonzero
// for isdword==true the 32bit of the register are tested
// for isdword==false the lowest 8bit of the register are tested
static System::UInt64 gen_create_branch_long_nonzero(HostReg reg,bool isdword) {
	// isdword: cmp reg32,0
	...
	cache_addd(0);
	return ((System::UInt64)cache.pos-4);
}

// compare 32bit-register against zero and jump if value less/equal than zero
static System::UInt64 gen_create_branch_long_leqzero(HostReg reg) {
...
static void gen_fill_branch_long(System::UInt64 data) {
	*(System::UInt32*)data=(System::UInt32)((System::UInt64)cache.pos-data-4);
}
...
static void gen_fill_function_ptr(System::Byte * pos,void* fct_ptr,unsigned flags_type) {
	...
	switch (flags_type) {
		...
		case t_ADDw:
		case t_ADDd:
			*(System::UInt32*)(pos+0)=0xf001f889;	// mov eax,edi; add eax,esi
			*(System::UInt32*)(pos+4)=0x909006eb;	// skip
			*(System::UInt32*)(pos+8)=0x90909090;
			break;
		case t_ORb:
		case t_ORw:
		case t_ORd:
			*(System::UInt32*)(pos+0)=0xf009f889;	// mov eax,edi; or eax,esi
			*(System::UInt32*)(pos+4)=0x909006eb;	// skip
			*(System::UInt32*)(pos+8)=0x90909090;
			break;
		case t_ANDb:
		case t_ANDw:
		case t_ANDd:
			*(System::UInt32*)(pos+0)=0xf021f889;	// mov eax,edi; and eax,esi
			*(System::UInt32*)(pos+4)=0x909006eb;	// skip
			*(System::UInt32*)(pos+8)=0x90909090;
			break;
		case t_SUBb:
		case t_SUBw:
		case t_SUBd:
			*(System::UInt32*)(pos+0)=0xf029f889;	// mov eax,edi; sub eax,esi
			*(System::UInt32*)(pos+4)=0x909006eb;	// skip
			*(System::UInt32*)(pos+8)=0x90909090;
			break;
		case t_XORb:
		case t_XORw:
		case t_XORd:
			*(System::UInt32*)(pos+0)=0xf031f889;	// mov eax,edi; xor eax,esi
			*(System::UInt32*)(pos+4)=0x909006eb;	// skip
			*(System::UInt32*)(pos+8)=0x90909090;
			break;
		case t_CMPb:
		...
		case t_TESTw:
		case t_TESTd:
			*(System::UInt32*)(pos+0)=0x90900aeb;	// skip
			*(System::UInt32*)(pos+4)=0x90909090;
			*(System::UInt32*)(pos+8)=0x90909090;
			break;
		case t_INCb:
		case t_INCw:
		case t_INCd:
			*(System::UInt32*)(pos+0)=0xc0fff889;	// mov eax,edi; inc eax
			*(System::UInt32*)(pos+4)=0x909006eb;	// skip
			*(System::UInt32*)(pos+8)=0x90909090;
			break;
		case t_DECb:
		case t_DECw:
		case t_DECd:
			*(System::UInt32*)(pos+0)=0xc8fff889;	// mov eax,edi; dec eax
			*(System::UInt32*)(pos+4)=0x909006eb;	// skip
			*(System::UInt32*)(pos+8)=0x90909090;
			break;
		case t_NEGb:
		case t_NEGw:
		case t_NEGd:
			*(System::UInt32*)(pos+0)=0xd8f7f889;	// mov eax,edi; neg eax
			*(System::UInt32*)(pos+4)=0x909006eb;	// skip
			*(System::UInt32*)(pos+8)=0x90909090;
			break;
		default:
			*(System::UInt64*)(pos+2)=(System::UInt64)fct_ptr;		// fill function pointer
			break;
	}
#else
	*(System::UInt64*)(pos+2)=(System::UInt64)fct_ptr;
#endif
}
#endif
