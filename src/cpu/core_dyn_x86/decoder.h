...
static bool MakeCodePage(unsigned lin_addr,CodePageHandler * &cph) {
	System::Byte rdval;
...
static System::Byte decode_fetchb(void) {
	if (GCC_UNLIKELY(decode.page.index>=4096)) {
		...
		decode.page.first++;
		unsigned fetchaddr=decode.page.first << 12;
		mem_readb(fetchaddr);
...
static INLINE void decode_increase_wmapmask(unsigned size) {
	unsigned mapidx;
...
static struct {
	save_info_type type;
	DynState state;
	System::Byte * branch_pos;
	System::UInt32 eip_change;
	unsigned cycles;
	System::Byte * return_pos;
} save_info[512];

unsigned used_save_info=0;


static BlockReturn DynRunException(System::UInt32 eip_add,System::UInt32 cycle_sub,System::UInt32 dflags) {
...
static void dyn_fill_blocks(void) {
	for (unsigned sct=0; sct<used_save_info; sct++) {
		...
		switch (save_info[sct].type) {
			...
			case fpu_restore:
				...
				cache_addb(0x25);
				cache_addd((System::UInt32)(&(dyn_dh_fpu.state[0])));
				cache_addb(0x89);	// mov fpu.state_used,1
				cache_addb(0x05|(gr1->index<<3));
				cache_addd((System::UInt32)(&(dyn_dh_fpu.state_used)));
				gen_releasereg(DREG(TMPB));
...
static void dyn_read_byte(DynReg * addr,DynReg * dst,unsigned high) {
	gen_protectflags();
...
static void dyn_write_byte(DynReg * addr,DynReg * val,unsigned high) {
	gen_protectflags();
...
static void dyn_read_byte_release(DynReg * addr,DynReg * dst,unsigned high) {
	gen_protectflags();
	gen_call_function((void *)&mem_readb_checked,"%Ddr%Id",addr,&core_dyn.readdata);
	dyn_check_bool_exception_al();
	gen_mov_host(&core_dyn.readdata,dst,1,high);
}
static void dyn_write_byte_release(DynReg * addr,DynReg * val,unsigned high) {
	gen_protectflags();
...
static void dyn_read_intro(DynReg * addr,bool release_addr=true) {
	...
	if (addr->genreg) {
		...
		System::Byte reg_idx=(System::Byte)addr->genreg->index;
		...
		cache_addd((System::UInt32)addr->data);
	}
...
bool mem_readb_checked_dcx86(PhysPt address) {
	return get_tlb_readhandler(address)->readb_checked(address, (System::Byte*)(&core_dyn.readdata));
}
...
static void dyn_read_byte(DynReg * addr,DynReg * dst,unsigned high) {
	...
	cache_addb(0x0c);
	cache_addw(0x048b);		// mov eax,paging.tlb.read[eax*TYPE System::UInt32]
	cache_addb(0x85);
	cache_addd((System::UInt32)(&paging.tlb.read[0]));
	cache_addw(0xc085);		// test eax,eax
	System::Byte* je_loc=gen_create_branch(BR_Z);


	cache_addw(0x048a);		// mov al,[eax+ecx]
	cache_addb(0x08);

	System::Byte* jmp_loc=gen_create_jump();
	gen_fill_branch(je_loc);
	cache_addb(0x51);		// push ecx
	cache_addb(0xe8);
	cache_addd(((System::UInt32)&mem_readb_checked_dcx86) - (System::UInt32)cache.pos-4);
	...
	cache_addd((System::UInt32)(&core_dyn.readdata));
...
static void dyn_read_word(DynReg * addr,DynReg * dst,bool dword) {
	if (dword) {
		...
		System::Byte* jb_loc1=gen_create_branch(BR_B);
		cache_addw(0xe8d1);		// shr eax,0x1
		System::Byte* jb_loc2=gen_create_branch(BR_B);
		...
		cache_addb(0x51);		// push ecx
		cache_addb(0xe8);
		cache_addd(((System::UInt32)&mem_readd_checked_dcx86) - (System::UInt32)cache.pos-4);
		cache_addw(0xc483);		// add esp,4
		cache_addb(0x04);
		cache_addw(0x012c);		// sub al,1
...
static void dyn_write_intro(DynReg * addr,bool release_addr=true) {
	...
	if (addr->genreg) {
		// addr in a register
		System::Byte reg_idx_addr=(System::Byte)addr->genreg->index;
...
static void dyn_write_byte(DynReg * addr,DynReg * val,bool high) {
	...
	cache_addb(0x0c);
	cache_addw(0x0c8b);		// mov ecx,paging.tlb.read[ecx*TYPE System::UInt32]
	cache_addb(0x8d);
	cache_addd((System::UInt32)(&paging.tlb.write[0]));
	...
	if (GCC_UNLIKELY(high)) cache_addw(0xe086+((genreg->index+(genreg->index<<3))<<8));
	cache_addb(0xe8);
	cache_addd(((System::UInt32)&mem_writeb_checked) - (System::UInt32)cache.pos-4);
...
static void dyn_write_word(DynReg * addr,DynReg * val,bool dword) {
	if (dword) {
		...
		cache_addb(0x50);	// push eax
		cache_addb(0xe8);
		cache_addd(((System::UInt32)&mem_writed_checked) - (System::UInt32)cache.pos-4);
		cache_addw(0xc483);		// add esp,8
		cache_addb(0x08);
		cache_addw(0x012c);		// sub al,1
		cache_addb(0x5a);		// pop edx
...
skip_extend_word:
		...
		int imm=0;
		DynReg * base=0;DynReg * scaled=0;unsigned scale=0;
		switch (decode.modrm.rm) {
		...
		case 5:
			if (decode.modrm.mod) {
				...
				imm=(System::Int32)decode_fetchd();segbase=DREG(DS);
			}
			break;
		case 6:base=DREG(ESI);segbase=DREG(DS);break;
		case 7:base=DREG(EDI);segbase=DREG(DS);break;
...
static void dyn_dop_byte_imm(DualOps op,DynReg * dr1,System::Byte di1) {
	unsigned val;
	if (decode_fetchb_imm(val)) {
		gen_dop_byte_imm_mem(op,dr1,di1,(void*)val);
	} else {
		gen_dop_byte_imm(op,dr1,di1,(System::Byte)val);
	}
}
...
static void dyn_mov_ebgb(void) {
	dyn_get_modrm();
	DynReg * rm_reg=&DynRegs[decode.modrm.reg&3];unsigned rm_regi=decode.modrm.reg&4;
	if (decode.modrm.mod<3) {
		dyn_fill_ea();
...
static void dyn_imul_gvev(unsigned immsize) {
	...
	case 1:gen_imul_word_imm(decode.big_op,rm_reg,src,(System::SByte)decode_fetchb());break;
	case 2:gen_imul_word_imm(decode.big_op,rm_reg,src,(System::Int16)decode_fetchw());break;
	case 4:gen_imul_word_imm(decode.big_op,rm_reg,src,(System::Int32)decode_fetchd());break;
	}
	gen_releasereg(DREG(TMPW));
}
...
static void dyn_cmpxchg_evgv(void) {
	...
	if (decode.modrm.mod<3) {
		...
		System::Byte * branch=gen_create_branch(BR_NZ);
		...
		System::Byte * jump=gen_create_jump();
...
static void dyn_grp1_ev_ivx(bool withbyte) {
	...
	if (decode.modrm.mod<3) {
		...
		if (!withbyte) {
			...
			gen_dop_word_imm(op,decode.big_op,DREG(TMPW),(System::SByte)decode_fetchb());
		}
		if (op!=DOP_CMP) dyn_write_word_release(DREG(EA),DREG(TMPW),decode.big_op);
		else gen_releasereg(DREG(EA));
		gen_releasereg(DREG(TMPW));
		...
		if (!withbyte) {
			...
			gen_dop_word_imm(op,decode.big_op,&DynRegs[decode.modrm.rm],(System::SByte)decode_fetchb());
		}
	}
}
...
static void dyn_grp2_eb(grp2_types type) {
	dyn_get_modrm();DynReg * src;System::Byte src_i;
	if (decode.modrm.mod<3) {
...
//	if (GCC_UNLIKELY((unsigned)(decode.segprefix))) IllegalOption("dyn_segprefix");
...
static void dyn_branched_exit(BranchTypes btype,System::Int32 eip_add) {
	...
	System::Byte * data=gen_create_branch(btype);
...
static void dyn_loop(LoopTypes type) {
	...
	unsigned eip_base=decode.code-decode.code_start;
	System::Byte * branch1=0;System::Byte * branch2=0;
	dyn_save_critical_regs();
	switch (type) {
	case LOOP_E:
		gen_needflags();
...
static void dyn_interrupt(unsigned num) {
	gen_protectflags();
	dyn_flags_gen_to_host();
	dyn_reduce_cycles();
	dyn_set_eip_last_end(DREG(TMPW));
...
static void dyn_add_iocheck(unsigned access_size) {
...
static CacheBlock * CreateCacheBlock(CodePageHandler * codepage,PhysPt start,unsigned max_opcodes) {
	int i;
/* Init a load of variables */
	...
	gen_save_host_direct(&cache.block.running,(uint32_t)decode.block);
	for (i=0;i<G_MAX;i++) {
...
restart_prefix:
		...
		case 0x0f:
		{
			unsigned dual_code=decode_fetchb();
			switch (dual_code) {
			/* Short conditional jumps */
			case 0x80:case 0x81:case 0x82:case 0x83:case 0x84:case 0x85:case 0x86:case 0x87:	
			case 0x88:case 0x89:case 0x8a:case 0x8b:case 0x8c:case 0x8d:case 0x8e:case 0x8f:	
				dyn_branched_exit((BranchTypes)(dual_code&0xf),
					decode.big_op ? (System::Int32)decode_fetchd() : (System::Int16)decode_fetchw());	
				goto finish_block;
			/* PUSH/POP FS */
		...
		case 0x6a:		/* PUSH Ibx */
			gen_dop_word_imm(DOP_MOV,true,DREG(TMPW),(System::SByte)decode_fetchb());
			dyn_push(DREG(TMPW));
			gen_releasereg(DREG(TMPW));
			break;
		/* Imul Ibx */
		...
		case 0x78:case 0x79:case 0x7a:case 0x7b:case 0x7c:case 0x7d:case 0x7e:case 0x7f:	
			dyn_branched_exit((BranchTypes)(opcode&0xf),(System::SByte)decode_fetchb());	
			goto finish_block;
		/* Group 1 */
		...
		case 0xa2:
			if (decode.big_addr) {
				...
				if (decode_fetchd_imm(val)) {
					...
					gen_lea(DREG(EA),decode.segprefix ? decode.segprefix : DREG(DS),0,0,(int)val);
				}
...
//		case 0xcd:dyn_interrupt(decode_fetchb());goto finish_block;
		...
		case 0xe8:		/* CALL Ivx */
			...
			goto finish_block;
		case 0xe9:		/* Jmp Ivx */
			dyn_exit_link(decode.big_op ? (System::Int32)decode_fetchd() : (System::Int16)decode_fetchw());
			goto finish_block;
		case 0xea:		/* JMP FAR Ip */
			dyn_jmp_far_imm();
			goto finish_block;
			/* Jmp Ibx */
		case 0xeb:dyn_exit_link((System::SByte)decode_fetchb());goto finish_block;
		/* IN AL/AX,DX*/
		case 0xec:
			dyn_add_iocheck(1);
