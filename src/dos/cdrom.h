...
private:
	class TrackFile {
	public:
		virtual bool read(System::Byte *buffer, int seek, int count) = 0;
	...
	public:
		...
		~BinaryFile();
		bool read(System::Byte *buffer, int seek, int count);
		int getLength();
	private:
		BinaryFile();
		std::ifstream *file;
...
public:
	CDROM_Interface_Image		(System::Byte subUnit);
	...
	bool	ReadSector		(System::Byte *buffer, bool raw, unsigned long sector);
...
private:
	// player
static	void	CDAudioCallBack(unsigned len);
...
private:
	...
	static void dx_CDAudioCallBack(unsigned len);
