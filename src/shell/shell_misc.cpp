...
static void outc(System::Byte c) {
	System::UInt16 n=1;
	DOS_WriteFile(STDOUT,&c,&n);
}

void DOS_Shell::InputCommand(char * line) {
	unsigned size=CMD_MAXLINE-2; //lastcharacter+0
	System::Byte c;System::UInt16 n=1;
	unsigned str_len=0;unsigned str_index=0;
	System::UInt16 len=0;
	...
	while (size) {
		...
		case 0x00:				/* Extended Keys */
			{
				...
				case 0x3d:		/* F3 */
					...
					if (it_history != l_history.end() && it_history->length() > str_len) {
						...
						str_len = str_index = (unsigned)it_history->length();
				...
				case 0x48:	/* UP */
					...
					len = (System::UInt16)it_history->length();
					str_len = str_index = len;
					size = CMD_MAXLINE - str_index - 2;
					DOS_WriteFile(STDOUT, (System::Byte *)line, &len);
					it_history ++;
					break;
				...
				case 0x53:/* DELETE */
					{
						...
						System::UInt16 a=str_len-str_index-1;
						System::Byte* text=reinterpret_cast<System::Byte*>(&line[str_index+1]);
						DOS_WriteFile(STDOUT,text,&a);//write buffer to screen
						outc(' ');outc(8);
						for(unsigned i=str_index;i<str_len-1;i++) {
							line[i]=line[i+1];
							outc(8);
						}
						line[--str_len]=0;
		...
		case 0x08:				/* BackSpace */
			if (str_index) {
				outc(8);
				System::UInt32 str_remain=str_len - str_index;
				size++;
				if (str_remain) {
					...
					str_index --;
					/* Go back to redraw */
					for (System::UInt16 i=str_index; i < str_len; i++)
						outc(line[i]);
				} else {
					line[--str_index] = '\0';
					str_len--;
		...
		case'\t':
			{
				if (l_completion.size()) {
					...
					if (p_completion_start) {
						p_completion_start ++;
						completion_index = (System::UInt16)(str_len - strlen(p_completion_start));
					} else {
					...
					char *path;
					if ((path = strrchr(line+completion_index,'\\'))) completion_index = (System::UInt16)(path-line+1);
					if ((path = strrchr(line+completion_index,'/'))) completion_index = (System::UInt16)(path-line+1);
					...
					DOS_DTA dta(dos.dta());
					char name[DOS_NAMELENGTH_ASCII];System::UInt32 sz;System::UInt16 date;System::UInt16 time;System::Byte att;
				...
				if (l_completion.size() && it_completion->length()) {
					...
					len = (System::UInt16)it_completion->length();
					str_len = str_index = completion_index + len;
					size = CMD_MAXLINE - str_index - 2;
					DOS_WriteFile(STDOUT, (System::Byte *)it_completion->c_str(), &len);
				}
			}
			break;
		case 0x1b:   /* ESC */
		...
		default:
			...
			if(str_index < str_len && true) { //mem_readb(BIOS_KEYBOARD_FLAGS1)&0x80) dev_con.h ?
				...
				System::UInt16 a = str_len - str_index;
				System::Byte* text=reinterpret_cast<System::Byte*>(&line[str_index]);
				DOS_WriteFile(STDOUT,text,&a);//write buffer to screen
				outc(8);//undo the cursor the right.
				for(unsigned i=str_len;i>str_index;i--) {
 ...
 * return false =>       check for hardware changes in do_command */
	...
	{	/* only .bat .exe .com extensions maybe be executed by the shell */
		...
		MEM_BlockWrite(Real2Phys(file_name),fullname,(unsigned)(strlen(fullname)+1));
		...
		cmdtail.count=(System::Byte)strlen(line);
