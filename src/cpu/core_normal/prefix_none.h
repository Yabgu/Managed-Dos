 ...
 */
	...
	CASE_W(0x62)												/* BOUND */
		{
			System::Int16 bound_min, bound_max;
			GetRMrw;GetEAa;
			bound_min=LoadMw(eaa);
			bound_max=LoadMw(eaa+2);
			if ( (((System::Int16)*rmrw) < bound_min) || (((System::Int16)*rmrw) > bound_max) ) {
				EXCEPTION(5);
			}
		}
		break;
	CASE_W(0x63)												/* ARPL Ew,Rw */
		{
			...
			GetRMrw;
			if (rm >= 0xc0 ) {
				GetEArw;unsigned new_sel=*earw;
				CPU_ARPL(new_sel,*rmrw);
				*earw=(System::UInt16)new_sel;
			} else {
				GetEAa;unsigned new_sel=LoadMw(eaa);
				CPU_ARPL(new_sel,*rmrw);
				SaveMw(eaa,(System::UInt16)new_sel);
			}
		}
		break;
	CASE_B(0x64)												/* SEG FS: */
	...
	CASE_W(0x81)												/* Grpl Ew,Iw */
		{
			GetRM;unsigned which=(rm>>3)&7;
			if (rm>= 0xc0) {
				GetEArw;System::UInt16 iw=Fetchw();
				switch (which) {
				...
				GetEAa;System::UInt16 iw=Fetchw();
				switch (which) {
	...
	CASE_W(0x83)												/* Grpl Ew,Ix */
		{
			GetRM;unsigned which=(rm>>3)&7;
			if (rm>= 0xc0) {
				GetEArw;System::UInt16 iw=(System::Int16)Fetchbs();
				switch (which) {
				case 0x00:ADDW(*earw,iw,LoadRw,SaveRw);break;
				case 0x01: ORW(*earw,iw,LoadRw,SaveRw);break;
				case 0x02:ADCW(*earw,iw,LoadRw,SaveRw);break;
				...
				GetEAa;System::UInt16 iw=(System::Int16)Fetchbs();
				switch (which) {
				case 0x00:ADDW(eaa,iw,LoadMw,SaveMw);break;
				case 0x01: ORW(eaa,iw,LoadMw,SaveMw);break;
				case 0x02:ADCW(eaa,iw,LoadMw,SaveMw);break;
	...
	CASE_W(0x8c)												/* Mov Ew,Sw */
		{
			GetRM;System::UInt16 val;unsigned which=(rm>>3)&7;
			switch (which) {
			case 0x00:					/* MOV Ew,ES */
	...
	CASE_W(0x8d)												/* LEA Gw */
		{
			...
			GetRMrw;
			if (TEST_PREFIX_ADDR) {
				*rmrw=(System::UInt16)(*EATable[256+rm])();
			} else {
				*rmrw=(System::UInt16)(*EATable[rm])();
			}
			break;
		}
	...
	CASE_W(0x91)												/* XCHG CX,AX */
		{ System::UInt16 temp=reg_ax;reg_ax=reg_cx;reg_cx=temp; }
		break;
	CASE_W(0x92)												/* XCHG DX,AX */
		{ System::UInt16 temp=reg_ax;reg_ax=reg_dx;reg_dx=temp; }
		break;
	CASE_W(0x93)												/* XCHG BX,AX */
		{ System::UInt16 temp=reg_ax;reg_ax=reg_bx;reg_bx=temp; }
		break;
	CASE_W(0x94)												/* XCHG SP,AX */
		{ System::UInt16 temp=reg_ax;reg_ax=reg_sp;reg_sp=temp; }
		break;
	CASE_W(0x95)												/* XCHG BP,AX */
		{ System::UInt16 temp=reg_ax;reg_ax=reg_bp;reg_bp=temp; }
		break;
	CASE_W(0x96)												/* XCHG SI,AX */
		{ System::UInt16 temp=reg_ax;reg_ax=reg_si;reg_si=temp; }
		break;
	CASE_W(0x97)												/* XCHG DI,AX */
		{ System::UInt16 temp=reg_ax;reg_ax=reg_di;reg_di=temp; }
		break;
	CASE_W(0x98)												/* CBW */
		reg_ax=(System::SByte)reg_al;break;
	CASE_W(0x99)												/* CWD */
	...
	CASE_W(0x9a)												/* CALL Ap */
		{ 
			FillFlags();
			System::UInt16 newip=Fetchw();System::UInt16 newcs=Fetchw();
	...
	CASE_W(0xc8)												/* ENTER Iw,Ib */
		{
			unsigned bytes=Fetchw();
			unsigned level=Fetchb();
	...
	CASE_B(0xd7)												/* XLAT */
		if (TEST_PREFIX_ADDR) {
			reg_al=LoadMb(BaseDS+(System::UInt32)(reg_ebx+reg_al));
		} else {
			reg_al=LoadMb(BaseDS+(System::UInt16)(reg_bx+reg_al));
		}
		break;
#ifdef CPU_FPU
	...
	CASE_W(0xe8)												/* CALL Jw */
		{ 
			System::UInt16 addip=Fetchws();
			SAVEIP;
			Push_16(reg_eip);
			reg_eip=(System::UInt16)(reg_eip+addip);
			continue;
		}
	CASE_W(0xe9)												/* JMP Jw */
		{ 
	...
	CASE_W(0xea)												/* JMP Ap */
		{ 
			System::UInt16 newip=Fetchw();
			System::UInt16 newcs=Fetchw();
			FillFlags();
			...
			case 0x03:										/* CALL Ep */
				{
					if (rm >= 0xc0) goto illegal_opcode;
					GetEAa;
					System::UInt16 newip=LoadMw(eaa);
					System::UInt16 newcs=LoadMw(eaa+2);
					FillFlags();
					CPU_CALL(false,newcs,newip,GETIP);
#if CPU_TRAP_CHECK
					if (GETFLAG(TF)) {	
