...
static void write_crtc_index_other(unsigned /*port*/,unsigned val,unsigned /*iolen*/) {
	vga.other.index=(System::Byte)val;
}

static unsigned read_crtc_index_other(unsigned /*port*/,unsigned /*iolen*/) {
	return vga.other.index;
}

static void write_crtc_data_other(unsigned /*port*/,unsigned val,unsigned /*iolen*/) {
	switch (vga.other.index) {
	case 0x00:		//Horizontal total
		if (vga.other.htotal ^ val) VGA_StartResize();
		vga.other.htotal=(System::Byte)val;
		break;
	case 0x01:		//Horizontal displayed chars
		if (vga.other.hdend ^ val) VGA_StartResize();
		vga.other.hdend=(System::Byte)val;
		break;
	case 0x02:		//Horizontal sync position
		vga.other.hsyncp=(System::Byte)val;
		break;
	case 0x03:		//Horizontal sync width
		if (machine==MCH_TANDY) vga.other.vsyncw=(System::Byte)(val >> 4);
		else vga.other.vsyncw = 16; // The MC6845 has a fixed v-sync width of 16 lines
		vga.other.hsyncw=(System::Byte)(val & 0xf);
		break;
	case 0x04:		//Vertical total
		if (vga.other.vtotal ^ val) VGA_StartResize();
		vga.other.vtotal=(System::Byte)val;
		break;
	case 0x05:		//Vertical display adjust
		if (vga.other.vadjust ^ val) VGA_StartResize();
		vga.other.vadjust=(System::Byte)val;
		break;
	case 0x06:		//Vertical rows
		if (vga.other.vdend ^ val) VGA_StartResize();
		vga.other.vdend=(System::Byte)val;
		break;
	case 0x07:		//Vertical sync position
		vga.other.vsyncp=(System::Byte)val;
		break;
	case 0x09:		//Max scanline
		...
		vga.other.max_scanline=(System::Byte)val;
		break;
	case 0x0A:	/* Cursor Start Register */
		vga.other.cursor_start = (System::Byte)(val & 0x3f);
		vga.draw.cursor.sline = (System::Byte)(val&0x1f);
		vga.draw.cursor.enabled = ((val & 0x60) != 0x20);
		break;
	case 0x0B:	/* Cursor End Register */
		vga.other.cursor_end = (System::Byte)(val&0x1f);
		vga.draw.cursor.eline = (System::Byte)(val&0x1f);
		break;
	...
	case 0x0E:	/*Cursor Location High Register */
		...
		vga.config.cursor_start|=((System::Byte)val) << 8;
		break;
	case 0x0F:	/* Cursor Location Low Register */
		vga.config.cursor_start&=0xff00;
		vga.config.cursor_start|=(System::Byte)val;
		break;
	case 0x10:	/* Light Pen High */
		vga.other.lightpen &= 0xff;
		vga.other.lightpen |= (val & 0x3f)<<8;		// only 6 int
		break;
	case 0x11:	/* Light Pen Low */
		vga.other.lightpen &= 0xff00;
		vga.other.lightpen |= (System::Byte)val;
		break;
	default:
		LOG(LOG_VGAMISC,LOG_NORMAL)("MC6845:Write %X to illegal index %x",val,vga.other.index);
	}
}
static unsigned read_crtc_data_other(unsigned /*port*/,unsigned /*iolen*/) {
	switch (vga.other.index) {
	case 0x00:		//Horizontal total
	...
	case 0x0C:	/* Start Address High Register */
		return (System::Byte)(vga.config.display_start >> 8);
	case 0x0D:	/* Start Address Low Register */
		return (System::Byte)(vga.config.display_start & 0xff);
	case 0x0E:	/*Cursor Location High Register */
		return (System::Byte)(vga.config.cursor_start >> 8);
	case 0x0F:	/* Cursor Location Low Register */
		return (System::Byte)(vga.config.cursor_start & 0xff);
	case 0x10:	/* Light Pen High */
		return (System::Byte)(vga.other.lightpen >> 8);
	case 0x11:	/* Light Pen Low */
		return (System::Byte)(vga.other.lightpen & 0xff);
	default:
...
static System::Byte cga16_val = 0;
static void update_cga16_color(void);
static System::Byte herc_pal = 0;

static void cga16_color_select(System::Byte val) {
	cga16_val = val;
	update_cga16_color();
}
...
// It uses an avarage over the int to give smooth transitions from colour to colour
...
// to match an entry that is generated here.
	...
	unsigned colorBit1,colorBit2,colorBit3,colorBit4,index;
	...
	for(unsigned i = 0; i < 16;i++) {
		for(unsigned j = 0;j < 5;j++) {
			...
			I -= (double) colorBit3;
			Q -= (double) colorBit4;
			Y  = (double) j / 4.0; //calculated avarage is over 4 int
			...
			RENDER_SetPal((System::Byte)index,static_cast<System::Byte>(R*baseR),static_cast<System::Byte>(G*baseG),static_cast<System::Byte>(B*baseB));
		}
	}
}
...
static void write_color_select(System::Byte val) {
	...
	case M_TANDY4:
		{
			...
			System::Byte base=(val & 0x10) ? 0x08 : 0;
			/* Check for BW Mode */
...
static void TandyCheckLineMask(void ) {
	...
	if ( vga.tandy.line_mask ) {
		...
		vga.tandy.addr_mask = (unsigned)(~0);
		vga.tandy.line_shift = 0;
	}
}

static void write_tandy_reg(System::Byte val) {
...
static void write_cga(unsigned port,unsigned val,unsigned /*iolen*/) {
	switch (port) {
	case 0x3d8:
		vga.tandy.mode_control=(System::Byte)val;
		vga.attr.disabled = (val&0x8)? 0: 1; 
		if (vga.tandy.mode_control & 0x2) {
			if (vga.tandy.mode_control & 0x10) {
		...
		break;
	case 0x3d9:
		write_color_select((System::Byte)val);
		break;
	}
}

static void write_tandy(unsigned port,unsigned val,unsigned /*iolen*/) {
	switch (port) {
	case 0x3d8:
		vga.tandy.mode_control=(System::Byte)val;
		TandyCheckLineMask();
	...
	case 0x3d9:
		...
		break;
	case 0x3da:
		vga.tandy.reg_index=(System::Byte)val;
		break;
	...
	case 0x3dc:	// Preset lightpen latch
		if (!vga.other.lightpen_triggered) {
			...
			unsigned current_scanline = (unsigned)(timeInFrame / vga.draw.delay.htotal);
			
			vga.other.lightpen = (System::UInt16)((vga.draw.address_add/2) * (current_scanline/2));
			vga.other.lightpen += (System::UInt16)((timeInLine / vga.draw.delay.hdend) *
...
//	case 0x3dd:	//Extended ram page address register:
		break;
	case 0x3de:
		write_tandy_reg((System::Byte)val);
		break;
	case 0x3df:
		vga.tandy.line_mask = (System::Byte)(val >> 6);
...
static void write_pcjr(unsigned port,unsigned val,unsigned /*iolen*/) {
	switch (port) {
	case 0x3d9:
		write_color_select((System::Byte)val);
		break;
	case 0x3da:
		if (vga.tandy.pcjr_flipflop) write_tandy_reg((System::Byte)val);
		else vga.tandy.reg_index=(System::Byte)val;
...
static void write_hercules(unsigned port,unsigned val,unsigned /*iolen*/) {
	switch (port) {
	case 0x3b8: {
		// the protected int can always be cleared but only be set if the 
		// protection int are set
		if (vga.herc.mode_control&0x2) {
			...
			if ((val & 0x2) && (vga.herc.enable_int & 0x1)) {
				vga.herc.mode_control |= 0x2;
		...
		if (vga.herc.mode_control&0x80) {
			...
			if ((val & 0x80) && (vga.herc.enable_int & 0x2)) {
				vga.herc.mode_control |= 0x80;
	...
	case 0x3bf:
		vga.herc.enable_int=(System::Byte)val;
		break;
	}
}

/* static unsigned read_hercules(unsigned port,unsigned iolen) {
	LOG_MSG("read from Herc port %x",port);
	return 0;
} */

unsigned read_herc_status(unsigned /*port*/,unsigned /*iolen*/) {
	...
	double timeInFrame = PIC_FullIndex()-vga.draw.delay.framestart;
	System::Byte retval=0x72; // Hercules ident; from a working card (Winbond W86855AF)
...
void VGA_SetupOther(void) {
	...
	if (machine==MCH_CGA) {
		unsigned base=0x3d0;
		for (unsigned port_ct=0; port_ct<4; port_ct++) {
