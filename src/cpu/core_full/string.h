{
	...
	unsigned	si_index,di_index;
	unsigned	add_mask;
	unsigned	count,count_left;
	int	add_index;
	...
	if (!(inst.prefix & PREFIX_REP)) {
		...
		CPU_Cycles++;
		if ((count>(unsigned)CPU_Cycles) && (inst.code.op<R_SCASB)) {
	...
	case R_SCASW:
		{
			add_index<<=1;System::UInt16 val2;
			for (;count>0;) {
				count--;CPU_Cycles--;
	...
	case R_SCASD:
		{
			add_index<<=2;System::UInt32 val2;
			for (;count>0;) {
				count--;CPU_Cycles--;
	...
	case R_CMPSW:
		{
			add_index<<=1;System::UInt16 val1,val2;
			for (;count>0;) {
				count--;CPU_Cycles--;
	...
	case R_CMPSD:
		{
			add_index<<=2;System::UInt32 val1,val2;
			for (;count>0;) {
				count--;CPU_Cycles--;
