...
static INLINE bool InvalidHandle(unsigned handle) {
...
unsigned XMS_QueryFreeMemory(System::UInt16& largestFree, System::UInt16& totalFree) {
	...
	totalFree=(System::UInt16)(MEM_FreeTotal()*4);
	largestFree=(System::UInt16)(MEM_FreeLargest()*4);
	if (!totalFree) return XMS_OUT_OF_SPACE;
	return 0;
}

unsigned XMS_AllocateMemory(unsigned size, System::UInt16& handle) {	// size = kb
	/* Find free handle */
	System::UInt16 index=1;
	while (!xms_handles[index].free) {
		if (++index>=XMS_HANDLES) return XMS_OUT_OF_HANDLES;
	}
	MemHandle mem;
	if (size!=0) {
		unsigned pages=(size/4) + ((size & 3) ? 1 : 0);
...
unsigned XMS_MoveMemory(PhysPt bpt) {
	...
	unsigned length=mem_readd(bpt+offsetof(XMS_MemMove,length));
	unsigned src_handle=mem_readw(bpt+offsetof(XMS_MemMove,src_handle));
	union {
		RealPt realpt;
		System::UInt32 offset;
	} src,dest;
	src.offset=mem_readd(bpt+offsetof(XMS_MemMove,src.offset));
	unsigned dest_handle=mem_readw(bpt+offsetof(XMS_MemMove,dest_handle));
...
unsigned XMS_GetHandleInformation(unsigned handle, System::Byte& lockCount, System::Byte& numFree, System::UInt16& size) {
	...
	numFree=0;
	for (unsigned i=1;i<XMS_HANDLES;i++) {
		if (xms_handles[i].free) numFree++;
	}
	size=(System::UInt16)(xms_handles[handle].size);
	return 0;
}
...
unsigned XMS_ResizeMemory(unsigned handle, unsigned newSize) {
	...
	unsigned pages=newSize/4 + ((newSize & 3) ? 1 : 0);
...
INLINE void SET_RESULT(unsigned res,bool touch_bl_on_succes=true) {
	if(touch_bl_on_succes || res) reg_bl = (System::Byte)res;
	reg_ax = (res==0);
}

unsigned XMS_Handler(void) {
//	LOG(LOG_MISC,LOG_ERROR)("XMS: CALL %02X",reg_ah);
	...
	case XMS_LOCK_EXTENDED_MEMORY_BLOCK: {						/* 0c */
		...
		unsigned res = XMS_LockMemory(reg_dx, address);
		if(res) reg_bl = (System::Byte)res;
		reg_ax = (res==0);
		if (res==0) { // success
			reg_bx=(System::UInt16)(address & 0xFFFF);
			reg_dx=(System::UInt16)(address >> 16);
		};
		}; break;
	...
	case XMS_ALLOCATE_UMB: {									/* 10 */
		...
		System::UInt16 size=reg_dx;System::UInt16 seg;
	...
	case XMS_GET_EMB_HANDLE_INFORMATION_EXT: {					/* 8e */
		System::Byte free_handles;
		unsigned result = XMS_GetHandleInformation(reg_dx,reg_bh,free_handles,reg_dx);
