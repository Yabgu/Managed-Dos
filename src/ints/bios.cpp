...
static System::UInt16 size_extended;
static int other_memsystems=0;
void CMOS_SetRegister(unsigned regNr, System::Byte val); //For setting equipment word

static unsigned INT70_Handler(void) {
	/* Acknowledge irq with cmos */
	IO_Write(0x70,0xc);
	IO_Read(0x71);
	if (mem_readb(BIOS_WAIT_FLAG_ACTIVE)) {
		System::UInt32 count=mem_readd(BIOS_WAIT_FLAG_COUNT);
		if (count>997) {
...
static bool Tandy_InitializeSB() {
	...
	unsigned sbport, sbirq, sbdma;
	if (SB_Get_Address(sbport, sbirq, sbdma)) {
		tandy_sb.port=(System::UInt16)(sbport&0xffff);
		tandy_sb.irq =(System::Byte)(sbirq&0xff);
		tandy_sb.dma =(System::Byte)(sbdma&0xff);
		return true;
	} else {
...
static bool Tandy_InitializeTS() {
	...
	unsigned tsport, tsirq, tsdma;
	if (TS_Get_Address(tsport, tsirq, tsdma)) {
		tandy_dac.port=(System::UInt16)(tsport&0xffff);
		tandy_dac.irq =(System::Byte)(tsirq&0xff);
		tandy_dac.dma =(System::Byte)(tsdma&0xff);
		return true;
	} else {
...
static void Tandy_SetupTransfer(PhysPt bufpt,bool isplayback) {
	...
	System::Byte bufpage=(System::Byte)((bufpt>>16)&0xff);
	IO_Write(tandy_dma*2,(System::Byte)(bufpt&0xff));
	IO_Write(tandy_dma*2,(System::Byte)((bufpt>>8)&0xff));
	switch (tandy_dma) {
	...
	System::UInt32 tlength=length;
	if (tlength+(bufpt&0xffff)>0x10000) tlength=0x10000-(bufpt&0xffff);
	real_writew(0x40,0xd0,(System::UInt16)(length-tlength));	/* remaining buffer length */
	tlength--;

	/* set transfer size */
	IO_Write(tandy_dma*2+1,(System::Byte)(tlength&0xff));
	IO_Write(tandy_dma*2+1,(System::Byte)((tlength>>8)&0xff));

	System::UInt16 delay=(System::UInt16)(real_readw(0x40,0xd2)&0xfff);
	System::Byte amplitude=(System::Byte)((real_readw(0x40,0xd2)>>13)&0x7);
	if (tandy_sb.port) {
		...
		/* set transfer size */
		IO_Write(tandy_sb.port+0xc,(System::Byte)(tlength&0xff));
		IO_Write(tandy_sb.port+0xc,(System::Byte)((tlength>>8)&0xff));
	} else {
		...
		IO_Write(tandy_dac.port+2,(System::Byte)(delay&0xff));
		IO_Write(tandy_dac.port+3,(System::Byte)(((delay>>8)&0xf) | (amplitude<<5)));
	...
	if (!isplayback) {
		...
		real_writew(0x40,0xd2,(System::UInt16)(delay|0x1000));
	}
}

static unsigned IRQ_TandyDAC(void) {
	if (tandy_dac.port) {
	...
	if (real_readw(0x40,0xd0)) {	/* play/record next buffer */
		...
		System::Byte npage=real_readb(0x40,0xd4)+1;
		real_writeb(0x40,0xd4,npage);

		unsigned rb=real_readb(0x40,0xd3);
		if (rb&0x10) {
			/* start recording */
...
static void TandyDAC_Handler(System::Byte tfunction) {
	...
	case 0x85:	/* Tandy sound system reset */
		if (tandy_dac.port) {
			IO_Write(tandy_dac.port,(System::Byte)(IO_Read(tandy_dac.port)&0xe0));
		}
		reg_ah=0x00;
		CALLBACK_SCF(false);
		break;
...
static unsigned INT1A_Handler(void) {
	switch (reg_ah) {
	case 0x00:	/* Get System time */
		{
			System::UInt32 ticks=mem_readd(BIOS_TIMER);
			reg_al=0;		/* Midnight never passes :) */
			reg_cx=(System::UInt16)(ticks >> 16);
			reg_dx=(System::UInt16)(ticks & 0xffff);
			break;
		}
...
static unsigned INT11_Handler(void) {
...
#endif
static unsigned INT8_Handler(void) {
	/* Increase the bios tick counter */
	System::UInt32 value = mem_readd(BIOS_TIMER) + 1;
#if DOSBOX_CLOCKSYNC
	...
	if((value %50)==0) {
		if(((value %100)==0) && check) {
			...
			System::UInt32 ticksnu = (System::UInt32)((loctime->tm_hour*3600+loctime->tm_min*60+loctime->tm_sec)*(float)PIT_TICK_RATE/65536.0);
			System::Int32 bios = value;System::Int32 tn = ticksnu;
			System::Int32 diff = tn - bios;
			if(diff>0) {
	...
	System::Byte val = mem_readb(BIOS_DISK_MOTOR_TIMEOUT);
...
static unsigned INT1C_Handler(void) {
	return CBRET_NONE;
}

static unsigned INT12_Handler(void) {
	reg_ax=mem_readw(BIOS_MEMORY_SIZE);
	return CBRET_NONE;
}

static unsigned INT17_Handler(void) {
...
static System::Byte INT14_Wait(System::UInt16 port, System::Byte mask, System::Byte timeout) {
...
static unsigned INT14_Handler(void) {
	...
	System::UInt16 port = real_readw(0x40,reg_dx*2); // DX is always port number
	System::Byte timeout = mem_readb(BIOS_COM1_TIMEOUT + reg_dx);
	if (port==0)	{
	...
	case 0x00:	{
		...
		System::UInt16 baudresult;
		unsigned rawbaud=reg_al>>5;
		...
		baudresult = (System::UInt16)(115200 / baudrate);

		IO_WriteB(port+3, 0x80);	// enable divider access
		IO_WriteB(port, (System::Byte)baudresult&0xff);
		IO_WriteB(port+1, (System::Byte)(baudresult>>8));
		...
		// get result
		reg_ah=(System::Byte)(IO_ReadB(port+5)&0xff);
		reg_al=(System::Byte)(IO_ReadB(port+6)&0xff);
		CALLBACK_SCF(false);
		break;
	}
...
static unsigned INT15_Handler(void) {
	static System::UInt16 biosConfigSeg=0;
	switch (reg_ah) {
	case 0x06:
	...
	case 0x83:	/* BIOS - SET EVENT WAIT INTERVAL */
		{
			...
			System::UInt32 count=(reg_cx<<16)|reg_dx;
	...
	case 0x84:	/* BIOS - JOYSTICK SUPPORT (XT after 11/8/82,AT,XT286,PS) */
		if (reg_dx == 0x0000) {
			...
			if (JOYSTICK_IsEnabled(0)) {
				reg_ax = (System::UInt16)(JOYSTICK_GetMove_X(0)*127+128);
				reg_bx = (System::UInt16)(JOYSTICK_GetMove_Y(0)*127+128);
				if(JOYSTICK_IsEnabled(1)) {
					reg_cx = (System::UInt16)(JOYSTICK_GetMove_X(1)*127+128);
					reg_dx = (System::UInt16)(JOYSTICK_GetMove_Y(1)*127+128);
				}
				else {
					reg_cx = reg_dx = 0;
				}
...
static unsigned Reboot_Handler(void) {
...
public:
	BIOS(Section* configuration):Module_base(configuration){
		...
		for (System::UInt16 i=0;i<0x200;i++) real_writeb(0x40,i,0);

		/* Setup all the interrupt handlers the bios controls */

		/* INT 8 Clock IRQ Handler */
		unsigned call_irq0=CALLBACK_Allocate();	
		...
		/* Irq 2 */
		unsigned call_irq2=CALLBACK_Allocate();	
		...
		for(unsigned i = 0; i < strlen(b_type); i++) phys_writeb(0xfe00e + i,b_type[i]);
		
		// System BIOS version
		const char* const b_vers =
			"DOSBox FakeBIOS v1.0";
		for(unsigned i = 0; i < strlen(b_vers); i++) phys_writeb(0xfe061+i,b_vers[i]);

		// write system BIOS date
		const char* const b_date = "01/01/92";
		for(unsigned i = 0; i < strlen(b_date); i++) phys_writeb(0xffff5+i,b_date[i]);
		...
		if (use_tandyDAC) {
			...
			if (tandy_dac_type) {
				...
				RealPt current_irq=RealGetVec(tandy_irq_vector);
				real_writed(0x40,0xd6,current_irq);
				for (System::UInt16 i=0; i<0x10; i++) phys_writeb(PhysMake(0xf000,0xa084+i),0x80);
		...
		unsigned ppindex=0; // number of lpt ports
		...
		//System::UInt16 config=0x4400;	//1 Floppy, 2 serial and 1 parallel 
		System::UInt16 config = 0x0;
		...
		CMOS_SetRegister(0x14,(System::Byte)(config&0xff)); //Should be updated on changes
	...
	~BIOS(){
		...
		if (tandy_DAC_callback[0]) {
			System::UInt32 orig_vector=real_readd(0x40,0xd6);
...
void BIOS_SetComPorts(System::UInt16 baseaddr[]) {
	System::UInt16 portcount=0;
	System::UInt16 equipmentword;
	...
	CMOS_SetRegister(0x14,(System::Byte)(equipmentword&0xff)); //Should be updated on changes
}
