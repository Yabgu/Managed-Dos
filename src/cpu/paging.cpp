...
unsigned PageHandler::readb(PhysPt addr) {
	E_Exit("No byte handler for read from %d",addr);	
	return 0;
}
unsigned PageHandler::readw(PhysPt addr) {
	return 
		(readb(addr+0) << 0) |
		(readb(addr+1) << 8);
}
unsigned PageHandler::readd(PhysPt addr) {
	return 
		(readb(addr+0) << 0)  |
		(readb(addr+1) << 8)  |
		(readb(addr+2) << 16) |
...
void PageHandler::writeb(PhysPt addr,unsigned /*val*/) {
	E_Exit("No byte handler for write to %d",addr);	
}

void PageHandler::writew(PhysPt addr,unsigned val) {
	writeb(addr+0,(System::Byte) (val >> 0));
	writeb(addr+1,(System::Byte) (val >> 8));
}
void PageHandler::writed(PhysPt addr,unsigned val) {
	writeb(addr+0,(System::Byte) (val >> 0));
	writeb(addr+1,(System::Byte) (val >> 8));
	writeb(addr+2,(System::Byte) (val >> 16));
	writeb(addr+3,(System::Byte) (val >> 24));
}

HostPt PageHandler::GetHostReadPt(unsigned /*phys_page*/) {
	return 0;
}

HostPt PageHandler::GetHostWritePt(unsigned /*phys_page*/) {
	return 0;
}

bool PageHandler::readb_checked(PhysPt addr, System::Byte * val) {
	*val=(System::Byte)readb(addr);	return false;
}
bool PageHandler::readw_checked(PhysPt addr, System::UInt16 * val) {
	*val=(System::UInt16)readw(addr);	return false;
}
bool PageHandler::readd_checked(PhysPt addr, System::UInt32 * val) {
	*val=(System::UInt32)readd(addr);	return false;
}
bool PageHandler::writeb_checked(PhysPt addr,unsigned val) {
	writeb(addr,val);	return false;
}
bool PageHandler::writew_checked(PhysPt addr,unsigned val) {
	writew(addr,val);	return false;
}
bool PageHandler::writed_checked(PhysPt addr,unsigned val) {
...
static int PageFaultCore(void) {
	CPU_CycleLeft+=CPU_Cycles;
	CPU_Cycles=1;
	int ret=CPU_Core_Full_Run();
	CPU_CycleLeft+=CPU_Cycles;
...
void PAGING_PageFault(PhysPt lin_addr,unsigned page_addr,unsigned faultcode) {
...
static INLINE void InitPageUpdateLink(unsigned relink,PhysPt addr) {
	if (relink==0) return;
...
static INLINE void InitPageCheckPresence(PhysPt lin_addr,bool writing,X86PageEntry& table,X86PageEntry& entry) {
	...
	unsigned d_index=lin_page >> 10;
	unsigned t_index=lin_page & 0x3ff;
	unsigned table_addr=(paging.base.page<<12)+d_index*4;
	...
	unsigned entry_addr=(table.block.base<<12)+t_index*4;
...
static INLINE bool InitPage_CheckUseraccess(unsigned u1,unsigned u2) {
...
public:
	...
	unsigned readb(PhysPt addr) {
		unsigned needs_reset=InitPage(addr,false);
		System::Byte val=mem_readb(addr);
		InitPageUpdateLink(needs_reset,addr);
		return val;
	}
	unsigned readw(PhysPt addr) {
		unsigned needs_reset=InitPage(addr,false);
		System::UInt16 val=mem_readw(addr);
		InitPageUpdateLink(needs_reset,addr);
		return val;
	}
	unsigned readd(PhysPt addr) {
		unsigned needs_reset=InitPage(addr,false);
		System::UInt32 val=mem_readd(addr);
		InitPageUpdateLink(needs_reset,addr);
		return val;
	}
	void writeb(PhysPt addr,unsigned val) {
		unsigned needs_reset=InitPage(addr,true);
		mem_writeb(addr,val);
		InitPageUpdateLink(needs_reset,addr);
	}
	...
	bool readb_checked(PhysPt addr, System::Byte * val) {
		if (InitPageCheckOnly(addr,false)) {
			*val=mem_readb(addr);
			return false;
		} else return true;
	}
	bool readw_checked(PhysPt addr, System::UInt16 * val) {
		if (InitPageCheckOnly(addr,false)){
			*val=mem_readw(addr);
			return false;
		} else return true;
	}
	bool readd_checked(PhysPt addr, System::UInt32 * val) {
		if (InitPageCheckOnly(addr,false)) {
			*val=mem_readd(addr);
			return false;
		} else return true;
	...
	unsigned InitPage(unsigned lin_addr,bool writing) {
		unsigned lin_page=lin_addr >> 12;
		unsigned phys_page;
		if (paging.enabled) {
			X86PageEntry table;
...
//						priv_check=0;	// default
	...
	bool InitPageCheckOnly(unsigned lin_addr,bool writing) {
		unsigned lin_page=lin_addr >> 12;
		if (paging.enabled) {
			X86PageEntry table;
			X86PageEntry entry;
	...
	void InitPageForced(unsigned lin_addr) {
		unsigned lin_page=lin_addr >> 12;
		unsigned phys_page;
		if (paging.enabled) {
			X86PageEntry table;
...
public:
	...
	void writeb(PhysPt addr,unsigned val) {
		InitPage(addr,(System::Byte)(val&0xff));
		host_writeb(get_tlb_read(addr)+addr,(System::Byte)(val&0xff));
	}
	void writew(PhysPt addr,unsigned val) {
		InitPage(addr,(System::UInt16)(val&0xffff));
		host_writew(get_tlb_read(addr)+addr,(System::UInt16)(val&0xffff));
	}
	void writed(PhysPt addr,unsigned val) {
		InitPage(addr,(System::UInt32)val);
		host_writed(get_tlb_read(addr)+addr,(System::UInt32)val);
	}
	bool writeb_checked(PhysPt addr,unsigned val) {
		unsigned writecode=InitPageCheckOnly(addr,(System::Byte)(val&0xff));
		if (writecode) {
			HostPt tlb_addr;
			if (writecode>1) tlb_addr=get_tlb_read(addr);
			else tlb_addr=get_tlb_write(addr);
			host_writeb(tlb_addr+addr,(System::Byte)(val&0xff));
			return false;
		}
		return true;
	}
	bool writew_checked(PhysPt addr,unsigned val) {
		unsigned writecode=InitPageCheckOnly(addr,(System::UInt16)(val&0xffff));
		if (writecode) {
			HostPt tlb_addr;
			if (writecode>1) tlb_addr=get_tlb_read(addr);
			else tlb_addr=get_tlb_write(addr);
			host_writew(tlb_addr+addr,(System::UInt16)(val&0xffff));
			return false;
		}
		return true;
	}
	bool writed_checked(PhysPt addr,unsigned val) {
		unsigned writecode=InitPageCheckOnly(addr,(System::UInt32)val);
		if (writecode) {
			HostPt tlb_addr;
			if (writecode>1) tlb_addr=get_tlb_read(addr);
			else tlb_addr=get_tlb_write(addr);
			host_writed(tlb_addr+addr,(System::UInt32)val);
			return false;
		}
		return true;
	}
	void InitPage(unsigned lin_addr,unsigned val) {
		unsigned lin_page=lin_addr >> 12;
		unsigned phys_page;
		if (paging.enabled) {
	...
	unsigned InitPageCheckOnly(unsigned lin_addr,unsigned val) {
		unsigned lin_page=lin_addr >> 12;
		if (paging.enabled) {
...
bool PAGING_MakePhysPage(unsigned & page) {
	if (paging.enabled) {
		unsigned d_index=page >> 10;
		unsigned t_index=page & 0x3ff;
		X86PageEntry table;
...
unsigned PAGING_GetDirBase(void) {
	return paging.cr3;
}

bool PAGING_ForcePageInit(unsigned lin_addr) {
...
void PAGING_ClearTLB(void) {
	System::UInt32 * entries=&paging.links.entries[0];
	for (;paging.links.used>0;paging.links.used--) {
		unsigned page=*entries++;
...
void PAGING_UnlinkPages(unsigned lin_page,unsigned pages) {
	for (;pages>0;pages--) {
...
void PAGING_MapPage(unsigned lin_page,unsigned phys_page) {
...
void PAGING_LinkPage(unsigned lin_page,unsigned phys_page) {
	PageHandler * handler=MEM_GetPageHandler(phys_page);
	unsigned lin_base=lin_page << 12;
...
void PAGING_LinkPage_ReadOnly(unsigned lin_page,unsigned phys_page) {
	PageHandler * handler=MEM_GetPageHandler(phys_page);
	unsigned lin_base=lin_page << 12;
	if (lin_page>=TLB_SIZE || phys_page>=TLB_SIZE) 
		E_Exit("Illegal page");
...
void PAGING_SetDirBase(unsigned cr3) {
	paging.cr3=cr3;
