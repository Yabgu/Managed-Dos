...
unsigned call_int13;
unsigned diskparm0, diskparm1;
static System::Byte last_status;
static System::Byte last_drive;
System::UInt16 imgDTASeg;
RealPt imgDTAPtr;
DOS_DTA *imgDTA;
bool killRead;
...
void updateDPT(void) {
	System::UInt32 tmpheads, tmpcyl, tmpsect, tmpsize;
	if(imageDiskList[2] != NULL) {
		PhysPt dp0physaddr=CALLBACK_PhysPointer(diskparm0);
		imageDiskList[2]->Get_Geometry(&tmpheads, &tmpcyl, &tmpsect, &tmpsize);
		phys_writew(dp0physaddr,(System::UInt16)tmpcyl);
		phys_writeb(dp0physaddr+0x2,(System::Byte)tmpheads);
		phys_writew(dp0physaddr+0x3,0);
		phys_writew(dp0physaddr+0x5,(System::UInt16)-1);
		...
		phys_writew(dp0physaddr+0xc,(System::UInt16)tmpcyl);
		phys_writeb(dp0physaddr+0xe,(System::Byte)tmpsect);
	}
	if(imageDiskList[3] != NULL) {
		...
		phys_writew(dp1physaddr,(System::UInt16)tmpcyl);
		phys_writeb(dp1physaddr+0x2,(System::Byte)tmpheads);
		phys_writeb(dp1physaddr+0xe,(System::Byte)tmpsect);
	}
}
...
System::Byte imageDisk::Read_Sector(System::UInt32 head,System::UInt32 cylinder,System::UInt32 sector,void * data) {
	System::UInt32 sectnum;
...
System::Byte imageDisk::Read_AbsoluteSector(System::UInt32 sectnum, void * data) {
	System::UInt32 bytenum;
...
System::Byte imageDisk::Write_Sector(System::UInt32 head,System::UInt32 cylinder,System::UInt32 sector,void * data) {
	System::UInt32 sectnum;
...
System::Byte imageDisk::Write_AbsoluteSector(System::UInt32 sectnum, void *data) {
	System::UInt32 bytenum;
...
imageDisk::imageDisk(FILE *imgFile, System::Byte *imgName, System::UInt32 imgSizeK, bool isHardDisk) {
	heads = 0;
	cylinders = 0;
	sectors = 0;
	sector_size = 512;
	...
	if(!isHardDisk) {
		...
		if(!founddisk) {
			...
			System::UInt16 equipment=mem_readw(BIOS_CONFIGURATION);
			if(equipment&1) {
				unsigned numofdisks = (equipment>>6)&3;
				numofdisks++;
			...
			CMOS_SetRegister(0x14, (System::Byte)(equipment&0xff));
		}
	}
}

void imageDisk::Set_Geometry(System::UInt32 setHeads, System::UInt32 setCyl, System::UInt32 setSect, System::UInt32 setSectSize) {
	heads = setHeads;
	cylinders = setCyl;
	sectors = setSect;
...
void imageDisk::Get_Geometry(System::UInt32 * getHeads, System::UInt32 *getCyl, System::UInt32 *getSect, System::UInt32 *getSectSize) {
	*getHeads = heads;
	*getCyl = cylinders;
	*getSect = sectors;
...
System::Byte imageDisk::GetBiosType(void) {
	if(!hardDrive) {
		return (System::Byte)DiskGeometryList[floppytype].biosval;
	} else return 0;
}

System::UInt32 imageDisk::getSectSize(void) {
	return sector_size;
}

static unsigned GetDosDriveNumber(unsigned biosNum) {
	switch(biosNum) {
		case 0x0:
			return 0x0;
		case 0x1:
...
static bool driveInactive(unsigned driveNum) {
...
static unsigned INT13_DiskHandler(void) {
	System::UInt16 segat, bufptr;
	System::Byte sectbuf[512];
	unsigned  drivenum;
	unsigned  i,t;
	last_drive = reg_dl;
	...
	case 0x2: /* Read sectors */
		...
		bufptr = reg_bx;
		for(i=0;i<reg_al;i++) {
			last_status = imageDiskList[drivenum]->Read_Sector((System::UInt32)reg_dh, (System::UInt32)(reg_ch | ((reg_cl & 0xc0)<< 2)), (System::UInt32)((reg_cl & 63)+i), sectbuf);
	...
	case 0x3: /* Write sectors */
		...
		for(i=0;i<reg_al;i++) {
			...
			last_status = imageDiskList[drivenum]->Write_Sector((System::UInt32)reg_dh, (System::UInt32)(reg_ch | ((reg_cl & 0xc0) << 2)), (System::UInt32)((reg_cl & 63) + i), &sectbuf[0]);
			if(last_status != 0x00) {
            CALLBACK_SCF(true);
				return CBRET_NONE;
			}
	...
	case 0x08: /* Get drive parameters */
		...
		reg_ch = (System::Byte)(tmpcyl & 0xff);
		reg_cl = (System::Byte)(((tmpcyl >> 2) & 0xc0) | (tmpsect & 0x3f)); 
		reg_dh = (System::Byte)tmpheads;
		last_status = 0x00;
