...
static void FPU_FLD_16(PhysPt addr) {
	dyn_dh_fpu.temp.m1 = (System::UInt32)mem_readw(addr);
}

static void FPU_FST_16(PhysPt addr) {
	mem_writew(addr,(System::UInt16)dyn_dh_fpu.temp.m1);
}
...
static void FPU_FLDCW_DH(PhysPt addr){
	...
	dyn_dh_fpu.temp.m1 = (System::UInt32)(dyn_dh_fpu.cw|0x3f);
}

static void FPU_FNSTCW_DH(PhysPt addr){
	mem_writew(addr,(System::UInt16)(dyn_dh_fpu.cw&0xffff));
}
...
static void FPU_FSTENV_DH(PhysPt addr){
	if(!cpu.code.big) {
		mem_writew(addr+0,(System::UInt16)dyn_dh_fpu.cw);
		mem_writew(addr+2,(System::UInt16)dyn_dh_fpu.temp.m2);
...
static void FPU_FLDENV_DH(PhysPt addr){
	if(!cpu.code.big) {
		dyn_dh_fpu.cw = (System::UInt32)mem_readw(addr);
		dyn_dh_fpu.temp.m1 = dyn_dh_fpu.cw|0x3f;
		dyn_dh_fpu.temp.m2 = (System::UInt32)mem_readw(addr+2);
...
static void FPU_FSAVE_DH(PhysPt addr){
	if (!cpu.code.big) {
		mem_writew(addr,(System::UInt16)dyn_dh_fpu.cw);
		addr+=2;
		...
		for(unsigned i=28;i<108;i++) mem_writeb(addr++,dyn_dh_fpu.temp_state[i]);
	} else {
		mem_writew(addr,(System::UInt16)dyn_dh_fpu.cw);
		addr+=2;
		for(unsigned i=2;i<108;i++) mem_writeb(addr++,dyn_dh_fpu.temp_state[i]);
	}
}
...
static void FPU_FRSTOR_DH(PhysPt addr){
	if (!cpu.code.big) {
		...
		for(unsigned i=28;i<108;i++) dyn_dh_fpu.temp_state[i] = mem_readb(addr++);
	} else {
		dyn_dh_fpu.cw = (System::UInt32)mem_readw(addr);
		for(unsigned i=0;i<108;i++) dyn_dh_fpu.temp_state[i] = mem_readb(addr++);
...
static void dh_fpu_esc0(){
	...
	if (decode.modrm.val >= 0xc0) {
		...
		cache_addd((System::UInt32)(&(dyn_dh_fpu.temp.m1)));
	}
}
...
static void dh_fpu_esc1(){
	...
	if (decode.modrm.val >= 0xc0) {
		...
		unsigned group=(decode.modrm.val >> 3) & 7;
		unsigned sub=(decode.modrm.val & 7);
		dyn_fill_ea(); 
		switch(group){
		case 0x00: /* FLD float*/
			gen_call_function((void*)&FPU_FLD_32,"%Ddr",DREG(EA));
...
//				LOG(LOG_FPU,LOG_ERROR)("80267 protected mode (un)set. Nothing done");
		...
		case 0x03:	/* FISTP */
			...
			gen_call_function((void*)&FPU_FST_32,"%Ddr",DREG(EA));
			break;
		case 0x05:	/* FLD 80 int Real */
			gen_call_function((void*)&FPU_FLD_80,"%Ddr",DREG(EA));
			cache_addb(0xdb);
			cache_addb(0x05|(decode.modrm.reg<<3));
			cache_addd((System::UInt32)(&(dyn_dh_fpu.temp.m1)));
			break;
		case 0x07:	/* FSTP 80 int Real */
			cache_addb(0xdb);
			cache_addb(0x05|(decode.modrm.reg<<3));
			cache_addd((System::UInt32)(&(dyn_dh_fpu.temp.m1)));
...
static void dh_fpu_esc5(){
	...
	if (decode.modrm.val >= 0xc0) { 
		...
		case 0x04:	/* FRSTOR */
			...
			cache_addb(0xdd);
			cache_addb(0x05|(decode.modrm.reg<<3));
			cache_addd((System::UInt32)(&(dyn_dh_fpu.temp_state[0])));
			break;
		case 0x06:	/* FSAVE */
			cache_addb(0xdd);
			cache_addb(0x05|(decode.modrm.reg<<3));
...
static void dh_fpu_esc7(){
	...
	if (decode.modrm.val >= 0xc0) { 
		...
		case 0x01:
			...
			break;
		case 0x02:   /* FIST System::Int16 */
			cache_addb(0xdf);
			cache_addb(0x05|(decode.modrm.reg<<3));
			cache_addd((System::UInt32)(&(dyn_dh_fpu.temp.m1)));
			gen_call_function((void*)&FPU_FST_16,"%Ddr",DREG(EA));
			break;
		case 0x03:	/* FISTP System::Int16 */
			cache_addb(0xdf);
			cache_addb(0x05|(decode.modrm.reg<<3));
			cache_addd((System::UInt32)(&(dyn_dh_fpu.temp.m1)));
			gen_call_function((void*)&FPU_FST_16,"%Ddr",DREG(EA));
