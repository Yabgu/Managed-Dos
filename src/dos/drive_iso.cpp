...
public:
	isoFile(isoDrive *drive, const char *name, FileStat_Block *stat, System::UInt32 offset);
...
private:
	isoDrive *drive;
	System::Byte buffer[ISO_FRAMESIZE];
	int cachedSector;
	System::UInt32 fileBegin;
	System::UInt32 filePos;
	System::UInt32 fileEnd;
	System::UInt16 info;
};

isoFile::isoFile(isoDrive *drive, const char *name, FileStat_Block *stat, System::UInt32 offset) {
	this->drive = drive;
	time = stat->time;
	date = stat->date;
	attr = stat->attr;
...
bool isoFile::Read(System::Byte *data, System::UInt16 *size) {
	if (filePos + *size > fileEnd)
		*size = (System::UInt16)(fileEnd - filePos);
	
	System::UInt16 nowSize = 0;
	int sector = filePos / ISO_FRAMESIZE;
	System::UInt16 sectorPos = (System::UInt16)(filePos % ISO_FRAMESIZE);
	...
	while (nowSize < *size) {
		System::UInt16 remSector = ISO_FRAMESIZE - sectorPos;
		System::UInt16 remSize = *size - nowSize;
...
bool isoFile::Write(System::Byte* /*data*/, System::UInt16* /*size*/) {
	return false;
}

bool isoFile::Seek(System::UInt32 *pos, System::UInt32 type) {
	switch (type) {
		case DOS_SEEK_SET:
...
System::UInt16 isoFile::GetInformation(void) {
...
isoDrive::isoDrive(char driveLetter, const char *fileName, System::Byte mediaid, int &error) {
...
int isoDrive::UpdateMscdex(char driveLetter, const char* path, System::Byte& subUnit) {
...
bool isoDrive::FileOpen(DOS_File **file, char *name, System::UInt32 flags) {
...
bool isoDrive::FileCreate(DOS_File** /*file*/, char* /*name*/, System::UInt16 /*attributes*/) {
	DOS_SetError(DOSERR_ACCESS_DENIED);
	return false;
}
...
bool isoDrive::FindFirst(char *dir, DOS_DTA &dta, bool fcb_findfirst) {
	...
	dta.SetDirID((System::UInt16)dirIterator);
...
bool isoDrive::FindNext(DOS_DTA &dta) {
	...
	while (GetNextDirEntry(dirIterator, &de)) {
		...
		if (!(isRoot && de.ident[0]=='.') && WildFileCmp((char*)de.ident, pattern)
			...
			System::UInt32 findSize = DATA_LENGTH(de);
			System::UInt16 findDate = DOS_PackDate(1900 + de.dateYear, de.dateMonth, de.dateDay);
			System::UInt16 findTime = DOS_PackTime(de.timeHour, de.timeMin, de.timeSec);
...
bool isoDrive::GetFileAttr(char *name, System::UInt16 *attr) {
	*attr = 0;
	isoDirEntry de;
...
bool isoDrive::AllocationInfo(System::UInt16 *bytes_sector, System::Byte *sectors_cluster, System::UInt16 *total_clusters, System::UInt16 *free_clusters) {
	*bytes_sector = 2048;
...
System::Byte isoDrive::GetMediaByte(void) {
	return mediaid;
}
...
bool isoDrive::ReadCachedSector(System::Byte** buffer, const System::UInt32 sector) {
...
inline bool isoDrive :: readSector(System::Byte *buffer, System::UInt32 sector) {
	return CDROM_Interface_Image::images[subUnit]->ReadSector(buffer, false, sector);
}

int isoDrive :: readDirEntry(isoDirEntry *de, System::Byte *data) {	
...
bool isoDrive :: loadImage() {
	isoPVD pvd;
	dataCD = false;
	readSector((System::Byte*)(&pvd), ISO_FIRST_VD);
