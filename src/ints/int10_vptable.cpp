...
const System::Byte vparams_pcjr[] = {
	// 40x25 mode 0 and 1 crtc registers
...
const System::Byte vparams_tandy[] = {
	// 40x25 mode 0 and 1 crtc registers
...
const System::Byte vparams_tandy_td[] = {
	// 40x25 mode 0 and 1 crtc registers
	0x38, 0x28, 0x2d, 0x0a, 0x1f, 0x06, 0x19, 0x1c, 0x02, 0x07, 0x06, 0x07, 0,0,0,0,
	// 80x25 mode 2 and 3 crtc registers
	0x71, 0x50, 0x5a, 0x0a, 0x1f, 0x06, 0x19, 0x1c, 0x02, 0x07, 0x06, 0x07, 0,0,0,0,
...
static System::Byte video_parameter_table_vga[0x40*0x1d]={
...
static System::Byte video_parameter_table_ega[0x40*0x17]={
// video parameter table for mode 0 (cga emulation)
  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
System::UInt16 INT10_SetupVideoParameterTable(PhysPt basepos) {
	if (IS_VGA_ARCH) {
		for (unsigned i=0;i<0x40*0x1d;i++) {
			phys_writeb(basepos+i,video_parameter_table_vga[i]);
		}
		return 0x40*0x1d;
	} else {
		for (unsigned i=0;i<0x40*0x17;i++) {
...
void INT10_SetupBasicVideoParameterTable(void) {
	...
	switch (machine) {
	case MCH_TANDY:
		for (System::UInt16 i = 0; i < sizeof(vparams_tandy); i++) {
			phys_writeb(0xFF0A4+i,vparams_tandy[i]);
		}
		break;
	case MCH_PCJR:
		for (System::UInt16 i = 0; i < sizeof(vparams_pcjr); i++) {
			phys_writeb(0xFF0A4+i,vparams_pcjr[i]);
		}
		break;
	default:
		for (System::UInt16 i = 0; i < sizeof(vparams); i++) {
...
void INT10_GenerateVideoParameterTable(void) {
	...
	for (i=4; i<0x0f; i++) {
		...
		if ((i>=8) && (i<0x0d)) {
			...
			unsigned crtc_regs[0x19];
			System::UInt16 crt_addr=real_readw(BIOSMEM_SEG,BIOSMEM_CRTC_ADDRESS);
			for (ct=0; ct<0x19; ct++) {
