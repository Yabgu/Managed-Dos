...
System::Byte dos_copybuf[DOS_COPYBUFSIZE];
...
#include "cpu.h"
#endif
static inline void modify_cycles(int value) {
...
static unsigned DOS_21Handler(void) {
	...
	case 0x02:		/* Write character to STDOUT */
		{
			System::Byte c=reg_dl;System::UInt16 n=1;
	...
	case 0x03:		/* Read character from STDAUX */
		{
			System::UInt16 port = real_readw(0x40,0);
	...
	case 0x0a:		/* Buffered Input */
		{
			...
			System::Byte free=mem_readb(data);
			System::Byte read=0;System::Byte c;System::UInt16 n=1;
			if (!free) break;
			for(;;) {
				DOS_ReadFile(STDIN,&c,&n);
				if (c == 8) {			// Backspace
...
/*	Calculate how many miliseconds have passed */
			unsigned ticks=5*mem_readd(BIOS_TIMER);
			ticks = ((ticks / 59659u) << 16) + ((ticks % 59659u) << 16) / 59659u;
			unsigned seconds=(ticks/100);
			reg_ch=(System::Byte)(seconds/3600);
			reg_cl=(System::Byte)((seconds % 3600)/60);
			reg_dh=(System::Byte)(seconds % 60);
			reg_dl=(System::Byte)(ticks % 100);
		}
		//Simulate DOS overhead for timing-sensitive games
        	//Robomaze 2
		overhead();
	...
	case 0x35:		/* Get interrupt vector */
		reg_bx=real_readw(0,((System::UInt16)reg_al)*4);
		SegSet16(es,real_readw(0,((System::UInt16)reg_al)*4+2));
		break;
	case 0x36:		/* Get Free Disk Space */
		{
			System::UInt16 bytes,clusters,free;
			System::Byte sectors;
...
//TODO	Give errors for these functions to see if anyone actually uses this shit-
	...
	case 0x42:					/* LSEEK Set current file position */
		{
			System::UInt32 pos=(reg_cx<<16) + reg_dx;
			if (DOS_SeekFile(reg_bx,&pos,reg_al)) {
				reg_dx=(System::UInt16)(pos >> 16);
				reg_ax=(System::UInt16)(pos & 0xFFFF);
				CALLBACK_SCF(false);
			} else {
				reg_ax=dos.errorcode;
				CALLBACK_SCF(true);
	...
	case 0x47:					/* CWD Get current directory */
		if (DOS_GetCurrentDir(reg_dl,name1)) {
			MEM_BlockWrite(SegPhys(ds)+reg_si,name1,(unsigned)(strlen(name1)+1));	
			reg_ax=0x0100;
			CALLBACK_SCF(false);
		} else {
			reg_ax=dos.errorcode;
	...
	case 0x48:					/* Allocate memory */
		{
			System::UInt16 size=reg_bx;System::UInt16 seg;
...
//And will any game ever use this :)
	...
	case 0x5a:					/* Create temporary file */
		{
			...
			if (DOS_CreateTempFile(name1,&handle)) {
				reg_ax=handle;
				MEM_BlockWrite(SegPhys(ds)+reg_dx,name1,(unsigned)(strlen(name1)+1));
				CALLBACK_SCF(false);
			} else {
				reg_ax=dos.errorcode;
				CALLBACK_SCF(true);
	...
	case 0x60:					/* Canonicalize filename or path */
		...
		if (DOS_Canonicalize(name1,name2)) {
				MEM_BlockWrite(SegPhys(es)+reg_di,name2,(unsigned)(strlen(name2)+1));	
				CALLBACK_SCF(false);
			} else {
				reg_ax=dos.errorcode;
				CALLBACK_SCF(true);
	...
	case 0x65:					/* Get extented country information and a lot of other useless shit*/
		{ /* Todo maybe fully support this for now we set it standard for USA */ 
			...
			unsigned len = 0; /* For 0x21 and 0x22 */
			...
			case 0x01:
				...
				if(reg_cx > 0x08 ) {
					unsigned amount = (reg_cx>=0x29)?0x22:(reg_cx-7);
			...
			case 0x22: /* Capatilize ASCIZ string */
				...
				if(len) {
					...
					for (unsigned count = 0; count < len;count++)
						dos_copybuf[count] = (System::Byte)toupper(*reinterpret_cast<unsigned char*>(dos_copybuf+count));
...
static unsigned DOS_20Handler(void) {
	reg_ah=0x00;
	DOS_21Handler();
	return CBRET_NONE;
}

static unsigned DOS_27Handler(void) {
	// Terminate & stay resident
	System::UInt16 para = (reg_dx/16)+((reg_dx % 16)>0);
	System::UInt16 psp = dos.psp(); //mem_readw(SegPhys(ss)+reg_sp+2);
	if (DOS_ResizeMemory(psp,&para)) DOS_Terminate(psp,true,0);
	return CBRET_NONE;
}

static unsigned DOS_25Handler(void) {
	if(Drives[reg_al]==0){
		reg_ax=0x8002;
		SETFLAGBIT(CF,true);
	}else{
...
static unsigned DOS_26Handler(void) {
...
public:
	...
	// retf  <- int 21 4c jumps here to mimic a retf Cyber
		...
		dos.date.day=(System::Byte)loctime->tm_mday;
		dos.date.month=(System::Byte)loctime->tm_mon+1;
		dos.date.year=(System::UInt16)loctime->tm_year+1900;
		System::UInt32 ticks=(System::UInt32)((loctime->tm_hour*3600+loctime->tm_min*60+loctime->tm_sec)*(float)PIT_TICK_RATE/65536.0);
		mem_writed(BIOS_TIMER,ticks);
	}
	~DOS(){
		for (System::UInt16 i=0;i<DOS_DRIVES;i++) delete Drives[i];
	}
};
