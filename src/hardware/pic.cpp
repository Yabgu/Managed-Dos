...
struct PIC_Controller {
	...
	bool single;
	bool request_issr;
	System::Byte vector_base;
};

unsigned PIC_Ticks=0;
unsigned PIC_IRQCheck;
unsigned PIC_IRQOnSecondPicActive;
unsigned PIC_IRQActive;
...
static void write_command(unsigned port,unsigned val,unsigned iolen) {
	PIC_Controller * pic=&pics[port==0x20 ? 0 : 1];
	unsigned irq_base=port==0x20 ? 0 : 8;
	unsigned i;
	static System::UInt16 IRQ_priority_table[16] = 
...
static void write_data(unsigned port,unsigned val,unsigned iolen) {
	PIC_Controller * pic=&pics[port==0x21 ? 0 : 1];
	unsigned irq_base=(port==0x21) ? 0 : 8;
	unsigned i;
...
static unsigned read_command(unsigned port,unsigned iolen) {
	PIC_Controller * pic=&pics[port==0x20 ? 0 : 1];
	unsigned irq_base=(port==0x20) ? 0 : 8;
	unsigned i;System::Byte ret=0;System::Byte b=1;
	if (pic->request_issr) {
...
static unsigned read_data(unsigned port,unsigned iolen) {
	unsigned irq_base=(port==0x21) ? 0 : 8;
	unsigned i;System::Byte ret=0;System::Byte b=1;
...
void PIC_ActivateIRQ(unsigned irq) {
	if( irq < 8 ) {
...
void PIC_DeActivateIRQ(unsigned irq) {
	if (irq<16) {
...
static inline bool PIC_startIRQ(unsigned i) {
...
void PIC_runIRQs(void) {
	...
	static unsigned IRQ_priority_order[16] = 
		{ 0,1,2,8,9,10,11,12,13,14,15,3,4,5,6,7 };
	static System::UInt16 IRQ_priority_lookup[17] =
		{ 0,1,2,11,12,13,14,15,3,4,5,6,7,8,9,10,16 };
	System::UInt16 activeIRQ = PIC_IRQActive;
	if (activeIRQ == PIC_NOIRQ) activeIRQ = 16;
	/* Get the priority of the active irq */
	System::UInt16 Priority_Active_IRQ = IRQ_priority_lookup[activeIRQ];
...
void PIC_SetIRQMask(unsigned irq, bool masked) {
...
void PIC_AddEvent(PIC_EventHandler handler,float delay,unsigned val) {
...
void PIC_RemoveSpecificEvents(PIC_EventHandler handler, unsigned val) {
...
bool PIC_RunQueue(void) {
	...
	if (pic_queue.next_entry) {
		int cycles=(int)(pic_queue.next_entry->index*CPU_CycleMax-index_nd);
