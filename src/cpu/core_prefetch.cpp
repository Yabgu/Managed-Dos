...
static const System::UInt32 AddrMaskTable[2]={0x0000ffff,0xffffffff};
...
static System::Byte prefetch_buffer[MAX_PQ_SIZE];
static bool pq_valid=false;
static unsigned pq_start;

static System::Byte Fetchb() {
	System::Byte temp;
	if (pq_valid && (core.cseip>=pq_start) && (core.cseip<pq_start+CPU_PrefetchQueueSize)) {
		...
		if ((core.cseip+1>=pq_start+CPU_PrefetchQueueSize-4) &&
			...
			unsigned remaining_bytes=pq_start+CPU_PrefetchQueueSize-(core.cseip+1);
			for (unsigned i=0; i<remaining_bytes; i++) prefetch_buffer[i]=prefetch_buffer[core.cseip+1-pq_start+i];
			for (unsigned i=remaining_bytes; i<CPU_PrefetchQueueSize; i++) prefetch_buffer[i]=LoadMb(core.cseip+1+i);
			pq_start=core.cseip+1;
			pq_valid=true;
		}
	} else {
		for (unsigned i=0; i<CPU_PrefetchQueueSize; i++) prefetch_buffer[i]=LoadMb(core.cseip+i);
		pq_start=core.cseip;
		pq_valid=true;
...
static System::UInt16 Fetchw() {
	...
	if (pq_valid && (core.cseip>=pq_start) && (core.cseip+2<pq_start+CPU_PrefetchQueueSize)) {
		...
		if ((core.cseip+2>=pq_start+CPU_PrefetchQueueSize-4) &&
			...
			unsigned remaining_bytes=pq_start+CPU_PrefetchQueueSize-(core.cseip+2);
			for (unsigned i=0; i<remaining_bytes; i++) prefetch_buffer[i]=prefetch_buffer[core.cseip+2-pq_start+i];
			for (unsigned i=remaining_bytes; i<CPU_PrefetchQueueSize; i++) prefetch_buffer[i]=LoadMb(core.cseip+2+i);
...
static System::UInt32 Fetchd() {
	...
	if (pq_valid && (core.cseip>=pq_start) && (core.cseip+4<pq_start+CPU_PrefetchQueueSize)) {
		...
		if ((core.cseip+4>=pq_start+CPU_PrefetchQueueSize-4) &&
			...
			unsigned remaining_bytes=pq_start+CPU_PrefetchQueueSize-(core.cseip+4);
			for (unsigned i=0; i<remaining_bytes; i++) prefetch_buffer[i]=prefetch_buffer[core.cseip+4-pq_start+i];
			for (unsigned i=remaining_bytes; i<CPU_PrefetchQueueSize; i++) prefetch_buffer[i]=LoadMb(core.cseip+4+i);
...
#endif
restart_opcode:
		System::Byte next_opcode=Fetchb();
		invalidate_pq=false;
...
int CPU_Core_Prefetch_Trap_Run(void) {
	int oldCycles = CPU_Cycles;
	CPU_Cycles = 1;
	cpu.trap_skip = false;

	int ret=CPU_Core_Prefetch_Run();
