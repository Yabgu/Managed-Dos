...
static void write_p3ce(unsigned port,unsigned val,unsigned iolen) {
	gfx(index)=val & 0x0f;
}

static unsigned read_p3ce(unsigned port,unsigned iolen) {
	return gfx(index);
}

static void write_p3cf(unsigned port,unsigned val,unsigned iolen) {
	switch (gfx(index)) {
	case 0:	/* Set/Reset Register */
		...
		/*
			0	If in Write Mode 0 and bit 0 of 3CEh index 1 is set a write to
				display memory will set all the int in plane 0 of the byte to this
...
static unsigned read_p3cf(unsigned port,unsigned iolen) {
	switch (gfx(index)) {
	case 0:	/* Set/Reset Register */
	...
	default:
		...
		LOG(LOG_VGAMISC,LOG_NORMAL)("Reading from illegal index %2X in port %4X",static_cast<System::UInt32>(gfx(index)),port);
		break;
	}
