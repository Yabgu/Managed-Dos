...
DOS_Drive_Cache::DOS_Drive_Cache(void) {
	...
	label[0]		= 0;
	nextFreeFindFirst	= 0;
	for (System::UInt32 i=0; i<MAX_OPENDIRS; i++) { dirSearch[i] = 0; free[i] = true; dirFindFirst[i] = 0; };
...
DOS_Drive_Cache::~DOS_Drive_Cache(void) {
	Clear();
	for (System::UInt32 i=0; i<MAX_OPENDIRS; i++) { delete dirFindFirst[i]; dirFindFirst[i]=0; };
}

void DOS_Drive_Cache::Clear(void) {
	delete dirBase; dirBase = 0;
	nextFreeFindFirst	= 0;
	for (System::UInt32 i=0; i<MAX_OPENDIRS; i++) dirSearch[i] = 0;
}
...
void DOS_Drive_Cache::EmptyCache(void) {
	...
	save_dir	= 0;
	srchNr		= 0;
	for (System::UInt32 i=0; i<MAX_OPENDIRS; i++) free[i] = true; 
	SetBaseDir(basePath);
}
...
System::UInt16 DOS_Drive_Cache::GetFreeID(CFileInfo* dir) {
	for (System::UInt16 i=0; i<MAX_OPENDIRS; i++) if (free[i] || (dir==dirSearch[i])) return i;
	...
	if (pos) {
		...
		if (index>=0) {
			...
			if (dir) for (i=0; i<MAX_OPENDIRS; i++) {
				if ((dirSearch[i]==dir) && ((System::UInt32)index<=dirSearch[i]->nextEntry)) 
...
void DOS_Drive_Cache::CacheOut(const char* path, bool ignoreLastDir) {
	...
	if (ignoreLastDir) {
		...
		System::Int32 len=0;
		const char* pos = strrchr(path,CROSS_FILESPLIT);
		if (pos) len = (System::Int32)(pos - path);
		if (len>0) { 
...
//	LOG_DEBUG("DIR: Caching out %s : dir %s",expand,dir->orgname);
	// delete file objects...
	for(System::UInt32 i=0; i<dir->fileList.size(); i++) {
...
int DOS_Drive_Cache::CompareShortname(const char* compareName, const char* shortName) {
	...
	if (cpos) {
/* the following code is replaced as it's not safe when char* is 64 int */
/*		int compareCount1	= (int)cpos - (int)shortName;
		char* endPos		= strchr(cpos,'.');
		unsigned numberSize		= endPos ? int(endPos)-int(cpos) : strlen(cpos);
		
		char* lpos			= strchr(compareName,'.');
		int compareCount2	= lpos ? int(lpos)-int(compareName) : strlen(compareName);
	...
	while (low<=high) {
		...
		if (res<0)	high = mid-1; 
			...
			do {
				...
				mid++;
			} while((unsigned)mid<curDir->longNameList.size() && (CompareShortname(name,curDir->longNameList[mid]->shortname)==0));
			break;
		};
	}
	return foundNr+1;
...
int DOS_Drive_Cache::GetLongName(CFileInfo* curDir, char* shortName) {
...
void DOS_Drive_Cache::CreateShortName(CFileInfo* curDir, CFileInfo* info) {
	...
	if (pos) {
		...
		pos = strchr(tmpName,'.');
		if (pos)	len = (int)(pos - tmpName);
		else		len = (int)strlen(tmpName);
	} else {
	...
	if (createShort) {
		...
		if (len+buflen+1>8)	tocopy = (int)(8 - buflen - 1);
		else				tocopy = len;
...
bool DOS_Drive_Cache::OpenDir(const char* path, System::UInt16& id) {
	char expand[CROSS_LEN] = {0};
	CFileInfo* dir = FindDirInfo(path,expand);
...
bool DOS_Drive_Cache::OpenDir(CFileInfo* dir, const char* expand, System::UInt16& id) {
	id = GetFreeID(dir);
	dirSearch[id] = dir;
...
bool DOS_Drive_Cache::ReadDir(System::UInt16 id, char* &result) {
	// shouldnt happen...
...
bool DOS_Drive_Cache::SetResult(CFileInfo* dir, char* &result, unsigned entryNr)
{
...
// FindFirst / FindNext
bool DOS_Drive_Cache::FindFirst(char* path, System::UInt16& id) {
	System::UInt16	dirID;
	// Cache directory in 
	...
	System::UInt16 local_findcounter = 0;
	...
	System::UInt16	dirFindFirstID = this->nextFreeFindFirst++;
	...
	if (local_findcounter == MAX_OPENDIRS) { //Here is the reset from above.
		...
		for(unsigned n=0; n<MAX_OPENDIRS;n++) {	
	...
	for (unsigned i=0; i<dirSearch[dirID]->fileList.size(); i++) {
...
bool DOS_Drive_Cache::FindNext(System::UInt16 id, char* &result) {
	// out of range ?
