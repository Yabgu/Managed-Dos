...
class GenReg {
public:
	GenReg(System::Byte _index) {
		index=_index;
		notusable=false;dynreg=0;
	}
	DynReg  * dynreg;
	unsigned last_used;			//Keeps track of last assigned regs 
    System::Byte index;
	bool notusable;
	void Load(DynReg * _dynreg,bool stale=false) {
		if (!_dynreg) return;
		if (GCC_UNLIKELY((unsigned)dynreg)) Clear();
		dynreg=_dynreg;
		...
		if ((!stale) && (dynreg->flags & (DYNFLG_LOAD|DYNFLG_ACTIVE))) {
			...
			cache_addd((System::UInt32)dynreg->data);
		}
		dynreg->flags|=DYNFLG_ACTIVE;
	}
	void Save(void) {
		if (GCC_UNLIKELY(!((unsigned)dynreg))) IllegalOption("GenReg->Save");
		dynreg->flags&=~DYNFLG_CHANGED;
		cache_addw(0x0589+(index << (8+3)));		//Mov [data],reg
		cache_addd((System::UInt32)dynreg->data);
	}
	void Release(void) {
		if (GCC_UNLIKELY(!((unsigned)dynreg))) return;
...
static BlockReturn gen_runcode(System::Byte * code) {
	BlockReturn retval;
#if defined (_MSC_VER)
	__asm {
/* Prepare the flags */
...
#elif defined (MACOSX)
	register System::UInt32 tempflags=reg_flags & FMASK_TEST;
	__asm__ volatile (
		"pushl %%ebx						\n"
		"pushl %%ebp						\n"
...
static GenReg * FindDynReg(DynReg * dynreg,bool stale=false) {
	...
	if (dynreg->flags & DYNFLG_HAS8) {
		...
		for (i=first_index=0;i<=X86_REG_EBX;i++) {
			...
			if (genreg->last_used<(unsigned)first_used) {
...
static void gen_reinit(void) {
	x86gen.last_used=0;
	x86gen.flagsactive=false;
	for (unsigned i=0;i<X86_REGS;i++) {
		x86gen.regs[i]->dynreg=0;
	}
}


static void gen_load_host(void * data,DynReg * dr1,unsigned size) {
	...
	cache_addd((System::UInt32)data);
	dr1->flags|=DYNFLG_CHANGED;
}

static void gen_mov_host(void * data,DynReg * dr1,unsigned size,System::Byte di1=0) {
...
static void gen_dop_byte(DualOps op,DynReg * dr1,System::Byte di1,DynReg * dr2,System::Byte di2) {
...
static void gen_dop_byte_imm(DualOps op,DynReg * dr1,System::Byte di1,unsigned imm) {
...
static void gen_dop_byte_imm_mem(DualOps op,DynReg * dr1,System::Byte di1,void* data) {
	GenReg * gr1=FindDynReg(dr1);
	System::UInt16 tmp;
	switch (op) {
...
static void gen_sop_byte(SingleOps op,DynReg * dr1,System::Byte di1) {
	GenReg * gr1=FindDynReg(dr1);
	System::UInt16 tmp;
	switch (op) {
...
static void gen_extend_byte(bool sign,bool dword,DynReg * ddr,DynReg * dsr,System::Byte dsi) {
	GenReg * gsr=FindDynReg(dsr);
...
static void gen_lea(DynReg * ddr,DynReg * dsr1,DynReg * dsr2,unsigned scale,int imm) {
	GenReg * gdr=FindDynReg(ddr);
	unsigned imm_size;
	System::Byte rm_base=(gdr->index << 3);
	if (dsr1) {
		...
		if (dsr2) {
			...
			System::Byte sib=(gsr1->index)+(gsr2->index<<3)+(scale<<6);
			cache_addb(sib);
		} else {
		...
		if (dsr2) {
			...
			cache_addb(0x8d);			//LEA
			cache_addb(rm_base+0x4);	//The sib indicator
			System::Byte sib=(5+(gsr2->index<<3)+(scale<<6));
			cache_addb(sib);
			imm_size=4;
		} else {
			cache_addb(0x8d);			//LEA
...
static void gen_dop_word_imm(DualOps op,bool dword,DynReg * dr1,int imm) {
	GenReg * gr1=FindDynReg(dr1,dword && op==DOP_MOV);
	System::UInt16 tmp;
	if (!dword) cache_addb(0x66);
	switch (op) {
...
static void gen_imul_word_imm(bool dword,DynReg * dr1,DynReg * dr2,int imm) {
	GenReg * gr1=FindDynReg(dr1);GenReg * gr2=FindDynReg(dr2);
	if (!dword) cache_addb(0x66);
	if ((imm>=-128 && imm<=127)) {
		cache_addb(0x6b);
...
static void gen_shift_byte_cl(unsigned op,DynReg * dr1,System::Byte di1,DynReg * drecx) {
	ForceDynReg(x86gen.regs[X86_REG_ECX],drecx);
	GenReg * gr1=FindDynReg(dr1);
	cache_addw(0xc0d2+(((System::UInt16)op) << 11)+ ((gr1->index+di1)<<8));
	dr1->flags|=DYNFLG_CHANGED;
}

static void gen_shift_byte_imm(unsigned op,DynReg * dr1,System::Byte di1,System::Byte imm) {
	GenReg * gr1=FindDynReg(dr1);
	cache_addw(0xc0c0+(((System::UInt16)op) << 11) + ((gr1->index+di1)<<8));
	cache_addb(imm);
	dr1->flags|=DYNFLG_CHANGED;
}

static void gen_shift_word_cl(unsigned op,bool dword,DynReg * dr1,DynReg * drecx) {
	ForceDynReg(x86gen.regs[X86_REG_ECX],drecx);
	GenReg * gr1=FindDynReg(dr1);
	if (!dword) cache_addb(0x66);
	cache_addw(0xc0d3+(((System::UInt16)op) << 11) + ((gr1->index)<<8));
	dr1->flags|=DYNFLG_CHANGED;
}

static void gen_shift_word_imm(unsigned op,bool dword,DynReg * dr1,System::Byte imm) {
	GenReg * gr1=FindDynReg(dr1);
	dr1->flags|=DYNFLG_CHANGED;
	if (!dword) {
		cache_addd(0x66|((0xc0c1+((System::UInt16)op << 11) + (gr1->index<<8))|imm<<16)<<8);
	} else { 
		cache_addw(0xc0c1+((System::UInt16)op << 11) + (gr1->index<<8));
		cache_addb(imm);
	}
}
...
static void gen_mul_byte(bool imul,DynReg * dyn_ax,DynReg * dr1,System::Byte di1) {
	ForceDynReg(x86gen.regs[X86_REG_EAX],dyn_ax);
	GenReg * gr1=FindDynReg(dr1);
...
static void gen_dshift_imm(bool dword,bool left,DynReg * dr1,DynReg * dr2,unsigned imm) {
	GenReg * gr1=FindDynReg(dr1);
	GenReg * gr2=FindDynReg(dr2);
	if (!dword) cache_addb(0x66);
...
static void gen_call_function(void * func,char const* ops,...) {
	...
	if (ops) {
		...
		while (*ops) {
			if (*ops=='%') {
				...
				pinfo[pindex].value=va_arg(params,unsigned);
#if defined (MACOSX)
		...
		/* align stack */
		System::UInt32 stack_used=8;	// saving esp and return address on the stack
	...
	cache_addb(0xe8);
	cache_addd(reinterpret_cast<intptr_t>(func) - reinterpret_cast<intptr_t>(cache.pos)-4);
...
static void gen_call_write(DynReg * dr,System::UInt32 val,unsigned write_size) {
	/* Clear the EAX Genreg for usage */
	x86gen.regs[X86_REG_EAX]->Clear();
	x86gen.regs[X86_REG_EAX]->notusable=true;
	gen_protectflags();
	...
	cache_addb(0xe8);
	switch (write_size) {
		case 1: cache_addd((System::UInt32)mem_writeb_checked - (System::UInt32)cache.pos-4); break;
		case 2: cache_addd((System::UInt32)mem_writew_checked - (System::UInt32)cache.pos-4); break;
		case 4: cache_addd((System::UInt32)mem_writed_checked - (System::UInt32)cache.pos-4); break;
...
static System::Byte * gen_create_branch(BranchTypes type) {
	/* First free all registers */
	cache_addw(0x70+type);
	return (cache.pos-1);
}

static void gen_fill_branch(System::Byte * data,System::Byte * from=cache.pos) {
#if C_DEBUG
	int len=from-data;
	if (len<0) len=-len;
...
static System::Byte * gen_create_branch_long(BranchTypes type) {
	cache_addw(0x800f+(type<<8));
	cache_addd(0);
	return (cache.pos-4);
}

static void gen_fill_branch_long(System::Byte * data,System::Byte * from=cache.pos) {
	*(System::UInt32*)data=(from-data-4);
}

static System::Byte * gen_create_jump(System::Byte * to=0) {
	/* First free all registers */
	cache_addb(0xe9);
...
static void gen_fill_jump(System::Byte * data,System::Byte * to=cache.pos) {
	*(System::UInt32*)data=(to-data-4);
}


static void gen_jmp_ptr(void * ptr,int imm=0) {
	cache_addb(0xa1);
	cache_addd((System::UInt32)ptr);
...
static void gen_save_host_direct(void * data,int imm) {
...
static void gen_return_fast(BlockReturn retcode,bool ret_exception=false) {
	...
	cache_addd((System::UInt32)&cpu_regs.flags);
	if (!ret_exception) {
		cache_addw(0xc483);			//ADD ESP,4
		cache_addb(0x4);
		if (retcode==0) cache_addw(0xc033);		//MOV EAX, 0
