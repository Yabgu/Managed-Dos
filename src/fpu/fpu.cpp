...
void FPU_FLDCW(PhysPt addr){
	System::UInt16 temp = mem_readw(addr);
	FPU_SetCW(temp);
}

System::UInt16 FPU_GetTag(void){
	System::UInt16 tag=0;
...
void FPU_ESC0_EA(unsigned rm,PhysPt addr) {
	/* REGULAR TREE WITH 32 int REALS */
	FPU_FLD_F32_EA(addr);
	EATREE(rm);
}

void FPU_ESC0_Normal(unsigned rm) {
	unsigned group=(rm >> 3) & 7;
	unsigned sub=(rm & 7);
	switch (group){
...
void FPU_ESC1_EA(unsigned rm,PhysPt addr) {
// floats
	unsigned group=(rm >> 3) & 7;
	unsigned sub=(rm & 7);
	switch(group){
...
void FPU_ESC2_EA(unsigned rm,PhysPt addr) {
	/* 32 int integer operants */
	FPU_FLD_I32_EA(addr);
	EATREE(rm);
}
...
void FPU_ESC3_EA(unsigned rm,PhysPt addr) {
	unsigned group=(rm >> 3) & 7;
	unsigned sub=(rm & 7);
	switch(group){
	case 0x00:	/* FILD */
...
void FPU_ESC4_EA(unsigned rm,PhysPt addr) {
	/* REGULAR TREE WITH 64 int REALS */
	FPU_FLD_F64_EA(addr);
	EATREE(rm);
}
...
void FPU_ESC5_EA(unsigned rm,PhysPt addr) {
	unsigned group=(rm >> 3) & 7;
	unsigned sub=(rm & 7);
	switch(group){
...
void FPU_ESC6_EA(unsigned rm,PhysPt addr) {
...
void FPU_ESC7_EA(unsigned rm,PhysPt addr) {
	unsigned group=(rm >> 3) & 7;
	unsigned sub=(rm & 7);
	switch(group){
