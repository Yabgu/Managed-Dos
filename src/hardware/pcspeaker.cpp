...
static void PCSPEAKER_CallBack(unsigned len) {
	...
	while (count--) {
		...
		*stream++=(System::Int16)(value/sample_add);
	}
	if(spkr.chan) spkr.chan->AddSamples_m16(len,(System::Int16*)MixTemp);

	//Turn off speaker after 10 seconds of idle or one second idle when in off mode
	bool turnoff = false;
	unsigned test_ticks = PIC_Ticks;
