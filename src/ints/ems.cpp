...
static System::UInt16 GEMMIS_seg;
...
public:
	...
	System::UInt16 GetInformation(void) { return 0xc080; }
	bool ReadFromControlChannel(PhysPt bufptr, System::UInt16 size, System::UInt16 * retcode);
	bool WriteToControlChannel(PhysPt /*bufptr*/, System::UInt16 /*size*/, System::UInt16 * /*retcode*/) { return true; }
private:
	System::Byte cache;
};

bool device_EMM::ReadFromControlChannel(PhysPt bufptr, System::UInt16 size, System::UInt16 * retcode) {
	unsigned subfct = mem_readb(bufptr);
	switch (subfct) {
	case 0x00:
	...
	case 0x01: {
		...
		for (unsigned frct = 0; frct < EMM_PAGEFRAME4K / 4; frct++) {
		...
		for (unsigned frct = 0; frct < 0x10 / 4; frct++) {
			unsigned frnr = (frct + EMM_PAGEFRAME4K / 4) * 6;
			...
			mem_writeb(GEMMIS_addr + 0x0e + frnr, (System::Byte)(frct & 0xff));		// physical EMS page number
			mem_writeb(GEMMIS_addr + 0x0f + frnr, 0x00);		// EMS frame
		}
		/* build non-EMS ROM frames (0xf000-0x10000) */
		for (unsigned frct = (EMM_PAGEFRAME4K + 0x10) / 4; frct < 0xf0 / 4; frct++) {
			mem_writeb(GEMMIS_addr + 0x0a + frct * 6, 0x00);	// frame type: NONE
			mem_writeb(GEMMIS_addr + 0x0b + frct * 6, 0xff);	// owner: NONE
			mem_writew(GEMMIS_addr + 0x0c + frct * 6, 0xffff);	// non-EMS frame
			mem_writeb(GEMMIS_addr + 0x0e + frct * 6, 0xff);	// EMS page number (NONE)
	...
	case 0x03:
		...
		mem_writew(bufptr + 0x00, (System::UInt16)(MEM_TotalPages() * 4));	// max size (kb)
...
static struct {
	bool enabled;
	System::UInt16 ems_handle;
	unsigned pm_interface;
	MemHandle private_area;
	System::Byte pic1_remapping, pic2_remapping;
} vcpi;

struct MoveRegion {
	System::UInt32 bytes;
	System::Byte src_type;
	System::UInt16 src_handle;
	System::UInt16 src_offset;
	System::UInt16 src_page_seg;
	System::Byte dest_type;
	System::UInt16 dest_handle;
	System::UInt16 dest_offset;
	System::UInt16 dest_page_seg;
};

static System::UInt16 EMM_GetFreePages(void) {
	unsigned count = MEM_FreeTotal() / 4;
	if (count > 0x7fff) count = 0x7fff;
	return (System::UInt16)count;
}

static bool INLINE ValidHandle(System::UInt16 handle) {
	if (handle >= EMM_MAX_HANDLES) return false;
	if (emm_handles[handle].pages == NULL_HANDLE) return false;
	return true;
}

static System::Byte EMM_AllocateMemory(System::UInt16 pages, System::UInt16 & dhandle, bool can_allocate_zpages) {
...
static System::Byte EMM_AllocateSystemHandle(System::UInt16 pages) {
	/* Check for enough free pages */
	if ((MEM_FreeTotal() / 4) < pages) { return EMM_OUT_OF_LOG; }
	System::UInt16 handle = EMM_SYSTEM_HANDLE;	// emm system handle (reserved for OS usage)
...
static System::Byte EMM_ReallocatePages(System::UInt16 handle, System::UInt16 & pages) {
...
static System::Byte EMM_MapPage(unsigned phys_page, System::UInt16 handle, System::UInt16 log_page) {
...
static System::Byte EMM_MapSegment(unsigned segment, System::UInt16 handle, System::UInt16 log_page) {
	...
	if (((segment >= 0xa000) && (segment < 0xb000)) || ((segment >= EMM_PAGEFRAME - 0x1000) && (segment < EMM_PAGEFRAME + 0x1000))) {
		System::Int32 tphysPage = ((System::Int32)segment - EMM_PAGEFRAME) / (0x1000 / EMM_MAX_PHYS);
...
static System::Byte EMM_ReleaseMemory(System::UInt16 handle) {
	/* Check for valid handle */
	if (!ValidHandle(handle)) return EMM_INVALID_HANDLE;
...
static System::Byte EMM_SavePageMap(System::UInt16 handle) {
	/* Check for valid handle */
	...
	for (unsigned i = 0; i < EMM_MAX_PHYS; i++) {
...
static System::Byte EMM_RestoreMappingTable(void) {
	System::Byte result;
...
static System::Byte EMM_RestorePageMap(System::UInt16 handle) {
	/* Check for valid handle */
	if (handle >= EMM_MAX_HANDLES || emm_handles[handle].pages == NULL_HANDLE) {
		if (handle != 0) return EMM_INVALID_HANDLE;
	}
...
static System::Byte EMM_GetPagesForAllHandles(PhysPt table, System::UInt16 & handles) {
	handles = 0;
	for (System::UInt16 i = 0; i < EMM_MAX_HANDLES; i++) {
...
static System::Byte EMM_PartialPageMapping(void) {
	PhysPt list, data; System::UInt16 count;
	switch (reg_al) {
	case 0x00:	/* Save Partial Page Map */
		...
		for (; count > 0; count--) {
			System::UInt16 segment = mem_readw(list); list += 2;
			if ((segment >= EMM_PAGEFRAME) && (segment < EMM_PAGEFRAME + 0x1000)) {
				System::UInt16 page = (segment - EMM_PAGEFRAME) / (EMM_PAGE_SIZE >> 4);
	...
	case 0x01:	/* Restore Partial Page Map */
		...
		for (; count > 0; count--) {
			System::UInt16 segment = mem_readw(data); data += 2;
			if ((segment >= EMM_PAGEFRAME) && (segment < EMM_PAGEFRAME + 0x1000)) {
				System::UInt16 page = (segment - EMM_PAGEFRAME) / (EMM_PAGE_SIZE >> 4);
	...
	case 0x02:	/* Get Partial Page Map Array Size */
		reg_al = (System::Byte)(2 + reg_bx*(2 + sizeof(EMM_Mapping)));
		break;
	default:
...
static System::Byte HandleNameSearch(void) {
	char name[9];
	System::UInt16 handle = 0; PhysPt data;
	switch (reg_al) {
...
static System::Byte GetSetHandleName(void) {
	System::UInt16 handle = reg_dx;
	switch (reg_al) {
...
static System::Byte MemoryRegion(void) {
	MoveRegion region;
	System::Byte buf_src[MEM_PAGE_SIZE];
	System::Byte buf_dest[MEM_PAGE_SIZE];
	if (reg_al > 1) {
		LOG(LOG_MISC, LOG_ERROR)("EMS:Call %2X Subfunction %2X not supported", reg_ah, reg_al);
		return EMM_FUNC_NOSUP;
	}
	...
	unsigned src_off = 0, dest_off = 0; unsigned src_remain = 0, dest_remain = 0;
	if (!region.src_type) {
		...
		unsigned pages = region.src_page_seg * 4 + (region.src_offset / MEM_PAGE_SIZE);
	...
	if (!region.dest_type) {
		...
		unsigned pages = region.dest_page_seg * 4 + (region.dest_offset / MEM_PAGE_SIZE);
...
static unsigned INT67_Handler(void) {
	unsigned i;
	switch (reg_ah) {
	...
	case 0x42:		/* Get number of pages */
		reg_dx = (System::UInt16)(MEM_TotalPages() / 4);		//Not entirely correct but okay
	...
	case 0x50:	/* Map/Unmap multiple handle pages */
		...
		for (int i = 0; i < reg_cx; i++) {
			System::UInt16 logPage = mem_readw(data); data += 2;
			System::UInt16 physPage = mem_readw(data); data += 2;
	...
	case 0x58: // Get mappable physical array address array
		if (reg_al == 0x00) {
			...
			System::UInt16 step = 0x1000 / EMM_MAX_PHYS;
			for (System::UInt16 i = 0; i < EMM_MAX_PHYS; i++) {
	...
	case 0xDE:		/* VCPI Functions */
		if (!vcpi.enabled) {
			...
			case 0x01: {	/* VCPI Get Protected Mode Interface */
				...
				for (ct = 0; ct < 0xff; ct++) {
					real_writeb(SegValue(es), reg_di + ct * 4 + 0x00, 0x67);		// access int
				...
				for (ct = 0; ct < 4; ct++) {
					System::UInt16 handle = emm_mappings[ct].handle;
					if (handle != 0xffff) {
						System::UInt16 memh = (System::UInt16)MEM_NextHandleAt(emm_handles[handle].mem, emm_mappings[ct].page * 4);
						System::UInt16 entry_addr = reg_di + (EMM_PAGEFRAME >> 6) + (ct * 0x10);
				...
				System::UInt32 cbseg_low = (CALLBACK_GetBase() & 0xffff) << 16;
				System::UInt32 cbseg_high = (CALLBACK_GetBase() & 0x1f0000) >> 16;
			...
			case 0x06: {	/* VCPI Get Physical Address of Page in 1st MB */
				if (((reg_cx << 8) >= EMM_PAGEFRAME) && ((reg_cx << 8) < EMM_PAGEFRAME + 0x1000)) {
					...
					System::Byte phys_page;
					System::UInt16 mem_seg = reg_cx << 8;
					...
					System::UInt16 handle = emm_mappings[phys_page].handle;
					if (handle == 0xffff) {
						reg_ah = EMM_ILL_PHYS;
						break;
					}
			...
			case 0x0c: {	/* VCPI Switch from V86 to Protected Mode */
				...
				System::UInt32 new_cr3 = mem_readd(reg_esi);
				System::UInt32 new_gdt_addr = mem_readd(reg_esi + 4);
				System::UInt32 new_idt_addr = mem_readd(reg_esi + 8);
				System::UInt16 new_ldt = mem_readw(reg_esi + 0x0c);
				System::UInt16 new_tr = mem_readw(reg_esi + 0x0e);
				System::UInt32 new_eip = mem_readd(reg_esi + 0x10);
				System::UInt16 new_cs = mem_readw(reg_esi + 0x14);

				/* Get GDT and IDT entries */
				System::UInt16 new_gdt_limit = mem_readw(new_gdt_addr);
				System::UInt32 new_gdt_base = mem_readd(new_gdt_addr + 2);
				System::UInt16 new_idt_limit = mem_readw(new_idt_addr);
				System::UInt32 new_idt_base = mem_readd(new_idt_addr + 2);

				/* Switch to protected mode, paging enabled if necessary */
				System::UInt32 new_cr0 = CPU_GET_CRX(0) | 1;
				if (new_cr3 != 0) new_cr0 |= 0x80000000;
				CPU_SET_CRX(0, new_cr0);
				CPU_SET_CRX(3, new_cr3);

				PhysPt tbaddr = new_gdt_base + (new_tr & 0xfff8) + 5;
				System::Byte tb = mem_readb(tbaddr);
...
static unsigned VCPI_PM_Handler() {
...
static unsigned V86_Monitor() {
	/* Calculate which interrupt did occur */
	unsigned int_num = (mem_readw(SegPhys(ss) + (reg_esp & cpu.stack.mask)) - 0x2803);
	...
	if ((int_num == (0x0d * 4)) && ((reg_sp & 0xffff) != 0x1fda)) {
		...
		System::UInt16 v86_cs = mem_readw(SegPhys(ss) + ((reg_esp + 4) & cpu.stack.mask));
		System::UInt16 v86_ip = mem_readw(SegPhys(ss) + ((reg_esp + 0) & cpu.stack.mask));
		System::Byte v86_opcode = mem_readb((v86_cs << 4) + v86_ip);
		...
		case 0x0f:		// double byte opcode
			...
			break;
		case 0xe4:		// IN AL,Ib
			reg_al = (System::Byte)(IO_ReadB(mem_readb((v86_cs << 4) + v86_ip + 1)) & 0xff);
			mem_writew(SegPhys(ss) + ((reg_esp + 0) & cpu.stack.mask), v86_ip + 2);
			break;
		case 0xe5:		// IN AX,Ib
			reg_ax = (System::UInt16)(IO_ReadW(mem_readb((v86_cs << 4) + v86_ip + 1)) & 0xffff);
			mem_writew(SegPhys(ss) + ((reg_esp + 0) & cpu.stack.mask), v86_ip + 2);
			break;
		case 0xe6:		// OUT Ib,AL
		...
		case 0xe7:		// OUT Ib,AX
			...
			break;
		case 0xec:		// IN AL,DX
			reg_al = (System::Byte)(IO_ReadB(reg_dx) & 0xff);
			mem_writew(SegPhys(ss) + ((reg_esp + 0) & cpu.stack.mask), v86_ip + 1);
			break;
		case 0xed:		// IN AX,DX
			reg_ax = (System::UInt16)(IO_ReadW(reg_dx) & 0xffff);
			mem_writew(SegPhys(ss) + ((reg_esp + 0) & cpu.stack.mask), v86_ip + 1);
			break;
		case 0xee:		// OUT DX,AL
	...
	System::UInt16 vint_vector_seg = mem_readw(SegValue(ds) + int_num + 2);
	System::UInt16 vint_vector_ofs = mem_readw(int_num);
	...
	System::UInt16 return_ip = mem_readw(SegPhys(ss) + (reg_esp & cpu.stack.mask));
	System::UInt16 return_cs = mem_readw(SegPhys(ss) + ((reg_esp + 4) & cpu.stack.mask));
	System::UInt32 return_eflags = mem_readd(SegPhys(ss) + ((reg_esp + 8) & cpu.stack.mask));
	...
	System::UInt16 v86_ss = mem_readw(SegPhys(ss) + ((reg_esp + 0x10) & cpu.stack.mask));
	System::UInt16 v86_sp = mem_readw(SegPhys(ss) + ((reg_esp + 0x0c) & cpu.stack.mask)) - 6;
	...
	mem_writew((v86_ss << 4) + v86_sp + 4, (System::UInt16)(return_eflags & 0xffff));
	return CBRET_NONE;
}
...
static void SetupVCPI() {
	...
	System::UInt32 ldt_address = (vcpi.private_area + 0x1000);
	System::UInt16 ldt_limit = 0xff;
	System::UInt32 ldt_desc_part = ((ldt_address & 0xffff) << 16) | ldt_limit;
	...
	System::UInt32 tss_address = (vcpi.private_area + 0x3000);
	System::UInt32 tss_desc_part = ((tss_address & 0xffff) << 16) | (0x0068 + 0x200);
	...
	System::UInt32 cs_desc_part = ((vcpi.private_area & 0xffff) << 16) | 0xffff;
	...
	System::UInt32 ds_desc_part = ((vcpi.private_area & 0xffff) << 16) | 0xffff;
	...
	/* IDT setup */
	for (System::UInt16 int_ct = 0; int_ct < 0x100; int_ct++) {
	...
	/* TSS */
	for (unsigned tse_ct = 0; tse_ct < 0x68 + 0x200; tse_ct++) {
...
private:
	 ...
	 * stored  32 bytes.*/
	static System::UInt16 ems_baseseg;
...
public:
	EMS(Section* configuration) :Module_base(configuration) {
		...
		MEM_BlockWrite(PhysMake(ems_baseseg, 0xa), emsname, (unsigned)(strlen(emsname) + 1));
		...
		mem_writeb(vcpi.private_area + 0x2e00, (System::Byte)0xFE);       //GRP 4
		mem_writeb(vcpi.private_area + 0x2e01, (System::Byte)0x38);       //Extra Callback instruction
		mem_writew(vcpi.private_area + 0x2e02, call_v86mon.Get_callback());		//The immediate word
		mem_writeb(vcpi.private_area + 0x2e04, (System::Byte)0x66);
		mem_writeb(vcpi.private_area + 0x2e05, (System::Byte)0xCF);       //A IRETD Instruction
...
System::UInt16 EMS::ems_baseseg = 0;
