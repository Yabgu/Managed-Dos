...
TCPClientSocket::TCPClientSocket(int platformsocket) {
	sendbuffer=0;
	nativetcpstruct = new System::Byte[sizeof(struct _TCPsocketX)];
...
TCPClientSocket::TCPClientSocket(const char* destination, System::UInt16 port) {
#ifdef NATIVESOCKETS
	nativetcpstruct=0;
#endif
	sendbuffer=0;
...
bool TCPClientSocket::GetRemoteAddressString(System::Byte* buffer) {
	IPaddress* remote_ip;
	System::Byte b1, b2, b3, b4;
...
bool TCPClientSocket::ReceiveArray(System::Byte* data, unsigned* size) {
	if(SDLNet_CheckSockets(listensocketset,0))
	{
		int retval = SDLNet_TCP_Recv(mysock, data, *size);
		if(retval<1) {
			isopen=false;
			*size=0;
			return false;
...
int TCPClientSocket::GetcharNonBlock() {
// return:
// -1: no data
// -2: socket closed
// 0..255: data
...
bool TCPClientSocket::Putchar(System::Byte data) {
...
bool TCPClientSocket::SendArray(System::Byte* data, unsigned bufsize) {
...
bool TCPClientSocket::SendByteBuffered(System::Byte data) {
...
/*
bool TCPClientSocket::SendArrayBuffered(System::Byte* data, unsigned bufsize) {
...
void TCPClientSocket::SetSendBufferSize(unsigned bufsize) {
	if(sendbuffer) delete [] sendbuffer;
	sendbuffer = new System::Byte[bufsize];
	sendbuffersize=bufsize;
	sendbufferindex=0;
}


TCPServerSocket::TCPServerSocket(System::UInt16 port)
{
	isopen = false;
	mysock = 0;
	if(!SDLNetInited) {
