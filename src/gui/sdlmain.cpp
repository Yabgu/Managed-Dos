...
struct SDL_Block {
	...
	System::Byte laltstate;
	System::Byte raltstate;
};
...
void GFX_SetTitle(System::Int32 cycles, int frameskip, bool paused) {
	char title[200] = { 0 };
	static System::Int32 internal_cycles = 0;
...
static SDL_Surface * GFX_SetupSurfaceScaled(System::UInt32 sdl_flags, System::UInt32 bpp) {
	System::UInt16 fixedWidth;
	System::UInt16 fixedHeight;
	...
	if (fixedWidth && fixedHeight) {
		...
		if (ratio_w < ratio_h) {
			...
			sdl.clip.h = (System::UInt16)(sdl.draw.height*sdl.draw.scaley*ratio_w);
		}
		else {
			sdl.clip.w = (System::UInt16)(sdl.draw.width*sdl.draw.scalex*ratio_h);
			sdl.clip.h = (System::UInt16)fixedHeight;
		}
		if (sdl.desktop.fullscreen)
		...
		if (sdl.surface && sdl.surface->flags & SDL_FULLSCREEN) {
			sdl.clip.x = (System::Int16)((sdl.surface->w - sdl.clip.w) / 2);
			sdl.clip.y = (System::Int16)((sdl.surface->h - sdl.clip.h) / 2);
		}
		else {
			sdl.clip.x = 0;
			sdl.clip.y = 0;
		...
		sdl.clip.w = (System::UInt16)(sdl.draw.width*sdl.draw.scalex);
		sdl.clip.h = (System::UInt16)(sdl.draw.height*sdl.draw.scaley);
		sdl.surface = SDL_SetVideoMode(sdl.clip.w, sdl.clip.h, bpp, sdl_flags);
		return sdl.surface;
	}
}

unsigned GFX_SetSize(unsigned width, unsigned height, unsigned flags, double scalex, double scaley, GFX_CallBack_t callback) {
	if (sdl.updating)
		GFX_EndUpdate(0);
	...
	dosurface:
		...
		if (sdl.desktop.fullscreen) {
			if (sdl.desktop.full.fixed) {
				sdl.clip.x = (System::Int16)((sdl.desktop.full.width - width) / 2);
				sdl.clip.y = (System::Int16)((sdl.desktop.full.height - height) / 2);
...
bool GFX_StartUpdate(System::Byte * & pixels, unsigned & pitch) {
	...
	case SCREEN_SURFACE:
		if (sdl.blit.surface) {
			if (SDL_MUSTLOCK(sdl.blit.surface) && SDL_LockSurface(sdl.blit.surface))
				return false;
			pixels = (System::Byte *)sdl.blit.surface->pixels;
			pitch = sdl.blit.surface->pitch;
		}
		else {
			if (SDL_MUSTLOCK(sdl.surface) && SDL_LockSurface(sdl.surface))
				return false;
			pixels = (System::Byte *)sdl.surface->pixels;
	...
	case SCREEN_OVERLAY:
		...
		pixels = (System::Byte *)*(sdl.overlay->pixels);
		pitch = *(sdl.overlay->pitches);
		sdl.updating = true;
		return true;
#if C_OPENGL
	case SCREEN_OPENGL:
		pixels = (System::Byte *)sdl.opengl.framebuf;
...
void GFX_EndUpdate(const System::UInt16 *changedLines) {
	...
	case SCREEN_SURFACE:
		if (SDL_MUSTLOCK(sdl.surface)) {
			...
			unsigned y = 0, index = 0, rectCount = 0;
			while (y < sdl.draw.height) {
				if (!(index & 1)) {
					...
					rect->x = sdl.clip.x;
					rect->y = sdl.clip.y + y;
					rect->w = (System::UInt16)sdl.draw.width;
			...
			if (changedLines) {
				...
				while (y < sdl.draw.height) {
					if (!(index & 1)) {
						...
						System::Byte *pixels = (System::Byte *)sdl.opengl.framebuf + y * sdl.opengl.pitch;
						unsigned height = changedLines[index];
...
unsigned GFX_GetRGB(System::Byte red, System::Byte green, System::Byte blue) {
	...
	case SCREEN_OVERLAY:
	{
		System::Byte y = (9797 * (red)+19237 * (green)+3734 * (blue)) >> 15;
		System::Byte u = (18492 * ((blue)-(y)) >> 15) + 128;
		System::Byte v = (23372 * ((red)-(y)) >> 15) + 128;
...
static void OutputString(unsigned x, unsigned y, const char * text, System::UInt32 color, System::UInt32 color2, SDL_Surface * output_surface) {
	System::UInt32 * draw = (System::UInt32*)(((System::Byte *)output_surface->pixels) + ((y)*output_surface->pitch)) + x;
	while (*text) {
		System::Byte * font = &int10_font_14[(*text) * 14];
		unsigned i, j;
		System::UInt32 * draw_line = draw;
		for (i = 0; i < 14; i++) {
			System::Byte map = *font++;
			for (j = 0; j < 8; j++) {
				if (map & 0x80) *((System::UInt32*)(draw_line + j)) = color; else *((System::UInt32*)(draw_line + j)) = color2;
				map <<= 1;
			}
...
static void GUI_StartUp(Section * sec) {
	...
	if (fullresolution && *fullresolution) {
		...
		if (strcmp(fullresolution, "original")) {
			...
			if (height && * height) {
				*height = 0;
				sdl.desktop.full.height = (System::UInt16)atoi(height + 1);
				sdl.desktop.full.width = (System::UInt16)atoi(res);
			}
		}
	}
	...
	if (windowresolution && *windowresolution) {
		...
		if (strcmp(windowresolution, "original")) {
			...
			if (height && *height) {
				*height = 0;
				sdl.desktop.window.height = (System::UInt16)atoi(height + 1);
				sdl.desktop.window.width = (System::UInt16)atoi(res);
			}
		}
	}
...
#ifdef WIN32
		sdl.desktop.full.width = (System::UInt16)GetSystemMetrics(SM_CXSCREEN);
#else
...
#ifdef WIN32
		sdl.desktop.full.height = (System::UInt16)GetSystemMetrics(SM_CYSCREEN);
#else
	...
	//    System::UInt32 rmask = 0xff000000;
	//    System::UInt32 gmask = 0x00ff0000;
	//    System::UInt32 bmask = 0x0000ff00;
	//#else
	System::UInt32 rmask = 0x000000ff;
	System::UInt32 gmask = 0x0000ff00;
	System::UInt32 bmask = 0x00ff0000;
	//#endif
	...
	if (splash_surf) {
		...
		System::Byte* tmpbufp = new System::Byte[640 * 400 * 3];
		GIMP_IMAGE_RUN_LENGTH_DECODE(tmpbufp, gimp_image.rle_pixel_data, 640 * 400, 3);
		for (unsigned y = 0; y < 400; y++) {

			System::Byte* tmpbuf = tmpbufp + y * 640 * 3;
			System::UInt32 * draw = (System::UInt32*)(((System::Byte *)splash_surf->pixels) + ((y)*splash_surf->pitch));
		...
		static unsigned max_splash_loop = 600;
		static unsigned splash_fade = 100;
		static bool use_fadeout = true;

		for (System::UInt32 ct = 0, startticks = GetTicks(); ct < max_splash_loop; ct = GetTicks() - startticks) {
			SDL_Event evt;
			...
			if (ct < 1) {
				...
				if (use_fadeout) {
					SDL_FillRect(sdl.surface, NULL, SDL_MapRGB(sdl.surface->format, 0, 0, 0));
					SDL_SetAlpha(splash_surf, SDL_SRCALPHA, (System::Byte)((max_splash_loop - 1 - ct) * 255 / (splash_fade - 1)));
					SDL_BlitSurface(splash_surf, NULL, sdl.surface, NULL);
					SDL_Flip(sdl.surface);
				}
			}
