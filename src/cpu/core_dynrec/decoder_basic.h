...
static struct DynDecode {
	...
	unsigned cycles;			// number cycles used by currently translated code
	bool seg_prefix_used;	// segment overridden
	System::Byte seg_prefix;		// segment prefix (if seg_prefix_used==true)
	...
	struct {
		...
		unsigned index;		// index to the current byte of the instruction stream
		System::Byte * wmap;	// write map that indicates code presence for every byte of this page
		System::Byte * invmap;	// invalidation map
		unsigned first;		// page number 
	} page;
...
static bool MakeCodePage(unsigned lin_addr,CodePageHandlerDynRec * &cph) {
	System::Byte rdval;
	...
	unsigned lin_page=lin_addr>>12;
	unsigned phys_page=lin_page;
...
static void decode_advancepage(void) {
	...
	decode.page.first++;
	unsigned faddr=decode.page.first << 12;
	mem_readb(faddr);
...
static System::Byte decode_fetchb(void) {
...
static System::UInt16 decode_fetchw(void) {
	if (GCC_UNLIKELY(decode.page.index>=4095)) {
   		System::UInt16 val=decode_fetchb();
		val|=decode_fetchb() << 8;
		return val;
	}
	*(System::UInt16 *)&decode.page.wmap[decode.page.index]+=0x0101;
	decode.code+=2;decode.page.index+=2;
	return mem_readw(decode.code-2);
}
// fetch the next dword of the instruction stream
static System::UInt32 decode_fetchd(void) {
	if (GCC_UNLIKELY(decode.page.index>=4093)) {
   		System::UInt32 val=decode_fetchb();
		val|=decode_fetchb() << 8;
	...
	*(System::UInt32 *)&decode.page.wmap[decode.page.index]+=0x01010101;
...
// codefetch functions
static void INLINE decode_increase_wmapmask(unsigned size) {
	unsigned mapidx;
	...
	if (GCC_UNLIKELY(!activecb->cache.wmapmask)) {
		...
		activecb->cache.wmapmask=(System::Byte*)malloc(START_WMMEM);
		...
		if (GCC_UNLIKELY(mapidx+size>=activecb->cache.masklen)) {
			...
			unsigned newmasklen=activecb->cache.masklen*4;
			if (newmasklen<mapidx+size) newmasklen=((mapidx+size)&~3)*2;
			System::Byte* tempmem=(System::Byte*)malloc(newmasklen);
	...
	switch (size) {
		...
		case 2 : (*(System::UInt16*)&activecb->cache.wmapmask[mapidx])+=0x0101; break;
		case 4 : (*(System::UInt32*)&activecb->cache.wmapmask[mapidx])+=0x01010101; break;
	}
}

// fetch a byte, val points to the code location if possible,
// otherwise val contains the current value read from the position
static bool decode_fetchb_imm(unsigned & val) {
	if (GCC_UNLIKELY(decode.page.index>=4096)) {
		decode_advancepage();
	}
	...
	if (decode.page.invmap != NULL) {
		if (decode.page.invmap[decode.page.index] == 0) {
			...
			val=(System::UInt32)decode_fetchb();
			return false;
		}

		HostPt tlb_addr=get_tlb_read(decode.code);
		if (tlb_addr) {
			val=(unsigned)(tlb_addr+decode.code);
...
// otherwise val contains the current value read from the position
static bool decode_fetchw_imm(unsigned & val) {
...
// otherwise val contains the current value read from the position
static bool decode_fetchd_imm(unsigned & val) {
...
static INLINE void dyn_set_eip_last_end(HostReg reg) {
	...
	gen_add_imm(reg,(System::UInt32)(decode.code-decode.code_start));
...
static INLINE void dyn_set_eip_end(HostReg reg,System::UInt32 imm=0) {
	gen_mov_word_to_reg(reg,&reg_eip,decode.big_op);
	gen_add_imm(reg,(System::UInt32)(decode.code-decode.code_start+imm));
...
static DRC_PTR_SIZE_IM INLINE gen_call_function_R(void * func,unsigned op) {
	gen_load_param_reg(op,0);
	return gen_call_function_setup(func, 1);
}

static DRC_PTR_SIZE_IM INLINE gen_call_function_R3(void * func,unsigned op) {
	gen_load_param_reg(op,2);
	return gen_call_function_setup(func, 3, true);
}

static DRC_PTR_SIZE_IM INLINE gen_call_function_RI(void * func,unsigned op1,unsigned op2) {
	gen_load_param_imm(op2,1);
	gen_load_param_reg(op1,0);
	return gen_call_function_setup(func, 2);
}

static DRC_PTR_SIZE_IM INLINE gen_call_function_RA(void * func,unsigned op1,DRC_PTR_SIZE_IM op2) {
	gen_load_param_addr(op2,1);
	gen_load_param_reg(op1,0);
	return gen_call_function_setup(func, 2);
}

static DRC_PTR_SIZE_IM INLINE gen_call_function_RR(void * func,unsigned op1,unsigned op2) {
	gen_load_param_reg(op2,1);
	gen_load_param_reg(op1,0);
	return gen_call_function_setup(func, 2);
}

static DRC_PTR_SIZE_IM INLINE gen_call_function_IR(void * func,unsigned op1,unsigned op2) {
	gen_load_param_reg(op2,1);
	gen_load_param_imm(op1,0);
	return gen_call_function_setup(func, 2);
}

static DRC_PTR_SIZE_IM INLINE gen_call_function_I(void * func,unsigned op) {
	gen_load_param_imm(op,0);
	return gen_call_function_setup(func, 1);
}

static DRC_PTR_SIZE_IM INLINE gen_call_function_II(void * func,unsigned op1,unsigned op2) {
	gen_load_param_imm(op2,1);
	gen_load_param_imm(op1,0);
	return gen_call_function_setup(func, 2);
}

static DRC_PTR_SIZE_IM INLINE gen_call_function_III(void * func,unsigned op1,unsigned op2,unsigned op3) {
...
static DRC_PTR_SIZE_IM INLINE gen_call_function_IA(void * func,unsigned op1,DRC_PTR_SIZE_IM op2) {
	gen_load_param_addr(op2,1);
	gen_load_param_imm(op1,0);
	return gen_call_function_setup(func, 2);
}

static DRC_PTR_SIZE_IM INLINE gen_call_function_IIR(void * func,unsigned op1,unsigned op2,unsigned op3) {
...
static DRC_PTR_SIZE_IM INLINE gen_call_function_IIIR(void * func,unsigned op1,unsigned op2,unsigned op3,unsigned op4) {
...
static DRC_PTR_SIZE_IM INLINE gen_call_function_IRRR(void * func,unsigned op1,unsigned op2,unsigned op3,unsigned op4) {
	gen_load_param_reg(op4,3);
	gen_load_param_reg(op3,2);
	gen_load_param_reg(op2,1);
	gen_load_param_imm(op1,0);
...
static DRC_PTR_SIZE_IM INLINE gen_call_function_m(void * func,unsigned op) {
	gen_load_param_mem(op,2);
	return gen_call_function_setup(func, 3, true);
}

static DRC_PTR_SIZE_IM INLINE gen_call_function_mm(void * func,unsigned op1,unsigned op2) {
...
static BlockReturn DynRunException(System::UInt32 eip_add,System::UInt32 cycle_sub) {
	reg_eip+=eip_add;
...
static struct {
	...
	System::UInt32 eip_change;
	unsigned cycles;
} save_info_dynrec[512];

unsigned used_save_info_dynrec=0;
...
static void dyn_fill_blocks(void) {
	for (unsigned sct=0; sct<used_save_info_dynrec; sct++) {
...
bool DRC_CALL_CONV mem_readb_checked_drc(PhysPt address) {
	...
	if (tlb_addr) {
		*((System::Byte*)(&core_dynrec.readdata))=host_readb(tlb_addr+address);
		return false;
	} else {
		return get_tlb_readhandler(address)->readb_checked(address, (System::Byte*)(&core_dynrec.readdata));
	}
}

bool DRC_CALL_CONV mem_writeb_checked_drc(PhysPt address,System::Byte val) DRC_FC;
bool DRC_CALL_CONV mem_writeb_checked_drc(PhysPt address,System::Byte val) {
...
bool DRC_CALL_CONV mem_readw_checked_drc(PhysPt address) {
	if ((address & 0xfff)<0xfff) {
		HostPt tlb_addr=get_tlb_read(address);
		if (tlb_addr) {
			*((System::UInt16*)(&core_dynrec.readdata))=host_readw(tlb_addr+address);
			return false;
		} else return get_tlb_readhandler(address)->readw_checked(address, (System::UInt16*)(&core_dynrec.readdata));
	} else return mem_unalignedreadw_checked(address, ((System::UInt16*)(&core_dynrec.readdata)));
}
...
bool DRC_CALL_CONV mem_readd_checked_drc(PhysPt address) {
	if ((address & 0xfff)<0xffd) {
		HostPt tlb_addr=get_tlb_read(address);
		if (tlb_addr) {
			*((System::UInt32*)(&core_dynrec.readdata))=host_readd(tlb_addr+address);
			return false;
		} else return get_tlb_readhandler(address)->readd_checked(address, (System::UInt32*)(&core_dynrec.readdata));
	} else return mem_unalignedreadd_checked(address, ((System::UInt32*)(&core_dynrec.readdata)));
}

bool DRC_CALL_CONV mem_writew_checked_drc(PhysPt address,System::UInt16 val) DRC_FC;
bool DRC_CALL_CONV mem_writew_checked_drc(PhysPt address,System::UInt16 val) {
	if ((address & 0xfff)<0xfff) {
		HostPt tlb_addr=get_tlb_write(address);
		if (tlb_addr) {
...
bool DRC_CALL_CONV mem_writed_checked_drc(PhysPt address,System::UInt32 val) DRC_FC;
bool DRC_CALL_CONV mem_writed_checked_drc(PhysPt address,System::UInt32 val) {
	if ((address & 0xfff)<0xffd) {
		HostPt tlb_addr=get_tlb_write(address);
		if (tlb_addr) {
...
static void dyn_lea_mem_mem(HostReg ea_reg,void* op1,void* op2,unsigned scale,int imm) {
	if (scale || imm) {
		if (op1!=NULL) {
...
static void dyn_lea_regval_regval(HostReg ea_reg,unsigned op1_index,unsigned op2_index,unsigned scale,int imm) {
	if (scale || imm) {
...
static void dyn_lea_mem_regval(HostReg ea_reg,void* op1,unsigned op2_index,unsigned scale,int imm) {
	if (scale || imm) {
		if (op1!=NULL) {
			gen_mov_word_to_reg(ea_reg,op1,true);
			MOV_REG_VAL_TO_HOST_REG(TEMP_REG_DRC,op2_index);
...
static void dyn_lea_segphys_regval(HostReg ea_reg,unsigned op1_index,unsigned op2_index,unsigned scale,int imm) {
	if (scale || imm) {
...
static void dyn_lea_segphys_mem(HostReg ea_reg,unsigned op1_index,void* op2,unsigned scale,int imm) {
	if (scale || imm) {
		MOV_SEG_PHYS_TO_HOST_REG(ea_reg,op1_index);
		gen_mov_word_to_reg(TEMP_REG_DRC,op2,true);
...
static void dyn_fill_ea(HostReg ea_reg,bool addseg=true) {
	System::Byte seg_base=DRC_SEG_DS;
	if (!decode.big_addr) {
		int imm;
		switch (decode.modrm.mod) {
		case 0:imm=0;break;
		case 1:imm=(System::SByte)decode_fetchb();break;
		case 2:imm=(System::Int16)decode_fetchw();break;
		}
		...
		case 4:// SI
			...
			if (imm) gen_add_imm(ea_reg,(System::UInt32)imm);
			break;
		case 5:// DI
		...
		case 6:// imm/BP
			if (!decode.modrm.mod) {
				imm=decode_fetchw();
				gen_mov_dword_to_reg_imm(ea_reg,(System::UInt32)imm);
				goto skip_extend_word;
			} else {
				MOV_REG_VAL_TO_HOST_REG(ea_reg,DRC_REG_EBP);
				gen_add_imm(ea_reg,(System::UInt32)imm);
				seg_base=DRC_SEG_SS;
			}
			break;
		case 7: // BX
...
skip_extend_word:
		...
		int imm=0;
		System::Byte base_reg;
		System::Byte scaled_reg;
		unsigned scale=0;
		switch (decode.modrm.rm) {
		...
		case 4:	// SIB
			{
				unsigned sib=decode_fetchb();
				bool scaled_reg_used=false;
				static System::Byte scaledtable[8]={
				...
				switch (decode.modrm.mod) {
				case 1:
					imm=(System::SByte)decode_fetchb();
					break;
				case 2: {
					unsigned val;
					// try to get a pointer to the next dword code position
		...
		case 5:
			if (decode.modrm.mod) {
				...
				imm=(System::Int32)decode_fetchd();
				if (!addseg) {
					gen_mov_dword_to_reg_imm(ea_reg,(System::UInt32)imm);
				} else {
					MOV_SEG_PHYS_TO_HOST_REG(ea_reg,(decode.seg_prefix_used ? decode.seg_prefix : seg_base));
...
static void dyn_add_iocheck(HostReg reg_port,unsigned access_size) {
	if (cpu.pmode) {
...
static void dyn_add_iocheck_var(System::Byte accessed_port,unsigned access_size) {
	if (cpu.pmode) {
...
static unsigned mf_functions_num=0;
static struct {
	System::Byte* pos;
	void* fct_ptr;
	unsigned ftype;
...
static void InvalidateFlags(void) {
	...
	for (unsigned ct=0; ct<mf_functions_num; ct++) {
...
// because the current instruction destroys all condition flags and
// the flags are not required before
static void InvalidateFlags(void* current_simple_function,unsigned flags_type) {
#ifdef DRC_FLAGS_INVALIDATION
	for (unsigned ct=0; ct<mf_functions_num; ct++) {
		gen_fill_function_ptr(mf_functions[ct].pos,mf_functions[ct].fct_ptr,mf_functions[ct].ftype);
	}
...
static void InvalidateFlagsPartially(void* current_simple_function,unsigned flags_type) {
#ifdef DRC_FLAGS_INVALIDATION
...
// destroys all condition flags and the flags weren't needed in-between
// this function can be replaced by a simpler one as well
static void InvalidateFlagsPartially(void* current_simple_function,DRC_PTR_SIZE_IM cpos,unsigned flags_type) {
#ifdef DRC_FLAGS_INVALIDATION
	mf_functions[mf_functions_num].pos=(System::Byte*)cpos;
	mf_functions[mf_functions_num].fct_ptr=current_simple_function;
	mf_functions[mf_functions_num].ftype=flags_type;
	mf_functions_num++;
#endif
...
static void AcquireFlags(unsigned flags_mask) {
#ifdef DRC_FLAGS_INVALIDATION
	mf_functions_num=0;
#endif
}
