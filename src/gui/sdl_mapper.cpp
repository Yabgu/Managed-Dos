...
public:
	...
	virtual unsigned GetActivityCount(void) {
		return activity;
	}
...
//		event->bindlist.remove(this);
	...
	void ActivateBind(int _value,bool ev_trigger,bool skip_action=false) {
		if (event->IsTrigger()) {
...
public:
	...
	void ActivateBindList(CBindList * list,int value,bool ev_trigger);
...
static System::Byte scancode_map[MAX_SDLKEYS];
...
SDLKey MapSDLCode(unsigned skey) {
	if (usescancodes) {
...
unsigned GetKeyCode(SDL_keysym keysym) {
	if (usescancodes) {
		unsigned key=(unsigned)keysym.scancode;
		if (key==0
#if defined (MACOSX)
			...
			if (keysym.sym<MAX_SDLKEYS) key=scancode_map[(unsigned)keysym.sym];
		} 
		...
		switch (key) {
			...
			case 0x52:	// INSERT
			case 0x53:	// DELETE
				if (GFX_SDLUsingWinDIB()) key=scancode_map[(unsigned)keysym.sym];
				break;
		}
#endif
		return key;
		...
		if ((keysym.sym==SDLK_BACKSLASH) && (keysym.scancode==0x56)) return (unsigned)SDLK_LESS;
#endif
		return (unsigned)keysym.sym;
	}
}
...
public:
	...
	void BindName(char * buf) {
		sprintf(buf,"Key %s",SDL_GetKeyName(MapSDLCode((unsigned)key)));
	}
	void ConfigName(char * buf) {
		sprintf(buf,"key %d",MapSDLCode((unsigned)key));
	}
public:
	SDLKey key;
};
...
public:
	CKeyBindGroup(unsigned _keys) : CBindGroup (){
		lists=new CBindList[_keys];
		for (unsigned i=0;i<_keys;i++) lists[i].clear();
		keys=_keys;
		configname="key";
	}
...
//		LOG_MSG("key type %i is %x [%x %x]",event->type,key,event->key.keysym.sym,event->key.keysym.scancode);
	...
	CBind * CreateKeyBind(SDLKey _key) {
		if (!usescancodes) assert((unsigned)_key<keys);
		return new CKeyBind(&lists[(unsigned)_key],_key);
	}
private:
...
public:
	CJAxisBind(CBindList * _list,CBindGroup * _group,unsigned _axis,bool _positive) : CBind(_list){
		group = _group;
		axis = _axis;
		positive = _positive;
	}
...
public:
	CJButtonBind(CBindList * _list,CBindGroup * _group,unsigned _button) : CBind(_list) {
		group = _group;
		button=_button;
	}
	void ConfigName(char * buf) {
...
public:
	CJHatBind(CBindList * _list,CBindGroup * _group,unsigned _hat,System::Byte _dir) : CBind(_list) {
		group = _group;
		hat   = _hat;
		dir   = _dir;
...
public:
	CStickBindGroup(unsigned _stick,unsigned _emustick,bool _dummy=false) : CBindGroup (){
	...
	CBind * CreateConfigBind(char *& buf) {
		...
		if (!strcasecmp(type,"axis")) {
			unsigned ax=ConvDecWord(StripWord(buf));
			bool pos=ConvDecWord(StripWord(buf)) > 0;
			bind=CreateAxisBind(ax,pos);
		} else if (!strcasecmp(type,"button")) {
			unsigned but=ConvDecWord(StripWord(buf));			
			bind=CreateButtonBind(but);
		} else if (!strcasecmp(type,"hat")) {
			unsigned hat=ConvDecWord(StripWord(buf));			
			System::Byte dir=(System::Byte)ConvDecWord(StripWord(buf));			
	...
	void ActivateJoystickBoundEvents() {
		...
		for (i=0; i<axes_cap; i++) {
			System::Int16 caxis_pos=SDL_JoystickGetAxis(sdl_joystick,i);
			...
			if (caxis_pos>1) {
				...
				if (caxis_pos!=-32768) caxis_pos=(System::Int16)abs(caxis_pos);
		...
		for (i=0; i<hats_cap; i++) {
			System::Byte chat_state=SDL_JoystickGetHat(sdl_joystick,i);
...
private:
	CBind * CreateAxisBind(unsigned axis,bool positive) {
	...
	CBind * CreateButtonBind(unsigned button) {
		if (button<button_wrap) 
			return new CJButtonBind(&button_lists[button],this,button);
		return NULL;
	}
	CBind * CreateHatBind(unsigned hat,System::Byte value) {
		unsigned hat_dir;
...
protected:
	...
	CBindList * hat_lists;
	unsigned stick,emustick,axes,buttons,hats,emulated_axes,emulated_buttons,emulated_hats;
	unsigned button_wrap,button_cap,axes_cap,hats_cap;
	...
	System::Byte old_hat_state[16];
	bool is_dummy;
};

class C4AxisBindGroup : public  CStickBindGroup {
public:
	C4AxisBindGroup(unsigned _stick,unsigned _emustick) : CStickBindGroup (_stick,_emustick){
		emulated_axes=4;
		emulated_buttons=4;
...
public:
	CFCSBindGroup(unsigned _stick,unsigned _emustick) : CStickBindGroup (_stick,_emustick){
		emulated_axes=4;
		emulated_buttons=4;
		old_hat_position=0;
		emulated_hats=1;
...
private:
	System::Byte old_hat_position;

	void DecodeHatPosition(System::Byte hat_pos) {
		switch(hat_pos) {
...
public:
	CCHBindGroup(unsigned _stick,unsigned _emustick) : CStickBindGroup (_stick,_emustick){
		emulated_axes=4;
		emulated_buttons=6;
		emulated_hats=1;
		if (button_wrapping_enabled) button_wrap=emulated_buttons;
...
static struct CMapper {
	...
	unsigned mods;
	struct {
		unsigned num_groups,num;
		CStickBindGroup * stick[MAXSTICKS];
	} sticks;
	std::string filename;
} mapper;

void CBindGroup::ActivateBindList(CBindList * list,int value,bool ev_trigger) {
	unsigned validmod=0;
	CBindList_it it;
...
static void DrawText(unsigned x,unsigned y,const char * text,System::Byte color) {
	System::Byte * draw=((System::Byte *)mapper.surface->pixels)+(y*mapper.surface->pitch)+x;
	while (*text) {
		System::Byte * font=&int10_font_14[(*text)*14];
		unsigned i,j;System::Byte * draw_line=draw;
		for (i=0;i<14;i++) {
			System::Byte map=*font++;
			for (j=0;j<8;j++) {
...
public:
	virtual ~CButton(){};
	CButton(unsigned _x,unsigned _y,unsigned _dx,unsigned _dy) {
	...
	virtual void Draw(void) {
		if (!enabled) return;
		System::Byte * point=((System::Byte *)mapper.surface->pixels)+(y*mapper.surface->pitch)+x;
		for (unsigned lines=0;lines<dy;lines++)  {
			if (lines==0 || lines==(dy-1)) {
				for (unsigned cols=0;cols<dx;cols++) *(point+cols)=color;
			} else {
	...
	virtual bool OnTop(unsigned _x,unsigned _y) {
	...
	void SetColor(System::Byte _col) { color=_col; }
protected:
	unsigned x,y,dx,dy;
	System::Byte color;
	bool enabled;
...
public:
	CTextButton(unsigned _x,unsigned _y,unsigned _dx,unsigned _dy,const char * _text) : CButton(_x,_y,_dx,_dy) { text=_text;}
	void Draw(void) {
		if (!enabled) return;
		CButton::Draw();
...
public:
	CEventButton(unsigned _x,unsigned _y,unsigned _dx,unsigned _dy,const char * _text,CEvent * _event) 
...
public:
	CCaptionButton(unsigned _x,unsigned _y,unsigned _dx,unsigned _dy) : CButton(_x,_y,_dx,_dy){
		caption[0]=0;
	}
...
static void change_action_text(const char* text,System::Byte col);

static void MAPPER_SaveBinds(void);
class CBindButton : public CTextButton {
public:	
	CBindButton(unsigned _x,unsigned _y,unsigned _dx,unsigned _dy,const char * _text,BB_Types _type) 
	: CTextButton(_x,_y,_dx,_dy,_text) 	{ 
		type=_type;
	}
	void Click(void) {
...
public:	
	CCheckButton(unsigned _x,unsigned _y,unsigned _dx,unsigned _dy,const char * _text,BC_Types _type) 
	: CTextButton(_x,_y,_dx,_dy,_text) 	{ 
		type=_type;
	}
	void Draw(void) {
		...
		if (checked) {
			System::Byte * point=((System::Byte *)mapper.surface->pixels)+((y+2)*mapper.surface->pitch)+x+dx-dy+2;
			for (unsigned lines=0;lines<(dy-4);lines++)  {
...
public:
	CJAxisEvent(char const * const _entry,unsigned _stick,unsigned _axis,bool _positive,CJAxisEvent * _opposite_axis) : CContinuousEvent(_entry) {
		stick=_stick;
		axis=_axis;
		positive=_positive;
	...
	void Active(bool /*moved*/) {
		virtual_joysticks[stick].axis_pos[axis]=(System::Int16)(GetValue()*(positive?1:-1));
	}
	virtual unsigned GetActivityCount(void) {
...
public:
	CJButtonEvent(char const * const _entry,unsigned _stick,unsigned _button) : CTriggeredEvent(_entry) {
		stick=_stick;
		button=_button;
	}
...
protected:
	unsigned stick,button;
};

class CJHatEvent : public CTriggeredEvent {
public:
	CJHatEvent(char const * const _entry,unsigned _stick,unsigned _hat,unsigned _dir) : CTriggeredEvent(_entry) {
		stick=_stick;
		hat=_hat;
		dir=_dir;
	}
...
protected:
	unsigned stick,hat,dir;
};


class CModEvent : public CTriggeredEvent {
public:
	CModEvent(char const * const _entry,unsigned _wmod) : CTriggeredEvent(_entry) {
		wmod=_wmod;
	}
	void Active(bool yesno) {
...
public:
	CHandlerEvent(char const * const _entry,MAPPER_Handler * _handler,MapKeys _key,unsigned _mod,char const * const _buttonname) : CTriggeredEvent(_entry) {
		handler=_handler;
		defmod=_mod;
		defkey=_key;
...
static CKeyEvent * AddKeyButtonEvent(unsigned x,unsigned y,unsigned dx,unsigned dy,char const * const title,char const * const entry,KBD_KEYS key) {
	char buf[64];
	strcpy(buf,"key_");
	strcat(buf,entry);
...
static CJAxisEvent * AddJAxisButton(unsigned x,unsigned y,unsigned dx,unsigned dy,char const * const title,unsigned stick,unsigned axis,bool positive,CJAxisEvent * opposite_axis) {
	char buf[64];
...
static CJAxisEvent * AddJAxisButton_hidden(unsigned stick,unsigned axis,bool positive,CJAxisEvent * opposite_axis) {
	char buf[64];
	sprintf(buf,"jaxis_%d_%d%s",stick,axis,positive ? "+" : "-");
	return new CJAxisEvent(buf,stick,axis,positive,opposite_axis);
}

static void AddJButtonButton(unsigned x,unsigned y,unsigned dx,unsigned dy,char const * const title,unsigned stick,unsigned button) {
	char buf[64];
	sprintf(buf,"jbutton_%d_%d",stick,button);
	CJButtonEvent * event=new CJButtonEvent(buf,stick,button);
	new CEventButton(x,y,dx,dy,title,event);
}
static void AddJButtonButton_hidden(unsigned stick,unsigned button) {
	char buf[64];
	sprintf(buf,"jbutton_%d_%d",stick,button);
	new CJButtonEvent(buf,stick,button);
}

static void AddJHatButton(unsigned x,unsigned y,unsigned dx,unsigned dy,char const * const title,unsigned _stick,unsigned _hat,unsigned _dir) {
	char buf[64];
...
static void AddModButton(unsigned x,unsigned y,unsigned dx,unsigned dy,char const * const title,unsigned _mod) {
	char buf[64];
...
#define YO 8
	...
	unsigned xpos=3;unsigned ypos=11;
...
void MAPPER_AddHandler(MAPPER_Handler * handler,MapKeys key,unsigned mods,char const * const eventname,char const * const buttonname) {
...
void MAPPER_UpdateJoysticks(void) {
	for (unsigned i=0; i<mapper.sticks.num_groups; i++) {
...
//					const char* geom = XGetAtomName(info.info.x11.display, desc->names->geometry);
		...
		for (i=0; i<MAX_SCANCODES; i++) {
			...
			if (key<MAX_SDLKEYS) scancode_map[key]=(System::Byte)i;
		}
	}
