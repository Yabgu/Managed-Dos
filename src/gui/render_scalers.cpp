...
System::Byte Scaler_Aspect[SCALER_MAXHEIGHT];
System::UInt16 Scaler_ChangedLines[SCALER_MAXHEIGHT];
unsigned Scaler_ChangedLineIndex;

static union {
	System::UInt32 b32 [4][SCALER_MAXWIDTH*3];
	System::UInt16 b16 [4][SCALER_MAXWIDTH*3];
	System::Byte b8 [4][SCALER_MAXWIDTH*3];
} scalerWriteCache;
...
static INLINE void unsignedMove( void *_dst, const void * _src, unsigned size) {
	unsigned * dst=(unsigned *)(_dst);
	const unsigned * src=(unsigned *)(_src);
	size/=sizeof(unsigned);
	for (unsigned x=0; x<size;x++)
		dst[x] = src[x];
}

static INLINE void ScalerAddLines( unsigned changed, unsigned count ) {
...
#define unsignedMove2(_DST,_SRC,_SIZE)			\
{											\
	unsigned bsize=(_SIZE)/sizeof(unsigned);		\
	unsigned * bdst=(unsigned *)(_DST);				\
	unsigned * bsrc=(unsigned *)(_SRC);				\
