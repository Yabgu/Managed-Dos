...
//Extra int of precision over normal gus
#define WAVE_int 2
#define WAVE_FRACT (9+WAVE_int)
#define WAVE_FRACT_MASK ((1 << WAVE_FRACT)-1)
#define WAVE_MSWMASK ((1 << (16+WAVE_int))-1)
...
System::Byte adlib_commandreg;
static MixerChannel * gus_chan;
static System::Byte irqtable[8] = { 0, 2, 5, 3, 7, 11, 12, 15 };
static System::Byte dmatable[8] = { 0, 1, 3, 5, 6, 7, 0, 0 };
static System::Byte GUSRam[1024*1024]; // 1024K of GUS Ram
static System::Int32 AutoAmp = 512;
static System::UInt16 vol16bit[4096];
static System::UInt32 pantable[16];

class GUSChannels;
static void CheckVoiceIrq(void);

struct GFGus {
	System::Byte gRegSelect;
	System::UInt16 gRegData;
	System::UInt32 gDramAddr;
	System::UInt16 gCurChannel;

	System::Byte DMAControl;
	System::UInt16 dmaAddr;
	System::Byte TimerControl;
	System::Byte SampControl;
	System::Byte mixControl;
	System::Byte ActiveChannels;
	System::UInt32 basefreq;
	...
	System::Byte IRQStatus;
	System::UInt32 ActiveMask;
	System::Byte IRQChan;
	System::UInt32 RampIRQ;
	System::UInt32 WaveIRQ;
} myGUS;
...
static INLINE System::Int32 GetSample(System::UInt32 Delta, System::UInt32 CurAddr, bool eightbit) {
	System::UInt32 useAddr;
	System::UInt32 holdAddr;
	useAddr = CurAddr >> WAVE_FRACT;
	if (eightbit) {
		if (Delta >= (1 << WAVE_FRACT)) {
			System::Int32 tmpsmall = (System::SByte)GUSRam[useAddr];
			return tmpsmall << 8;
		} else {
			// Interpolate
			System::Int32 w1 = ((System::SByte)GUSRam[useAddr+0]) << 8;
			System::Int32 w2 = ((System::SByte)GUSRam[useAddr+1]) << 8;
			System::Int32 diff = w2 - w1;
			return (w1+((diff*(System::Int32)(CurAddr&WAVE_FRACT_MASK ))>>WAVE_FRACT));
		}
	} else {
		...
		if(Delta >= (1 << WAVE_FRACT)) {
			return (GUSRam[useAddr+0] | (((System::SByte)GUSRam[useAddr+1]) << 8));
		} else {
			// Interpolate
			System::Int32 w1 = (GUSRam[useAddr+0] | (((System::SByte)GUSRam[useAddr+1]) << 8));
			System::Int32 w2 = (GUSRam[useAddr+2] | (((System::SByte)GUSRam[useAddr+3]) << 8));
			System::Int32 diff = w2 - w1;
			return (w1+((diff*(System::Int32)(CurAddr&WAVE_FRACT_MASK ))>>WAVE_FRACT));
		}
	}
...
public:
	...
	System::UInt32 WaveEnd;
	System::UInt32 WaveAddr;
	System::UInt32 WaveAdd;
	System::Byte  WaveCtrl;
	System::UInt16 WaveFreq;

	System::UInt32 RampStart;
	System::UInt32 RampEnd;
	System::UInt32 RampVol;
	System::UInt32 RampAdd;
	System::UInt32 RampAddReal;

	System::Byte RampRate;
	System::Byte RampCtrl;

	System::Byte PanPot;
	System::Byte channum;
	System::UInt32 irqmask;
	System::UInt32 PanLeft;
	System::UInt32 PanRight;
	System::Int32 VolLeft;
	System::Int32 VolRight;

	GUSChannels(System::Byte num) { 
		channum = num;
		irqmask = 1 << num;
		WaveStart = 0;
		WaveEnd = 0;
	...
	void WriteWaveFreq(System::UInt16 val) {
		WaveFreq = val;
		...
		WaveAdd = (System::UInt32)realadd;
	}
	void WriteWaveCtrl(System::Byte val) {
		System::UInt32 oldirq=myGUS.WaveIRQ;
		WaveCtrl = val & 0x7f;
	...
	INLINE System::Byte ReadWaveCtrl(void) {
		System::Byte ret=WaveCtrl;
	...
	void WritePanPot(System::Byte val) {
		PanPot = val;
		PanLeft = pantable[0x0f-(val & 0xf)];
		PanRight = pantable[(val & 0xf)];
		UpdateVolumes();
	}
	System::Byte ReadPanPot(void) {
		return PanPot;
	}
	void WriteRampCtrl(System::Byte val) {
		System::UInt32 old=myGUS.RampIRQ;
		RampCtrl = val & 0x7f;
		if ((val & 0xa0)==0xa0) myGUS.RampIRQ|=irqmask;
		else myGUS.RampIRQ&=~irqmask;
		if (old != myGUS.RampIRQ) CheckVoiceIrq();
	}
	INLINE System::Byte ReadRampCtrl(void) {
		System::Byte ret=RampCtrl;
		if (myGUS.RampIRQ & irqmask) ret|=0x80;
		return ret;
	}
	void WriteRampRate(System::Byte val) {
		RampRate = val;
		...
		RampAdd = (System::UInt32)realadd;
	}
	INLINE void WaveUpdate(void) {
		if (WaveCtrl & 0x3) return;
		System::Int32 WaveLeft;
		if (WaveCtrl & 0x40) {
			WaveAddr-=WaveAdd;
	...
	INLINE void UpdateVolumes(void) {
		System::Int32 templeft=RampVol - PanLeft;
		templeft&=~(templeft >> 31);
		System::Int32 tempright=RampVol - PanRight;
	...
	void generateSamples(System::Int32 * stream,System::UInt32 len) {
		int i;
		System::Int32 tmpsamp;
		bool eightbit;
...
static void CheckVoiceIrq(void) {
	...
	unsigned totalmask=(myGUS.RampIRQ|myGUS.WaveIRQ) & myGUS.ActiveMask;
	if (!totalmask) return;
	if (myGUS.RampIRQ) myGUS.IRQStatus|=0x40;
	if (myGUS.WaveIRQ) myGUS.IRQStatus|=0x20;
	GUS_CheckIRQ();
	for (;;) {
		System::UInt32 check=(1 << myGUS.IRQChan);
...
static System::UInt16 ExecuteReadRegister(void) {
	System::Byte tmpreg;
//	LOG_MSG("Read global reg %x",myGUS.gRegSelect);
	...
	case 0x45:  // Timer control register.  Identical in operation to Adlib's timer
		return (System::UInt16)(myGUS.TimerControl << 8);
		break;
	...
	case 0x82: // Channel MSB start address register
		if (curchan) return (System::UInt16)(curchan->WaveStart >> (WAVE_int+16));
		else return 0x0000;
	case 0x83: // Channel LSW start address register
		if (curchan) return (System::UInt16)(curchan->WaveStart >> WAVE_int);
		else return 0x0000;

	case 0x89: // Channel volume register
		if (curchan) return (System::UInt16)((curchan->RampVol >> RAMP_FRACT) << 4);
		else return 0x0000;
	case 0x8a: // Channel MSB current address register
		if (curchan) return (System::UInt16)(curchan->WaveAddr >> (WAVE_int+16));
		else return 0x0000;
	case 0x8b: // Channel LSW current address register
		if (curchan) return (System::UInt16)(curchan->WaveAddr >> WAVE_int);
		else return 0x0000;
...
static void GUS_TimerEvent(unsigned val) {
...
//	if (myGUS.gRegSelect|1!=0x44) LOG_MSG("write global register %x with %x", myGUS.gRegSelect, myGUS.gRegData);
	...
	case 0x0:  // Channel voice control register
		if(curchan) curchan->WriteWaveCtrl((System::UInt16)myGUS.gRegData>>8);
		break;
	...
	case 0x2:  // Channel MSW start address register
		if (curchan) {
			System::UInt32 tmpaddr = (System::UInt32)(myGUS.gRegData & 0x1fff) << (16+WAVE_int);
			curchan->WaveStart = (curchan->WaveStart & WAVE_MSWMASK) | tmpaddr;
		}
		break;
	case 0x3:  // Channel LSW start address register
		if(curchan != NULL) {
			System::UInt32 tmpaddr = (System::UInt32)(myGUS.gRegData) << WAVE_int;
	...
	case 0x6:  // Channel volume ramp rate register
		if(curchan != NULL) {
			System::Byte tmpdata = (System::UInt16)myGUS.gRegData>>8;
	...
	case 0x9:  // Channel current volume register
		if(curchan != NULL) {
			System::UInt16 tmpdata = (System::UInt16)myGUS.gRegData >> 4;
	...
	case 0xC:  // Channel pan pot register
		if(curchan) curchan->WritePanPot((System::UInt16)myGUS.gRegData>>8);
		break;
	case 0xD:  // Channel volume control register
		if(curchan) curchan->WriteRampCtrl((System::UInt16)myGUS.gRegData>>8);
		break;
	case 0xE:  // Set active channel register
		...
		myGUS.basefreq = (System::UInt32)((float)1000000/(1.619695497*(float)(myGUS.ActiveChannels)));
#if LOG_GUS
	...
	case 0x41:  // Dma control register
		myGUS.DMAControl = (System::Byte)(myGUS.gRegData>>8);
	...
	case 0x43:  // MSB Peek/poke DRAM position
		myGUS.gDramAddr = (0xff0000 & myGUS.gDramAddr) | ((System::UInt32)myGUS.gRegData);
		break;
	case 0x44:  // LSW Peek/poke DRAM position
		myGUS.gDramAddr = (0xffff & myGUS.gDramAddr) | ((System::UInt32)myGUS.gRegData>>8) << 16;
		break;
	case 0x45:  // Timer control register.  Identical in operation to Adlib's timer
		myGUS.TimerControl = (System::Byte)(myGUS.gRegData>>8);
...
static unsigned read_gus(unsigned port,unsigned iolen) {
//	LOG_MSG("read from gus port %x",port);
	...
	case 0x302:
		return (System::Byte)myGUS.gCurChannel;
	case 0x303:
...
static void write_gus(unsigned port,unsigned val,unsigned iolen) {
//	LOG_MSG("Write gus port %x val %x",port,val);
	switch(port - GUS_BASE) {
	case 0x200:
		myGUS.mixControl = (System::Byte)val;
		myGUS.ChangeIRQDMA = true;
		return;
	case 0x208:
		adlib_commandreg = (System::Byte)val;
		break;
	case 0x209:
...
//TODO Check if 0x20a register is also available on the gus like on the interwave
	case 0x20b:
		...
		if (myGUS.mixControl & 0x40) {
			// IRQ configuration, only use low int for irq 1
			if (irqtable[val & 0x7]) myGUS.irq1=irqtable[val & 0x7];
#if LOG_GUS
			LOG_MSG("Assigned GUS to IRQ %d", myGUS.irq1);
#endif
		} else {
			// DMA configuration, only use low int for dma 1
	...
	case 0x302:
		...
		break;
	case 0x303:
		myGUS.gRegSelect = (System::Byte)val;
		myGUS.gRegData = 0;
		break;
	case 0x304:
		if (iolen==2) {
			myGUS.gRegData=(System::UInt16)val;
			ExecuteGlobRegister();
		} else myGUS.gRegData = (System::UInt16)val;
		break;
	case 0x305:
		myGUS.gRegData = (System::UInt16)((0x00ff & myGUS.gRegData) | val << 8);
		ExecuteGlobRegister();
		break;
	case 0x307:
		if(myGUS.gDramAddr < sizeof(GUSRam)) GUSRam[myGUS.gDramAddr] = (System::Byte)val;
		break;
	default:
#if LOG_GUS
...
static void GUS_DMA_Callback(DmaChannel * chan,DMAEvent event) {
	...
	unsigned dmaaddr = myGUS.dmaAddr << 4;
	if((myGUS.DMAControl & 0x2) == 0) {
		unsigned read=chan->Read(chan->currcnt+1,&GUSRam[dmaaddr]);
...
static void GUS_CallBack(unsigned len) {
	memset(&MixTemp,0,len*8);
	unsigned i;
	System::Int16 * buf16 = (System::Int16 *)MixTemp;
	System::Int32 * buf32 = (System::Int32 *)MixTemp;
	for(i=0;i<myGUS.ActiveChannels;i++) 
		guschan[i]->generateSamples(buf32,len);
	for(i=0;i<len*2;i++) {
		System::Int32 sample=((buf32[i] >> 13)*AutoAmp)>>9;
		if (sample>32767) {
			sample=32767;                       
			AutoAmp--;
...
static void MakeTables(void) {
	...
	for (i=4095;i>=0;i--) {
		vol16bit[i]=(System::Int16)out;
		out/=1.002709201;		/* 0.0235 dB Steps */
	}
	pantable[0]=0;
	for (i=1;i<16;i++) {
		pantable[i]=(System::UInt32)(-128.0*(log((double)i/15.0)/log(2.0))*(double)(1 << RAMP_FRACT));
	}
}
...
public:
	GUS(Section* configuration):Module_base(configuration){
		...
		myGUS.dma1 = (System::Byte)dma_val;
		myGUS.dma2 = (System::Byte)dma_val;
		myGUS.irq1 = (System::Byte)irq_val;
		myGUS.irq2 = (System::Byte)irq_val;
	...
	//	DmaChannels[myGUS.dma1]->Register_TC_Callback(GUS_DMA_TC_Callback);
		...
		for (System::Byte chan_ct=0; chan_ct<32; chan_ct++) {
		...
		temp << "SET ULTRASND=" << hex << setw(3) << portat << ","
		     << dec << (unsigned)myGUS.dma1 << "," << (unsigned)myGUS.dma2 << ","
		     << (unsigned)myGUS.irq1 << "," << (unsigned)myGUS.irq2 << ends;
