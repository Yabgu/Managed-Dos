...
System::UInt32 get_CF(void) {
	...
	case t_SARb:
		return (((System::SByte) lf_var1b) >> (lf_var2b - 1)) & 1;
	case t_SARw:
		return (((System::Int16) lf_var1w) >> (lf_var2b - 1)) & 1;
	case t_SARd:
		return (((System::Int32) lf_var1d) >> (lf_var2b - 1)) & 1;
	case t_NEGb:
		return lf_var1b;
	case t_NEGw:
		return lf_var1w;
...
/* AF     Adjust flag -- Set on carry from or borrow to the low order
            four int of   AL; cleared otherwise. Used for decimal
            arithmetic.
*/
System::UInt32 get_AF(void) {
	unsigned type=lflags.type;
	switch (type) {
	case t_UNKNOWN:
		return GETFLAG(AF);
	case t_ADDb:	
...
System::UInt16 parity_lookup[256] = {
...
unsigned FillFlags(void) {
//	if (lflags.type==t_UNKNOWN) return reg_flags;
	unsigned new_word=(reg_flags & ~FLAG_MASK);
...
unsigned FillFlags(void) {
	...
	case t_SARb:
		SET_FLAG(CF,(((System::SByte) lf_var1b) >> (lf_var2b - 1)) & 1);
		DOFLAG_ZFb;
		DOFLAG_SFb;
		SET_FLAG(OF,false);
		DOFLAG_PF;
		...
		break;
	case t_SARw:
		SET_FLAG(CF,(((System::Int16) lf_var1w) >> (lf_var2b - 1)) & 1);
		DOFLAG_ZFw;
		DOFLAG_SFw;
		SET_FLAG(OF,false);
		DOFLAG_PF;
		...
		break;
	case t_SARd:
		SET_FLAG(CF,(((System::Int32) lf_var1d) >> (lf_var2b - 1)) & 1);
		DOFLAG_ZFd;
		DOFLAG_SFd;
		SET_FLAG(OF,false);
		DOFLAG_PF;
