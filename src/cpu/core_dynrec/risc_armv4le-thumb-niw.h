...
// data pool variables
static System::Byte * cache_datapos = NULL;	// position of data pool in the cache block
static System::UInt32 cache_datasize = 0;		// total size of data pool
static System::UInt32 cache_dataindex = 0;		// used size of data pool = index of free data item (in bytes) in data pool
...
static void cache_checkinstr(System::UInt32 size) {
	if (cache_datasize == 0) {
	...
	{
		register System::Byte * newcachepos;
	...
	{
		cache_datapos = (System::Byte *) (((unsigned)cache.block.active->cache.start + cache.block.active->cache.size - CACHE_DATA_ALIGN) & ~(CACHE_DATA_ALIGN - 1));
	} else {
		register System::UInt32 cachemodsize;
		...
		{
			cache_datapos = (System::Byte *) (((unsigned)cache.pos + CACHE_DATA_MAX) & ~(CACHE_DATA_ALIGN - 1));
		} else {
			cache_datapos = (System::Byte *) (((unsigned)cache.pos + (CACHE_MAXSIZE - CACHE_DATA_ALIGN) - cachemodsize) & ~(CACHE_DATA_ALIGN - 1));
		}
	}
...
static System::Byte * cache_reservedata(void) {
	...
	if (GCC_UNLIKELY(cache_datapos == NULL)) {
		if (cache.pos + CACHE_DATA_MIN + CACHE_DATA_ALIGN < cache.block.active->cache.start + CACHE_DATA_MAX) {
			cache_datapos = (System::Byte *) (((unsigned)cache.block.active->cache.start + CACHE_DATA_MAX) & ~(CACHE_DATA_ALIGN - 1));
		}
	}
...
static void gen_mov_dword_to_reg_imm(HostReg dest_reg,System::UInt32 imm) {
	if ((imm & 0xffffff00) == 0) {
		cache_checkinstr(2);
		...
		diff = imm - ((System::UInt32)cache.pos+4);
		
		if ((diff < 1024) && ((imm & 0x03) == 0)) {
			if (((System::UInt32)cache.pos & 0x03) == 0) {
...
static void gen_mov_word_to_reg_helper(HostReg dest_reg,void* data,bool dword,HostReg data_reg) {
	// alignment....
	if (dword) {
		if ((System::UInt32)data & 3) {
			if ( ((System::UInt32)data & 3) == 2 ) {
				cache_checkinstr(8);
...
static void gen_mov_word_to_reg(HostReg dest_reg,void* data,bool dword) {
	gen_mov_dword_to_reg_imm(templo2, (System::UInt32)data);
	gen_mov_word_to_reg_helper(dest_reg, data, dword, templo2);
}

// move a 16bit constant value into dest_reg
// the upper 16bit of the destination register may be destroyed
static void INLINE gen_mov_word_to_reg_imm(HostReg dest_reg,System::UInt16 imm) {
	gen_mov_dword_to_reg_imm(dest_reg, (System::UInt32)imm);
}
...
static void gen_mov_word_from_reg_helper(HostReg src_reg,void* dest,bool dword, HostReg data_reg) {
	// alignment....
	if (dword) {
		if ((System::UInt32)dest & 3) {
			if ( ((System::UInt32)dest & 3) == 2 ) {
				cache_checkinstr(8);
...
static void gen_mov_word_from_reg(HostReg src_reg,void* dest,bool dword) {
	gen_mov_dword_to_reg_imm(templo2, (System::UInt32)dest);
...
static void gen_mov_byte_to_reg_low(HostReg dest_reg,void* data) {
	gen_mov_dword_to_reg_imm(templo1, (System::UInt32)data);
	cache_checkinstr(2);
...
// this function does not use FC_OP1/FC_OP2 as dest_reg as these
// registers might not be directly byte-accessible on some architectures
static void gen_mov_byte_to_reg_low_imm(HostReg dest_reg,System::Byte imm) {
	cache_checkinstr(2);
	cache_addw( MOV_IMM(dest_reg, imm) );      // mov dest_reg, #(imm)
}
...
// this function can use FC_OP1/FC_OP2 as dest_reg which are
// not directly byte-accessible on some architectures
static void INLINE gen_mov_byte_to_reg_low_imm_canuseword(HostReg dest_reg,System::Byte imm) {
	gen_mov_byte_to_reg_low_imm(dest_reg, imm);
}

// move the lowest 8bit of a register into memory
static void gen_mov_byte_from_reg_low(HostReg src_reg,void* dest) {
	gen_mov_dword_to_reg_imm(templo1, (System::UInt32)dest);
	cache_checkinstr(2);
...
static void gen_add_imm(HostReg reg,System::UInt32 imm) {
	if(!imm) return;
...
static void gen_and_imm(HostReg reg,System::UInt32 imm) {
...
static void gen_mov_direct_dword(void* dest,System::UInt32 imm) {
...
static void INLINE gen_mov_direct_ptr(void* dest,DRC_PTR_SIZE_IM imm) {
	gen_mov_direct_dword(dest,(System::UInt32)imm);
}

// add an 8bit constant value to a dword memory value
static void gen_add_direct_byte(void* dest,System::SByte imm) {
	if(!imm) return;
	gen_mov_dword_to_reg_imm(templo2, (System::UInt32)dest);
	gen_mov_word_to_reg_helper(templo3, dest, 1, templo2);
	cache_checkinstr(2);
	if (imm >= 0) {
		cache_addw( ADD_IMM8(templo3, (System::Int32)imm) );      // add templo3, #(imm)
	} else {
		cache_addw( SUB_IMM8(templo3, -((System::Int32)imm)) );      // sub templo3, #(-imm)
	}
	gen_mov_word_from_reg_helper(templo3, dest, 1, templo2);
}

// add a 32bit (dword==true) or 16bit (dword==false) constant value to a memory value
static void gen_add_direct_word(void* dest,System::UInt32 imm,bool dword) {
	if(!imm) return;
	if (dword && ( (imm<128) || (imm>=0xffffff80) ) ) {
		gen_add_direct_byte(dest,(System::SByte)imm);
		return;
	}
	gen_mov_dword_to_reg_imm(templo2, (System::UInt32)dest);
	...
	if (dword) {
		...
		gen_mov_word_to_reg_imm(templo1, (System::UInt16)imm);
	}
	cache_checkinstr(2);
...
static void gen_sub_direct_byte(void* dest,System::SByte imm) {
	if(!imm) return;
	gen_mov_dword_to_reg_imm(templo2, (System::UInt32)dest);
	gen_mov_word_to_reg_helper(templo3, dest, 1, templo2);
	cache_checkinstr(2);
...
static void gen_sub_direct_word(void* dest,System::UInt32 imm,bool dword) {
	if(!imm) return;
	if (dword && ( (imm<128) || (imm>=0xffffff80) ) ) {
		gen_sub_direct_byte(dest,(System::SByte)imm);
		return;
	}
	gen_mov_dword_to_reg_imm(templo2, (System::UInt32)dest);
	gen_mov_word_to_reg_helper(templo3, dest, dword, templo2);
...
static INLINE void gen_lea(HostReg dest_reg,HostReg scale_reg,unsigned scale,int imm) {
	if (scale) {
		cache_checkinstr(4);
...
static INLINE void gen_lea(HostReg dest_reg,unsigned scale,int imm) {
	if (scale) {
		cache_checkinstr(2);
...
static void gen_call_function_helper(void * func) {
	...
	datapos = cache_reservedata();
	*(System::UInt32*)datapos=(System::UInt32)func;
...
static System::UInt32 INLINE gen_call_function_setup(void * func,unsigned paramcount,bool fastcall=false) {
	cache_checkinstr(18);
	System::UInt32 proc_addr = (System::UInt32)cache.pos;
	gen_call_function_helper(func);
	return proc_addr;
...
static void INLINE gen_load_param_imm(unsigned imm,unsigned param) {
	gen_mov_dword_to_reg_imm(param, imm);
}

// load an address as param'th function parameter
static void INLINE gen_load_param_addr(unsigned addr,unsigned param) {
	gen_mov_dword_to_reg_imm(param, addr);
}

// load a host-register as param'th function parameter
static void INLINE gen_load_param_reg(unsigned reg,unsigned param) {
	gen_mov_regs(param, reg);
}

// load a value from memory as param'th function parameter
static void INLINE gen_load_param_mem(unsigned mem,unsigned param) {
...
static void gen_jmp_ptr(void * ptr,int imm=0) {
...
static System::UInt32 gen_create_branch_on_zero(HostReg reg,bool dword) {
	cache_checkinstr(4);
	if (dword) {
	...
	return ((System::UInt32)cache.pos-2);
}

// short conditional jump (+-127 bytes) if register is nonzero
// the destination is set by gen_fill_branch() later
static System::UInt32 gen_create_branch_on_nonzero(HostReg reg,bool dword) {
	cache_checkinstr(4);
	if (dword) {
		cache_addw( CMP_IMM(reg, 0) );      // cmp reg, #0
	} else {
...
#if C_DEBUG
	int len=(System::UInt32)cache.pos-(data+4);
	if (len<0) len=-len;
	if (len>252) LOG_MSG("Big jump %d",len);
#endif
	*(System::Byte*)data=(System::Byte)( ((System::UInt32)cache.pos-(data+4)) >> 1 );
}
...
static System::UInt32 gen_create_branch_long_nonzero(HostReg reg,bool isdword) {
	System::Byte *datapos;
	...
	// nobranch:
	return ((System::UInt32)datapos);
}

// compare 32bit-register against zero and jump if value less/equal than zero
static System::UInt32 gen_create_branch_long_leqzero(HostReg reg) {
	System::Byte *datapos;
...
static void INLINE gen_fill_branch_long(System::UInt32 data) {
	// this is an absolute branch
	*(System::UInt32*)data=((System::UInt32)cache.pos) + 1; // add 1 to keep processor in thumb state
}
...
static void gen_run_code(void) {
	...
	// adr: 64
	cache_addd((System::UInt32)&Segs);      // address of "Segs"
	// adr: 68
	cache_addd((System::UInt32)&cpu_regs);  // address of "cpu_regs"
}
...
static void INLINE gen_create_branch_short(void * func) {
	cache_addw( B_FWD((System::UInt32)func - ((System::UInt32)cache.pos + 4)) );      // b func
}
...
static void gen_fill_function_ptr(System::Byte * pos,void* fct_ptr,unsigned flags_type) {
	if ((*(System::UInt16*)pos & 0xf000) == 0xe000) {
		if ((*(System::UInt16*)pos & 0x0fff) >= ((CACHE_DATA_ALIGN / 2) - 1) &&
			(*(System::UInt16*)pos & 0x0fff) < 0x0800)
		{
			pos = (System::Byte *) ( ( ( (System::UInt32)(*(System::UInt16*)pos & 0x0fff) ) << 1 ) + ((System::UInt32)pos + 4) );
		}
	}

#ifdef DRC_FLAGS_INVALIDATION_DCODE
	if (((System::UInt32)pos & 0x03) == 0)
	{
		...
		switch (flags_type) {
			...
			case t_ADDw:
			case t_ADDd:
				*(System::UInt16*)pos=ADD_REG(HOST_a1, HOST_a1, HOST_a2);	// add a1, a1, a2
				*(System::UInt16*)(pos+2)=B_FWD(10);						// b after_call (pc+10)
				break;
			case t_ORb:
			case t_ORw:
			case t_ORd:
				*(System::UInt16*)pos=ORR(HOST_a1, HOST_a2);				// orr a1, a2
				*(System::UInt16*)(pos+2)=B_FWD(10);						// b after_call (pc+10)
				break;
			case t_ANDb:
			case t_ANDw:
			case t_ANDd:
				*(System::UInt16*)pos=AND(HOST_a1, HOST_a2);				// and a1, a2
				*(System::UInt16*)(pos+2)=B_FWD(10);						// b after_call (pc+10)
				break;
			case t_SUBb:
			case t_SUBw:
			case t_SUBd:
				*(System::UInt16*)pos=SUB_REG(HOST_a1, HOST_a1, HOST_a2);	// sub a1, a1, a2
				*(System::UInt16*)(pos+2)=B_FWD(10);						// b after_call (pc+10)
				break;
			case t_XORb:
			case t_XORw:
			case t_XORd:
				*(System::UInt16*)pos=EOR(HOST_a1, HOST_a2);				// eor a1, a2
				*(System::UInt16*)(pos+2)=B_FWD(10);						// b after_call (pc+10)
				break;
			case t_CMPb:
			case t_CMPw:
			...
			case t_TESTw:
			case t_TESTd:
				*(System::UInt16*)pos=B_FWD(12);							// b after_call (pc+12)
				break;
			case t_INCb:
			case t_INCw:
			case t_INCd:
				*(System::UInt16*)pos=ADD_IMM3(HOST_a1, HOST_a1, 1);		// add a1, a1, #1
				*(System::UInt16*)(pos+2)=B_FWD(10);						// b after_call (pc+10)
				break;
			case t_DECb:
			case t_DECw:
			case t_DECd:
				*(System::UInt16*)pos=SUB_IMM3(HOST_a1, HOST_a1, 1);		// sub a1, a1, #1
				*(System::UInt16*)(pos+2)=B_FWD(10);						// b after_call (pc+10)
				break;
			case t_SHLb:
			case t_SHLw:
			case t_SHLd:
				*(System::UInt16*)pos=LSL_REG(HOST_a1, HOST_a2);			// lsl a1, a2
				*(System::UInt16*)(pos+2)=B_FWD(10);						// b after_call (pc+10)
				break;
			case t_SHRb:
				*(System::UInt16*)pos=LSL_IMM(HOST_a1, HOST_a1, 24);		// lsl a1, a1, #24
				*(System::UInt16*)(pos+2)=LSR_IMM(HOST_a1, HOST_a1, 24);	// lsr a1, a1, #24
				*(System::UInt16*)(pos+4)=LSR_REG(HOST_a1, HOST_a2);		// lsr a1, a2
				*(System::UInt16*)(pos+6)=B_FWD(6);							// b after_call (pc+6)
				break;
			case t_SHRw:
				*(System::UInt16*)pos=LSL_IMM(HOST_a1, HOST_a1, 16);		// lsl a1, a1, #16
				*(System::UInt16*)(pos+2)=LSR_IMM(HOST_a1, HOST_a1, 16);	// lsr a1, a1, #16
				*(System::UInt16*)(pos+4)=LSR_REG(HOST_a1, HOST_a2);		// lsr a1, a2
				*(System::UInt16*)(pos+6)=B_FWD(6);							// b after_call (pc+6)
				break;
			case t_SHRd:
				*(System::UInt16*)pos=LSR_REG(HOST_a1, HOST_a2);			// lsr a1, a2
				*(System::UInt16*)(pos+2)=B_FWD(10);						// b after_call (pc+10)
				break;
			case t_SARb:
				*(System::UInt16*)pos=LSL_IMM(HOST_a1, HOST_a1, 24);		// lsl a1, a1, #24
				*(System::UInt16*)(pos+2)=ASR_IMM(HOST_a1, HOST_a1, 24);	// asr a1, a1, #24
				*(System::UInt16*)(pos+4)=ASR_REG(HOST_a1, HOST_a2);		// asr a1, a2
				*(System::UInt16*)(pos+6)=B_FWD(6);							// b after_call (pc+6)
				break;
			case t_SARw:
				*(System::UInt16*)pos=LSL_IMM(HOST_a1, HOST_a1, 16);		// lsl a1, a1, #16
				*(System::UInt16*)(pos+2)=ASR_IMM(HOST_a1, HOST_a1, 16);	// asr a1, a1, #16
				*(System::UInt16*)(pos+4)=ASR_REG(HOST_a1, HOST_a2);		// asr a1, a2
				*(System::UInt16*)(pos+6)=B_FWD(6);							// b after_call (pc+6)
				break;
			case t_SARd:
				*(System::UInt16*)pos=ASR_REG(HOST_a1, HOST_a2);			// asr a1, a2
				*(System::UInt16*)(pos+2)=B_FWD(10);						// b after_call (pc+10)
				break;
			case t_RORb:
				*(System::UInt16*)pos=LSL_IMM(HOST_a1, HOST_a1, 24);		// lsl a1, a1, #24
				*(System::UInt16*)(pos+2)=LSR_IMM(templo1, HOST_a1, 8);		// lsr templo1, a1, #8
				*(System::UInt16*)(pos+4)=ORR(HOST_a1, templo1);			// orr a1, templo1
				*(System::UInt16*)(pos+6)=NOP;								// nop
				*(System::UInt16*)(pos+8)=LSR_IMM(templo1, HOST_a1, 16);	// lsr templo1, a1, #16
				*(System::UInt16*)(pos+10)=NOP;								// nop
				*(System::UInt16*)(pos+12)=ORR(HOST_a1, templo1);			// orr a1, templo1
				*(System::UInt16*)(pos+14)=ROR_REG(HOST_a1, HOST_a2);		// ror a1, a2
				break;
			case t_RORw:
				*(System::UInt16*)pos=LSL_IMM(HOST_a1, HOST_a1, 16);		// lsl a1, a1, #16
				*(System::UInt16*)(pos+2)=LSR_IMM(templo1, HOST_a1, 16);	// lsr templo1, a1, #16
				*(System::UInt16*)(pos+4)=ORR(HOST_a1, templo1);			// orr a1, templo1
				*(System::UInt16*)(pos+6)=ROR_REG(HOST_a1, HOST_a2);		// ror a1, a2
				*(System::UInt16*)(pos+8)=B_FWD(4);							// b after_call (pc+4)
				break;
			case t_RORd:
				*(System::UInt16*)pos=ROR_REG(HOST_a1, HOST_a2);			// ror a1, a2
				*(System::UInt16*)(pos+2)=B_FWD(10);						// b after_call (pc+10)
				break;
			case t_ROLb:
				*(System::UInt16*)pos=LSL_IMM(HOST_a1, HOST_a1, 24);		// lsl a1, a1, #24
				*(System::UInt16*)(pos+2)=NEG(HOST_a2, HOST_a2);			// neg a2, a2
				*(System::UInt16*)(pos+4)=LSR_IMM(templo1, HOST_a1, 8);		// lsr templo1, a1, #8
				*(System::UInt16*)(pos+6)=ADD_IMM8(HOST_a2, 32);			// add a2, #32
				*(System::UInt16*)(pos+8)=ORR(HOST_a1, templo1);			// orr a1, templo1
				*(System::UInt16*)(pos+10)=LSR_IMM(templo1, HOST_a1, 16);	// lsr templo1, a1, #16
				*(System::UInt16*)(pos+12)=ORR(HOST_a1, templo1);			// orr a1, templo1
				*(System::UInt16*)(pos+14)=ROR_REG(HOST_a1, HOST_a2);		// ror a1, a2
				break;
			case t_ROLw:
				*(System::UInt16*)pos=LSL_IMM(HOST_a1, HOST_a1, 16);		// lsl a1, a1, #16
				*(System::UInt16*)(pos+2)=NEG(HOST_a2, HOST_a2);			// neg a2, a2
				*(System::UInt16*)(pos+4)=LSR_IMM(templo1, HOST_a1, 16);	// lsr templo1, a1, #16
				*(System::UInt16*)(pos+6)=ADD_IMM8(HOST_a2, 32);			// add a2, #32
				*(System::UInt16*)(pos+8)=ORR(HOST_a1, templo1);			// orr a1, templo1
				*(System::UInt16*)(pos+10)=NOP;								// nop
				*(System::UInt16*)(pos+12)=ROR_REG(HOST_a1, HOST_a2);		// ror a1, a2
				*(System::UInt16*)(pos+14)=NOP;								// nop
				break;
			case t_ROLd:
				*(System::UInt16*)pos=NEG(HOST_a2, HOST_a2);				// neg a2, a2
				*(System::UInt16*)(pos+2)=ADD_IMM8(HOST_a2, 32);			// add a2, #32
				*(System::UInt16*)(pos+4)=ROR_REG(HOST_a1, HOST_a2);		// ror a1, a2
				*(System::UInt16*)(pos+6)=B_FWD(6);							// b after_call (pc+6)
				break;
			case t_NEGb:
			case t_NEGw:
			case t_NEGd:
				*(System::UInt16*)pos=NEG(HOST_a1, HOST_a1);				// neg a1, a1
				*(System::UInt16*)(pos+2)=B_FWD(10);						// b after_call (pc+10)
				break;
			default:
				*(System::UInt32*)( ( ((System::UInt32) (*pos)) << 2 ) + ((System::UInt32)pos + 4) ) = (System::UInt32)fct_ptr;		// simple_func
				break;
		}
	}
	else
	{
		...
		switch (flags_type) {
			...
			case t_ADDd:
				*(System::UInt16*)pos=ADD_REG(HOST_a1, HOST_a1, HOST_a2);	// add a1, a1, a2
				*(System::UInt16*)(pos+2)=B_FWD(12);						// b after_call (pc+12)
				break;
			case t_ORb:
			case t_ORw:
			case t_ORd:
			...
			case t_TESTw:
			case t_TESTd:
				*(System::UInt16*)pos=B_FWD(14);							// b after_call (pc+14)
				break;
			case t_INCb:
			case t_INCw:
			case t_INCd:
			...
			case t_SHRb:
				...
				*(System::UInt16*)(pos+2)=LSR_IMM(HOST_a1, HOST_a1, 24);	// lsr a1, a1, #24
				*(System::UInt16*)(pos+4)=LSR_REG(HOST_a1, HOST_a2);		// lsr a1, a2
				*(System::UInt16*)(pos+6)=B_FWD(8);							// b after_call (pc+8)
				break;
			case t_SHRw:
				*(System::UInt16*)pos=LSL_IMM(HOST_a1, HOST_a1, 16);		// lsl a1, a1, #16
				*(System::UInt16*)(pos+2)=LSR_IMM(HOST_a1, HOST_a1, 16);	// lsr a1, a1, #16
			...
			case t_RORb:
				...
				*(System::UInt16*)(pos+12)=ORR(HOST_a1, templo1);			// orr a1, templo1
				*(System::UInt16*)(pos+14)=NOP;								// nop
				*(System::UInt16*)(pos+16)=ROR_REG(HOST_a1, HOST_a2);		// ror a1, a2
				break;
			case t_RORw:
				*(System::UInt16*)pos=LSL_IMM(HOST_a1, HOST_a1, 16);		// lsl a1, a1, #16
				*(System::UInt16*)(pos+2)=LSR_IMM(templo1, HOST_a1, 16);	// lsr templo1, a1, #16
				*(System::UInt16*)(pos+4)=ORR(HOST_a1, templo1);			// orr a1, templo1
				*(System::UInt16*)(pos+6)=ROR_REG(HOST_a1, HOST_a2);		// ror a1, a2
				*(System::UInt16*)(pos+8)=B_FWD(6);							// b after_call (pc+6)
				break;
			case t_RORd:
				*(System::UInt16*)pos=ROR_REG(HOST_a1, HOST_a2);			// ror a1, a2
				*(System::UInt16*)(pos+2)=B_FWD(12);						// b after_call (pc+12)
			...
			case t_NEGd:
				...
				*(System::UInt16*)(pos+2)=B_FWD(12);						// b after_call (pc+12)
				break;
			default:
				*(System::UInt32*)( ( ((System::UInt32) (*pos)) << 2 ) + ((System::UInt32)pos + 2) ) = (System::UInt32)fct_ptr;		// simple_func
				break;
		}
...
// 16bit moves may destroy the upper 16bit of the destination register
static void gen_mov_seg16_to_reg(HostReg dest_reg,unsigned index) {
	cache_checkinstr(4);
...
static void gen_mov_seg32_to_reg(HostReg dest_reg,unsigned index) {
	cache_checkinstr(4);
	cache_addw( MOV_LO_HI(templo1, FC_SEGS_ADDR) );      // mov templo1, FC_SEGS_ADDR
...
static void gen_add_seg32_to_reg(HostReg reg,unsigned index) {
	cache_checkinstr(6);
	cache_addw( MOV_LO_HI(templo1, FC_SEGS_ADDR) );      // mov templo1, FC_SEGS_ADDR
...
// 16bit moves may destroy the upper 16bit of the destination register
static void gen_mov_regval16_to_reg(HostReg dest_reg,unsigned index) {
	cache_checkinstr(4);
...
static void gen_mov_regval32_to_reg(HostReg dest_reg,unsigned index) {
	cache_checkinstr(4);
	cache_addw( MOV_LO_HI(templo2, FC_REGS_ADDR) );      // mov templo2, FC_REGS_ADDR
...
// 16bit moves may destroy the upper 16bit of the destination register
static void gen_mov_regword_to_reg(HostReg dest_reg,unsigned index,bool dword) {
	cache_checkinstr(4);
	cache_addw( MOV_LO_HI(templo2, FC_REGS_ADDR) );      // mov templo2, FC_REGS_ADDR
	if (dword) {
		cache_addw( LDR_IMM(dest_reg, templo2, index) );      // ldr dest_reg, [templo2, #index]
...
// this function does not use FC_OP1/FC_OP2 as dest_reg as these
// registers might not be directly byte-accessible on some architectures
static void gen_mov_regbyte_to_reg_low(HostReg dest_reg,unsigned index) {
	cache_checkinstr(4);
	cache_addw( MOV_LO_HI(templo2, FC_REGS_ADDR) );      // mov templo2, FC_REGS_ADDR
...
// this function can use FC_OP1/FC_OP2 as dest_reg which are
// not directly byte-accessible on some architectures
static void INLINE gen_mov_regbyte_to_reg_low_canuseword(HostReg dest_reg,unsigned index) {
	cache_checkinstr(4);
	cache_addw( MOV_LO_HI(templo2, FC_REGS_ADDR) );      // mov templo2, FC_REGS_ADDR
	cache_addw( LDRB_IMM(dest_reg, templo2, index) );      // ldrb dest_reg, [templo2, #index]
}
...
static void gen_add_regval32_to_reg(HostReg reg,unsigned index) {
	cache_checkinstr(6);
	cache_addw( MOV_LO_HI(templo2, FC_REGS_ADDR) );      // mov templo2, FC_REGS_ADDR
...
static void gen_mov_regval16_from_reg(HostReg src_reg,unsigned index) {
	cache_checkinstr(4);
...
static void gen_mov_regval32_from_reg(HostReg src_reg,unsigned index) {
	cache_checkinstr(4);
	cache_addw( MOV_LO_HI(templo1, FC_REGS_ADDR) );      // mov templo1, FC_REGS_ADDR
...
static void gen_mov_regword_from_reg(HostReg src_reg,unsigned index,bool dword) {
	cache_checkinstr(4);
	cache_addw( MOV_LO_HI(templo1, FC_REGS_ADDR) );      // mov templo1, FC_REGS_ADDR
	if (dword) {
		cache_addw( STR_IMM(src_reg, templo1, index) );      // str src_reg, [templo1, #index]
...
static void gen_mov_regbyte_from_reg_low(HostReg src_reg,unsigned index) {
	cache_checkinstr(4);
	cache_addw( MOV_LO_HI(templo1, FC_REGS_ADDR) );      // mov templo1, FC_REGS_ADDR
