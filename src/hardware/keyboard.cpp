...
#define KEYDELAY 0.300f			//Considering 20-30 khz serial clock and 11 int/char
...
static void KEYBOARD_SetPort60(System::Byte val) {
...
static void KEYBOARD_TransferBuffer(unsigned val) {
...
static void KEYBOARD_AddBuffer(System::Byte data) {
	if (keyb.used>=KEYBUFSIZE) {
		LOG(LOG_KEYBOARD,LOG_NORMAL)("Buffer full, dropping code");
		return;
	}
	unsigned start=keyb.pos+keyb.used;
...
static unsigned read_p60(unsigned port,unsigned iolen) {
...
static void write_p60(unsigned port,unsigned val,unsigned iolen) {
	switch (keyb.command) {
	case CMD_NONE:	/* None */
...
static System::Byte port_61_data = 0;
static unsigned read_p61(unsigned port,unsigned iolen) {
	port_61_data^=0x20;
	port_61_data^=0x10;
	return port_61_data;
}
...
static void write_p61(unsigned port,unsigned val,unsigned iolen) {
...
static void write_p64(unsigned port,unsigned val,unsigned iolen) {
	switch (val) {
...
static unsigned read_p64(unsigned port,unsigned iolen) {
	System::Byte status= 0x1c | (keyb.p60changed? 0x1 : 0x0);
	return status;
}

void KEYBOARD_AddKey(KBD_KEYS keytype,bool pressed) {
	System::Byte ret=0;bool extend=false;
	switch (keytype) {
