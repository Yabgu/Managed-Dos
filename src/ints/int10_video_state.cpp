...
bool INT10_VideoState_Save(unsigned state,RealPt buffer) {
	...
	unsigned base_seg=RealSeg(buffer);
	unsigned base_dest=RealOff(buffer)+0x20;

	if (state&1)  {
		real_writew(base_seg,RealOff(buffer),base_dest);

		System::UInt16 crt_reg=real_readw(BIOSMEM_SEG,BIOSMEM_CRTC_ADDRESS);
		...
		// save some registers
		IO_WriteB(0x3c4,2);
		System::Byte crtc_2=IO_ReadB(0x3c5);
		IO_WriteB(0x3c4,4);
		System::Byte crtc_4=IO_ReadB(0x3c5);
		IO_WriteB(0x3ce,6);
		System::Byte gfx_6=IO_ReadB(0x3cf);
		IO_WriteB(0x3ce,5);
		System::Byte gfx_5=IO_ReadB(0x3cf);
		IO_WriteB(0x3ce,4);
		System::Byte gfx_4=IO_ReadB(0x3cf);
	...
	if (state&4)  {
		...
		unsigned dac_state=IO_ReadB(0x3c7)&1;
		unsigned dac_windex=IO_ReadB(0x3c8);
...
bool INT10_VideoState_Restore(unsigned state,RealPt buffer) {
	...
	System::UInt16 base_seg=RealSeg(buffer);
	System::UInt16 base_dest;

	if (state&1)  {
		base_dest=real_readw(base_seg,RealOff(buffer));
		System::UInt16 crt_reg=real_readw(base_seg,base_dest+0x40);
	...
	if (state&4)  {
		...
		unsigned dac_state=real_readb(base_seg,base_dest+0x000);
		if (dac_state==0) {
	...
	if ((svgaCard==SVGA_S3Trio) && (state&8))  {
		...
		unsigned seq_idx=IO_ReadB(0x3c4);
		IO_WriteB(0x3c4,0x08);
//		unsigned seq_8=IO_ReadB(0x3c5);
		IO_ReadB(0x3c5);
//		real_writeb(base_seg,base_dest+0x00,IO_ReadB(0x3c5));
...
//		unsigned crtc_idx=IO_ReadB(0x3d4);
...
/*		IO_WriteB(crt_reg,0x40);
		unsigned sysval1=IO_ReadB(crt_reg+1);
		IO_WriteB(crt_reg+1,sysval|1);
		IO_WriteB(crt_reg,0x53);
		unsigned sysva2=IO_ReadB(crt_reg+1);
