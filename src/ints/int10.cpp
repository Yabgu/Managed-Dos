...
//		reg_ah=0;
	...
	case 0x05:								/* Set Active Page */
		if ((reg_al & 0x80) && IS_TANDY_ARCH) {
			System::Byte crtcpu=real_readb(BIOSMEM_SEG, BIOSMEM_CRTCPU_PAGE);		
			switch (reg_al) {
			case 0x80:
				reg_bh=crtcpu & 7;
				reg_bl=(crtcpu >> 3) & 0x7;
	...
	case 0x0F:								/* Get videomode */
		...
		reg_ah=(System::Byte)real_readw(BIOSMEM_SEG,BIOSMEM_NB_COLS);
		break;					
...
/* General */
	...
	case 0x12:								/* alternate function select */
		...
		case 0x30:							/* Select vertical resolution */
			{   
				...
				System::Byte video_switches = real_readb(BIOSMEM_SEG,BIOSMEM_SWITCHES)&0xf0;
				switch(reg_al) {
				case 0: // 200
					modeset_ctl &= 0xef;
					modeset_ctl |= 0x80;
		...
		case 0x31:							/* Palette loading on modeset */
			{   
				...
				System::Byte temp = real_readb(BIOSMEM_SEG,BIOSMEM_MODESET_CTL) & 0xf7;
		...
		case 0x33: /* SWITCH GRAY-SCALE SUMMING */
			{   
				...
				System::Byte temp = real_readb(BIOSMEM_SEG,BIOSMEM_MODESET_CTL) & 0xfd;
		...
		case 0x34: /* ALTERNATE FUNCTION SELECT (VGA) - CURSOR EMULATION */
			{   
				...
				System::Byte temp = real_readb(BIOSMEM_SEG,BIOSMEM_VIDEO_CTL) & 0xfe;
		...
		case 0x36: {						/* VGA Refresh control */
			...
			IO_Write(0x3c4,0x1);
			System::Byte clocking = IO_Read(0x3c5);
	...
	case 0x1A:								/* Display Combination */
		...
		if (reg_al==0) {	// get dcc
			...
			if (svstable) {
				...
				if (entries) {
					unsigned ct;
					System::UInt16 swpidx=reg_bh|(reg_bl<<8);
					// search the ddc index in the dcc table
					for (ct=0; ct<entries; ct++) {
						System::UInt16 dccentry=real_readw(RealSeg(dcctable),RealOff(dcctable)+0x04+ct*2);
						if ((dccentry==reg_bx) || (dccentry==swpidx)) {
							newidx=(System::Byte)ct;
							break;
						}
					}
				}
	...
	case 0x1C:	/* Video Save Area */
		...
		switch (reg_al) {
			case 0: {
				unsigned ret=INT10_VideoState_GetSize(reg_cx);
				if (ret) {
					reg_al=0x1c;
					reg_bx=(System::UInt16)ret;
				} else reg_al=0;
				}
				break;
			case 1:
...
static void SetupTandyBios(void) {
	static System::Byte TandyConfig[130]= {
