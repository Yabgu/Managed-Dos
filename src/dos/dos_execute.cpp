...
#endif
struct EXE_Header {
	System::UInt16 signature;					/* EXE Signature MZ or ZM */
	System::UInt16 extrabytes;					/* Bytes on the last page */
	System::UInt16 pages;						/* Pages in file */
	System::UInt16 relocations;					/* Relocations in file */
	System::UInt16 headersize;					/* Paragraphs in header */
	System::UInt16 minmemory;					/* Minimum amount of memory */
	System::UInt16 maxmemory;					/* Maximum amount of memory */
	System::UInt16 initSS;
	System::UInt16 initSP;
	System::UInt16 checksum;
	System::UInt16 initIP;
	System::UInt16 initCS;
	System::UInt16 reloctable;
	System::UInt16 overlay;
...
void DOS_Terminate(System::UInt16 pspseg,bool tsr,System::Byte exitcode) {
	...
	dos.return_mode=(tsr)?(System::Byte)RETURN_TSR:(System::Byte)RETURN_EXIT;
...
static bool MakeEnv(char * name,System::UInt16 * segment) {
	...
	System::UInt16 size=long2para(envsize+ENV_KEEPFREE);
...
//		mem_memcpy(envwrite,envread,envsize);
	...
	if (DOS_Canonicalize(name,namebuf)) {
		MEM_BlockWrite(envwrite,namebuf,(unsigned)(strlen(namebuf)+1));
		return true;
	} else return false;
}

bool DOS_NewPSP(System::UInt16 segment, System::UInt16 size) {
	DOS_PSP psp(segment);
	psp.MakeNew(size);
	System::UInt16 parent_psp_seg=psp.GetParent();
...
bool DOS_ChildPSP(System::UInt16 segment, System::UInt16 size) {
	DOS_PSP psp(segment);
	psp.MakeNew(size);
	System::UInt16 parent_psp_seg = psp.GetParent();
	DOS_PSP psp_parent(parent_psp_seg);
...
static void SetupPSP(System::UInt16 pspseg,System::UInt16 memsize,System::UInt16 envseg) {
	/* Fix the PSP for psp and environment MCB's */
	DOS_MCB mcb((System::UInt16)(pspseg-1));
	mcb.SetPSPSeg(pspseg);
	mcb.SetPt((System::UInt16)(envseg-1));
	mcb.SetPSPSeg(pspseg);
...
static void SetupCMDLine(System::UInt16 pspseg,DOS_ParamBlock & block) {
	DOS_PSP psp(pspseg);
	// if cmdtail==0 it will inited as empty in SetCommandTail
	psp.SetCommandTail(block.exec.cmdtail);
}

bool DOS_Execute(char * name,PhysPt block_pt,System::Byte flags) {
	EXE_Header head;unsigned i;
	System::UInt16 fhandle;System::UInt16 len;System::UInt32 pos;
	System::UInt16 pspseg,envseg,loadseg,memsize,readsize;
	PhysPt loadaddress;RealPt relocpt;
	unsigned headersize=0,imagesize=0;
...
//		E_Exit("DOS:Not supported execute mode %d for file %s",flags,name);
	...
	if (!DOS_ReadFile(fhandle,(System::Byte *)&head,&len)) {
	...
	if (len<sizeof(EXE_Header)) {
		...
		for (i=0;i<sizeof(EXE_Header)/2;i++) {
			*((System::UInt16 *)endian)=host_readw(endian);
			endian+=2;
		}
	...
	System::Byte * loadbuf=(System::Byte *)new System::Byte[0x10000];
	if (flags!=OVERLAY) {
		...
		/* Get Memory */		
		System::UInt16 minsize,maxsize;System::UInt16 maxfree=0xffff;DOS_AllocateMemory(&pspseg,&maxfree);
		if (iscom) {
			minsize=0x1000;maxsize=0xffff;
			if (machine==MCH_PCJR) {
				/* try to load file into memory below 96k */ 
				pos=0;DOS_SeekFile(fhandle,&pos,DOS_SEEK_SET);	
				System::UInt16 dataread=0x1800;
		...
		if (iscom && (machine==MCH_PCJR) && (pspseg<0x2000)) {
			...
			if ((real_readb(0x2000,0)==0x5a) && (real_readw(0x2000,1)==0) && (real_readw(0x2000,3)==0x7ffe)) {
				...
				if (pspseg+maxsize==0x17ff) {
					DOS_MCB cmcb((System::UInt16)(pspseg-1));
		...
		if (!iscom) {
			...
			if ((head.minmemory == 0) && (head.maxmemory == 0))
				loadseg = (System::UInt16)(((pspseg+memsize)*0x10-imagesize)/0x10);
		}
...
//			if (readsize!=0x8000) LOG(LOG_EXEC,LOG_NORMAL)("Illegal header");
		...
		if (imagesize>0) {
			readsize=(System::UInt16)imagesize;DOS_ReadFile(fhandle,loadbuf,&readsize);
			MEM_BlockWrite(loadaddress,loadbuf,readsize);
//			if (readsize!=imagesize) LOG(LOG_EXEC,LOG_NORMAL)("Illegal header");
		}
		/* Relocate the exe image */
		System::UInt16 relocate;
		...
		for (i=0;i<head.relocations;i++) {
			readsize=4;DOS_ReadFile(fhandle,(System::Byte *)&relocpt,&readsize);
	...
	if (flags==LOADNGO) {
		...
		char stripname[8]= { 0 };unsigned index=0;
		while (char chr=*name++) {
			switch (chr) {
