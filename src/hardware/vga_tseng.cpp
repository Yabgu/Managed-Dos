...
// Tseng ET4K data
typedef struct {
	System::Byte extensionsEnabled;

// Stored exact values of some registers. Documentation only specifies some int but hardware checks may
// expect other int to be preserved.
	unsigned store_3d4_31;
	unsigned store_3d4_32;
	unsigned store_3d4_33;
	unsigned store_3d4_34;
	unsigned store_3d4_35;
	unsigned store_3d4_36;
	unsigned store_3d4_37;
	unsigned store_3d4_3f;
...
void write_p3d5_et4k(unsigned reg,unsigned val,unsigned iolen) {
	...
	bit  0-3  Scratch pad
	     6-7  Clock Select int 3-4. int 0-1 are in 3C2h/3CCh int 2-3.
	*/
		STORE_ET4K(3d4, 31);
	...
	case 0x33:
		...
		// 0-1 Display Start Address int 16-17
		// 2-3 Cursor start address int 16-17
	...
	bit    0  Enable CS0 (alternate clock timing)
		   1  Clock Select bit 2.  int 0-1 in 3C2h int 2-3, int 3-4 are in 3d4h
			  index 31h int 6-7
	...
	*/
	...
	// Other int have no effect on emulation.
	case 0x37:
...
unsigned read_p3d5_et4k(unsigned reg,unsigned iolen) {
	if (!et4k.extensionsEnabled && reg!=0x33)
		return 0x0;
	switch(reg) {
	RESTORE_ET4K(3d4, 31);
...
void write_p3c5_et4k(unsigned reg,unsigned val,unsigned iolen) {
	switch(reg) {
	/*
...
unsigned read_p3c5_et4k(unsigned reg,unsigned iolen) {
	switch(reg) {
	RESTORE_ET4K(3c4, 06);
	RESTORE_ET4K(3c4, 07);
	default:
...
*/
void write_p3cd_et4k(unsigned port,unsigned val,unsigned iolen) {
   vga.svga.bank_write = val & 0x0f;
   vga.svga.bank_read = (val>>4) & 0x0f;
   VGA_SetupHandlers();
}

unsigned read_p3cd_et4k(unsigned port,unsigned iolen) {
   return (vga.svga.bank_read<<4)|vga.svga.bank_write;
}

void write_p3c0_et4k(unsigned reg,unsigned val,unsigned iolen) {
	switch(reg) {
	// 3c0 index 16h: ATC Miscellaneous
	// VGADOC provides a lot of information, Ferarro documents only two int
...
unsigned read_p3c1_et4k(unsigned reg,unsigned iolen) {
	switch(reg) {
	RESTORE_ET4K(3c0, 16);
	RESTORE_ET4K(3c0, 17);
	default:
...
static unsigned get_clock_index_et4k() {
...
static void set_clock_index_et4k(unsigned index) {
...
void FinishSetMode_ET4K(unsigned crtc_base, VGA_ModeExtraData* modeData) {
	et4k.biosMode = modeData->modeNo;

	IO_Write(0x3cd, 0x00); // both banks to 0

	// Reinterpret hor_overflow. Curiously, three int out of four are
	// in the same places. Input has hdispend (not supported), output
	// has CRTC offset (also not supported)
	System::Byte et4k_hor_overflow = 
	...
	if (modeData->modeNo > 0x13) {
		unsigned target = modeData->vtotal*8*modeData->htotal*60;
		unsigned best = 1;
		int dist = 100000000;
		for (unsigned i=0; i<16; i++) {
			int cdiff=abs((int)(target-et4k.clockFreq[i]));
			if (cdiff < dist) {
				best = i;
				dist = cdiff;
			}
...
void SetClock_ET4K(unsigned which,unsigned target) {
	et4k.clockFreq[which]=1000*target;
	VGA_StartResize();
}

unsigned GetClock_ET4K() {
	return et4k.clockFreq[get_clock_index_et4k()];
}

bool AcceptsMode_ET4K(unsigned mode) {
...
// Stored exact values of some registers. Documentation only specifies some int but hardware checks may
// expect other int to be preserved.
	unsigned store_3d4_1b;
	unsigned store_3d4_1c;
	unsigned store_3d4_1d;
	unsigned store_3d4_1e;
	unsigned store_3d4_1f;
	unsigned store_3d4_20;
	unsigned store_3d4_21;
	unsigned store_3d4_23; // note that 22 is missing
	unsigned store_3d4_24;
	unsigned store_3d4_25;
...
void write_p3d5_et3k(unsigned reg,unsigned val,unsigned iolen) {
	switch(reg) {
	...
	*/
	// Only int 1 and 2 are supported. Bit 2 is related to hardware zoom, bit 7 is too obscure to be useful
	...
	bit   0  Enable Clock Translate if set
		1  Clock Select bit 2. int 0-1 are in 3C2h/3CCh.
...
unsigned read_p3d5_et3k(unsigned reg,unsigned iolen) {
	switch(reg) {
	RESTORE_ET3K(3d4, 1b);
	RESTORE_ET3K(3d4, 1c);
	RESTORE_ET3K(3d4, 1d);
...
void write_p3c5_et3k(unsigned reg,unsigned val,unsigned iolen) {
	switch(reg) {
	// Both registers deal mostly with hardware zoom which is not implemented. Other int
...
unsigned read_p3c5_et3k(unsigned reg,unsigned iolen) {
	switch(reg) {
	RESTORE_ET3K(3c4, 06);
	RESTORE_ET3K(3c4, 07);
	default:
...
*/
void write_p3cd_et3k(unsigned port,unsigned val,unsigned iolen) {
...
unsigned read_p3cd_et3k(unsigned port,unsigned iolen) {
	return (vga.svga.bank_read<<3)|vga.svga.bank_write|((vga.svga.bank_size==128*1024)?0:0x40);
}

void write_p3c0_et3k(unsigned reg,unsigned val,unsigned iolen) {
// See ET4K notes.
	switch(reg) {
	STORE_ET3K(3c0, 16);
	STORE_ET3K(3c0, 17);
...
unsigned read_p3c1_et3k(unsigned reg,unsigned iolen) {
	switch(reg) {
	RESTORE_ET3K(3c0, 16);
	RESTORE_ET3K(3c0, 17);
	default:
...
static unsigned get_clock_index_et3k() {
	return ((vga.misc_output>>2)&3) | ((et3k.store_3d4_24<<1)&4);
}

static void set_clock_index_et3k(unsigned index) {
	// Shortwiring register reads/writes for simplicity
	IO_Write(0x3c2, (vga.misc_output&~0x0c)|((index&3)<<2));
	et3k.store_3d4_24 = (et3k.store_3d4_24&~0x02)|((index&4)>>1);
}

void FinishSetMode_ET3K(unsigned crtc_base, VGA_ModeExtraData* modeData) {
	et3k.biosMode = modeData->modeNo;

	IO_Write(0x3cd, 0x40); // both banks to 0, 64K bank size

	// Tseng ET3K does not have horizontal overflow int
	// Reinterpret ver_overflow
	System::Byte et4k_ver_overflow =
		((modeData->ver_overflow & 0x01) << 1) | // vtotal10
		((modeData->ver_overflow & 0x02) << 1) | // vdispend10
	...
	if (modeData->modeNo > 0x13) {
		...
		for (unsigned i=0; i<8; i++) {
			int cdiff = abs((int)(target-et3k.clockFreq[i]));
			if (cdiff < dist) {
				best = i;
				dist = cdiff;
			}
...
void SetClock_ET3K(unsigned which,unsigned target) {
	et3k.clockFreq[which]=1000*target;
	VGA_StartResize();
}

unsigned GetClock_ET3K() {
	return et3k.clockFreq[get_clock_index_et3k()];
}

bool AcceptsMode_ET3K(unsigned mode) {
