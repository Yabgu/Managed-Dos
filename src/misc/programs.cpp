...
unsigned call_program;

/* This registers a file on the virtual drive and creates the correct structure for it*/

static System::Byte exe_block[] = {
...
void PROGRAMS_MakeFile(char const * const name, PROGRAMS_Main * main) {
	System::Byte * comdata = (System::Byte *)malloc(32); //MEM LEAK
	memcpy(comdata, &exe_block, sizeof(exe_block));
	comdata[CB_POS] = (System::Byte)(call_program & 0xff);
	comdata[CB_POS + 1] = (System::Byte)((call_program >> 8) & 0xff);
	...
	System::Byte index = (System::Byte)internal_progs.size();
	internal_progs.push_back(main);

	memcpy(&comdata[sizeof(exe_block)], &index, sizeof(index));
	System::UInt32 size = sizeof(exe_block) + sizeof(index);
	VFILE_Register(name, comdata, size);
}



static unsigned PROGRAMS_Handler(void) {
	/* This sets up everything for a program start up call */
	unsigned size = sizeof(System::Byte);
	System::Byte index;
...
void Program::WriteOut_NoParsing(const char * format) {
	...
	//	DOS_WriteFile(STDOUT,(System::Byte *)format,&size);
}
...
bool Program::GetEnvNum(unsigned num, std::string & result) {
	char env_string[1024 + 1];
	PhysPt env_read = PhysMake(psp->GetEnvironment(), 0);
	do {
		MEM_StrCopy(env_read, env_string, 1024);
...
unsigned Program::GetEnvCount(void) {
	PhysPt env_read = PhysMake(psp->GetEnvironment(), 0);
	unsigned num = 0;
...
bool Program::SetEnv(const char * entry, const char * new_string) {
	...
	do {
		...
		MEM_BlockWrite(env_write, env_string, (unsigned)(strlen(env_string) + 1));
