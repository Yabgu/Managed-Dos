...
bool DOS_IOCTL(void) {
	unsigned handle=0;System::Byte drive=0;
	...
	case 0x00:		/* Get Device Information */
		if (Files[handle]->GetInformation() & 0x8000) {	//Check for device
			...
			System::Byte hdrive=Files[handle]->GetDrive();
			if (hdrive==0xff) {
				LOG(LOG_IOCTL,LOG_NORMAL)("00:No drive set");
				hdrive=2;	// defaulting to C:
			}
			/* return drive number in lower 5 int for block devices */
	...
	case 0x01:		/* Set Device Information */
		if (reg_dh != 0) {
			...
			if (Files[handle]->GetInformation() & 0x8000) {	//Check for device
				reg_al=(System::Byte)(Files[handle]->GetInformation() & 0xff);
			} else {
				DOS_SetError(DOSERR_FUNCTION_NUMBER_INVALID);
				return false;
			}
	...
	case 0x06:      /* Get Input Status */
		if (Files[handle]->GetInformation() & 0x8000) {		//Check for device
			...
			System::UInt32 oldlocation=0;
			Files[handle]->Seek(&oldlocation, DOS_SEEK_CUR);
			System::UInt32 endlocation=0;
	...
	case 0x0D:		/* Generic block device request */
		{
			...
			case 0x66:	/* Volume label */
				{			
					...
					if (find_ext) {
						unsigned size=(unsigned)(find_ext-bufin);
						if (size>8) size=8;
...
bool DOS_GetSTDINStatus(void) {
	System::UInt32 handle=RealHandle(STDIN);
