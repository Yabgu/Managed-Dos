...
public:
	...
	void LinkTo(unsigned index,CacheBlockDynRec * toblock) {
		assert(toblock);
	...
	struct {
		System::UInt16 start,end;		// where in the page is the original code
...
static struct {
	...
	System::Byte * pos;		// position in the cache block
...
public:
	...
	CacheBlockDynRec * FindCacheBlock(unsigned start) {
...
private:
	...
	unsigned active_blocks;		// the number of cache blocks in this page
	unsigned active_count;		// delaying parameter to not immediately release a page
	HostPt hostmem;	
	unsigned phys_page;
};
...
static void cache_closeblock(void) {
	...
	unsigned written=(unsigned)(cache.pos-block->cache.start);
...
static INLINE void cache_addb(System::Byte val) {
	*cache.pos++=val;
}

// place a 16bit value into the cache
static INLINE void cache_addw(System::UInt16 val) {
	*(System::UInt16*)cache.pos=val;
	cache.pos+=2;
}

// place a 32bit value into the cache
static INLINE void cache_addd(System::UInt32 val) {
	*(System::UInt32*)cache.pos=val;
	cache.pos+=4;
}

// place a 64bit value into the cache
static INLINE void cache_addq(System::UInt64 val) {
	*(System::UInt64*)cache.pos=val;
	cache.pos+=8;
}
...
static void cache_init(bool enable) {
	...
	if (enable) {
		...
		if (cache_code_start_ptr==NULL) {
			...
			cache_code=(System::Byte*)(((long)cache_code_start_ptr + PAGESIZE_TEMP-1) & ~(PAGESIZE_TEMP-1)); //MEM LEAK. store old pointer if you want to free it.
		...
		core_dynrec.runcode=(BlockReturn (*)(System::Byte*))cache.pos;
//		link_blocks[1].cache.start=cache.pos;
		dyn_run_code();
