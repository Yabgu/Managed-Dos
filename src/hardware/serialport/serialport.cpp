...
bool device_COM::Read(System::Byte * data,System::UInt16 * size) {
	// DTR + RTS on
	sclass->Write_MCR(0x03);
	for (System::UInt16 i=0; i<*size; i++)
	{
		System::Byte status;
...
bool device_COM::Write(System::Byte * data,System::UInt16 * size) {
	// DTR + RTS on
	sclass->Write_MCR(0x03);
	for (System::UInt16 i=0; i<*size; i++)
	{
...
bool device_COM::Seek(System::UInt32 * pos,System::UInt32 type) {
	*pos = 0;
	return true;
}
...
System::UInt16 device_COM::GetInformation(void) {
	return 0x80A0;
}
...
static unsigned SERIAL_Read (unsigned port, unsigned iolen) {
	unsigned i;
	unsigned retval;
	unsigned index = port & 0x7;
	switch(port&0xff8) {
...
static void SERIAL_Write (unsigned port, unsigned val, unsigned) {
	unsigned i;
	unsigned index = port & 0x7;
	switch(port&0xff8) {
		case 0x3f8: i=0; break;
...
/*****************************************************************************/
void CSerial::rise (System::Byte priority) {
#if SERIAL_DEBUG
...
void CSerial::clear (System::Byte priority) {
...
void CSerial::ComputeInterrupts () {
	...
	unsigned val = IER & waiting_interrupts;
...
/* A byte was received                                                      **/
/*****************************************************************************/
void CSerial::receiveByteEx (System::Byte data, System::Byte error) {
#if SERIAL_DEBUG
	...
	if(error) {
		...
		if(FCR&FCR_ACTIVATE) {
			...
			if(!errorfifo->isFull()) {
				...
				System::Byte toperror=errorfifo->getTop();
...
void CSerial::receiveByte (System::Byte data) {
...
/*****************************************************************************/
void CSerial::Write_THR (System::Byte data) {
	// 0-7 transmit data
...
/*****************************************************************************/
unsigned CSerial::Read_RHR () {
	// 0-7 received data
	if ((LCR & LCR_DIVISOR_Enable_MASK)) return baud_divider&0xff;
	else {
		System::Byte data=rxfifo->getb();
		if(FCR&FCR_ACTIVATE) {
			System::Byte error=errorfifo->getb();
...
void CSerial::Write_IER (System::Byte data) {
	if ((LCR & LCR_DIVISOR_Enable_MASK)) {	// write to DLM
		baud_divider&=0xff;
		baud_divider |= ((System::UInt16)data)<<8;
		changeLineProperties();
	} else {
...
void CSerial::Write_FCR (System::Byte data) {
...
// - parity, stopint, word length
// - send break
// - switch between RHR/THR and baud rate registers
// Modified by:
// - writing to it.
unsigned CSerial::Read_LCR () {
	// 0-1	word length
	// 2	stop int
	// 3	parity enable
	// 4-5	parity type
...
void CSerial::Write_LCR (System::Byte data) {
	System::Byte lcr_old = LCR;
	LCR = data;
...
void CSerial::Write_MCR (System::Byte data) {
...
// - loopback
unsigned CSerial::Read_LSR () {
	unsigned retval = LSR & (LSR_ERROR_MASK|LSR_TX_EMPTY_MASK);
	...
	LSR &= (~LSR_ERROR_MASK);			// clear error int on read
	clear (ERROR_PRIORITY);
	return retval;
}

void CSerial::Write_MSR (System::Byte val) {
...
unsigned CSerial::Read_SPR () {
	return SPR;
}

void CSerial::Write_SPR (System::Byte data) {
	SPR = data;
}
...
/* Write_reserved                                                           **/
/*****************************************************************************/
void CSerial::Write_reserved (System::Byte data, System::Byte address) {
...
void CSerial::Init_Registers () {
	...
	System::UInt32 initbps = 9600;
	System::Byte bytesize = 8;
	char parity = 'N';
							  
	System::Byte lcrresult = 0;
	System::UInt16 baudresult = 0;
	...
	// make lcr: byte size, parity, stopint, baudrate

	if (bytesize == 5)
		lcrresult |= LCR_DATAint_5;
	else if (bytesize == 6)
		lcrresult |= LCR_DATAint_6;
	else if (bytesize == 7)
		lcrresult |= LCR_DATAint_7;
	else
		lcrresult |= LCR_DATAint_8;
	...
	// baudrate
	if (initbps > 0)
		baudresult = (System::UInt16) (115200 / initbps);
	else
		baudresult = 12;			// = 9600 baud

	Write_MCR (0);
	Write_LCR (LCR_DIVISOR_Enable_MASK);
	Write_THR ((System::Byte) baudresult & 0xff);
	Write_IER ((System::Byte) (baudresult >> 8));
	Write_LCR (lcrresult);
	updateMSR();
	Read_MSR();
	PIC_DeActivateIRQ(irq);
...
CSerial::CSerial(unsigned id, CommandLine* cmd) {
	idnumber=id;
	System::UInt16 base = serial_baseaddr[id];

	irq = serial_defaultirq[id];
	getunsignedSubstring("irq:",&irq, cmd);
...
bool CSerial::getunsignedSubstring(const char* name,unsigned* data, CommandLine* cmd) {
...
CSerial::~CSerial(void) {
	...
	for(unsigned i = 0; i <= SERIAL_BASE_EVENT_COUNT; i++)
		removeEvent(i);
};
bool CSerial::Getchar(System::Byte* data, System::Byte* lsr, bool wait_dsr, unsigned timeout) {
...
bool CSerial::Putchar(System::Byte data, bool wait_dsr, bool wait_cts, unsigned timeout) {
...
public:
	SERIALPORTS (Section * configuration):Module_base (configuration) {
		System::UInt16 biosParameter[4] = { 0, 0, 0, 0 };
