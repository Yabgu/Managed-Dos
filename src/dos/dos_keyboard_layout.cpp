...
public:
	...
	unsigned read_codepage_file(const char* codepage_file_name, System::Int32 codepage_id);
	System::UInt16 extract_codepage(const char* keyboard_file_name);
	// read in a keyboard layout from a .kl-file
	unsigned read_keyboard_file(const char* keyboard_file_name, System::Int32 req_cp);

	// call layout_key to apply the current language layout
	bool layout_key(unsigned key, System::Byte flags1, System::Byte flags2, System::Byte flags3);

	unsigned switch_keyboard_layout(const char* new_layout, keyboard_layout* &created_layout, System::Int32& tried_cp);
...
private:
	static const System::Byte layout_pages=12;
	System::UInt16 current_layout[(MAX_SCAN_CODE+1)*layout_pages];
	struct {
		System::UInt16 required_flags,forbidden_flags;
		System::UInt16 required_userflags,forbidden_userflags;
	} current_layout_planes[layout_pages-4];
	System::Byte additional_planes,used_lock_modifiers;

	// diacritics table
	System::Byte diacritics[2048];
	System::UInt16 diacritics_entries;
	System::UInt16 diacritics_character;
	System::UInt16 user_keys;
	...
	char** language_codes;
	unsigned language_code_count;

	void reset();
	void read_keyboard_file(System::Int32 specific_layout);
	unsigned read_keyboard_file(const char* keyboard_file_name, System::Int32 specific_layout, System::Int32 requested_codepage);
	bool map_key(unsigned key, System::UInt16 layouted_key, bool is_command, bool is_keypair);
};


keyboard_layout::~keyboard_layout() {
	if (language_codes) {
		for (unsigned i=0; i<language_code_count; i++)
...
void keyboard_layout::reset() {
	for (System::UInt32 i=0; i<(MAX_SCAN_CODE+1)*layout_pages; i++) current_layout[i]=0;
	for (System::UInt32 i=0; i<layout_pages-4; i++) {
...
unsigned keyboard_layout::read_keyboard_file(const char* keyboard_file_name, System::Int32 req_cp) {
	return this->read_keyboard_file(keyboard_file_name, -1, req_cp);
}

// switch to a different layout
void keyboard_layout::read_keyboard_file(System::Int32 specific_layout) {
...
static System::UInt32 read_kcl_file(const char* kcl_file_name, const char* layout_id, bool first_id_only) {
	FILE* tempfile = OpenDosboxFile(kcl_file_name);
	if (tempfile==0) return 0;

	static System::Byte rbuf[8192];

	// check ID-bytes of file
	System::UInt32 dr=(System::UInt32)fread(rbuf, sizeof(System::Byte), 7, tempfile);
	...
	for (;;) {
		System::UInt32 cur_pos=(System::UInt32)(ftell(tempfile));
		dr=(System::UInt32)fread(rbuf, sizeof(System::Byte), 5, tempfile);
		if (dr<5) break;
		System::UInt16 len=host_readw(&rbuf[0]);

		System::Byte data_len=rbuf[2];

		char lng_codes[258];
		fseek(tempfile, -2, SEEK_CUR);
		// get all language codes for this layout
		for (unsigned i=0; i<data_len;) {
			fread(rbuf, sizeof(System::Byte), 2, tempfile);
			System::UInt16 lcnum=host_readw(&rbuf[0]);
			i+=2;
			unsigned lcpos=0;
			for (;i<data_len;) {
				fread(rbuf, sizeof(System::Byte), 1, tempfile);
				i++;
...
static System::UInt32 read_kcl_data(System::Byte * kcl_data, System::UInt32 kcl_data_size, const char* layout_id, bool first_id_only) {
	// check ID-bytes
	if ((kcl_data[0]!=0x4b) || (kcl_data[1]!=0x43) || (kcl_data[2]!=0x46)) {
		return 0;
	}

	System::UInt32 dpos=7+kcl_data[6];

	for (;;) {
		if (dpos+5>kcl_data_size) break;
		System::UInt32 cur_pos=dpos;
		System::UInt16 len=host_readw(&kcl_data[dpos]);
		System::Byte data_len=kcl_data[dpos+2];
		dpos+=5;

		char lng_codes[258];
		// get all language codes for this layout
		for (unsigned i=0; i<data_len;) {
			System::UInt16 lcnum=host_readw(&kcl_data[dpos-2]);
			i+=2;
			unsigned lcpos=0;
			for (;i<data_len;) {
...
unsigned keyboard_layout::read_keyboard_file(const char* keyboard_file_name, System::Int32 specific_layout, System::Int32 requested_codepage) {
	this->reset();
	...
	static System::Byte read_buf[65535];
	System::UInt32 read_buf_size, read_buf_pos, bytes_read;
	System::UInt32 start_pos=5;
	...
	if (tempfile==NULL) {
		...
		if ((start_pos=read_kcl_file("keyboard.sys",keyboard_file_name,true))) {
			...
			read_buf_size=0;
			for (unsigned ct=start_pos+2; ct<33196; ct++) read_buf[read_buf_size++]=layout_keyboardsys[ct];
		} else if ((start_pos=read_kcl_data(layout_keybrd2sys,25431,keyboard_file_name,true))) {
			read_buf_size=0;
			for (unsigned ct=start_pos+2; ct<25431; ct++) read_buf[read_buf_size++]=layout_keybrd2sys[ct];
		} else if ((start_pos=read_kcl_data(layout_keybrd3sys,27122,keyboard_file_name,true))) {
			read_buf_size=0;
			for (unsigned ct=start_pos+2; ct<27122; ct++) read_buf[read_buf_size++]=layout_keybrd3sys[ct];
		...
		if (tempfile) {
			...
			read_buf_size=(System::UInt32)fread(read_buf, sizeof(System::Byte), 65535, tempfile);
			fclose(tempfile);
		}
		start_pos=0;
	} else {
		// check ID-bytes of file
		System::UInt32 dr=(System::UInt32)fread(read_buf, sizeof(System::Byte), 4, tempfile);
	...
	System::Byte data_len,submappings;
	...
	// four pages always occupied by normal,shift,flags,commandint
	...
	for (System::UInt16 cplane=0; cplane<additional_planes; cplane++) {
		System::UInt16 plane_flags;
	...
	for (System::UInt16 sub_map=0; (sub_map<submappings) && (!found_matching_layout); sub_map++) {
		System::UInt16 submap_cp, table_offset;

		if ((sub_map!=0) && (specific_layout!=-1)) sub_map=(System::UInt16)(specific_layout&0xffff);
		...
		for (System::UInt32 i=0; i<bytes_read;) {
			System::Byte scan=read_buf[read_buf_pos++];
			if (scan==0) break;
			System::Byte scan_length=(read_buf[read_buf_pos]&7)+1;		// length of data struct
			read_buf_pos+=2;
			i+=3;
			if (((scan&0x7f)<=MAX_SCAN_CODE) && (scan_length>0)) {
				// add all available mappings
				for (System::UInt16 addmap=0; addmap<scan_length; addmap++) {
					if (addmap>additional_planes+2) break;
					unsigned charptr=read_buf_pos+addmap*((read_buf[read_buf_pos-2]&0x80)?2:1);
					System::UInt16 kchar=read_buf[charptr];
...
bool keyboard_layout::layout_key(unsigned key, System::Byte flags1, System::Byte flags2, System::Byte flags3) {
	...
	System::UInt16 current_flags=(flags1&0x7f) | (((flags2&3) | (flags3&0xc))<<8);
	...
	for (System::UInt16 cplane=0; cplane<additional_planes; cplane++) {
		System::UInt16 req_flags=current_layout_planes[cplane].required_flags;
		System::UInt16 req_userflags=current_layout_planes[cplane].required_userflags;
		// test flags
	...
	if (diacritics_character>0) {
		...
		switch(key) {
			...
			default:
				...
				System::UInt16 diacritics_start=0;
				// search start of subtable
				for (System::UInt16 i=0; i<diacritics_character-200; i++)
					diacritics_start+=diacritics[diacritics_start+1]*2+2;

				BIOS_AddKeyToBuffer((System::UInt16)(key<<8) | diacritics[diacritics_start]);
				diacritics_character=0;
		}
	}
...
bool keyboard_layout::map_key(unsigned key, System::UInt16 layouted_key, bool is_command, bool is_keypair) {
	if (is_command) {
		System::Byte key_command=(System::Byte)(layouted_key&0xff);
		...
		if (diacritics_character>0) {
			if (diacritics_character-200>=diacritics_entries) diacritics_character = 0;
				...
				System::Byte diacritics_length=diacritics[diacritics_start+1];
				diacritics_start+=2;
				diacritics_character=0;	// reset

				// search scancode
				for (System::UInt16 i=0; i<diacritics_length; i++) {
					if (diacritics[diacritics_start+i*2]==(layouted_key&0xff)) {
						// add diacritics to keybuf
						BIOS_AddKeyToBuffer((System::UInt16)(key<<8) | diacritics[diacritics_start+i*2+1]);
						return true;
					}
				}
				// add standard-diacritics to keybuf
				BIOS_AddKeyToBuffer((System::UInt16)(key<<8) | diacritics[diacritics_start-2]);
			}
		}

		// add remapped key to keybuf
		if (is_keypair) BIOS_AddKeyToBuffer(layouted_key);
		else BIOS_AddKeyToBuffer((System::UInt16)(key<<8) | (layouted_key&0xff));
...
System::UInt16 keyboard_layout::extract_codepage(const char* keyboard_file_name) {
	if (!strcmp(keyboard_file_name,"none")) return 437;

	System::UInt32 read_buf_size;
	static System::Byte read_buf[65535];
	System::UInt32 start_pos=5;
	...
	// check all submappings and use them if general submapping or same codepage submapping
	for (System::UInt16 sub_map=0; (sub_map<submappings); sub_map++) {
		System::UInt16 submap_cp;
...
unsigned keyboard_layout::read_codepage_file(const char* codepage_file_name, System::Int32 codepage_id) {
	...
	System::UInt32 start_pos;
	System::UInt16 number_of_codepages;
	...
	static System::Byte cpi_buf[65536];
	System::UInt32 cpi_buf_size=0,size_of_cpxdata=0;;
	bool upxfound=false;
	System::UInt16 found_at_pos=5;
	if (tempfile==NULL) {
		// check if build-in codepage is available
		switch (codepage_id) {
			case 437:	case 850:	case 852:	case 853:	case 857:	case 858:	
						for (unsigned bct=0; bct<6322; bct++) cpi_buf[bct]=font_ega_cpx[bct];
						cpi_buf_size=6322;
						break;
			case 771:	case 772:	case 808:	case 855:	case 866:	case 872:
						for (unsigned bct=0; bct<5455; bct++) cpi_buf[bct]=font_ega3_cpx[bct];
						cpi_buf_size=5455;
						break;
			case 737:	case 851:	case 869:
						for (unsigned bct=0; bct<5720; bct++) cpi_buf[bct]=font_ega5_cpx[bct];
						cpi_buf_size=5720;
						break;
			default: 
				return KEYB_INVALIDCPFILE;
		...
		System::UInt32 dr=(System::UInt32)fread(cpi_buf, sizeof(System::Byte), 5, tempfile);
		...
		if ((cpi_buf[0]!=0xff) || (cpi_buf[1]!=0x46) || (cpi_buf[2]!=0x4f) || 
			...
			System::Byte next_byte=0;
			for (unsigned i=0; i<100; i++) {
				fread(&next_byte, sizeof(System::Byte), 1, tempfile);	found_at_pos++;
				while (next_byte==0x55) {
					...
					if (next_byte==0x50) {
						...
						if (next_byte==0x58) {
							...
							if (next_byte==0x21) {
								// read version ID
								fread(&next_byte, sizeof(System::Byte), 1, tempfile);
								found_at_pos++;
								upxfound=true;
								break;
							}
			...
			if (!upxfound) {
				...
				fseek(tempfile, 0, SEEK_SET);
				size_of_cpxdata=(unsigned)fread(cpi_buf, sizeof(System::Byte), 65536, tempfile);
			}
		} else {
			// standard uncompressed cpi-file
			fseek(tempfile, 0, SEEK_SET);
			cpi_buf_size=(System::UInt32)fread(cpi_buf, sizeof(System::Byte), 65536, tempfile);
		}
	}
	...
	if (upxfound) {
		...
		// setup segments
		System::UInt16 save_ds=SegValue(ds);
		System::UInt16 save_es=SegValue(es);
		System::UInt16 save_ss=SegValue(ss);
		System::UInt32 save_esp=reg_esp;
		SegSet16(ds,seg);
		SegSet16(es,seg);
	...
	for (System::UInt16 test_codepage=0; test_codepage<number_of_codepages; test_codepage++) {
		System::UInt16 device_type, font_codepage, font_type;
		...
		System::UInt32 font_data_header_pt;
		...
		if ((device_type==0x0001) && (font_type==0x0001) && (font_codepage==codepage_id)) {
			...
			System::UInt16 number_of_fonts,font_data_length;
			...
			System::UInt32 font_data_start=font_data_header_pt+0x06;

			// load all fonts if possible
			for (System::UInt16 current_font=0; current_font<number_of_fonts; current_font++) {
				System::Byte font_height=cpi_buf[font_data_start];
				font_data_start+=6;
				if (font_height==0x10) {
					// 16x8 font
			...
			dos.loaded_codepage=(System::UInt16)(codepage_id&0xffff);
...
unsigned keyboard_layout::switch_keyboard_layout(const char* new_layout, keyboard_layout*& created_layout, System::Int32& tried_cp) {
	if (strncasecmp(new_layout,"US",2)) {
		...
		if (language_code_found) {
			...
			unsigned req_codepage=temp_layout->extract_codepage(new_layout);
			tried_cp = req_codepage;
			unsigned kerrcode=temp_layout->read_keyboard_file(new_layout, req_codepage);
			if (kerrcode) {
				delete temp_layout;
				return kerrcode;
			}
...
unsigned DOS_LoadKeyboardLayout(const char * layoutname, System::Int32 codepage, const char * codepagefile) {
	keyboard_layout * temp_layout=new keyboard_layout();
	// try to read the layout for the specified codepage
	unsigned kerrcode=temp_layout->read_keyboard_file(layoutname, codepage);
	if (kerrcode) {
		delete temp_layout;
		return kerrcode;
	}
...
unsigned DOS_SwitchKeyboardLayout(const char* new_layout, System::Int32& tried_cp) {
	if (loaded_layout) {
		keyboard_layout* changed_layout=NULL;
		unsigned ret_code=loaded_layout->switch_keyboard_layout(new_layout, changed_layout, tried_cp);
		if (changed_layout) {
...
/*				case 1064:
		...
		if (wants_dos_codepage>0) {
			if ((loaded_layout->read_codepage_file("auto", (unsigned)wants_dos_codepage)) == KEYB_NOERROR) {
		...
		if (extract_codepage) {
			...
			unsigned req_codepage = loaded_layout->extract_codepage(layoutname);
