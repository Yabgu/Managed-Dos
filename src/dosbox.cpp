...
static System::UInt32 ticksRemain;
static System::UInt32 ticksLast;
static System::UInt32 ticksAdded;
System::Int32 ticksDone;
System::UInt32 ticksScheduled;
bool ticksLocked;

static unsigned Normal_Loop(void) {
	int ret;
	while (1) {
		if (PIC_RunQueue()) {
			ret=(*cpudecoder)();
			...
			if (ret>0) {
				unsigned blah=(*CallBack_Handlers[ret])();
		...
		if (ticksNew > ticksLast) {
			...
			if (CPU_CycleAutoAdjust && !CPU_SkipCycleAutoAdjust) {
				if (ticksScheduled >= 250 || ticksDone >= 250 || (ticksAdded > 15 && ticksScheduled >= 5) ) {
					...
					System::Int32 ratio = (ticksScheduled * (CPU_CyclePercUsed*90*1024/100/100)) / ticksDone;
					System::Int32 new_cmax = CPU_CycleMax;
					System::Int64 cproc = (System::Int64)CPU_CycleMax * (System::Int64)ticksScheduled;
					if (cproc > 0) {
						/* ignore the cycles added due to the io delay code in order
						   to have smoother auto cycle adjustments */
						double ratioremoved = (double) CPU_IODelayRemoved / (double) cproc;
						if (ratioremoved < 1.0) {
							ratio = (System::Int32)((double)ratio * (1 - ratioremoved));
							/* Don't allow very high ratio which can cause us to lock as we don't scale down
							 * for very low ratios. High ratio might result because of timing resolution */
							if (ticksScheduled >= 250 && ticksDone < 10 && ratio > 20480) 
								ratio = 20480;
							System::Int64 cmax_scaled = (System::Int64)CPU_CycleMax * (System::Int64)ratio;
							if (ratio <= 1024) 
								new_cmax = (System::Int32)(cmax_scaled / (System::Int64)1024);
							else 
								new_cmax = (System::Int32)(1 + (CPU_CycleMax >> 1) + cmax_scaled / (System::Int64)2048);
						}
					}
