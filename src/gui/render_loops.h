...
lastagain:
	...
	System::Byte * changed = &CC[render.scale.outLine][1];
	unsigned b;
	for (b=0;b<render.scale.blocks;b++) {
		...
		const unsigned changeType = changed[b];
		changed[b] = 0;
		switch (changeType) {
		case 0:
		...
		case SCALE_LEFT:
#if (SCALERHEIGHT > 1) 
			line1 = (PTYPE *)(((System::Byte*)line0)+ render.scale.outPitch);
#endif
#if (SCALERHEIGHT > 2) 
			line2 = (PTYPE *)(((System::Byte*)line0)+ render.scale.outPitch * 2);
#endif
			SCALERFUNC;
			line0 += SCALERWIDTH * SCALER_BLOCKSIZE;
			fc += SCALER_BLOCKSIZE;
		...
		default:
			...
			for (unsigned i = 0; i<SCALER_BLOCKSIZE;i++) {
				SCALERFUNC;
				line0 += SCALERWIDTH;
#if (SCALERHEIGHT > 1) 
				line1 += SCALERWIDTH;
...
#if defined(SCALERLINEAR)
#if (SCALERHEIGHT > 1) 
			unsignedMove((System::Byte*)(&line0[-SCALER_BLOCKSIZE*SCALERWIDTH])+render.scale.outPitch  ,WC[0], SCALER_BLOCKSIZE *SCALERWIDTH*PSIZE);
#endif
#if (SCALERHEIGHT > 2) 
			unsignedMove((System::Byte*)(&line0[-SCALER_BLOCKSIZE*SCALERWIDTH])+render.scale.outPitch*2,WC[1], SCALER_BLOCKSIZE *SCALERWIDTH*PSIZE);
#endif
#endif //defined(SCALERLINEAR)
			break;
		}
...
#else
	unsigned scaleLines = Scaler_Aspect[ render.scale.outLine ];
	if ( ((int)(scaleLines - SCALERHEIGHT)) > 0 ) {
