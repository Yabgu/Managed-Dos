...
static System::UInt32 last_write = 0;
...
static unsigned read_p201(unsigned port,unsigned iolen) {
...
static unsigned read_p201_timed(unsigned port,unsigned iolen) {
	System::Byte ret=0xff;
...
static void write_p201(unsigned port,unsigned val,unsigned iolen) {
	/* Store writetime index */
	write_active = true;
	last_write = PIC_Ticks;
	if (stick[0].enabled) {
		stick[0].xcount=(unsigned)((stick[0].xpos*RANGE)+RANGE);
		stick[0].ycount=(unsigned)((stick[0].ypos*RANGE)+RANGE);
	}
	if (stick[1].enabled) {
		stick[1].xcount=(unsigned)(((swap34? stick[1].ypos : stick[1].xpos)*RANGE)+RANGE);
		stick[1].ycount=(unsigned)(((swap34? stick[1].xpos : stick[1].ypos)*RANGE)+RANGE);
	}

}
static void write_p201_timed(unsigned port,unsigned val,unsigned iolen) {
	// Store writetime index
