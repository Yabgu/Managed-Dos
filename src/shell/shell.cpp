...
unsigned call_shellstop;
/* Larger scope so shell_del autoexec can use it to
 * remove things from the environment */
Program * first_shell = 0;

static unsigned shellstop_handler(void) {
	return CBRET_STOP;
}
...
void AutoexecObject::CreateAutoexec(void) {
	...
	autoexec_data[0] = 0;
	for (auto_it it = autoexec_strings.begin(), end = autoexec_strings.end(); it != end; it++) {
	...
	if (first_shell) VFILE_Register("AUTOEXEC.BAT", (System::Byte *)autoexec_data, (System::UInt32)strlen(autoexec_data));
}
...
unsigned DOS_Shell::GetRedirection(char *s, char **ifn, char **ofn, bool * append) {
...
void DOS_Shell::ParseLine(char * line) {
	...
	System::UInt16 dummy, dummy2;
	System::UInt32 bigdummy = 0;
	unsigned num = 0;		/* Number of commands in this line */
	bool append;
...
public:
	AUTOEXEC(Section* configuration) :Module_base(configuration) {
		...
		while (control->cmdline->FindString("-c", line, true) && (i <= 11)) {
			...
			for (unsigned temp = 0; temp < line.size(); ++temp) if (line[temp] == '\'') line[temp] = '\"';
	...
	nomount:
		VFILE_Register("AUTOEXEC.BAT", (System::Byte *)autoexec_data, (System::UInt32)strlen(autoexec_data));
	}
};
...
void SHELL_Init() {
	...
	System::UInt16 psp_seg = DOS_FIRST_SHELL;
	System::UInt16 env_seg = DOS_FIRST_SHELL + 19; //DOS_GetMemory(1+(4096/16))+1;
	System::UInt16 stack_seg = DOS_GetMemory(2048 / 16);
	...
	real_writed(0, 0x24 * 4, ((System::UInt32)psp_seg << 16) | ((16 + 1) << 4));

	/* Set up int 23 to "int 20" in the psp. Fixes what.exe */
	real_writed(0, 0x23 * 4, ((System::UInt32)psp_seg << 16));

	/* Setup MCBs */
	DOS_MCB pspmcb((System::UInt16)(psp_seg - 1));
	pspmcb.SetPSPSeg(psp_seg);	// MCB of the command shell psp
	pspmcb.SetSize(0x10 + 2);
	pspmcb.SetType(0x4d);
	DOS_MCB envmcb((System::UInt16)(env_seg - 1));
	...
	MEM_BlockWrite(env_write, path_string, (unsigned)(strlen(path_string) + 1));
	env_write += (PhysPt)(strlen(path_string) + 1);
	MEM_BlockWrite(env_write, comspec_string, (unsigned)(strlen(comspec_string) + 1));
	env_write += (PhysPt)(strlen(comspec_string) + 1);
	mem_writeb(env_write++, 0);
	mem_writew(env_write, 1);
	env_write += 2;
	MEM_BlockWrite(env_write, full_name, (unsigned)(strlen(full_name) + 1));
	...
	CommandTail tail;
	tail.count = (System::Byte)strlen(init_line);
