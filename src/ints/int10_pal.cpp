...
static INLINE void WriteTandyACTL(System::Byte creg,System::Byte val) {
...
//	state&=0x01;
	...
	if (state<=1) {
		System::Byte msrval=real_readb(BIOSMEM_SEG,BIOSMEM_CURRENT_MSR)&0xdf;
...
void INT10_SetSingleDacRegister(System::Byte index,System::Byte red,System::Byte green,System::Byte blue) {
	IO_Write(VGAREG_DAC_WRITE_ADDRESS,(System::Byte)index);
...
void INT10_GetDACBlock(System::UInt16 index,System::UInt16 count,PhysPt data) {
 	IO_Write(VGAREG_DAC_READ_ADDRESS,(System::Byte)index);
	for (;count>0;count--) {
...
void INT10_SelectDACPage(System::Byte function,System::Byte mode) {
	ResetACTL();
	IO_Write(VGAREG_ACTL_ADDRESS,0x10);
	System::Byte old10=IO_Read(VGAREG_ACTL_READ_DATA);
...
void INT10_GetDACPage(System::Byte* mode,System::Byte* page) {
	ResetACTL();
	IO_Write(VGAREG_ACTL_ADDRESS,0x10);
	System::Byte reg10=IO_Read(VGAREG_ACTL_READ_DATA);
...
void INT10_SetBackgroundBorder(System::Byte val) {
	System::Byte temp=real_readb(BIOSMEM_SEG,BIOSMEM_CURRENT_PAL);
...
void INT10_PerformGrayScaleSumming(System::UInt16 start_reg,System::UInt16 count) {
	...
	for (unsigned ct=0; ct<count; ct++) {
		...
		System::Byte red=IO_Read(VGAREG_DAC_DATA);
		System::Byte green=IO_Read(VGAREG_DAC_DATA);
		System::Byte blue=IO_Read(VGAREG_DAC_DATA);

		/* calculate clamped intensity, taken from VGABIOS */
		System::UInt32 i=(( 77*red + 151*green + 28*blue ) + 0x80) >> 8;
		System::Byte ic=(i>0x3f) ? 0x3f : ((System::Byte)(i & 0xff));
