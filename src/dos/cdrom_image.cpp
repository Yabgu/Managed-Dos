...
bool CDROM_Interface_Image::BinaryFile::read(System::Byte *buffer, int seek, int count)
{
...
bool CDROM_Interface_Image::AudioFile::read(System::Byte *buffer, int seek, int count)
{
	if (lastCount != count) {
...
CDROM_Interface_Image::CDROM_Interface_Image(System::Byte subUnit)
{
	images[subUnit] = this;
	if (refCount == 0) {
...
bool CDROM_Interface_Image::SetDevice(char* path, int forceCD)
{
	...
	System::UInt16 size = (System::UInt16)strlen(buf);
	DOS_WriteFile(STDOUT, (System::Byte*)buf, &size);
	return false;
}
...
bool CDROM_Interface_Image::ReadSectors(PhysPt buffer, bool raw, unsigned long sector, unsigned long num)
{
	...
	unsigned buflen = num * sectorSize;
	System::Byte* buf = new System::Byte[buflen];
...
bool CDROM_Interface_Image::ReadSector(System::Byte *buffer, bool raw, unsigned long sector)
{
...
void CDROM_Interface_Image::CDAudioCallBack(unsigned len)
{
	len *= 4;       // 16 bit, stereo
	if (!len) return;
	...
	player.channel->AddSamples_s16_nonnative(len/4,(System::Int16 *)player.buffer);
#else
	player.channel->AddSamples_s16(len/4,(System::Int16 *)player.buffer);
#endif
...
{
	System::Byte pvd[COOKED_SECTOR_SIZE];
...
{
	in >> keyword;
	for(unsigned i = 0; i < keyword.size(); i++) keyword[i] = toupper(keyword[i]);
