...
static unsigned call_int33,call_int74,int74_ret_callback,call_mouse_bd;
static System::UInt16 ps2cbseg,ps2cbofs;
static bool useps2callback,ps2callbackinit;
static unsigned call_ps2;
static RealPt ps2_callback;
static System::Int16 oldmouseX, oldmouseY;
// forward
...
#define MOUSE_IRQ 12
#define POS_X ((System::Int16)(mouse.x) & mouse.granMask)
#define POS_Y (System::Int16)(mouse.y)

#define CURSORX 16
#define CURSORY 16
#define HIGHESTBIT (1<<(CURSORX-1))

static System::UInt16 defaultTextAndMask = 0x77FF;
static System::UInt16 defaultTextXorMask = 0x7700;

static System::UInt16 defaultScreenMask[CURSORY] = {
...
static System::UInt16 defaultCursorMask[CURSORY] = {
...
static System::UInt16 userdefScreenMask[CURSORY];
static System::UInt16 userdefCursorMask[CURSORY];

static struct {
	System::Byte buttons;
	System::UInt16 times_pressed[MOUSE_BUTTONS];
	System::UInt16 times_released[MOUSE_BUTTONS];
	System::UInt16 last_released_x[MOUSE_BUTTONS];
	System::UInt16 last_released_y[MOUSE_BUTTONS];
	System::UInt16 last_pressed_x[MOUSE_BUTTONS];
	System::UInt16 last_pressed_y[MOUSE_BUTTONS];
	System::UInt16 hidden;
	float add_x,add_y;
	System::Int16 min_x,max_x,min_y,max_y;
	float mickey_x,mickey_y;
	float x,y;
	button_event event_queue[QUEUE_SIZE];
	System::Byte events;//Increase if QUEUE_SIZE >255 (currently 32)
	System::UInt16 sub_seg,sub_ofs;
	System::UInt16 sub_mask;

	bool	background;
	System::Int16	backposx, backposy;
	System::Byte	backData[CURSORX*CURSORY];
	System::UInt16*	screenMask;
	System::UInt16* cursorMask;
	System::Int16	clipx,clipy;
	System::Int16  hotx,hoty;
	System::UInt16  textAndMask, textXorMask;

	float	mickeysPerPixel_x;
	float	mickeysPerPixel_y;
	float	pixelPerMickey_x;
	float	pixelPerMickey_y;
	System::UInt16	senv_x_val;
	System::UInt16	senv_y_val;
	System::UInt16	dspeed_val;
	float	senv_x;
	float	senv_y;
	System::UInt16  updateRegion_x[2];
	System::UInt16  updateRegion_y[2];
	System::UInt16  doubleSpeedThreshold;
	System::UInt16  language;
	System::UInt16  cursorType;
	System::UInt16	oldhidden;
	System::Byte  page;
	bool enabled;
	bool inhibit_draw;
...
void Mouse_ChangePS2Callback(System::UInt16 pseg, System::UInt16 pofs) {
	if ((pseg==0) && (pofs==0)) {
...
void DoPS2Callback(System::UInt16 data, System::Int16 mouseX, System::Int16 mouseY) {
	if (useps2callback) {
		System::UInt16 mdat = (data & 0x03) | 0x08;
		System::Int16 xdiff = mouseX-oldmouseX;
		System::Int16 ydiff = oldmouseY-mouseY;
		oldmouseX = mouseX;
		oldmouseY = mouseY;
		...
		CPU_Push16((System::UInt16)mdat); 
		CPU_Push16((System::UInt16)(xdiff % 256)); 
		CPU_Push16((System::UInt16)(ydiff % 256)); 
		CPU_Push16((System::UInt16)0); 
...
void MOUSE_Limit_Events(unsigned /*val*/) {
...
INLINE void Mouse_AddEvent(System::Byte type) {
	if (mouse.events<QUEUE_SIZE) {
		if (mouse.events>0) {
			 ...
			 */
			for(unsigned i = mouse.events ; i ; i--)
...
extern void WriteChar(System::UInt16 col,System::UInt16 row,System::Byte page,System::Byte chr,System::Byte attr,bool useattr);
extern void ReadCharAttr(System::UInt16 col,System::UInt16 row,System::Byte page,System::UInt16 * result);
...
void DrawCursorText() {	
	...
	System::Byte page = real_readb(BIOSMEM_SEG,BIOSMEM_CURRENT_PAGE);
	System::UInt16 result;

	ReadCharAttr(mouse.backposx,mouse.backposy,page,&result);
	mouse.backData[0]	= (System::Byte)(result & 0xFF);
	mouse.backData[1]	= (System::Byte)(result>>8);
	mouse.background	= true;
	// Write Cursor
	result = (result & mouse.textAndMask) ^ mouse.textXorMask;
	WriteChar(mouse.backposx,mouse.backposy,page,(System::Byte)(result&0xFF),(System::Byte)(result>>8),true);
}
...
static System::Byte gfxReg3CE[9];
static System::Byte index3C4,gfxReg3C5;
void SaveVgaRegisters() {
	if (IS_VGA_ARCH) {
		for (System::Byte i=0; i<9; i++) {
			IO_Write	(0x3CE,i);
...
void ClipCursorArea(System::Int16& x1, System::Int16& x2, System::Int16& y1, System::Int16& y2,
					System::UInt16& addx1, System::UInt16& addx2, System::UInt16& addy) {
	addx1 = addx2 = addy = 0;
	// Clip up
	if (y1<0) {
		addy += (-y1);
...
void RestoreCursorBackground() {
	...
	if (mouse.background) {
		// Restore background
		System::Int16 x,y;
		System::UInt16 addx1,addx2,addy;
		System::UInt16 dataPos	= 0;
		System::Int16 x1		= mouse.backposx;
		System::Int16 y1		= mouse.backposy;
		System::Int16 x2		= x1 + CURSORX - 1;
		System::Int16 y2		= y1 + CURSORY - 1;	
...
/*	if ((POS_X >= mouse.updateRegion_x[0]) && (POS_X <= mouse.updateRegion_x[1]) &&
   ...
   */ /*Not sure yet what to do update region should be set to ??? */
	...
	mouse.clipx = (System::Int16)((int)CurMode->swidth-1);	/* Get from bios ? */
	mouse.clipy = (System::Int16)((int)CurMode->sheight-1);

	/* might be vidmode == 0x13?2:1 */
	System::Int16 xratio = 640;
	...
	System::UInt16 addx1,addx2,addy;
	System::UInt16 dataPos	= 0;
	System::Int16 x1		= POS_X / xratio - mouse.hotx;
	System::Int16 y1		= POS_Y - mouse.hoty;
	System::Int16 x2		= x1 + CURSORX - 1;
	System::Int16 y2		= y1 + CURSORY - 1;	
	...
	dataPos = addy * CURSORX;
	for (y=y1; y<=y2; y++) {
		System::UInt16 scMask = mouse.screenMask[addy+y-y1];
		System::UInt16 cuMask = mouse.cursorMask[addy+y-y1];
...
void Mouse_ButtonPressed(System::Byte button) {
	switch (button) {
#if (MOUSE_BUTTONS >= 1)
	case 0:
		mouse.buttons|=1;
...
void Mouse_ButtonReleased(System::Byte button) {
	switch (button) {
#if (MOUSE_BUTTONS >= 1)
	case 0:
		mouse.buttons&=~1;
...
static void Mouse_SetMickeyPixelRate(System::Int16 px, System::Int16 py){
	if ((px!=0) && (py!=0)) {
...
static void Mouse_SetSensitivity(System::UInt16 px, System::UInt16 py, System::UInt16 dspeed){
	if(px>100) px=100;
	if(py>100) py=100;
...
void Mouse_NewVideoMode(void) {
	...
	System::Byte mode=mem_readb(BIOS_VIDEO_MODE);
	...
	case 0x02:
	case 0x03: {
		unsigned rows=real_readb(BIOSMEM_SEG,BIOSMEM_NB_ROWS);
	...
	oldmouseX = static_cast<System::Int16>(mouse.x);
	oldmouseY = static_cast<System::Int16>(mouse.y);
...
static unsigned INT33_Handler(void) {
//	LOG(LOG_MOUSE,LOG_NORMAL)("MOUSE: %04X %X %X %d %d",reg_ax,reg_bx,reg_cx,POS_X,POS_Y);
	...
	case 0x04:	/* Position Mouse */
		...
		if ((System::Int16)reg_cx >= mouse.max_x) mouse.x = static_cast<float>(mouse.max_x);
		else if (mouse.min_x >= (System::Int16)reg_cx) mouse.x = static_cast<float>(mouse.min_x); 
		else if ((System::Int16)reg_cx != POS_X) mouse.x = static_cast<float>(reg_cx);

		if ((System::Int16)reg_dx >= mouse.max_y) mouse.y = static_cast<float>(mouse.max_y);
		else if (mouse.min_y >= (System::Int16)reg_dx) mouse.y = static_cast<float>(mouse.min_y); 
		else if ((System::Int16)reg_dx != POS_Y) mouse.y = static_cast<float>(reg_dx);
		DrawCursor();
		break;
	case 0x05:	/* Return Button Press Data */
		{
			System::UInt16 but=reg_bx;
	...
	case 0x07:	/* Define horizontal cursor range */
		{	//lemmings set 1-640 and wants that. iron seeds set 0-640 but doesn't like 640
			...
			System::Int16 max,min;
			if ((System::Int16)reg_cx<(System::Int16)reg_dx) { min=(System::Int16)reg_cx;max=(System::Int16)reg_dx;}
			else { min=(System::Int16)reg_dx;max=(System::Int16)reg_cx;}
			mouse.min_x=min;
			mouse.max_x=max;
	...
	case 0x0b:	/* Read Motion Data */
		reg_cx=(System::Int16)(mouse.mickey_x*mouse.mickeysPerPixel_x);
		reg_dx=(System::Int16)(mouse.mickey_y*mouse.mickeysPerPixel_y);
		mouse.mickey_x=0;
		mouse.mickey_y=0;
		break;
	...
	case 0x14: /* Exchange event-handler */ 
		{	
			System::UInt16 oldSeg = mouse.sub_seg;
			System::UInt16 oldOfs = mouse.sub_ofs;
			System::UInt16 oldMask= mouse.sub_mask;
			// Set new values
			mouse.sub_mask= reg_cx;
			mouse.sub_seg = SegValue(es);
			mouse.sub_ofs = reg_dx;
	...
	case 0x26: /* Get Maximum virtual coordinates */
		...
		reg_cx=(System::UInt16)mouse.max_x;
		reg_dx=(System::UInt16)mouse.max_y;
		break;
	case 0x31: /* Get Current Minimum/Maximum virtual coordinates */
		reg_ax=(System::UInt16)mouse.min_x;
		reg_bx=(System::UInt16)mouse.min_y;
		reg_cx=(System::UInt16)mouse.max_x;
		reg_dx=(System::UInt16)mouse.max_y;
		break;
	default:
...
static unsigned MOUSE_BD_Handler(void) {
	// the stack contains offsets to register values
	System::UInt16 raxpt=real_readw(SegValue(ss),reg_sp+0x0a);
	System::UInt16 rbxpt=real_readw(SegValue(ss),reg_sp+0x08);
	System::UInt16 rcxpt=real_readw(SegValue(ss),reg_sp+0x06);
	System::UInt16 rdxpt=real_readw(SegValue(ss),reg_sp+0x04);

	// read out the actual values, registers ARE overwritten
	System::UInt16 rax=real_readw(SegValue(ds),raxpt);
	reg_ax=rax;
...
static unsigned INT74_Handler(void) {
	if (mouse.events>0) {
		mouse.events--;
		...
		if (mouse.sub_mask & mouse.event_queue[mouse.events].type) {
			...
			reg_cx=POS_X;
			reg_dx=POS_Y;
			reg_si=(System::Int16)(mouse.mickey_x*mouse.mickeysPerPixel_x);
			reg_di=(System::Int16)(mouse.mickey_y*mouse.mickeysPerPixel_y);
...
unsigned MOUSE_UserInt_CB_Handler(void) {
	mouse.in_UIR = false;
	if (mouse.events) {
		if (!mouse.timer_in_progress) {
			mouse.timer_in_progress = true;
...
//	RealPt i33loc=RealMake(CB_SEG+1,(call_int33*CB_SIZE)-0x10);
	...
	System::Byte hwvec=(MOUSE_IRQ>7)?(0x70+MOUSE_IRQ-8):(0x8+MOUSE_IRQ);
