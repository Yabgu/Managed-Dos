...
static void VGA_DAC_SendColor( unsigned index, unsigned src ) {
	const System::Byte red = vga.dac.rgb[src].red;
	const System::Byte green = vga.dac.rgb[src].green;
	const System::Byte blue = vga.dac.rgb[src].blue;
...
static void VGA_DAC_UpdateColor( unsigned index ) {
	unsigned maskIndex = index & vga.dac.pel_mask;
	VGA_DAC_SendColor( index, maskIndex );
}

static void write_p3c6(unsigned port,unsigned val,unsigned iolen) {
...
static unsigned read_p3c6(unsigned port,unsigned iolen) {
	return vga.dac.pel_mask;
}


static void write_p3c7(unsigned port,unsigned val,unsigned iolen) {
...
static unsigned read_p3c7(unsigned port,unsigned iolen) {
	if (vga.dac.state==DAC_READ) return 0x3;
	else return 0x0;
}

static void write_p3c8(unsigned port,unsigned val,unsigned iolen) {
	vga.dac.write_index=val;
	vga.dac.pel_index=0;
	vga.dac.state=DAC_WRITE;
}

static unsigned read_p3c8(unsigned port, unsigned iolen){
	return vga.dac.write_index;
}

static void write_p3c9(unsigned port,unsigned val,unsigned iolen) {
	val&=0x3f;
	...
	case 2:
		...
		case M_LIN8:
			...
			if ( GCC_UNLIKELY( vga.dac.pel_mask != 0xff)) {
				unsigned index = vga.dac.write_index;
...
static unsigned read_p3c9(unsigned port,unsigned iolen) {
	System::Byte ret;
	switch (vga.dac.pel_index) {
	case 0:
