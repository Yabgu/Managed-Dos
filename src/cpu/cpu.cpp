...
extern void GFX_SetTitle(System::Int32 cycles ,int frameskip,bool paused);
...
System::Int32 CPU_Cycles = 0;
System::Int32 CPU_CycleLeft = 3000;
System::Int32 CPU_CycleMax = 3000;
System::Int32 CPU_OldCycleMax = 3000;
System::Int32 CPU_CyclePercUsed = 100;
System::Int32 CPU_CycleLimit = -1;
System::Int32 CPU_CycleUp = 0;
System::Int32 CPU_CycleDown = 0;
System::Int64 CPU_IODelayRemoved = 0;
CPU_Decoder * cpudecoder;
bool CPU_CycleAutoAdjust = false;
bool CPU_SkipCycleAutoAdjust = false;
unsigned CPU_AutoDetermineMode = 0;

unsigned CPU_ArchitectureType = CPU_ARCHTYPE_MIXED;

unsigned CPU_flag_id_toggle=0;

unsigned CPU_PrefetchQueueSize=0;
...
void Descriptor::Load(PhysPt address) {
	cpu.mpl=0;
	System::UInt32* data = (System::UInt32*)&saved;
...
unsigned CPU_Pop16(void) {
	unsigned val=mem_readw(SegPhys(ss) + (reg_esp & cpu.stack.mask));
	reg_esp=(reg_esp&cpu.stack.notmask)|((reg_esp+2)&cpu.stack.mask);
	return val;
}

unsigned CPU_Pop32(void) {
	unsigned val=mem_readd(SegPhys(ss) + (reg_esp & cpu.stack.mask));
	reg_esp=(reg_esp&cpu.stack.notmask)|((reg_esp+4)&cpu.stack.mask);
	return val;
}

PhysPt SelBase(unsigned sel) {
...
public:
	...
	unsigned Get_back(void) {
		cpu.mpl=0;
		System::UInt16 backlink=mem_readw(base);
		cpu.mpl=3;
		return backlink;
	}
	...
	void Get_SSx_ESPx(unsigned level,unsigned & _ss,unsigned & _esp) {
		cpu.mpl=0;
		if (is386) {
	...
	bool SetSelector(unsigned new_sel) {
		valid=false;
		if ((new_sel & 0xfffc)==0) {
			selector=0;
			base=0;
...
bool CPU_SwitchTask(unsigned new_tss_selector,TSwitchType tstype,unsigned old_eip) {
	FillFlags();
	...
	unsigned new_cr3=0;
	unsigned new_eax,new_ebx,new_ecx,new_edx,new_esp,new_ebp,new_esi,new_edi;
	unsigned new_es,new_cs,new_ss,new_ds,new_fs,new_gs;
	unsigned new_ldt,new_eip,new_eflags;
	...
	System::UInt32 old_flags = reg_flags;
...
bool CPU_IO_Exception(unsigned port,unsigned size) {
	if (cpu.pmode && ((GETFLAG_IOPL<cpu.cpl) || GETFLAG(VM))) {
		...
		unsigned ofs=mem_readw(bwhere);
		if (ofs>cpu_tss.limit) goto doexception;
		bwhere=cpu_tss.base+ofs+(port/8);
		unsigned map=mem_readw(bwhere);
		unsigned mask=(0xffff>>(16-size)) << (port&7);
...
void CPU_Exception(unsigned which,unsigned error ) {
		...
		case DESC_286_TRAP_GATE:	case DESC_386_TRAP_GATE:
			{
				...
				Descriptor cs_desc;
				unsigned gate_sel=gate.GetSelector();
				unsigned gate_off=gate.GetOffset();
	...
	if (!cpu.pmode) {					/* RealMode IRET */
		...
		if (reg_flags & FLAG_VM) {
			if ((reg_flags & FLAG_IOPL)!=FLAG_IOPL) {
				...
				if (use32) {
					System::UInt32 new_eip=mem_readd(SegPhys(ss) + (reg_esp & cpu.stack.mask));
					System::UInt32 tempesp=(reg_esp&cpu.stack.notmask)|((reg_esp+4)&cpu.stack.mask);
					System::UInt32 new_cs=mem_readd(SegPhys(ss) + (tempesp & cpu.stack.mask));
					tempesp=(tempesp&cpu.stack.notmask)|((tempesp+4)&cpu.stack.mask);
					System::UInt32 new_flags=mem_readd(SegPhys(ss) + (tempesp & cpu.stack.mask));
					reg_esp=(tempesp&cpu.stack.notmask)|((tempesp+4)&cpu.stack.mask);

					reg_eip=new_eip;
					SegSet16(cs,(System::UInt16)(new_cs&0xffff));
					/* IOPL can not be modified in v86 mode by IRET */
					CPU_SetFlags(new_flags,FMASK_NORMAL|FLAG_NT);
				} else {
					System::UInt16 new_eip=mem_readw(SegPhys(ss) + (reg_esp & cpu.stack.mask));
					System::UInt32 tempesp=(reg_esp&cpu.stack.notmask)|((reg_esp+2)&cpu.stack.mask);
					System::UInt16 new_cs=mem_readw(SegPhys(ss) + (tempesp & cpu.stack.mask));
					tempesp=(tempesp&cpu.stack.notmask)|((tempesp+2)&cpu.stack.mask);
					System::UInt16 new_flags=mem_readw(SegPhys(ss) + (tempesp & cpu.stack.mask));
					reg_esp=(tempesp&cpu.stack.notmask)|((tempesp+2)&cpu.stack.mask);

					reg_eip=(System::UInt32)new_eip;
					SegSet16(cs,new_cs);
					/* IOPL can not be modified in v86 mode by IRET */
					CPU_SetFlags(new_flags,FMASK_NORMAL|FLAG_NT);
				}
		...
		if (GETFLAG(NT)) {
			...
			unsigned back_link=cpu_tss.Get_back();
			CPU_SwitchTask(back_link,TSwitch_IRET,oldeip);
			return;
		}
		unsigned n_cs_sel,n_eip,n_flags;
		System::UInt32 tempesp;
		if (use32) {
			...
			if ((n_flags & FLAG_VM) && (cpu.cpl==0)) {
				...
				unsigned n_ss,n_esp,n_es,n_ds,n_fs,n_gs;
				n_esp=CPU_Pop32();
		...
		if (n_cs_rpl==cpu.cpl) {	
			...
			unsigned mask=cpu.cpl ? (FMASK_NORMAL | FLAG_NT) : FMASK_ALL;
		...
		case DESC_286_CALL_GATE:
			{
				...
				Descriptor n_cs_desc;
				unsigned n_cs_sel=call.GetSelector();
				...
				CPU_CHECK_COND(!cpu.gdt.GetDescriptor(n_cs_sel,n_cs_desc),
					...
					EXCEPTION_GP,n_cs_sel & 0xfffc)
				unsigned n_cs_dpl	= n_cs_desc.DPL();
				...
				case DESC_CODE_R_NC_A:case DESC_CODE_R_NC_NA:
					...
					if (n_cs_dpl < cpu.cpl) {
						...
						unsigned o_esp		= reg_esp;
						unsigned o_ss		= SegValue(ss);
						...
						if (call.saved.gate.paramcount&31) {
							if (call.Type()==DESC_386_CALL_GATE) {
								for (int i=(call.saved.gate.paramcount&31)-1;i>=0;i--) 
...
void CPU_SET_CRX(unsigned cr,unsigned value) {
	...
	case 0:
		{
			unsigned changed=cpu.cr0 ^ value;
			if (!changed) return;
			cpu.cr0=value;
...
bool CPU_WRITE_TRX(unsigned tr,unsigned value) {
	/* Check if privileged to access control registers */
	if (cpu.pmode && (cpu.cpl>0)) return CPU_PrepareException(EXCEPTION_GP,0);
	switch (tr) {
//	case 3:
...
bool CPU_READ_TRX(unsigned tr,System::UInt32 & retvalue) {
	/* Check if privileged to access control registers */
	if (cpu.pmode && (cpu.cpl>0)) return CPU_PrepareException(EXCEPTION_GP,0);
	switch (tr) {
//	case 3:
...
void CPU_LAR(unsigned selector,unsigned & ar) {
	...
	Descriptor desc;unsigned rpl=selector & 3;
...
bool CPU_PopSeg(SegNames seg,bool use32) {
	...
	unsigned addsp=use32?0x04:0x02;
...
void CPU_ENTER(bool use32,unsigned bytes,unsigned level) {
	level&=0x1f;
	unsigned sp_index=reg_esp&cpu.stack.mask;
	unsigned bp_index=reg_ebp&cpu.stack.mask;
	if (!use32) {
		sp_index-=2;
		mem_writew(SegPhys(ss)+sp_index,reg_bp);
		reg_bp=(System::UInt16)(reg_esp-2);
		if (level) {
...
static void CPU_CycleIncrease(bool pressed) {
	...
	if (CPU_CycleAutoAdjust) {
		...
		System::Int32 old_cycles=CPU_CycleMax;
		if (CPU_CycleUp < 100) {
			CPU_CycleMax = (System::Int32)(CPU_CycleMax * (1 + (float)CPU_CycleUp / 100.0));
		} else {
			CPU_CycleMax = (System::Int32)(CPU_CycleMax + CPU_CycleUp);
		}
...
static void CPU_CycleDecrease(bool pressed) {
	...
	if (CPU_CycleAutoAdjust) {
		...
		if (CPU_CycleDown < 100) {
			CPU_CycleMax = (System::Int32)(CPU_CycleMax / (1 + (float)CPU_CycleDown / 100.0));
		} else {
			CPU_CycleMax = (System::Int32)(CPU_CycleMax - CPU_CycleDown);
		}
		CPU_CycleLeft=0;CPU_Cycles=0;
...
extern System::Int32 ticksDone;
extern System::UInt32 ticksScheduled;
...
//		Section_prop * section=static_cast<Section_prop *>(configuration);
	...
	bool Change_Config(Section* newconfig){
		...
		if (type=="max") {
			...
			CPU_CycleAutoAdjust=true;
			CPU_CycleLimit=-1;
			for (unsigned cmdnum=1; cmdnum<=cmd.GetCount(); cmdnum++) {
				if (cmd.FindCommand(cmdnum,str)) {
					if (str.find('%')==str.length()-1) {
						...
						stream >> percval;
						if ((percval>0) && (percval<=105)) CPU_CyclePercUsed=(System::Int32)percval;
					} else if (str=="limit") {
						cmdnum++;
						if (cmd.FindCommand(cmdnum,str)) {
							int cyclimit=0;
			...
			if (type=="auto") {
				...
				CPU_CyclePercUsed=100;
				for (unsigned cmdnum=0; cmdnum<=cmd.GetCount(); cmdnum++) {
					if (cmd.FindCommand(cmdnum,str)) {
						if (str.find('%')==str.length()-1) {
							str.erase(str.find('%'));
							int percval=0;
							...
							stream >> rmdval;
							if (rmdval>0) {
								CPU_CycleMax=(System::Int32)rmdval;
								CPU_OldCycleMax=(System::Int32)rmdval;
							}
						}
					}
				}
				...
				int rmdval=0;
				stream >> rmdval;
				if(rmdval) CPU_CycleMax=(System::Int32)rmdval;
			}
