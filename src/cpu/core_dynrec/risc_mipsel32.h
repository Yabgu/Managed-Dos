...
#define DRC_PTR_SIZE_IM System::UInt32
...
static System::UInt32 temp1_value;
...
static void gen_mov_dword_to_reg_imm(HostReg dest_reg,System::UInt32 imm) {
	if(imm < 65536) {
		cache_addw((System::UInt16)imm);		// ori dest_reg, $0, imm
		cache_addw(0x3400+dest_reg);
	} else if(((System::Int32)imm < 0) && ((System::Int32)imm >= -32768)) {
		cache_addw((System::UInt16)imm);		// addiu dest_reg, $0, imm
		cache_addw(0x2400+dest_reg);
	} else if(!(imm & 0xffff)) {
		cache_addw((System::UInt16)(imm >> 16));	// lui dest_reg, %hi(imm)
		cache_addw(0x3c00+dest_reg);
	} else {
		cache_addw((System::UInt16)(imm >> 16));	// lui dest_reg, %hi(imm)
		cache_addw(0x3c00+dest_reg);
		cache_addw((System::UInt16)imm);		// ori dest_reg, dest_reg, %lo(imm)
		cache_addw(0x3400+(dest_reg<<5)+dest_reg);
	}
}

// this is the only place temp1 should be modified
static void INLINE mov_imm_to_temp1(System::UInt32 imm) {
...
static System::Int16 gen_addr_temp1(System::UInt32 addr) {
	System::UInt32 hihalf = addr & 0xffff0000;
	System::Int16 lohalf = addr & 0xffff;
...
static void gen_mov_word_to_reg(HostReg dest_reg,void* data,bool dword) {
	System::Int16 lohalf = gen_addr_temp1((System::UInt32)data);
	// alignment....
	if (dword) {
...
static void gen_mov_word_to_reg_imm(HostReg dest_reg,System::UInt16 imm) {
...
static void gen_mov_word_from_reg(HostReg src_reg,void* dest,bool dword) {
	System::Int16 lohalf = gen_addr_temp1((System::UInt32)dest);
	// alignment....
	if (dword) {
...
// this function does not use FC_OP1/FC_OP2 as dest_reg as these
// registers might not be directly byte-accessible on some architectures
static void INLINE gen_mov_byte_to_reg_low_imm(HostReg dest_reg,System::Byte imm) {
...
static void gen_add_imm(HostReg reg,System::UInt32 imm) {
	if(!imm) return;
	if(((System::Int32)imm >= -32768) && ((System::Int32)imm < 32768)) {
		cache_addw((System::UInt16)imm);	// addiu reg, reg, imm
...
static void gen_and_imm(HostReg reg,System::UInt32 imm) {
	if(imm < 65536) { 
		cache_addw((System::UInt16)imm);      // andi reg, reg, imm 
		cache_addw(0x3000+(reg<<5)+reg); 
	} else { 
		mov_imm_to_temp1((System::UInt32)imm); 
...
static void INLINE gen_mov_direct_dword(void* dest,System::UInt32 imm) {
...
static void INLINE gen_add_direct_word(void* dest,System::UInt32 imm,bool dword) {
	if(!imm) return;
...
static void INLINE gen_add_direct_byte(void* dest,System::SByte imm) {
	gen_add_direct_word(dest, (System::Int32)imm, 1);
}

// subtract an 8bit constant value from a dword memory value
static void INLINE gen_sub_direct_byte(void* dest,System::SByte imm) {
	gen_add_direct_word(dest, -((System::Int32)imm), 1);
}

// subtract a 32bit (dword==true) or 16bit (dword==false) constant value from a memory value
static void INLINE gen_sub_direct_word(void* dest,System::UInt32 imm,bool dword) {
	gen_add_direct_word(dest, -(System::Int32)imm, dword);
}
...
#endif
	temp1_valid = false;
	cache_addd(0x0c000000+(((System::UInt32)func>>2)&0x3ffffff));		// jal func
	DELAY;
}
...
static void INLINE gen_jmp_ptr(void * ptr,int imm=0) {
	...
	temp1_valid = false;
	cache_addw((System::UInt16)imm);	// lw temp2, imm(temp2)
...
static System::UInt32 INLINE gen_create_branch_on_zero(HostReg reg,bool dword) {
	temp1_valid = false;
	if(!dword) { 
	...
	DELAY;
	return ((System::UInt32)cache.pos-8);
}

// short conditional jump (+-127 bytes) if register is nonzero
// the destination is set by gen_fill_branch() later
static System::UInt32 INLINE gen_create_branch_on_nonzero(HostReg reg,bool dword) {
	temp1_valid = false;
	if(!dword) { 
		cache_addw(0xffff);	// andi temp1, reg, 0xffff
		cache_addw(0x3000+(reg<<5)+temp1);
...
#if C_DEBUG
	int len=(System::UInt32)cache.pos-data;
	if (len<0) len=-len;
	if (len>126) LOG_MSG("Big jump %d",len);
#endif
	temp1_valid = false;			// this is a branch target
	*(System::UInt16*)data=((System::UInt16)((System::UInt32)cache.pos-data-4)>>2);
}
...
static System::UInt32 INLINE gen_create_branch_long_leqzero(HostReg reg) {
	temp1_valid = false;
...
static void INLINE gen_fill_branch_long(System::UInt32 data) {
	...
	*(System::UInt32*)data=0x08000000+(((System::UInt32)cache.pos>>2)&0x3ffffff);
}
#else		
// conditional jump if register is nonzero
// for isdword==true the 32bit of the register are tested
...
static void gen_fill_function_ptr(System::Byte * pos,void* fct_ptr,unsigned flags_type) {
	...
	switch (flags_type) {
		...
		case t_ADDw:
		case t_ADDd:
			*(System::UInt32*)pos=0x00851021;					// addu $v0, $a0, $a1
			break;
		case t_ORb:
		case t_ORw:
		case t_ORd:
			*(System::UInt32*)pos=0x00851025;					// or $v0, $a0, $a1
			break;
		case t_ANDb:
		case t_ANDw:
		case t_ANDd:
			*(System::UInt32*)pos=0x00851024;					// and $v0, $a0, $a1
			break;
		case t_SUBb:
		case t_SUBw:
		case t_SUBd:
			*(System::UInt32*)pos=0x00851023;					// subu $v0, $a0, $a1
			break;
		case t_XORb:
		case t_XORw:
		case t_XORd:
			*(System::UInt32*)pos=0x00851026;					// xor $v0, $a0, $a1
			break;
		case t_CMPb:
		case t_CMPw:
		case t_CMPd:
		...
		case t_TESTw:
		case t_TESTd:
			*(System::UInt32*)pos=0;							// nop
			break;
		case t_INCb:
		case t_INCw:
		case t_INCd:
			*(System::UInt32*)pos=0x24820001;					// addiu $v0, $a0, 1
			break;
		case t_DECb:
		case t_DECw:
		case t_DECd:
			*(System::UInt32*)pos=0x2482ffff;					// addiu $v0, $a0, -1
			break;
		case t_SHLb:
		case t_SHLw:
		case t_SHLd:
			*(System::UInt32*)pos=0x00a41004;					// sllv $v0, $a0, $a1
			break;
		case t_SHRb:
		case t_SHRw:
		case t_SHRd:
			*(System::UInt32*)pos=0x00a41006;					// srlv $v0, $a0, $a1
			break;
		case t_SARd:
			*(System::UInt32*)pos=0x00a41007;					// srav $v0, $a0, $a1
			break;
#if (_MIPS_ISA==MIPS32R2) || defined(PSP)
		case t_RORd:
			*(System::UInt32*)pos=0x00a41046;					// rotr $v0, $a0, $a1
			break;
#endif
		case t_NEGb:
		case t_NEGw:
		case t_NEGd:
			*(System::UInt32*)pos=0x00041023;					// subu $v0, $0, $a0
			break;
		default:
			*(System::UInt32*)pos=0x0c000000+((((System::UInt32)fct_ptr)>>2)&0x3ffffff);		// jal simple_func
			break;
	}
#else
	*(System::UInt32*)pos=0x0c000000+(((System::UInt32)fct_ptr)>>2)&0x3ffffff);		// jal simple_func
...
static void cache_block_closing(System::Byte* block_start,unsigned block_size) {
#ifdef PSP
// writeback dcache and invalidate icache
	System::UInt32 inval_start = ((System::UInt32)block_start) & ~63;
	System::UInt32 inval_end = (((System::UInt32)block_start) + block_size + 64) & ~63;
