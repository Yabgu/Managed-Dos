...
CSerialModem::CSerialModem(unsigned id, CommandLine* cmd):CSerial(id, cmd) {
	...
	clientsocket = 0;
	serversocket = 0;
	getunsignedSubstring("listenport:", &listenport, cmd);
...
CSerialModem::~CSerialModem() {
	...
	// remove events
	for(unsigned i = SERIAL_BASE_EVENT_COUNT+1;	i <= SERIAL_MODEM_EVENT_COUNT; i++)
		removeEvent(i);
}

void CSerialModem::handleUpperEvent(System::UInt16 type) {
	switch (type) {
	case SERIAL_RX_EVENT: {
		// check for bytes to be sent to port
		if(CSerial::CanReceiveByte())
			if(rqueue->inuse() && (CSerial::getRTS()||(flowcontrol!=3))) {
				System::Byte rbyte = rqueue->getb();
...
void CSerialModem::SendLine(const char *line) {
	rqueue->addb(0xd);
	rqueue->addb(0xa);
	rqueue->adds((System::Byte *)line,strlen(line));
	rqueue->addb(0xd);
	rqueue->addb(0xa);
}

// only for numbers < 1000...
void CSerialModem::SendNumber(unsigned val) {
	rqueue->addb(0xd);
	rqueue->addb(0xa);
...
void CSerialModem::SendRes(ResTypes response) {
	char const * string;unsigned code;
	switch (response)
	{
		case ResNONE:		return;
...
bool CSerialModem::Dial(char * host) {
	...
	if (hasport) {
		*hasport++=0;
		port=(System::UInt16)atoi(hasport);
	}
...
unsigned CSerialModem::ScanNumber(char * & scan) {
	unsigned ret=0;
	while (char c=*scan) {
		if (c>='0' && c<='9') {
			ret*=10;
	...
	while (1) {
		...
		case 'D': { // Dial
			...
			if (strlen(foundstr) >= 12) {
				...
				bool isNum = true;
				for (unsigned i=0; i<strlen(foundstr); i++)
		...
		case 'Q': {
			...
			unsigned val = ScanNumber(scanbuf);	
			if(!(val>2)) {
				doresponse=val;
				break;
			} else {
		...
		case 'S': { // Registers	
			unsigned index=ScanNumber(scanbuf);
			if(index>=SREGS) {
				SendRes(ResERROR);
...
void CSerialModem::TelnetEmulation(System::Byte * data, unsigned size) {
	unsigned i;
	System::Byte c;
	for(i=0;i<size;i++) {
		c = data[i];
...
void CSerialModem::transmitByte(System::Byte val, bool first) {
...
void CSerialModem::updatePortConfig(System::UInt16, System::Byte lcr) { 
