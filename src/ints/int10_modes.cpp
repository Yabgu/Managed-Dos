...
static System::Byte text_palette[64][3]=
{
...
static System::Byte mtext_palette[64][3]=
{
...
static System::Byte mtext_s3_palette[64][3]=
{
  {0x00,0x00,0x00},{0x00,0x00,0x00},{0x00,0x00,0x00},{0x00,0x00,0x00},{0x00,0x00,0x00},{0x00,0x00,0x00},{0x00,0x00,0x00},{0x00,0x00,0x00},
  {0x2a,0x2a,0x2a},{0x2a,0x2a,0x2a},{0x2a,0x2a,0x2a},{0x2a,0x2a,0x2a},{0x2a,0x2a,0x2a},{0x2a,0x2a,0x2a},{0x2a,0x2a,0x2a},{0x2a,0x2a,0x2a},
  {0x2a,0x2a,0x2a},{0x2a,0x2a,0x2a},{0x2a,0x2a,0x2a},{0x2a,0x2a,0x2a},{0x2a,0x2a,0x2a},{0x2a,0x2a,0x2a},{0x2a,0x2a,0x2a},{0x2a,0x2a,0x2a},
...
static System::Byte ega_palette[64][3]=
{
...
static System::Byte cga_palette[16][3]=
{
	{0x00,0x00,0x00}, {0x00,0x00,0x2a}, {0x00,0x2a,0x00}, {0x00,0x2a,0x2a}, {0x2a,0x00,0x00}, {0x2a,0x00,0x2a}, {0x2a,0x15,0x00}, {0x2a,0x2a,0x2a},
	{0x15,0x15,0x15}, {0x15,0x15,0x3f}, {0x15,0x3f,0x15}, {0x15,0x3f,0x3f}, {0x3f,0x15,0x15}, {0x3f,0x15,0x3f}, {0x3f,0x3f,0x15}, {0x3f,0x3f,0x3f},
};

static System::Byte cga_palette_2[64][3]=
{
	{0x00,0x00,0x00}, {0x00,0x00,0x2a}, {0x00,0x2a,0x00}, {0x00,0x2a,0x2a}, {0x2a,0x00,0x00}, {0x2a,0x00,0x2a}, {0x2a,0x15,0x00}, {0x2a,0x2a,0x2a},
	{0x00,0x00,0x00}, {0x00,0x00,0x2a}, {0x00,0x2a,0x00}, {0x00,0x2a,0x2a}, {0x2a,0x00,0x00}, {0x2a,0x00,0x2a}, {0x2a,0x15,0x00}, {0x2a,0x2a,0x2a},
	{0x15,0x15,0x15}, {0x15,0x15,0x3f}, {0x15,0x3f,0x15}, {0x15,0x3f,0x3f}, {0x3f,0x15,0x15}, {0x3f,0x15,0x3f}, {0x3f,0x3f,0x15}, {0x3f,0x3f,0x3f},
...
static System::Byte vga_palette[256][3]=
{
  {0x00,0x00,0x00},{0x00,0x00,0x2a},{0x00,0x2a,0x00},{0x00,0x2a,0x2a},{0x2a,0x00,0x00},{0x2a,0x00,0x2a},{0x2a,0x15,0x00},{0x2a,0x2a,0x2a},
  {0x15,0x15,0x15},{0x15,0x15,0x3f},{0x15,0x3f,0x15},{0x15,0x3f,0x3f},{0x3f,0x15,0x15},{0x3f,0x15,0x3f},{0x3f,0x3f,0x15},{0x3f,0x3f,0x3f},
...
static bool SetCurMode(VideoModeBlock modeblock[],System::UInt16 mode) {
	unsigned i=0;
...
static void FinishSetMode(bool clearmem) {
	...
	if (clearmem) {
		...
		case M_CGA2:
		case M_TANDY16:
			for (System::UInt16 ct=0;ct<16*1024;ct++) {
				real_writew( 0xb800,ct*2,0x0000);
			}
			break;
		case M_TEXT: {
			System::UInt16 seg = (CurMode->mode==7)?0xb000:0xb800;
			for (System::UInt16 ct=0;ct<16*1024;ct++) real_writew(seg,ct*2,0x0720);
			break;
		}
		case M_EGA:	
		case M_VGA:
	...
	/* Setup the BIOS */
	if (CurMode->mode<128) real_writeb(BIOSMEM_SEG,BIOSMEM_CURRENT_MODE,(System::Byte)CurMode->mode);
	else real_writeb(BIOSMEM_SEG,BIOSMEM_CURRENT_MODE,(System::Byte)(CurMode->mode-0x98));	//Looks like the s3 bios
	real_writew(BIOSMEM_SEG,BIOSMEM_NB_COLS,(System::UInt16)CurMode->twidth);
	real_writew(BIOSMEM_SEG,BIOSMEM_PAGE_SIZE,(System::UInt16)CurMode->plength);
	real_writew(BIOSMEM_SEG,BIOSMEM_CRTC_ADDRESS,((CurMode->mode==7 )|| (CurMode->mode==0x0f)) ? 0x3b4 : 0x3d4);
	real_writeb(BIOSMEM_SEG,BIOSMEM_NB_ROWS,(System::Byte)(CurMode->theight-1));
	real_writew(BIOSMEM_SEG,BIOSMEM_CHAR_HEIGHT,(System::UInt16)CurMode->cheight);
	...
	for (System::Byte ct=0;ct<8;ct++) INT10_SetCursorPos(0,0,ct);
	// Set active page 0
...
bool INT10_SetVideoMode_OTHER(System::UInt16 mode,bool clearmem) {
	switch (machine) {
	case MCH_CGA:
...
//	VGA_SetMode(CurMode->type);
	/* Setup the CRTC */
	unsigned crtc_base=machine==MCH_HERC ? 0x3b4 : 0x3d4;
	//Horizontal total
	...
	for (System::Byte ct=0;ct<16;ct++) VGA_DAC_SetEntry(ct,cga_palette[ct][0],cga_palette[ct][1],cga_palette[ct][2]);
	//Setup the tandy palette
	for (System::Byte ct=0;ct<16;ct++) VGA_DAC_CombineColor(ct,ct);
	//Setup the special registers for each machine type
	System::Byte mode_control_list[0xa+1]={
		0x2c,0x28,0x2d,0x29,	//0-3
		0x2a,0x2e,0x1e,0x29,	//4-7
		0x2a,0x2b,0x3b			//8-a
	};
	System::Byte mode_control_list_pcjr[0xa+1]={
		0x0c,0x08,0x0d,0x09,	//0-3
		0x0a,0x0e,0x0e,0x09,	//4-7		
		0x1a,0x1b,0x0b			//8-a
	};
	System::Byte mode_control,color_select;
	switch (machine) {
	case MCH_HERC:
	...
	if ((vparams != RealMake(0xf000,0xf0a4)) && (mode < 8)) {
		...
		System::UInt16 crtc_block_index = 0;
...
//	System::Byte video_ctl=real_readb(BIOSMEM_SEG,BIOSMEM_VIDEO_CTL);
//	System::Byte vga_switches=real_readb(BIOSMEM_SEG,BIOSMEM_SWITCHES);
	System::Byte modeset_ctl=real_readb(BIOSMEM_SEG,BIOSMEM_MODESET_CTL);
	...
	System::Byte misc_output=0x2 | (mono_mode ? 0x0 : 0x1);
	...
	for (System::Byte ct=0;ct<SEQ_REGS;ct++) {
		IO_Write(0x3c4,ct);
	...
	/* Clear all the regs */
	for (System::Byte ct=0x0;ct<=0x18;ct++) {
		IO_Write(crtc_base,ct);IO_Write(crtc_base+1,0);
	}
	System::Byte overflow=0;System::Byte max_scanline=0;
	System::Byte ver_overflow=0;System::Byte hor_overflow=0;
	/* Horizontal Total */
	IO_Write(crtc_base,0x00);IO_Write(crtc_base+1,(System::Byte)(CurMode->htotal-5));
	hor_overflow|=((CurMode->htotal-5) & 0x100) >> 8;
	/* Horizontal Display End */
	IO_Write(crtc_base,0x01);IO_Write(crtc_base+1,(System::Byte)(CurMode->hdispend-1));
	hor_overflow|=((CurMode->hdispend-1) & 0x100) >> 7;
	/* Start horizontal Blanking */
	IO_Write(crtc_base,0x02);IO_Write(crtc_base+1,(System::Byte)CurMode->hdispend);
	hor_overflow|=((CurMode->hdispend) & 0x100) >> 6;
	/* End horizontal Blanking */
	unsigned blank_end=(CurMode->htotal-2) & 0x7f;
	...
	IO_Write(crtc_base,0x04);IO_Write(crtc_base+1,(System::Byte)ret_start);
	...
	IO_Write(crtc_base,0x05);IO_Write(crtc_base+1,(System::Byte)(ret_end | (blank_end & 0x20) << 2));

	/* Vertical Total */
	IO_Write(crtc_base,0x06);IO_Write(crtc_base+1,(System::Byte)(CurMode->vtotal-2));
	...
	IO_Write(crtc_base,0x10);IO_Write(crtc_base+1,(System::Byte)vretrace);
	...
	IO_Write(crtc_base,0x15);IO_Write(crtc_base+1,(System::Byte)(CurMode->vdispend+vblank_trim));
	...
	/* Vertical Blank End */
	IO_Write(crtc_base,0x16);IO_Write(crtc_base+1,(System::Byte)(CurMode->vtotal-vblank_trim-2));

	/* Line Compare */
	unsigned line_compare=(CurMode->vtotal < 1024) ? 1023 : 2047;
	...
	if (svgaCard == SVGA_S3Trio) {
		...
		/* This register actually has more int but only use the extended offset ones */
		IO_Write(crtc_base,0x51);
		IO_Write(crtc_base + 1,(System::Byte)((offset & 0x300) >> 4));
		/* Clear remaining int of the display start */
	...
	if (svgaCard == SVGA_S3Trio) {
		...
		if (CurMode->mode>=0x100) {
			...
			unsigned clock=CurMode->vtotal*8*CurMode->htotal*70;
	...
	for (System::Byte ct=0;ct<GFX_REGS;ct++) {
		IO_Write(0x3ce,ct);
		IO_Write(0x3cf,gfx_data[ct]);
	}
	System::Byte att_data[ATT_REGS];
	...
	case M_TANDY16:
		att_data[0x10]=0x01;		//Color Graphics
		for (System::Byte ct=0;ct<16;ct++) att_data[ct]=ct;
		break;
	case M_TEXT:
		if (CurMode->cwidth==9) {
...
att_text16:
	...
	case M_CGA4:
		...
		att_data[6]=0x06;
		att_data[7]=0x07;
		for (System::Byte ct=0x8;ct<0x10;ct++) 
			att_data[ct] = ct + 0x8;
		real_writeb(BIOSMEM_SEG,BIOSMEM_CURRENT_PAL,0x30);
		break;
	case M_VGA:
	...
	if ((modeset_ctl & 8)==0) {
		for (System::Byte ct=0;ct<ATT_REGS;ct++) {
			IO_Write(0x3c0,ct);
...
dac_text16:
		...
		for (System::Byte ct=0x10;ct<ATT_REGS;ct++) {
	...
	System::Byte feature=real_readb(BIOSMEM_SEG,BIOSMEM_INITIAL_MODE);
	switch (CurMode->type) {
	case M_CGA2:
...
//	real_writeb(BIOSMEM_SEG,BIOSMEM_INITIAL_MODE,feature);
	...
	if (svgaCard == SVGA_S3Trio) {
		...
		IO_Write(crtc_base+1,(System::Byte)((S3_LFB_BASE >> 24)&0xff));
		IO_Write(crtc_base,0x5a);
		IO_Write(crtc_base+1,(System::Byte)((S3_LFB_BASE >> 16)&0xff));
