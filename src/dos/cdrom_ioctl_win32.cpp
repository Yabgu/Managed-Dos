...
bool CDROM_Interface_Ioctl::GetAudioTracks(int& stTrack, int& endTrack, TMSF& leadOut) {
	...
	if ((use_mciplay || use_dxplay) && (!track_start_valid)) {
		...
		for (int i=toc.FirstTrack; i<=toc.LastTrack+1; i++) {
...
bool CDROM_Interface_Ioctl::ReadSector(System::Byte *buffer, bool raw, unsigned long sector) {
	BOOL  bStat;
	DWORD byteCount = 0;

	unsigned	buflen	= raw ? RAW_SECTOR_SIZE : COOKED_SECTOR_SIZE;
...
bool CDROM_Interface_Ioctl::ReadSectors(PhysPt buffer, bool raw, unsigned long sector, unsigned long num) {
	...
	unsigned	buflen	= raw ? num*RAW_SECTOR_SIZE : num*COOKED_SECTOR_SIZE;
	System::Byte*	bufdata = new System::Byte[buflen];
...
void CDROM_Interface_Ioctl::dx_CDAudioCallBack(unsigned len) {
	len *= 4;       // 16 bit, stereo
	if (!len) return;
