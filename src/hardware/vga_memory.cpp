...
template <class Size>
static INLINE void hostWrite(HostPt off, unsigned val) {
	if ( sizeof( Size ) == 1)
		host_writeb( off, (System::Byte)val );
	else if ( sizeof( Size ) == 2)
		host_writew( off, (System::UInt16)val );
	else if ( sizeof( Size ) == 4)
		host_writed( off, (System::UInt32)val );
}

template <class Size>
static INLINE unsigned  hostRead(HostPt off ) {
	if ( sizeof( Size ) == 1)
...
INLINE static System::UInt32 RasterOp(System::UInt32 input,System::UInt32 mask) {
...
INLINE static System::UInt32 ModeOperation(System::Byte val) {
	System::UInt32 full;
	switch (vga.config.write_mode) {
	case 0x00:
		// Write Mode 0: In this mode, the host data is first rotated as per the Rotate Count field, then the Enable Set/Reset mechanism selects data from this or the Set/Reset field. Then the selected Logical Operation is performed on the resulting data and the data in the latch register. Then the Bit Mask field is used to select which int come from the resulting data and which come from the latch register. Finally, only the bit planes enabled by the Memory Plane Write Enable field are written to memory. 
	...
	case 0x01:
		...
		break;
	case 0x02:
		//Write Mode 2: In this mode, the int 3-0 of the host data are replicated across all 8 int of their respective planes. Then the selected Logical Operation is performed on the resulting data and the data in the latch register. Then the Bit Mask field is used to select which int come from the resulting data and which come from the latch register. Finally, only the bit planes enabled by the Memory Plane Write Enable field are written to memory. 
		full=RasterOp(FillTable[val&0xF],vga.config.full_bit_mask);
		break;
	case 0x03:
		// Write Mode 3: In this mode, the data in the Set/Reset field is used as if the Enable Set/Reset field were set to 1111b. Then the host data is first rotated as per the Rotate Count field, then logical ANDed with the value of the Bit Mask field. The resulting value is used on the data obtained from the Set/Reset field in the same way that the Bit Mask field would ordinarily be used. to select which int come from the expansion of the Set/Reset field and which come from the latch register. Finally, only the bit planes enabled by the Memory Plane Write Enable field are written to memory.
		val=((val >> vga.config.data_rotate) | (val << (8-vga.config.data_rotate)));
...
public:
	unsigned readHandler(PhysPt start) {
		vga.latch.d=((System::UInt32*)vga.mem.linear)[start];
		...
		case 1:
			...
			return (System::Byte)~(templatch.b[0] | templatch.b[1] | templatch.b[2] | templatch.b[3]);
		}
		return 0;
	}
public:
	unsigned readb(PhysPt addr) {
		addr = PAGING_GetPhysicalAddress(addr) & vgapages.mask;
		addr += vga.svga.bank_read_full;
		addr = CHECKED2(addr);
		return readHandler(addr);
	}
	unsigned readw(PhysPt addr) {
		addr = PAGING_GetPhysicalAddress(addr) & vgapages.mask;
		addr += vga.svga.bank_read_full;
		addr = CHECKED2(addr);
		return 
...
public:
	unsigned readHandler(PhysPt addr) {
		return vga.mem.linear[addr];
	}
	void writeHandler(PhysPt start, System::Byte val) {
		ModeOperation(val);
		/* Update video memory and the pixel buffer */
		VGA_Latch pixels;
		vga.mem.linear[start] = val;
		start >>= 2;
		pixels.d=((System::UInt32*)vga.mem.linear)[start];

		System::Byte * write_pixels=&vga.fastmem[start<<3];

		System::UInt32 colors0_3, colors4_7;
		...
		*(System::UInt32 *)write_pixels=colors0_3;
		...
		colors4_7 = 
			...
			Expand16Table[2][temp.b[2]] |
			Expand16Table[3][temp.b[3]];
		*(System::UInt32 *)(write_pixels+4)=colors4_7;
	}
public:	
	...
	void writeb(PhysPt addr,unsigned val) {
		...
		addr = CHECKED(addr);
		MEM_CHANGED( addr << 3);
		writeHandler(addr+0,(System::Byte)(val >> 0));
	}
	void writew(PhysPt addr,unsigned val) {
		...
		MEM_CHANGED( addr << 3);
		writeHandler(addr+0,(System::Byte)(val >> 0));
		writeHandler(addr+1,(System::Byte)(val >> 8));
	}
	void writed(PhysPt addr,unsigned val) {
		...
		writeHandler(addr+0,(System::Byte)(val >> 0));
		writeHandler(addr+1,(System::Byte)(val >> 8));
		writeHandler(addr+2,(System::Byte)(val >> 16));
		writeHandler(addr+3,(System::Byte)(val >> 24));
	}
	unsigned readb(PhysPt addr) {
		addr = PAGING_GetPhysicalAddress(addr) & vgapages.mask;
		addr += vga.svga.bank_read_full;
...
public:
	...
	void writeHandler(PhysPt start, System::Byte val) {
		System::UInt32 data=ModeOperation(val);
		/* Update video memory and the pixel buffer */
		VGA_Latch pixels;
		pixels.d=((System::UInt32*)vga.mem.linear)[start];
		pixels.d&=vga.config.full_not_map_mask;
		pixels.d|=(data & vga.config.full_map_mask);
		((System::UInt32*)vga.mem.linear)[start]=pixels.d;
		System::Byte * write_pixels=&vga.fastmem[start<<3];
...
public:	
	...
	void writeb(PhysPt addr,unsigned val) {
		...
		addr = CHECKED2(addr);
		MEM_CHANGED( addr << 3);
		writeHandler<true>(addr+0,(System::Byte)(val >> 0));
	}
	void writew(PhysPt addr,unsigned val) {
		addr = PAGING_GetPhysicalAddress(addr) & vgapages.mask;
		addr += vga.svga.bank_write_full;
		...
		MEM_CHANGED( addr << 3);
		writeHandler<true>(addr+0,(System::Byte)(val >> 0));
		writeHandler<true>(addr+1,(System::Byte)(val >> 8));
	}
	void writed(PhysPt addr,unsigned val) {
		addr = PAGING_GetPhysicalAddress(addr) & vgapages.mask;
		addr += vga.svga.bank_write_full;
		...
		writeHandler<true>(addr+0,(System::Byte)(val >> 0));
		writeHandler<true>(addr+1,(System::Byte)(val >> 8));
		writeHandler<true>(addr+2,(System::Byte)(val >> 16));
		writeHandler<true>(addr+3,(System::Byte)(val >> 24));
	}
};

//Slighly unusual version, will directly write 8,16,32 int values
...
public:
	...
	template <class Size>
	static INLINE unsigned readHandler(PhysPt addr ) {
		return hostRead<Size>( &vga.mem.linear[((addr&~3)<<2)+(addr&3)] );
	}
	template <class Size>
	static INLINE void writeCache(PhysPt addr, unsigned val) {
	...
	template <class Size>
	static INLINE void writeHandler(PhysPt addr, unsigned val) {
	...
	unsigned readb(PhysPt addr ) {
		...
		addr += vga.svga.bank_read_full;
		addr = CHECKED(addr);
		return readHandler<System::Byte>( addr );
	}
	unsigned readw(PhysPt addr ) {
		addr = PAGING_GetPhysicalAddress(addr) & vgapages.mask;
		addr += vga.svga.bank_read_full;
		...
		if (GCC_UNLIKELY(addr & 1))
			return
				(readHandler<System::Byte>( addr+0 ) << 0 ) | 
				(readHandler<System::Byte>( addr+1 ) << 8 );
		else
			return readHandler<System::UInt16>( addr );
	}
	unsigned readd(PhysPt addr ) {
		addr = PAGING_GetPhysicalAddress(addr) & vgapages.mask;
		addr += vga.svga.bank_read_full;
		...
		if (GCC_UNLIKELY(addr & 3))
			return
				(readHandler<System::Byte>( addr+0 ) << 0 ) | 
				(readHandler<System::Byte>( addr+1 ) << 8 ) | 
				(readHandler<System::Byte>( addr+2 ) << 16 ) | 
				(readHandler<System::Byte>( addr+3 ) << 24 );
		else
			return readHandler<System::UInt32>( addr );
	}
	void writeb(PhysPt addr, unsigned val ) {
		addr = PAGING_GetPhysicalAddress(addr) & vgapages.mask;
		addr += vga.svga.bank_write_full;
		addr = CHECKED(addr);
		MEM_CHANGED( addr );
		writeHandler<System::Byte>( addr, val );
		writeCache<System::Byte>( addr, val );
	}
	void writew(PhysPt addr,unsigned val) {
		addr = PAGING_GetPhysicalAddress(addr) & vgapages.mask;
		addr += vga.svga.bank_write_full;
...
//		MEM_CHANGED( addr + 1);
		if (GCC_UNLIKELY(addr & 1)) {
			writeHandler<System::Byte>( addr+0, val >> 0 );
			writeHandler<System::Byte>( addr+1, val >> 8 );
		} else {
			writeHandler<System::UInt16>( addr, val );
		}
		writeCache<System::UInt16>( addr, val );
	}
	void writed(PhysPt addr,unsigned val) {
		addr = PAGING_GetPhysicalAddress(addr) & vgapages.mask;
		addr += vga.svga.bank_write_full;
...
//		MEM_CHANGED( addr + 3);
		if (GCC_UNLIKELY(addr & 3)) {
			writeHandler<System::Byte>( addr+0, val >> 0 );
			writeHandler<System::Byte>( addr+1, val >> 8 );
			writeHandler<System::Byte>( addr+2, val >> 16 );
			writeHandler<System::Byte>( addr+3, val >> 24 );
		} else {
			writeHandler<System::UInt32>( addr, val );
		}
		writeCache<System::UInt32>( addr, val );
	}
};

class VGA_UnchainedVGA_Handler : public VGA_UnchainedRead_Handler {
public:
	void writeHandler( PhysPt addr, System::Byte val ) {
		System::UInt32 data=ModeOperation(val);
		VGA_Latch pixels;
		pixels.d=((System::UInt32*)vga.mem.linear)[addr];
		pixels.d&=vga.config.full_not_map_mask;
		pixels.d|=(data & vga.config.full_map_mask);
		((System::UInt32*)vga.mem.linear)[addr]=pixels.d;
//		if(vga.config.compatible_chain4)
//			((System::UInt32*)vga.mem.linear)[CHECKED2(addr+64*1024)]=pixels.d; 
	}
public:
...
public:
	...
	void writeb(PhysPt addr,unsigned val){
		...
		if (vga.seq.map_mask & 0x4) {
			vga.draw.font[addr]=(System::Byte)val;
		}
	}
};
...
public:
	...
	unsigned readb(PhysPt addr) {
		...
		addr += vga.svga.bank_read_full;
		addr = CHECKED(addr);
		return hostRead<System::Byte>( &vga.mem.linear[addr] );
	}
	unsigned readw(PhysPt addr) {
		addr = PAGING_GetPhysicalAddress(addr) & vgapages.mask;
		addr += vga.svga.bank_read_full;
		addr = CHECKED(addr);
		return hostRead<System::UInt16>( &vga.mem.linear[addr] );
	}
	unsigned readd(PhysPt addr) {
		addr = PAGING_GetPhysicalAddress(addr) & vgapages.mask;
		addr += vga.svga.bank_read_full;
		addr = CHECKED(addr);
		return hostRead<System::UInt32>( &vga.mem.linear[addr] );
	}
	void writeb(PhysPt addr,unsigned val) {
		addr = PAGING_GetPhysicalAddress(addr) & vgapages.mask;
		addr += vga.svga.bank_write_full;
		addr = CHECKED(addr);
		MEM_CHANGED( addr );
		hostWrite<System::Byte>( &vga.mem.linear[addr], val );
	}
	void writew(PhysPt addr,unsigned val) {
		addr = PAGING_GetPhysicalAddress(addr) & vgapages.mask;
		addr += vga.svga.bank_write_full;
		addr = CHECKED(addr);
		MEM_CHANGED( addr );
		hostWrite<System::UInt16>( &vga.mem.linear[addr], val );
	}
	void writed(PhysPt addr,unsigned val) {
		addr = PAGING_GetPhysicalAddress(addr) & vgapages.mask;
		addr += vga.svga.bank_write_full;
		addr = CHECKED(addr);
		MEM_CHANGED( addr );	
		hostWrite<System::UInt32>( &vga.mem.linear[addr], val );
	}
};
...
public:
	...
	void writeb(PhysPt addr,unsigned val) {
		...
		addr = CHECKED4(addr);
		MEM_CHANGED( addr << 3 );
		writeHandler<false>(addr+0,(System::Byte)(val >> 0));
	}
	void writew(PhysPt addr,unsigned val) {
		addr = vga.svga.bank_write_full + (PAGING_GetPhysicalAddress(addr) & 0xffff);
		addr = CHECKED4(addr);
		MEM_CHANGED( addr << 3 );
		writeHandler<false>(addr+0,(System::Byte)(val >> 0));
		writeHandler<false>(addr+1,(System::Byte)(val >> 8));
	}
	void writed(PhysPt addr,unsigned val) {
		addr = vga.svga.bank_write_full + (PAGING_GetPhysicalAddress(addr) & 0xffff);
		addr = CHECKED4(addr);
		...
		writeHandler<false>(addr+0,(System::Byte)(val >> 0));
		writeHandler<false>(addr+1,(System::Byte)(val >> 8));
		writeHandler<false>(addr+2,(System::Byte)(val >> 16));
		writeHandler<false>(addr+3,(System::Byte)(val >> 24));
	}
	unsigned readb(PhysPt addr) {
...
extern void XGA_Write(unsigned port, unsigned val, unsigned len);
extern unsigned XGA_Read(unsigned port, unsigned len);
...
public:
	...
	void writeb(PhysPt addr,unsigned val) {
		unsigned port = PAGING_GetPhysicalAddress(addr) & 0xffff;
...
void VGA_SetupMemory(Section* sec) {
	...
	System::UInt32 vga_allocsize=vga.vmemsize;
	// Keep lower limit at 512k
	if (vga_allocsize<512*1024) vga_allocsize=512*1024;
	// We reserve extra 2K for one scan line
	vga_allocsize+=2048;
	vga.mem.linear_orgptr = new System::Byte[vga_allocsize+16];
	vga.mem.linear=(System::Byte*)(((unsigned)vga.mem.linear_orgptr + 16-1) & ~(16-1));
	memset(vga.mem.linear,0,vga_allocsize);

	vga.fastmem_orgptr = new System::Byte[(vga.vmemsize<<1)+4096+16];
	vga.fastmem=(System::Byte*)(((unsigned)vga.fastmem_orgptr + 16-1) & ~(16-1));
	...
	vga.changes.map = new System::Byte[changesMapSize];
