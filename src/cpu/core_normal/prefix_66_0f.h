 ...
 */
			...
			case 0x01:	/* STR */
				{
					...
					if (rm >= 0xc0) {GetEArw;*earw=(System::UInt16)saveval;}
	...
	CASE_0F_D(0x01)												/* Group 7 Ed */
		{
			...
			if (rm < 0xc0)	{ //First ones all use EA
				...
				switch (which) {
				case 0x00:										/* SGDT */
					SaveMw(eaa,(System::UInt16)CPU_SGDT_limit());
					SaveMd(eaa+2,(System::UInt32)CPU_SGDT_base());
					break;
				case 0x01:										/* SIDT */
					SaveMw(eaa,(System::UInt16)CPU_SIDT_limit());
					SaveMd(eaa+2,(System::UInt32)CPU_SIDT_base());
					break;
				case 0x02:										/* LGDT */
				...
				case 0x03:										/* LIDT */
					...
					break;
				case 0x04:										/* SMSW */
					SaveMw(eaa,(System::UInt16)CPU_SMSW());
					break;
				case 0x06:										/* LMSW */
					limit=LoadMw(eaa);
					if (CPU_LMSW((System::UInt16)limit)) RUNEXCEPTION();
					break;
				case 0x07:										/* INVLPG */
					if (cpu.pmode && cpu.cpl) EXCEPTION(EXCEPTION_GP);
					PAGING_ClearTLB();
	...
	CASE_0F_D(0xa3)												/* BT Ed,Gd */
		{
			FillFlags();GetRMrd;
			System::UInt32 mask=1 << (*rmrd & 31);
			if (rm >= 0xc0 ) {
				GetEArd;
				SETFLAGBIT(CF,(*eard & mask));
			} else {
				GetEAa;eaa+=(((System::Int32)*rmrd)>>5)*4;
				System::UInt32 old=LoadMd(eaa);
	...
	CASE_0F_D(0xba)												/* GRP8 Ed,Ib */
		{
			...
			if (rm >= 0xc0 ) {
				GetEArd;
				System::UInt32 mask=1 << (Fetchb() & 31);
				SETFLAGBIT(CF,(*eard & mask));
				switch (rm & 0x38) {
				case 0x20:											/* BT */
					break;
				...
				GetEAa;System::UInt32 old=LoadMd(eaa);
				System::UInt32 mask=1 << (Fetchb() & 31);
				SETFLAGBIT(CF,(old & mask));
				switch (rm & 0x38) {
				case 0x20:											/* BT */
	...
	CASE_0F_D(0xbe)												/* MOVSX Gd,Eb */
		{
			GetRMrd;															
			if (rm >= 0xc0 ) {GetEArb;*rmrd=*(System::SByte *)earb;}
	...
	CASE_0F_D(0xbf)												/* MOVSX Gd,Ew */
		{
			GetRMrd;															
			if (rm >= 0xc0 ) {GetEArw;*rmrd=*(System::Int16 *)earw;}
