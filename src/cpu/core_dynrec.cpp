...
static struct {
	BlockReturn (*runcode)(System::Byte*);		// points to code that can start a block
	unsigned callback;				// the occurred callback
	unsigned readdata;				// spare space used when reading from memory
	System::UInt32 protected_regs[8];	// space to save/restore register values
} core_dynrec;
...
CacheBlockDynRec * LinkBlocks(BlockReturn ret) {
	...
	unsigned temp_ip=SegPhys(cs)+reg_eip;
	...
	for (;;) {
		...
		if (!block) {
			...
			if (!chandler->invalidation_map || (chandler->invalidation_map[ip_point&4095]<4)) {
				...
				unsigned old_cycles=CPU_Cycles;
				CPU_Cycles=1;
				int nc_retcode=CPU_Core_Normal_Run();
				if (!nc_retcode) {
