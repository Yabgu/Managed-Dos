...
typedef struct _dac_channel {
	System::Byte buffer[DISNEY_SIZE];	// data buffer
	unsigned used;					// current data buffer level
...
static struct {
	...
	unsigned state;
	unsigned interface_det;
	unsigned interface_det_ext;
} disney;
...
static void DISNEY_CallBack(unsigned len);

static void DISNEY_disable(unsigned) {
	if(disney.mo) {
...
static void DISNEY_enable(unsigned freq) {
...
static void DISNEY_analyze(unsigned channel){
	switch(disney.state) {
		...
		case DS_FINISH: 
		{
			...
			int st_diff = disney.da[0].used - disney.da[1].used;
			...
			for(unsigned i = 0; i < 2; i++) {
				ch_speed[i] = (unsigned)(1.0/((disney.da[i].speedcheck_sum/1000.0) /
...
static void disney_write(unsigned port,unsigned val,unsigned iolen) {
...
static unsigned disney_read(unsigned port,unsigned iolen) {
	unsigned retval;
	switch (port-DISNEY_BASE) {
	case 0:		/* Data Port */
...
static void DISNEY_PlayStereo(unsigned len, System::Byte* l, System::Byte* r) {
	static System::Byte stereodata[DISNEY_SIZE*2];
...
static void DISNEY_CallBack(unsigned len) {
	...
	// TODO: len > DISNEY
		...
		if(disney.stereo) {
			System::Byte gapfiller0 = 128;
			System::Byte gapfiller1 = 128;
			if(real_used) {
			...
			System::Byte gapfiller = 128; //Keep the middle
			if(real_used) {
