...
struct Dynamic_Functionality {
	...
	System::Byte cur_mode;				/* 04h   BYTE   video mode in effect */
	System::UInt16 num_cols;			/* 05h   WORD   number of columns */
	System::UInt16 regen_size;			/* 07h   WORD   length of regen buffer in bytes */
	System::UInt16 regen_start;			/* 09h   WORD   starting address of regen buffer*/
	System::UInt16 cursor_pos[8];		/* 0Bh   WORD   cursor position for page 0-7 */
	System::UInt16 cursor_type;			/* 1Bh   WORD   cursor type */
	System::Byte active_page;			/* 1Dh   BYTE   active display page */
	System::UInt16 crtc_address;		/* 1Eh   WORD   CRTC port address */
	System::Byte reg_3x8;				/* 20h   BYTE   current setting of register (3?8) */
	System::Byte reg_3x9;				/* 21h   BYTE   current setting of register (3?9) */
	System::Byte num_rows;				/* 22h   BYTE   number of rows */
	System::UInt16 bytes_per_char;		/* 23h   WORD   bytes/character */
	System::Byte dcc;					/* 25h   BYTE   display combination code of active display */
	System::Byte dcc_alternate;		/* 26h   BYTE   DCC of alternate display */
	System::UInt16 num_colors;			/* 27h   WORD   number of colors supported in current mode */
	System::Byte num_pages;			/* 29h   BYTE   number of pages supported in current mode */
	System::Byte num_scanlines;		/* 2Ah   BYTE   number of scan lines active mode (0,1,2,3) = (200,350,400,480) */
	System::Byte pri_char_block;		/* 2Bh   BYTE   primary character block */
	System::Byte sec_char_block;		/* 2Ch   BYTE   secondary character block */
	System::Byte misc_flags;			/* 2Dh   BYTE   miscellaneous flags
									...
									7 reserved
								*/
	System::Byte reserved1[3];			/* 2Eh  3 BYTEs reserved (00h) */
	System::Byte vid_mem;				/* 31h   BYTE   video memory available 00h = 64K, 01h = 128K, 02h = 192K, 03h = 256K */
	System::Byte savep_state_flag;		/* 32h   BYTE   save pointer state flags 
									...
									7 reserved
								*/
	System::Byte reserved2[13];		/*  33h 13 BYTEs reserved (00h) */
...
//	mem_writeb(save+0x25,real_readb(BIOSMEM_SEG,BIOSMEM_DCC_INDEX));
	...
	if (svstable) {
		...
		System::Byte entries=real_readb(RealSeg(dcctable),RealOff(dcctable)+0x00);
		System::Byte idx=real_readb(BIOSMEM_SEG,BIOSMEM_DCC_INDEX);
		// check if index within range
		if (idx<entries) {
			System::UInt16 dccentry=real_readw(RealSeg(dcctable),RealOff(dcctable)+0x04+idx*2);
			if ((dccentry&0xff)==0) dccode=(System::Byte)((dccentry>>8)&0xff);
			else dccode=(System::Byte)(dccentry&0xff);
		}
	}
	mem_writeb(save+0x25,dccode);

	System::UInt16 col_count=0;
	switch (CurMode->type) {
	case M_TEXT:
...
static void EGA_RIL(System::UInt16 dx, unsigned& port, unsigned& regs) {
	port = 0;
...
void INT10_EGA_RIL_ReadRegisterRange(System::Byte ch, System::Byte cl, System::UInt16 dx, PhysPt dst) {
	...
	if(regs == 0) {
		...
		if(ch<regs) {
			if ((unsigned)ch+cl>regs) cl=(System::Byte)(regs-ch);
...
void INT10_EGA_RIL_WriteRegisterRange(System::Byte ch, System::Byte cl, System::UInt16 dx, PhysPt src) {
	unsigned port = 0;
	unsigned regs = 0;
	EGA_RIL(dx,port,regs);
	if(regs == 0) {
...
void INT10_EGA_RIL_ReadRegisterSet(System::UInt16 cx, PhysPt tbl) {
	...
	for (unsigned i=0; i<cx; i++) {
		System::Byte vl=mem_readb(tbl+2);
...
void INT10_EGA_RIL_WriteRegisterSet(System::UInt16 cx, PhysPt tbl) {
	...
	for (unsigned i=0; i<cx; i++) {
		...
		System::Byte vl=mem_readb(tbl+3);
		if(regs == 0) {
			if(port) IO_Write(port,vl);
		} else {
			System::Byte idx=mem_readb(tbl+2);
			if(port == 0x3c0) {
				IO_Read(real_readw(BIOSMEM_SEG,BIOSMEM_CRTC_ADDRESS) + 6);
				IO_Write(port,idx);
				IO_Write(port,vl);
