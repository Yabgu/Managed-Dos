...
static void dyn_xchg_ax(System::Byte reg) {
...
static void dyn_dop_byte_imm(DualOps op,System::Byte reg,System::Byte idx) {
...
static void dyn_dop_byte_imm_mem(DualOps op,System::Byte reg,System::Byte idx) {
	MOV_REG_BYTE_TO_HOST_REG_LOW_CANUSEWORD(FC_OP1,reg,idx);
	unsigned val;
	if (decode_fetchb_imm(val)) {
		gen_mov_byte_to_reg_low_canuseword(FC_OP2,(void*)val);
	} else {
		gen_mov_byte_to_reg_low_imm_canuseword(FC_OP2,(System::Byte)val);
	}
	dyn_dop_byte_gencall(op);
	if ((op!=DOP_CMP) && (op!=DOP_TEST)) MOV_REG_BYTE_FROM_HOST_REG_LOW(FC_RETOP,reg,idx);
}

static void dyn_prep_word_imm(System::Byte reg) {
	unsigned val;
	if (decode.big_op) {
	...
	if (decode.big_op) gen_mov_dword_to_reg_imm(FC_OP2,(System::UInt32)val);
	else gen_mov_word_to_reg_imm(FC_OP2,(System::UInt16)val);
}

static void dyn_dop_word_imm(DualOps op,System::Byte reg) {
...
static void dyn_dop_word_imm_old(DualOps op,System::Byte reg,unsigned imm) {
	MOV_REG_WORD_TO_HOST_REG(FC_OP1,reg,decode.big_op);
	if (decode.big_op) gen_mov_dword_to_reg_imm(FC_OP2,(System::UInt32)imm);
	else gen_mov_word_to_reg_imm(FC_OP2,(System::UInt16)imm);
	dyn_dop_word_gencall(op,decode.big_op);
	if ((op!=DOP_CMP) && (op!=DOP_TEST)) MOV_REG_WORD_FROM_HOST_REG(FC_RETOP,reg,decode.big_op);
}

static void dyn_mov_byte_imm(System::Byte reg,System::Byte idx,System::Byte imm) {
	gen_mov_byte_to_reg_low_imm(FC_TMP_BA1,imm);
	MOV_REG_BYTE_FROM_HOST_REG_LOW(FC_TMP_BA1,reg,idx);
}

static void dyn_mov_word_imm(System::Byte reg) {
	unsigned val;
	if (decode.big_op) {
		if (decode_fetchd_imm(val)) {
	...
	if (decode.big_op) gen_mov_dword_to_reg_imm(FC_OP1,(System::UInt32)val);
	else gen_mov_word_to_reg_imm(FC_OP1,(System::UInt16)val);
	MOV_REG_WORD_FROM_HOST_REG(FC_OP1,reg,decode.big_op);
}


static void dyn_sop_word(SingleOps op,System::Byte reg) {
	MOV_REG_WORD_TO_HOST_REG(FC_OP1,reg,decode.big_op);
...
static void dyn_mov_byte_al_direct(unsigned imm) {
...
static void dyn_mov_byte_ax_direct(unsigned imm) {
	MOV_SEG_PHYS_TO_HOST_REG(FC_ADDR,(decode.seg_prefix_used ? decode.seg_prefix : DRC_SEG_DS));
	gen_add_imm(FC_ADDR,imm);
	dyn_read_word(FC_ADDR,FC_OP1,decode.big_op);
...
static void dyn_mov_byte_direct_al() {
	...
	if (decode.big_addr) {
		...
		if (decode_fetchd_imm(val)) {
			...
			gen_add_imm(FC_ADDR,(System::UInt32)val);
		}
	} else {
...
static void dyn_mov_byte_direct_ax(unsigned imm) {
	MOV_SEG_PHYS_TO_HOST_REG(FC_ADDR,(decode.seg_prefix_used ? decode.seg_prefix : DRC_SEG_DS));
	gen_add_imm(FC_ADDR,imm);
	MOV_REG_WORD_TO_HOST_REG(FC_OP1,DRC_REG_EAX,decode.big_op);
	dyn_write_word(FC_ADDR,FC_OP1,decode.big_op);
...
static void dyn_push_seg(System::Byte seg) {
...
static void dyn_segprefix(System::Byte seg) {
...
static void dyn_load_seg_off_ea(System::Byte seg) {
	if (decode.modrm.mod<3) {
		dyn_fill_ea(FC_ADDR);
		gen_protect_addr_reg();
		dyn_read_word(FC_ADDR,FC_OP1,decode.big_op);
...
static void dyn_imul_gvev(unsigned immsize) {
	dyn_get_modrm();
	if (decode.modrm.mod<3) {
		dyn_fill_ea(FC_ADDR);
		dyn_read_word(FC_ADDR,FC_OP1,decode.big_op);
	...
	switch (immsize) {
		case 0:
			...
			break;
		case 1:
			if (decode.big_op) gen_mov_dword_to_reg_imm(FC_OP2,(System::SByte)decode_fetchb());
			else  gen_mov_word_to_reg_imm(FC_OP2,(System::SByte)decode_fetchb());
			break;
		case 2:
			if (decode.big_op) gen_mov_dword_to_reg_imm(FC_OP2,(System::Int16)decode_fetchw());
			else gen_mov_word_to_reg_imm(FC_OP2,(System::Int16)decode_fetchw());
			break;
		case 4:
			if (decode.big_op) gen_mov_dword_to_reg_imm(FC_OP2,(System::Int32)decode_fetchd());
			else gen_mov_word_to_reg_imm(FC_OP2,(System::UInt16)((System::Int32)decode_fetchd()));
			break;
	}
...
static void dyn_grp1_ev_iv(bool withbyte) {
	...
	if (decode.modrm.mod<3) {
		...
		if (!withbyte) {
			...
			int imm=(System::SByte)decode_fetchb(); 
			if (decode.big_op) gen_mov_dword_to_reg_imm(FC_OP2,(System::UInt32)imm);
			else gen_mov_word_to_reg_imm(FC_OP2,(System::UInt16)imm);
		}
		...
		if (!withbyte) {
			...
			int imm=withbyte ? (System::SByte)decode_fetchb() : (decode.big_op ? decode_fetchd(): decode_fetchw()); 
...
static void dyn_grp2_ev(grp2_types type) {
	...
	case grp2_imm: {
		...
		System::Byte imm=(System::Byte)val;
		if (imm) {
			gen_mov_byte_to_reg_low_imm_canuseword(FC_OP2,imm&0x1f);
			dyn_shift_word_gencall((ShiftOps)decode.modrm.reg,decode.big_op);
		} else return;
...
static unsigned dyn_grp4_ev(void) {
	dyn_get_modrm();
	if (decode.modrm.mod<3) {
		dyn_fill_ea(FC_ADDR);
	...
	case 0x2:	// CALL Ev
		...
		gen_add_imm(FC_OP1,(System::UInt32)(decode.code-decode.code_start));
		if (decode.big_op) gen_call_function_raw((void*)&dynrec_push_dword);
		else gen_call_function_raw((void*)&dynrec_push_word);
...
static void dyn_exit_link(int eip_change) {
...
static void dyn_branched_exit(BranchTypes btype,System::Int32 eip_add) {
	unsigned eip_base=decode.code-decode.code_start;
	dyn_reduce_cycles();
...
static void dyn_loop(LoopTypes type) {
	dyn_reduce_cycles();
	int eip_add=(System::SByte)decode_fetchb();
	unsigned eip_base=decode.code-decode.code_start;
	...
	case LOOP_NONE:
		...
		gen_add_imm(FC_OP1,(System::UInt32)(-1));
...
static void dyn_ret_near(unsigned bytes) {
	dyn_reduce_cycles();
...
static void dyn_call_near_imm(void) {
	int imm;
	if (decode.big_op) imm=(System::Int32)decode_fetchd();
	else imm=(System::Int16)decode_fetchw();
...
static void dyn_ret_far(unsigned bytes) {
	dyn_reduce_cycles();
	dyn_set_eip_last_end(FC_RETOP);
...
static void dyn_interrupt(System::Byte num) {
	dyn_reduce_cycles();
	dyn_set_eip_last_end(FC_RETOP);
...
static void dyn_string(StringOps op) {
	...
	System::Byte di_base_addr=decode.seg_prefix_used ? decode.seg_prefix : DRC_SEG_DS;
	switch (op) {
		case STR_MOVSB:
			if (decode.big_addr) gen_call_function_mm((void*)&dynrec_movsb_dword,(unsigned)DRCD_SEG_PHYS(di_base_addr),(unsigned)DRCD_SEG_PHYS(DRC_SEG_ES));
			else gen_call_function_mm((void*)&dynrec_movsb_word,(unsigned)DRCD_SEG_PHYS(di_base_addr),(unsigned)DRCD_SEG_PHYS(DRC_SEG_ES));
			break;
		case STR_MOVSW:
			if (decode.big_addr) gen_call_function_mm((void*)&dynrec_movsw_dword,(unsigned)DRCD_SEG_PHYS(di_base_addr),(unsigned)DRCD_SEG_PHYS(DRC_SEG_ES));
			else gen_call_function_mm((void*)&dynrec_movsw_word,(unsigned)DRCD_SEG_PHYS(di_base_addr),(unsigned)DRCD_SEG_PHYS(DRC_SEG_ES));
			break;
		case STR_MOVSD:
			if (decode.big_addr) gen_call_function_mm((void*)&dynrec_movsd_dword,(unsigned)DRCD_SEG_PHYS(di_base_addr),(unsigned)DRCD_SEG_PHYS(DRC_SEG_ES));
			else gen_call_function_mm((void*)&dynrec_movsd_word,(unsigned)DRCD_SEG_PHYS(di_base_addr),(unsigned)DRCD_SEG_PHYS(DRC_SEG_ES));
			break;

		case STR_LODSB:
			if (decode.big_addr) gen_call_function_m((void*)&dynrec_lodsb_dword,(unsigned)DRCD_SEG_PHYS(di_base_addr));
			else gen_call_function_m((void*)&dynrec_lodsb_word,(unsigned)DRCD_SEG_PHYS(di_base_addr));
			break;
		case STR_LODSW:
			if (decode.big_addr) gen_call_function_m((void*)&dynrec_lodsw_dword,(unsigned)DRCD_SEG_PHYS(di_base_addr));
			else gen_call_function_m((void*)&dynrec_lodsw_word,(unsigned)DRCD_SEG_PHYS(di_base_addr));
			break;
		case STR_LODSD:
			if (decode.big_addr) gen_call_function_m((void*)&dynrec_lodsd_dword,(unsigned)DRCD_SEG_PHYS(di_base_addr));
			else gen_call_function_m((void*)&dynrec_lodsd_word,(unsigned)DRCD_SEG_PHYS(di_base_addr));
			break;

		case STR_STOSB:
			if (decode.big_addr) gen_call_function_m((void*)&dynrec_stosb_dword,(unsigned)DRCD_SEG_PHYS(DRC_SEG_ES));
			else gen_call_function_m((void*)&dynrec_stosb_word,(unsigned)DRCD_SEG_PHYS(DRC_SEG_ES));
			break;
		case STR_STOSW:
			if (decode.big_addr) gen_call_function_m((void*)&dynrec_stosw_dword,(unsigned)DRCD_SEG_PHYS(DRC_SEG_ES));
			else gen_call_function_m((void*)&dynrec_stosw_word,(unsigned)DRCD_SEG_PHYS(DRC_SEG_ES));
			break;
		case STR_STOSD:
			if (decode.big_addr) gen_call_function_m((void*)&dynrec_stosd_dword,(unsigned)DRCD_SEG_PHYS(DRC_SEG_ES));
			else gen_call_function_m((void*)&dynrec_stosd_word,(unsigned)DRCD_SEG_PHYS(DRC_SEG_ES));
			break;
...
static void dyn_write_port_byte_direct(System::Byte port) {
	dyn_add_iocheck_var(port,1);
...
static void dyn_write_port_word_direct(System::Byte port) {
	dyn_add_iocheck_var(port,decode.big_op?4:2);
...
static void dyn_enter(void) {
	unsigned bytes=decode_fetchw();
	unsigned level=decode_fetchb();
...
static void dynrec_leave_word(void) {
	...
	reg_bp=(System::UInt16)CPU_Pop16();
}
...
static void dynrec_popa_word(void) {
	reg_di=(System::UInt16)CPU_Pop16();reg_si=(System::UInt16)CPU_Pop16();
	reg_bp=(System::UInt16)CPU_Pop16();CPU_Pop16();		//Don't save SP
	reg_bx=(System::UInt16)CPU_Pop16();reg_dx=(System::UInt16)CPU_Pop16();
	reg_cx=(System::UInt16)CPU_Pop16();reg_ax=(System::UInt16)CPU_Pop16();
}
