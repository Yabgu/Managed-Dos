...
CDirectSerial::CDirectSerial (unsigned id, CommandLine* cmd)
					:CSerial (id, cmd) {
...
void CDirectSerial::handleUpperEvent(System::UInt16 type) {
	switch(type) {
...
bool CDirectSerial::doReceive() {
	...
	if(value) {
		receiveByteEx((System::Byte)(value&0xff),(System::Byte)((value&0xff00)>>8));
		return true;
	}
	return false;
}
...
void CDirectSerial::updatePortConfig (System::UInt16 divider, System::Byte lcr) {
	System::Byte parity = 0;
	...
	System::Byte bytelength = (lcr & 0x3)+5;
	...
	System::Byte stopint;
	if (lcr & 0x4) {
		if (bytelength == 5) stopint = SERIAL_15STOP;
		else stopint = SERIAL_2STOP;
	} else stopint = SERIAL_1STOP;

	if(!SERIAL_setCommParameters(comport, baudrate, parity, stopint, bytelength)) {
#if SERIAL_DEBUG
		log_ser(dbg_aux,"Serial port settings not supported by host." );
#endif
		LOG_MSG ("Serial%d: Desired serial mode not supported (%d,%d,%c,%d)",
			COMNUMBER, baudrate,bytelength,parity,stopint);
	} 
...
void CDirectSerial::transmitByte (System::Byte val, bool first) {
