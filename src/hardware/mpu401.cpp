...
static void MPU401_Event(unsigned);
...
static void MPU401_WriteData(unsigned port,unsigned val,unsigned iolen);
...
static struct {
	...
	MpuMode mode;
	unsigned irq;
	System::Byte queue[MPU401_QUEUE];
	unsigned queue_pos,queue_used;
	struct track {
		int counter;
		System::Byte value[8],sys_val;
		System::Byte vlength,length;
		MpuDataType type;
	} playbuf[8],condbuf;
	struct {
		...
		System::Byte tmask,cmask,amask;
		System::UInt16 midi_mask;
		System::UInt16 req_mask;
		System::Byte channel,old_chan;
	} state;
	struct {
		System::Byte timebase,old_timebase;
		System::Byte tempo,old_tempo;
		System::Byte tempo_rel,old_tempo_rel;
		System::Byte tempo_grad;
		System::Byte cth_rate,cth_counter;
		bool clock_to_host,cth_active;
	} clock;
} mpu;


static void QueueByte(System::Byte data) {
	...
	if (mpu.queue_used<MPU401_QUEUE) {
		unsigned pos=mpu.queue_used+mpu.queue_pos;
...
static unsigned MPU401_ReadStatus(unsigned port,unsigned iolen) {
	System::Byte ret=0x3f;	/* int 6 and 7 clear */
	if (!mpu.queue_used) ret|=0x80;
	return ret;
}

static void MPU401_WriteCommand(unsigned port,unsigned val,unsigned iolen) {
	mpu.state.reset=0;
	if (val<=0x2f) {
		...
		switch (val&0xc) {
			case  0x4:	/* Stop */
				...
				for (unsigned i=0xb0;i<0xbf;i++) {	/* All notes off */
					MIDI_RawOutByte(i);
...
static unsigned MPU401_ReadData(unsigned port,unsigned iolen) {
	System::Byte ret=MSG_MPU_ACK;
	if (mpu.queue_used) {
		if (mpu.queue_pos>=MPU401_QUEUE) mpu.queue_pos-=MPU401_QUEUE;
...
static void MPU401_WriteData(unsigned port,unsigned val,unsigned iolen) {
	...
	switch (mpu.state.command_byte) {	/* 0xe# command data */
		...
		case 0xef: /* Set 9-16 MIDI channel mask */
			...
			mpu.state.midi_mask|=((System::UInt16)val)<<8;
			return;
...
static void MPU401_IntelligentOut(System::Byte chan) {
	unsigned val;
	switch (mpu.playbuf[chan].type) {
		...
		case T_MARK:
			...
			break;
		case T_MIDI_NORM:
			for (unsigned i=0;i<mpu.playbuf[chan].vlength;i++)
...
static void UpdateTrack(System::Byte chan) {
...
static void MPU401_Event(unsigned val) {
	if (mpu.mode==M_UART) return;
	if (mpu.state.irq_pending) goto next_event;
	for (unsigned i=0;i<8;i++) { /* Decrease counters */
...
static void MPU401_Reset(void) {
	...
	mpu.condbuf.counter=0;
	mpu.condbuf.type=T_OVERFLOW;
	for (unsigned i=0;i<8;i++) {mpu.playbuf[i].type=T_OVERFLOW;mpu.playbuf[i].counter=0;}
}
