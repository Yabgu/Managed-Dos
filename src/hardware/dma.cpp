...
System::UInt32 ems_board_mapping[LINK_START];
...
static void DMA_BlockRead(PhysPt spage,PhysPt offset,void * data,unsigned size,System::Byte dma16) {
	System::Byte * write=(System::Byte *) data;
	unsigned highpart_addr_page = spage>>12;
	size <<= dma16;
	offset <<= dma16;
	System::UInt32 dma_wrap = ((0xffff<<dma16)+dma16) | dma_wrapping;
	for ( ; size ; size--, offset++) {
		if (offset>(dma_wrapping<<dma16)) E_Exit("DMA segbound wrapping (read)");
		offset &= dma_wrap;
		unsigned page = highpart_addr_page+(offset >> 12);
...
static void DMA_BlockWrite(PhysPt spage,PhysPt offset,void * data,unsigned size,System::Byte dma16) {
	System::Byte * read=(System::Byte *) data;
	unsigned highpart_addr_page = spage>>12;
	size <<= dma16;
	offset <<= dma16;
	System::UInt32 dma_wrap = ((0xffff<<dma16)+dma16) | dma_wrapping;
...
static void DMA_Write_Port(unsigned port,unsigned val,unsigned /*iolen*/) {
	if (port<0x10) {
		...
		switch (port) {
			...
			case 0x81:GetDMAChannel(2)->SetPage((System::Byte)val);break;
			case 0x82:GetDMAChannel(3)->SetPage((System::Byte)val);break;
			case 0x83:GetDMAChannel(1)->SetPage((System::Byte)val);break;
			case 0x89:GetDMAChannel(6)->SetPage((System::Byte)val);break;
			case 0x8a:GetDMAChannel(7)->SetPage((System::Byte)val);break;
			case 0x8b:GetDMAChannel(5)->SetPage((System::Byte)val);break;
		}
	}
}

static unsigned DMA_Read_Port(unsigned port,unsigned iolen) {
	if (port<0x10) {
...
void DmaController::WriteControllerReg(unsigned reg,unsigned val,unsigned /*len*/) {
	DmaChannel * chan;
	switch (reg) {
	/* set base address of DMA transfer (1st byte low part, 2nd byte high part) */
	case 0x0:case 0x2:case 0x4:case 0x6:
		UpdateEMSMapping();
		chan=GetChannel((System::Byte)(reg >> 1));
		flipflop=!flipflop;
		if (flipflop) {
...
unsigned DmaController::ReadControllerReg(unsigned reg,unsigned /*len*/) {
	DmaChannel * chan;unsigned ret;
	switch (reg) {
...
DmaChannel::DmaChannel(System::Byte num, bool dma16) {
	masked = true;
	callback = NULL;
	if(num == 4) return;
	channum = num;
...
unsigned DmaChannel::Read(unsigned want, System::Byte * buffer) {
	unsigned done=0;
	curraddr &= dma_wrapping;
again:
	unsigned left=(currcnt+1);
	if (want<left) {
...
unsigned DmaChannel::Write(unsigned want, System::Byte * buffer) {
	unsigned done=0;
	curraddr &= dma_wrapping;
again:
	unsigned left=(currcnt+1);
...
void DMA_SetWrapping(unsigned wrap) {
	dma_wrapping = wrap;
}
