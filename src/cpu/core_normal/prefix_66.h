 ...
 */
	...
	CASE_D(0x62)												/* BOUND Ed */
		{
			System::Int32 bound_min, bound_max;
			GetRMrd;GetEAa;
			bound_min=LoadMd(eaa);
			bound_max=LoadMd(eaa+4);
			if ( (((System::Int32)*rmrd) < bound_min) || (((System::Int32)*rmrd) > bound_max) ) {
				EXCEPTION(5);
			}
		}
		break;
	CASE_D(0x63)												/* ARPL Ed,Rd */
		{
			...
			GetRMrw;
			if (rm >= 0xc0 ) {
				GetEArd;unsigned new_sel=(System::UInt16)*eard;
				CPU_ARPL(new_sel,*rmrw);
				*eard=(System::UInt32)new_sel;
			} else {
				GetEAa;unsigned new_sel=LoadMw(eaa);
				CPU_ARPL(new_sel,*rmrw);
				SaveMd(eaa,(System::UInt32)new_sel);
			}
		}
		break;
	CASE_D(0x68)												/* PUSH Id */
	...
	CASE_D(0x81)												/* Grpl Ed,Id */
		{
			...
			if (rm >= 0xc0) {
				GetEArd;System::UInt32 id=Fetchd();
				switch (which) {
				...
				GetEAa;System::UInt32 id=Fetchd();
				switch (which) {
	...
	CASE_D(0x83)												/* Grpl Ed,Ix */
		{
			GetRM;unsigned which=(rm>>3)&7;
			if (rm >= 0xc0) {
				GetEArd;System::UInt32 id=(System::Int32)Fetchbs();
				switch (which) {
				case 0x00:ADDD(*eard,id,LoadRd,SaveRd);break;
				case 0x01: ORD(*eard,id,LoadRd,SaveRd);break;
				case 0x02:ADCD(*eard,id,LoadRd,SaveRd);break;
				...
				GetEAa;System::UInt32 id=(System::Int32)Fetchbs();
				switch (which) {
				case 0x00:ADDD(eaa,id,LoadMd,SaveMd);break;
				case 0x01: ORD(eaa,id,LoadMd,SaveMd);break;
				case 0x02:ADCD(eaa,id,LoadMd,SaveMd);break;
	...
	CASE_D(0x87)												/* XCHG Ed,Gd */
		{	
			GetRMrd;System::UInt32 oldrmrd=*rmrd;
	...
	CASE_D(0x8d)												/* LEA Gd */
		{
			...
			BaseDS=BaseSS=0;
			if (TEST_PREFIX_ADDR) {
				*rmrd=(System::UInt32)(*EATable[256+rm])();
			} else {
				*rmrd=(System::UInt32)(*EATable[rm])();
			}
			break;
		}
	CASE_D(0x8f)												/* POP Ed */
		{
			System::UInt32 val=Pop_32();
			GetRM;
			if (rm >= 0xc0 ) {GetEArd;*eard=val;}
	...
	CASE_D(0x91)												/* XCHG ECX,EAX */
		{ System::UInt32 temp=reg_eax;reg_eax=reg_ecx;reg_ecx=temp;break;}
	CASE_D(0x92)												/* XCHG EDX,EAX */
		{ System::UInt32 temp=reg_eax;reg_eax=reg_edx;reg_edx=temp;break;}
		break;
	CASE_D(0x93)												/* XCHG EBX,EAX */
		{ System::UInt32 temp=reg_eax;reg_eax=reg_ebx;reg_ebx=temp;break;}
		break;
	CASE_D(0x94)												/* XCHG ESP,EAX */
		{ System::UInt32 temp=reg_eax;reg_eax=reg_esp;reg_esp=temp;break;}
		break;
	CASE_D(0x95)												/* XCHG EBP,EAX */
		{ System::UInt32 temp=reg_eax;reg_eax=reg_ebp;reg_ebp=temp;break;}
		break;
	CASE_D(0x96)												/* XCHG ESI,EAX */
		{ System::UInt32 temp=reg_eax;reg_eax=reg_esi;reg_esi=temp;break;}
		break;
	CASE_D(0x97)												/* XCHG EDI,EAX */
		{ System::UInt32 temp=reg_eax;reg_eax=reg_edi;reg_edi=temp;break;}
		break;
	CASE_D(0x98)												/* CWDE */
		reg_eax=(System::Int16)reg_ax;break;
	CASE_D(0x99)												/* CDQ */
	...
	CASE_D(0x9a)												/* CALL FAR Ad */
		{ 
			System::UInt32 newip=Fetchd();System::UInt16 newcs=Fetchw();
			FillFlags();
			...
			case 0x03:											/* CALL FAR Ed */
				{
					if (rm >= 0xc0) goto illegal_opcode;
					GetEAa;
					System::UInt32 newip=LoadMd(eaa);
					System::UInt16 newcs=LoadMw(eaa+4);
					FillFlags();
					CPU_CALL(true,newcs,newip,GETIP);
#if CPU_TRAP_CHECK
					if (GETFLAG(TF)) {	
