...
bool localDrive::FileCreate(DOS_File * * file,char * name,System::UInt16 /*attributes*/) {
...
bool localDrive::FileOpen(DOS_File * * file,char * name,System::UInt32 flags) {
	const char* type;
	switch (flags&0xf) {
...
bool localDrive::FileUnlink(char * name) {
	...
	if (unlink(fullname)) {
		...
		for(unsigned i = 0;i < DOS_FILES;i++){
			if(Files[i] && Files[i]->IsName(name)) {
				unsigned max = DOS_FILES;
...
bool localDrive::FindNext(DOS_DTA & dta) {
	...
	System::Byte srch_attr;char srch_pattern[DOS_NAMELENGTH_ASCII];
	System::Byte find_attr;

	dta.GetSearchParams(srch_attr,srch_pattern);
	System::UInt16 id = dta.GetDirID();
...
again:
	...
	char find_name[DOS_NAMELENGTH_ASCII];System::UInt16 find_date,find_time;System::UInt32 find_size;
	...
	find_size=(System::UInt32) stat_block.st_size;
	struct tm *time;
	if((time=localtime(&stat_block.st_mtime))!=0){
		find_date=DOS_PackDate((System::UInt16)(time->tm_year+1900),(System::UInt16)(time->tm_mon+1),(System::UInt16)time->tm_mday);
		find_time=DOS_PackTime((System::UInt16)time->tm_hour,(System::UInt16)time->tm_min,(System::UInt16)time->tm_sec);
	} else {
		find_time=6; 
		find_date=4;
	}
...
bool localDrive::GetFileAttr(char * name,System::UInt16 * attr) {
	char newname[CROSS_LEN];
	strcpy(newname,basedir);
	strcat(newname,name);
	CROSS_FILENAME(newname);
...
bool localDrive::AllocationInfo(System::UInt16 * _bytes_sector,System::Byte * _sectors_cluster,System::UInt16 * _total_clusters,System::UInt16 * _free_clusters) {
...
bool localDrive::FileStat(const char* name, FileStat_Block * const stat_block) {
	...
	if((time=localtime(&temp_stat.st_mtime))!=0) {
		stat_block->time=DOS_PackTime((System::UInt16)time->tm_hour,(System::UInt16)time->tm_min,(System::UInt16)time->tm_sec);
		stat_block->date=DOS_PackDate((System::UInt16)(time->tm_year+1900),(System::UInt16)(time->tm_mon+1),(System::UInt16)time->tm_mday);
	} else {

	}
	stat_block->size=(System::UInt32)temp_stat.st_size;
	return true;
}


System::Byte localDrive::GetMediaByte(void) {
...
int localDrive::UnMount(void) { 
	delete this;
	return 0; 
}

localDrive::localDrive(const char * startdir,System::UInt16 _bytes_sector,System::Byte _sectors_cluster,System::UInt16 _total_clusters,System::UInt16 _free_clusters,System::Byte _mediaid) {
...
bool localFile::Read(System::Byte * data,System::UInt16 * size) {
	...
	last_action=READ;
	*size=(System::UInt16)fread(data,1,*size,fhandle);
	...
	System::Byte mask = IO_Read(0x21);
	if(mask & 0x4 ) IO_Write(0x21,mask&0xfb);
	return true;
}

bool localFile::Write(System::Byte * data,System::UInt16 * size) {
    ...
    {
		*size=(System::UInt16)fwrite(data,1,*size,fhandle);
		return true;
    }
}

bool localFile::Seek(System::UInt32 * pos,System::UInt32 type) {
	int seektype;
	switch (type) {
	...
	int ret=fseek(fhandle,*reinterpret_cast<System::Int32*>(pos),seektype);
	if (ret!=0) {
	...
	System::UInt32 * fake_pos=(System::UInt32*)&temppos;
	*pos=*fake_pos;
#endif
	*pos=(System::UInt32)ftell(fhandle);
	last_action=NONE;
	return true;
}
...
System::UInt16 localFile::GetInformation(void) {
...
bool localFile::UpdateDateTimeFromHost(void) {
	...
	if((ltime=localtime(&temp_stat.st_mtime))!=0) {
		time=DOS_PackTime((System::UInt16)ltime->tm_hour,(System::UInt16)ltime->tm_min,(System::UInt16)ltime->tm_sec);
		date=DOS_PackDate((System::UInt16)(ltime->tm_year+1900),(System::UInt16)(ltime->tm_mon+1),(System::UInt16)ltime->tm_mday);
	} else {
		time=1;date=1;
	}
	return true;
...
cdromDrive::cdromDrive(const char driveLetter, const char * startdir,System::UInt16 _bytes_sector,System::Byte _sectors_cluster,System::UInt16 _total_clusters,System::UInt16 _free_clusters,System::Byte _mediaid, int& error)
...
bool cdromDrive::FileOpen(DOS_File * * file,char * name,System::UInt32 flags) {
...
bool cdromDrive::FileCreate(DOS_File * * /*file*/,char * /*name*/,System::UInt16 /*attributes*/) {
	DOS_SetError(DOSERR_ACCESS_DENIED);
	return false;
}
...
bool cdromDrive::GetFileAttr(char * name,System::UInt16 * attr) {
