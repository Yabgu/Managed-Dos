...
//TODO Maybe support confirmation for *.* like dos does.	
	...
	char name[DOS_NAMELENGTH_ASCII];System::UInt32 size;System::UInt16 time,date;System::Byte attr;
	DOS_DTA dta(dos.dta());
	while (res) {
...
void DOS_Shell::CMD_HELP(char * args){
	...
	System::UInt32 cmd_index=0,write_count=0;
	while (cmd_list[cmd_index].name) {
...
void DOS_Shell::CMD_CHDIR(char * args) {
	...
	StripSpaces(args);
	if (!*args) {
		System::Byte drive=DOS_GetDefaultDrive()+'A';
...
static void FormatNumber(unsigned num,char * buf) {
	unsigned numm,numk,numb,numg;
	numb=num % 1000;
	num/=1000;
	numk=num % 1000;
	num/=1000;
...
void DOS_Shell::CMD_DIR(char * args) {
	...
	System::UInt32 byte_count,file_count,dir_count;
	unsigned w_count=0;
	unsigned p_count=0;
	unsigned w_size = optW?5:1;
	...
	*(strrchr(path,'\\')+1)=0;
	...
	do {    /* File name and extension */
		char name[DOS_NAMELENGTH_ASCII];System::UInt32 size;System::UInt16 date;System::UInt16 time;System::Byte attr;
		dta.GetResult(name,size,date,time,attr);
		...
		if (optB) {
			...
			System::Byte day	= (System::Byte)(date & 0x001f);
			System::Byte month	= (System::Byte)((date >> 5) & 0x000f);
			System::UInt16 year = (System::UInt16)((date >> 9) + 1980);
			System::Byte hour	= (System::Byte)((time >> 5 ) >> 6);
			System::Byte minute = (System::Byte)((time >> 5) & 0x003f);
	...
	if (!optB) {
		...
		System::Byte drive=dta.GetSearchDrive();
		//TODO Free Space
		unsigned free_space=1024*1024*100;
		if (Drives[drive]) {
			System::UInt16 bytes_sector;System::Byte sectors_cluster;System::UInt16 total_clusters;System::UInt16 free_clusters;
...
void DOS_Shell::CMD_COPY(char * args) {
	...
	dos.dta(dos.tables.tempdta);
	DOS_DTA dta(dos.dta());
	System::UInt32 size;System::UInt16 date;System::UInt16 time;System::Byte attr;
	...
	while(sources.size()) {
		...
		System::UInt16 sourceHandle,targetHandle;
				...
				if (DOS_OpenFile(nameSource,0,&sourceHandle)) {
					...
					if (oldsource.concat || DOS_CreateFile(nameTarget,0,&targetHandle)) {
						...
						if (!oldsource.concat || (DOS_OpenFile(nameTarget,OPEN_READWRITE,&targetHandle) && 
							...
							// Copy 
							static System::Byte buffer[0x8000]; // static, otherwise stack overflow possible.
							bool	failed = false;
							System::UInt16	toread = 0x8000;
							do {
...
void DOS_Shell::CMD_LOADHIGH(char *args){
	...
	System::Byte old_memstrat=(System::Byte)(DOS_GetMemAllocStrategy()&0xff);
	if (umb_start==0x9fff) {
...
void DOS_Shell::CMD_CHOICE(char * args){
	...
	c = optS?c:(System::Byte)toupper(c);
	DOS_WriteFile (STDOUT,&c, &n);
	dos.return_code = (System::Byte)(ptr-rem+1);
}
...
void DOS_Shell::CMD_VER(char *args) {
	...
	if(args && *args) {
		...
		word = StripWord(args);
		dos.version.major = (System::Byte)(atoi(word));
		dos.version.minor = (System::Byte)(atoi(args));
