...
static void getPixel(int x, int y, int &r, int &g, int &b, int shift)
{
	if (x >= (int)render.src.width) x = (int)render.src.width-1;
	if (y >= (int)render.src.height) x = (int)render.src.height-1;
	if (x < 0) x = 0;
	if (y < 0) y = 0;

	System::Byte* src = (System::Byte *)&scalerSourceCache;
	System::UInt32 pixel;
	switch (render.scale.inMode) {
	case scalerMode8:
		pixel = *(x+(System::Byte*)(src+y*render.scale.cachePitch));
		r += render.pal.rgb[pixel].red >> shift;
		g += render.pal.rgb[pixel].green >> shift;
		b += render.pal.rgb[pixel].blue >> shift;
		break;
	case scalerMode15:
		pixel = *(x+(System::UInt16*)(src+y*render.scale.cachePitch));
	...
	case scalerMode16:
		...
		break;
	case scalerMode32:
		pixel = *(x+(System::UInt32*)(src+y*render.scale.cachePitch));
...
static GUI::ScreenSDL *UI_Startup(GUI::ScreenSDL *screen) {
	...
	for (int y = 0; y < h; y++) {
		System::UInt32 *bg = (System::UInt32*)(y*screenshot->pitch + (char*)screenshot->pixels);
		for (int x = 0; x < w; x++) {
			int r = 0, g = 0, b = 0;
	...
	for (int y = 0; y < h; y++) {
		System::UInt32 *bg = (System::UInt32*)(y*background->pitch + (char*)background->pixels);
		for (int x = 0; x < w; x++) {
			int r = 0, g = 0, b = 0;
