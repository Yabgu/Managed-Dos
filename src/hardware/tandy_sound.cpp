...
#define FB_WNOISE 0x14002	/* (16int) bit16 = bit0(out) ^ bit2 ^ bit15 */
...
static struct {
	...
	struct {
		...
		struct {
			unsigned rate;
			System::Byte buf[TDAC_DMA_BUFSIZE];
			System::Byte last_sample;
			DmaChannel * chan;
			bool transfer_done;
		} dma;
		System::Byte mode,control;
		System::UInt16 frequency;
		System::Byte amplitude;
		bool irq_activated;
	} dac;
} tandy;


static void SN76496Write(unsigned /*port*/,unsigned data,unsigned /*iolen*/) {
	struct SN76496 *R = &sn;
...
static void SN76496Update(unsigned length) {
	...
	int i;
	struct SN76496 *R = &sn;
	System::Int16 * buffer=(System::Int16 *)MixTemp;
	...
	while (count)
	{
		...
		*(buffer++) = (System::Int16)(out / STEP);

		count--;
	}
	tandy.chan->AddSamples_m16(length,(System::Int16 *)MixTemp);
}
...
bool TS_Get_Address(unsigned& tsaddr, unsigned& tsirq, unsigned& tsdma) {
	tsaddr=0;
	tsirq =0;
	tsdma =0;
...
static void TandyDACModeChanged(void) {
	...
	case 3:
		...
		if (tandy.dac.frequency!=0) {
			...
			tandy.dac.chan->SetFreq((unsigned)freq);
...
static void TandyDACWrite(unsigned port,unsigned data,unsigned /*iolen*/) {
	switch (port) {
	case 0xc4: {
		unsigned oldmode = tandy.dac.mode;
		tandy.dac.mode = (System::Byte)(data&0xff);
		if ((data&3)!=(oldmode&3)) {
			TandyDACModeChanged();
		}
	...
	case 0xc5:
		...
		case 0:
			...
			break;
		case 1:
			tandy.dac.control = (System::Byte)(data&0xff);
			break;
		case 2:
			break;
		case 3:
		...
		break;
	case 0xc6:
		tandy.dac.frequency = tandy.dac.frequency & 0xf00 | (System::Byte)(data&0xff);
		switch (tandy.dac.mode&3) {
		case 0:
			// joystick mode
			break;
		...
		break;
	case 0xc7:
		tandy.dac.frequency = tandy.dac.frequency & 0x00ff | (((System::Byte)(data&0xf))<<8);
		tandy.dac.amplitude = (System::Byte)(data>>5);
		switch (tandy.dac.mode&3) {
		case 0:
			// joystick mode
			break;
...
static unsigned TandyDACRead(unsigned port,unsigned /*iolen*/) {
	switch (port) {
	case 0xc4:
		return (tandy.dac.mode&0x77) | (tandy.dac.irq_activated ? 0x08 : 0x00);
	case 0xc6:
		return (System::Byte)(tandy.dac.frequency&0xff);
	case 0xc7:
		return (System::Byte)(((tandy.dac.frequency>>8)&0xf) | (tandy.dac.amplitude<<5));
	}
	LOG_MSG("Tandy DAC: Read from unknown %X",port);
	return 0xff;
}

static void TandyDACGenerateDMASound(unsigned length) {
	if (length) {
		unsigned read=tandy.dac.dma.chan->Read(length,tandy.dac.dma.buf);
		tandy.dac.chan->AddSamples_m8(read,tandy.dac.dma.buf);
		if (read < length) {
			if (read>0) tandy.dac.dma.last_sample=tandy.dac.dma.buf[read-1];
			for (unsigned ct=read; ct < length; ct++) {
...
//				ReadHandler[3].Install(0x1e4,TandyDACRead,IO_MB,4);
		...
		System::UInt32 sample_rate = section->Get_int("tandyrate");
